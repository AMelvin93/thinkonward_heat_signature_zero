#!/usr/bin/env python3
"""
Run script for W2 Sigma 0.18/0.22 with Higher Thresholds optimizer.

EXP019: sigma_0.18_0.22_higher_threshold
Config:
- sigma0_1src: 0.18 (works well)
- sigma0_2src: 0.22 (works well)
- threshold_1src: 0.38 (higher - may reduce time)
- threshold_2src: 0.48 (higher - may reduce time)
- fallback_fevals: 18

Hypothesis: Sigma 0.18/0.22 works. Try with higher thresholds (0.38/0.48) to reduce time closer to 60 min.
Based on: EXP010: 1.1337 @ 63.5 min with thresholds 0.35/0.45. Slightly higher thresholds may save 2-3 min.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Sigma 0.18/0.22 with Higher Thresholds')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP019 specific
    parser.add_argument('--sigma0-1src', type=float, default=0.18)  # Good sigma
    parser.add_argument('--sigma0-2src', type=float, default=0.22)  # Good sigma
    parser.add_argument('--threshold-1src', type=float, default=0.38)  # Higher
    parser.add_argument('--threshold-2src', type=float, default=0.48)  # Higher
    parser.add_argument('--fallback-fevals', type=int, default=18)
    # Standard primary fevals
    ...
    print(f"\n[W2] Sigma 0.18/0.22 + Higher Thresholds Optimizer")
    print(f"=" * 70)
    print(f"EXP019: sigma_0.18_0.22_higher_threshold")
    print(f"-" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Sigma: 1-src={args.sigma0_1src}, 2-src={args.sigma0_2src}")
    print(f"Threshold: 1-src={args.threshold_1src}, 2-src={args.threshold_2src} (HIGHER)")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Sigma 0.18/0.22 + Higher Thresholds (EXP019)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Config: sigma={args.sigma0_1src}/{args.sigma0_2src}, thresh={args.threshold_1src}/{args.threshold_2src} (HIGHER)")
    print(f"Baseline: 1.1247 @ 57.2 min")
    print(f"EXP010 (same sigma, lower thresh): 1.1337 @ 63.5 min")
    print(f"Target: ~60 min with score near EXP010")
