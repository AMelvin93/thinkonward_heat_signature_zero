#!/usr/bin/env python3
"""
Run script for W1 Sigma 0.20/0.25 Reduced Fevals optimizer.

EXP029: Best sigma (0.20/0.25) with significantly reduced fevals to bring
69.2 min into budget (<60 min).

Config:
- sigma0_1src: 0.20 (best sigma)
- sigma0_2src: 0.25 (best sigma)
- threshold_1src: 0.35 (standard - NEVER CHANGE)
- threshold_2src: 0.45 (standard - NEVER CHANGE)
- fevals_1src: 14 (reduced from 20, ~30% reduction)
- fevals_2src: 26 (reduced from 36, ~28% reduction)
- fallback_fevals: 13 (reduced from 18)

Goal: Achieve score > 1.1247 (baseline) within 60 min budget.
Reference: EXP002 got 1.1362 @ 69.2 min with standard fevals.
"""
...
from optimizer import Sigma020_025ReducedFevalsOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = Sigma020_025ReducedFevalsOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Sigma 0.20/0.25 Reduced Fevals (EXP029)')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--max-fevals-1src', type=int, default=14)  # Reduced from 20
    parser.add_argument('--max-fevals-2src', type=int, default=26)  # Reduced from 36
    ...
    parser.add_argument('--fallback-fevals', type=int, default=13)  # Reduced from 18
    ...
    print(f"\nW1 Sigma 0.20/0.25 Reduced Fevals (EXP029)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Sigma 0.20/0.25 Reduced Fevals (EXP029)")
    print(f"{'='*70}")
    ...
    print(f"EXP002 ref:    1.1362 @ 69.2 min (sigma 0.20/0.25, thresh 0.35/0.45, fevals 20/36)")
    print(f"This run:      {score:.4f} @ {projected_400:.1f} min (sigma 0.20/0.25, thresh 0.35/0.45, fevals 14/26)")
