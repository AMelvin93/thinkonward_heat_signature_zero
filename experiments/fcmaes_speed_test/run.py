#!/usr/bin/env python3
"""Run script for fcmaes speed test with MLflow logging."""
...
from optimizer import FcmaesOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = FcmaesOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run fcmaes Speed Test')
    ...
    print(f"CMA-ES library: fcmaes (C++/Numba backend)")
    print(f"=" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - fcmaes Speed Test")
    print(f"{'='*70}")
    ...
    print()
    print(f"pycma Baseline (1.1688 @ 58.4 min):")
    print(f"  fcmaes run:    {score:.4f} @ {projected_400:.1f} min")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"fcmaes_speed_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param('experiment_id', 'EXP_FCMAES_SPEED_001')
            mlflow.log_param('worker', 'W2')
            mlflow.log_param('optimizer', 'FcmaesOptimizer')
            mlflow.log_param('cmaes_library', 'fcmaes')
