"""
fcmaes Speed Test Optimizer

Uses fcmaes library instead of pycma for CMA-ES optimization.
fcmaes has a C++/Numba backend that may be significantly faster.

Based on: experiments/early_timestep_filtering/optimizer_with_polish.py (1.1688 baseline)

Key changes:
- Replace 'import cma' with fcmaes equivalent
- Use fcmaes.cmaes.minimize() instead of CMAEvolutionStrategy
"""
...
# Import fcmaes instead of pycma
from fcmaes import cmaes as fcmaes_cma
...
class FcmaesOptimizer:
    """
    Optimizer using fcmaes library for CMA-ES instead of pycma.
    Otherwise identical to the 1.1688 baseline.
    """
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run optimization with fcmaes instead of pycma."""
        ...
        n_sims = [0]
        all_evaluated = []  # Track all evaluated solutions
        ...
        if n_sources == 1:
            def objective_coarse(xy_params):
                ...
                all_evaluated.append((np.array(xy_params), rmse))
                return rmse
        ...
        # Use fcmaes for optimization
        for init_params, init_type in initializations:
            bounds = Bounds(lb, ub)

            # fcmaes.minimize()
            result = fcmaes_cma.minimize(
                fun=objective_coarse,
                bounds=bounds,
                x0=init_params,
                max_evaluations=fevals_per_init,
                input_sigma=sigma0,
            )

            all_solutions.append((result.x, result.fun, init_type))

            # Also add any intermediate solutions that were evaluated
            # (fcmaes doesn't expose them directly, but we track them via all_evaluated)

        # Get best solutions from tracking
        for params, rmse in all_evaluated:
            # Check if this is a distinct solution
            is_new = True
            for existing, _, _ in all_solutions:
                if np.linalg.norm(params - existing) < 0.01:
                    is_new = False
                    break
            if is_new:
                all_solutions.append((params, rmse, 'evaluated'))
