...
Validate the new best config: 4 perturbations + perturb_nm_iters=2

Prior finding: 1.1524 @ 58.3 min (single run)
Goal: 3-run validation to establish confidence
"""
...
def main():
    ...
    # New best config: 4-pert + perturb_nm_iters=2
    config = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'max_fevals_1src': 20,
        ...
        'perturb_nm_iters': 2,            # Reduced from 3 to 2
    ...
    print("="*60)
    print("VALIDATION: 4-pert + perturb_nm_iters=2")
    print("="*60)
    print(f"Prior result: {PRIOR_RESULT} @ 58.3 min")
    print(f"3-pert baseline: {BASELINE_3PERT} @ 55.7 min")
    print(f"Goal: 3-run validation")

    results = []

    for run_num in range(1, 4):
        result = run_single_test(run_num, config, "4pert_nm2", data)
        results.append(result)
    ...
    print("\n" + "="*60)
    print("VALIDATION SUMMARY: 4-pert + perturb_nm_iters=2 (3 runs)")
    print("="*60)
    ...
    print(f"  vs 3-pert baseline: {mean_score - BASELINE_3PERT:+.4f}")
    print(f"  Runs in budget: {runs_in_budget}/3 ({runs_in_budget*100/3:.0f}%)")
    print(f"\n  Gap to Top 10 (1.1585): {1.1585 - mean_score:+.4f}")

    # Decision
    if mean_score > BASELINE_3PERT and runs_in_budget == 3:
        print("\n*** VALIDATED SUCCESS! Promote to production. ***")
    elif mean_score > BASELINE_3PERT and runs_in_budget >= 2:
        print("\n*** PARTIAL SUCCESS! Improvement found but budget risk. ***")
    else:
        print("\n*** VALIDATION FAILED. Keep 3-pert as production. ***")
    ...
    with open('run_output.json', 'w') as f:
        json.dump({
            ...
            'max_score': max_score,
            'mean_time': mean_time,
            'vs_baseline': mean_score - BASELINE_3PERT,
