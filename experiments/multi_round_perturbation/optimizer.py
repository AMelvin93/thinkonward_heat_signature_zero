"""
Multi-Round Perturbation Optimizer

Apply perturbation twice: perturb -> NM optimize -> perturb -> NM optimize
Hypothesis: Single perturbation may not escape deep local minima.
Two rounds of perturbation+optimization may find better basins.

Based on perturbed_extended_polish which achieved 1.1464 @ 51.2 min with single perturbation.
"""
...
class MultiRoundPerturbationOptimizer:
    """
    Optimizer with multi-round perturbation:
    perturb -> NM optimize -> perturb -> NM optimize
    """
    ...
    def __init__(
        ...
        # Multi-round perturbation parameters
        enable_perturbation: bool = True,
        perturb_top_n: int = 1,
        perturbation_scale: float = 0.05,
        perturb_rounds: int = 2,  # Number of perturbation rounds
        perturb_nm_iters_per_round: int = 2,  # NM iterations per round
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.perturb_nm_iters_per_round = perturb_nm_iters_per_round
    ...
    def _generate_perturbation(self, pos_params, lb, ub):
        """Generate a single perturbation of the position parameters."""
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # NM polish on top candidates (COARSE grid)
        refined_solutions = []
        ...
        if self.enable_perturbation:
            ...
            for idx in range(n_to_perturb):
                ...
                current_best = pos_params
                current_rmse = rmse

                # Multi-round perturbation
                for round_num in range(self.perturb_rounds):
                    # Generate perturbation
                    perturbed_pos = self._generate_perturbation(current_best, lb, ub)
                    ...
                    result = minimize(
                        ...
                        method='Nelder-Mead',
                        options={
                            'maxiter': self.perturb_nm_iters_per_round,
                            'xatol': 0.01,
                            'fatol': 0.001,
                        }
                    )
                ...
                # If final result is better than original, add to candidates
                if current_rmse < rmse:
                    new_candidates.append((current_best, current_rmse, 'multi_perturbed'))
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Multi-round perturbation: {self.perturb_rounds} rounds, {self.perturb_nm_iters_per_round} NM iters/round")
