#!/usr/bin/env python3
"""
Run script for Concentrated Fevals optimizer.
"""
...
from optimizer import ConcentratedFevalsOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = ConcentratedFevalsOptimizer(
        max_fevals_1src=config['max_fevals_1src'],
        max_fevals_2src=config['max_fevals_2src'],
        probe_fevals=config['probe_fevals'],
        ...
        popsize_factor=config.get('popsize_factor', 1.0),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Concentrated Fevals optimizer')
    ...
    parser.add_argument('--probe-fevals', type=int, default=8)
    ...
    parser.add_argument('--popsize-factor', type=float, default=1.0)
    ...
    print(f"Fevals: {args.max_fevals_1src}/{args.max_fevals_2src}, Probe: {args.probe_fevals}")
    ...
    config = {
        ...
        'probe_fevals': args.probe_fevals,
    ...
    print()
    print(f"Baseline: 1.1233 @ 55.3 min")
