#!/usr/bin/env python3
"""
Run script for Diagonal Decoding CMA-ES (dd-CMA-ES) optimizer.

Tests whether CMA_diagonal_decoding=1 improves convergence for the
coordinate-wise ill-conditioned domain (Lx=2.0, Ly=1.0).
"""
...
from optimizer import DiagonalDecodingCMAESOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = DiagonalDecodingCMAESOptimizer(
        max_fevals_1src=config['fevals_1src'],
        max_fevals_2src=config['fevals_2src'],
        sigma0_1src=config['sigma_1src'],
        sigma0_2src=config['sigma_2src'],
        ...
        diagonal_decoding=config['diagonal_decoding'],
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run dd-CMA-ES optimizer')
    ...
    # Diagonal decoding parameter
    parser.add_argument('--diagonal-decoding', type=float, default=1.0)
    # Shared parameters
    ...
    print(f"\nDiagonal Decoding CMA-ES (dd-CMA-ES) Optimizer")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Diagonal decoding: {args.diagonal_decoding}")
    ...
    config = {
        ...
        'diagonal_decoding': args.diagonal_decoding,
    }
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - dd-CMA-ES (diagonal_decoding={args.diagonal_decoding})")
    print(f"{'='*70}")
    ...
    run_name = args.mlflow_run_name or f"dd_cmaes_dd{args.diagonal_decoding}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        mlflow.log_param("experiment_id", "EXP_DD_CMAES_001")
        mlflow.log_param("worker", "W1")
        mlflow.log_param("algorithm", "dd_cmaes")
        mlflow.log_param("platform", "wsl")
        mlflow.log_param("diagonal_decoding", args.diagonal_decoding)
