...
Key modification: Use CMA_diagonal_decoding=1 option in pycma.

dd-CMA adds adaptive diagonal decoding on top of full covariance, which helps
with coordinate-wise scaling when the domain has different scales (Lx=2.0, Ly=1.0).

Based on: "Diagonal Acceleration for CMA-ES" (Akimoto & Hansen, EC 2020)
"""
...
class DiagonalDecodingCMAESOptimizer:
    """
    CMA-ES with diagonal decoding (dd-CMA-ES).

    Key change from baseline: CMA_diagonal_decoding=1
    This adds adaptive diagonal learning ON TOP of full covariance.
    """
    ...
    def __init__(
        ...
        # NEW: Diagonal decoding parameter
        diagonal_decoding: float = 1.0,  # Learning rate multiplier for diagonal update
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.diagonal_decoding = diagonal_decoding
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        for init_params, init_type in initializations:
            ...
            opts['tolfun'] = 1e-6
            opts['tolx'] = 1e-6
            # KEY CHANGE: Enable diagonal decoding
            opts['CMA_diagonal_decoding'] = self.diagonal_decoding
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Diagonal decoding: {self.diagonal_decoding}")
