...
we quickly evaluate all inits and focus ALL fevals on the best one.

Old approach (wasteful):
    - 3 inits × 8 fevals each = 24 total fevals
    - But only 1 init wins, wasting ~65% of compute

New approach (efficient):
    - Evaluate 3 inits with 1 sim each = 3 sims for selection
    - Run CMA-ES with 21 fevals on best init = 21 focused fevals
    - Same total budget, much better convergence
"""
...
class SmartInitOptimizer:
    """
    CMA-ES optimizer with SMART initialization selection.

    Key Innovation: Instead of splitting fevals across all inits,
    we quickly evaluate each init and focus ALL fevals on the best one.

    Old approach:
        - 3 inits × (max_fevals/3) each = wasteful
        - Each init gets insufficient fevals to converge

    New approach:
        - Evaluate each init with 1 simulation (selection phase)
        - Run CMA-ES with ALL fevals on best init (optimization phase)
        - Much better convergence, same total budget
    """
    ...
    def estimate_sources(
        ...
        """
        Estimate sources with SMART init selection.

        Phase 1: Evaluate all inits (1 sim each)
        Phase 2: Run CMA-ES with ALL fevals on best init
        """
        ...
        # Run CMA-ES with FULL budget on best init
        opts = cma.CMAOptions()
