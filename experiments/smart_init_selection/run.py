#!/usr/bin/env python
"""
Run Smart Init Selection Optimizer experiment.

Key Innovation: Instead of splitting fevals across multiple initializations,
we quickly evaluate all inits (1 sim each) and focus ALL fevals on the best one.
...
Target: Achieve score > 1.0 within 60-minute budget

Usage:
    # Test with 12/24 fevals (same as asymmetric moderate)
    uv run python experiments/smart_init_selection/run.py --workers 7 --shuffle

    # Test with higher fevals
    uv run python experiments/smart_init_selection/run.py --max-fevals-1src 15 --max-fevals-2src 28 --workers 7 --shuffle
"""
...
from experiments.smart_init_selection.optimizer import (
    SmartInitOptimizer,
...
def process_sample(sample, meta, config, history_1src, history_2src):
    """Process a single sample with smart init selection optimizer."""
    optimizer = SmartInitOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Smart Init Selection Optimizer')
    parser.add_argument('--workers', type=int, default=7,
                        help='Number of workers (7 for G4dn, -1 for all CPUs)')
    parser.add_argument('--batch-size', type=int, default=20,
                        help='Samples per batch for transfer learning')
    # Default to 12/24 (same as asymmetric moderate that achieved 1.0278)
    ...
    # Print header
    print("=" * 70)
    print("SMART INIT SELECTION OPTIMIZER")
    print("=" * 70)
    ...
    print()
    print("KEY INNOVATION: Smart init selection")
    print("  - Evaluate ALL inits quickly (1 sim each)")
    print("  - Focus ALL fevals on best init")
    print("  - Expected: ~40% time reduction on 2-source")
    print()
    print(f"Config:")
    ...
    if is_g4dn_simulation:
        ...
        run_name = f"smart_init_{config['max_fevals_1src']}_{config['max_fevals_2src']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param("optimizer", "SmartInitOptimizer")
    ...
    print("RESULTS")
    print("=" * 70)
    print(f"Config:           {config['max_fevals_1src']}/{config['max_fevals_2src']} fevals (smart init selection)")
    ...
    print()
    print("Best init types (selected by smart init):")
    ...
    # Compare with baseline and asymmetric
    baseline_score = 0.9973
    baseline_time = 56.6
    asymmetric_score = 1.0278
    asymmetric_time = 81.2

    print(f"Baseline (15/20):         Score {baseline_score:.4f} @ {baseline_time:.1f} min")
    print(f"Asymmetric (12/24):       Score {asymmetric_score:.4f} @ {asymmetric_time:.1f} min (OVER BUDGET)")
    print(f"Smart Init ({config['max_fevals_1src']}/{config['max_fevals_2src']}):       Score {final_score:.4f} @ {projected_400:.1f} min")
    print()

    score_vs_baseline = final_score - baseline_score
    time_vs_asymmetric = projected_400 - asymmetric_time

    if final_score > 1.0 and projected_400 < 60:
        print("[SUCCESS] Score > 1.0 achieved within 60-min budget!")
        ...
        print(f"[INFO] Within budget but score not improved ({score_vs_baseline:+.4f})")
    else:
        print(f"[FAIL] Over budget by {projected_400 - 60:.1f} min")

    if projected_400 < asymmetric_time:
        print(f"[TIME SAVED] {-time_vs_asymmetric:.1f} min faster than asymmetric 12/24")
