"""
Experiment: perturbation_scale_tuning
Test different perturbation scales (0.04, 0.06) vs baseline 0.05.

Current best: sigma 0.18/0.22 + fevals 20/44 + 4 perturbations + scale 0.05 = 1.1437 @ 42.2 min
"""
...
def main():
    ...
    # Summary
    print("\n" + "="*60)
    print("SUMMARY: Perturbation Scale Tuning")
    print("="*60)
    print(f"Current best (scale=0.05): 1.1437 @ 42.2 min")
    print()
    for r in results:
        delta = r['score'] - CURRENT_BEST
        print(f"  scale={r['scale']}: {r['score']:.4f} @ {r['projected_400_min']:.1f} min ({delta:+.4f})")

    best = max(results, key=lambda x: x['score'])
    print()
    print(f"Best: scale={best['scale']} = {best['score']:.4f}")
