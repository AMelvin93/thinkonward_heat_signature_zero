"""
Run 2-3: Validate perturb_nm_iters=4 as potential new best
"""
...
def run_experiment(config, run_num, data):
    ...
    print(f"\n=== Validation Run {run_num}: perturb_nm_iters=4 ===")
...
def main():
    ...
    # Config with perturb_nm_iters=4
    config = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'max_fevals_1src': 20,
    ...
    run1 = {'run': 1, 'score': 1.1499, 'avg_n_cands': 2.85, 'projected_400_min': 36.5}
    ...
    print("\n" + "="*60)
    print("SUMMARY: Perturb NM Iters=4 Validation")
    print("="*60)
    print(f"Previous best (nm_iters=3): 1.1437 @ 42.2 min")
    print()
    for r in results:
    ...
    if np.mean(scores) > PREV_BEST:
        print(f"\nNEW BEST CONFIRMED! {improvement:.2f}% improvement")
    else:
