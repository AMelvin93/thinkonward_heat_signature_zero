"""
Experiment: perturb_nm_iters_tuning
Test different perturb_nm_iters (2, 4) vs baseline 3.

Current best: sigma 0.18/0.22 + fevals 20/44 + 4 perturbations + perturb_nm_iters=3 = 1.1437 @ 42.2 min
"""
...
def run_experiment(config, config_name, data):
    ...
    nm_iters = config.get('perturb_nm_iters')
    print(f"\nConfig: {config_name} (perturb_nm_iters={nm_iters})")
...
def main():
    ...
    # Test different perturb_nm_iters
    for nm_iters in [2, 4, 3]:
        config = {**base_config, 'perturb_nm_iters': nm_iters}
        name = f"nm_iters_{nm_iters}"
        result = run_experiment(config, name, data)
        results.append(result)

    # Summary
    print("\n" + "="*60)
    print("SUMMARY: Perturb NM Iters Tuning")
    print("="*60)
    print(f"Current best (perturb_nm_iters=3): 1.1437 @ 42.2 min")
    print()
    for r in results:
        delta = r['score'] - CURRENT_BEST
        print(f"  nm_iters={r['perturb_nm_iters']}: {r['score']:.4f} @ {r['projected_400_min']:.1f} min ({delta:+.4f})")

    best = max(results, key=lambda x: x['score'])
    print()
    print(f"Best: nm_iters={best['perturb_nm_iters']} = {best['score']:.4f}")
