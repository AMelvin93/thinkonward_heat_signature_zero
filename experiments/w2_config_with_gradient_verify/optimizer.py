"""
W2 Config with Gradient Verification Optimizer

Based on perturbation_plus_verification but with 40% temporal fidelity (W2 config).
Reduced NM polish to fit in budget.

Hypothesis: 40% temporal might improve accuracy over 25%, even with less polish.
"""
...
class W2ConfigWithVerificationOptimizer:
    """
    Optimizer with 40% temporal + gradient verification.
    Uses reduced NM polish to fit in budget.
    """
    ...
    def __init__(
        ...
        refine_maxiter: int = 5,  # Reduced from 8 to fit budget
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.4,
        rmse_threshold_2src: float = 0.5,
        timestep_fraction: float = 0.4,  # W2 config: 40% temporal
        # Minimal perturbation
        enable_perturbation: bool = True,
        perturb_top_n: int = 1,
        n_perturbations: int = 2,
        perturbation_scale: float = 0.05,
        perturb_nm_iters: int = 2,
        # Verification
