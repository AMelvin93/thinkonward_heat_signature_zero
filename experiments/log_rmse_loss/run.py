#!/usr/bin/env python3
"""Run script for Log-RMSE Loss experiment with MLflow logging."""
...
from optimizer import LogRmseLossOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = LogRmseLossOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Log-RMSE Loss Experiment')
    ...
    parser.add_argument('--no-log-loss', action='store_true', help='Use regular RMSE instead of log(1+RMSE)')
    ...
    use_log_loss = not args.no_log_loss
    loss_type = "log(1+RMSE)" if use_log_loss else "RMSE"

    print(f"\nLog-RMSE Loss Experiment")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"CMA-ES objective: {loss_type}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Log-RMSE Loss ({loss_type})")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (RMSE obj): 1.1688 @ 58.4 min (documented)")
    print(f"This run ({loss_type}): {score:.4f} @ {projected_400:.1f} min")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"log_rmse_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_LOG_RMSE_LOSS_001")
            mlflow.log_param("worker", "W2")
            mlflow.log_param("tuning_run", args.tuning_run)
            mlflow.log_param("platform", "wsl")
            mlflow.log_param("n_samples", n_samples)
            mlflow.log_param("objective", loss_type)
            mlflow.log_param("use_log_loss", use_log_loss)
