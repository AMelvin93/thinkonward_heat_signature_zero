"""
Log-RMSE Loss Optimizer - Uses log(1+RMSE) instead of RMSE for CMA-ES objective

Key insight: log(1+RMSE) is a MONOTONIC transformation of RMSE, so:
1. The minimum is at the same point (same optimum as RMSE)
2. But the landscape shape changes - large errors are compressed, small differences amplified
3. This may affect CMA-ES convergence behavior

The final score is still computed with regular RMSE.
"""
...
class LogRmseLossOptimizer:
    """
    Optimizer that uses log(1+RMSE) as CMA-ES objective.

    Key insight: log is monotonic, so same optimum as RMSE but different landscape.
    """
    ...
    def __init__(
        ...
        use_log_loss: bool = True,  # Key parameter
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """
        Run optimization with log(1+RMSE) objective for CMA-ES.
        """
        ...
        # Objective using log(1+RMSE) instead of RMSE
        if n_sources == 1:
            def objective_coarse(xy_params):
                ...
                # Return log(1+rmse) instead of rmse for CMA-ES
                if self.use_log_loss:
                    return np.log(1 + rmse)
                return rmse
        else:
            def objective_coarse(xy_params):
        ...
        for init_params, init_type in initializations:
            ...
            while not es.stop():
                ...
                for sol, fit in zip(solutions, fitness):
                    # Store the actual RMSE (not log) for ranking
                    if self.use_log_loss:
                        actual_rmse = np.exp(fit) - 1  # Inverse: rmse = exp(log(1+rmse)) - 1
                    else:
                        actual_rmse = fit
                    all_solutions.append((np.array(sol), actual_rmse, init_type))

        # Sort by actual RMSE
        all_solutions.sort(key=lambda x: x[1])

        # Refine top N solutions - use regular RMSE for NM polish
        if n_sources == 1:
            def objective_refine(xy_params):
                x, y = xy_params
                n_sims[0] += 1
                q, Y_pred, rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_coarse, dt, nt_reduced, T0, sensors_xy, q_range)
                return rmse  # Regular RMSE for NM
        else:
            def objective_refine(xy_params):
                x1, y1, x2, y2 = xy_params
                n_sims[0] += 2
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            loss_type = "log(1+RMSE)" if self.use_log_loss else "RMSE"
            print(f"  Using {loss_type} objective with {nt_reduced}/{nt_full} timesteps ({self.timestep_fraction*100:.0f}%)")
