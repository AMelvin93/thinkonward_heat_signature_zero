"""
W3 High Sigma Optimizer.

Key idea from W1's findings: Higher sigma for exploration helps find better solutions.
Combined with moderate thresholds (0.40/0.50) to balance accuracy and time.
"""
...
class HighSigmaOptimizer:
    """
    Optimizer with configurable sigma for exploration.

    Higher sigma helps explore the search space more effectively.
    """
    ...
    def __init__(
        ...
        sigma0_1src: float = 0.20,   # Higher default sigma
        sigma0_2src: float = 0.25,   # Higher default sigma
        ...
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.40,  # Moderate threshold
        rmse_threshold_2src: float = 0.50,  # Moderate threshold
        fallback_fevals_1src: int = 14,
        fallback_fevals_2src: int = 18,
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, early_frac, max_fevals_override=None, sigma_override=None):
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if best_rmse_initial > threshold:
            ...
            # Use slightly larger sigma for fallback exploration
