#!/usr/bin/env python3
"""
Run script for W3 High Sigma optimizer.

Based on W1's finding that higher sigma (0.20/0.25) helps exploration.
"""
...
from optimizer import HighSigmaOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = HighSigmaOptimizer(
        ...
        fallback_fevals_2src=config.get('fallback_fevals_2src', 18),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run High Sigma optimizer')
    ...
    parser.add_argument('--fallback-fevals-2src', type=int, default=18)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            flag = " [FB]" if result['best_rmse'] > args.threshold_1src and result['n_sources'] == 1 else ""
            flag = " [FB]" if result['best_rmse'] > args.threshold_2src and result['n_sources'] == 2 else flag
