"""
Temporal Fidelity Optimizer with Final Polish

Same as TemporalFidelityOptimizer but adds a final NM refinement stage
using FULL timesteps on the best candidates.
"""
...
class TemporalFidelityWithPolishOptimizer:
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # NO dissimilarity filtering - just take top 3 by RMSE
        # (testing hypothesis that top 3 by RMSE are naturally distinct)
        candidates_raw.sort(key=lambda c: c[2])  # Sort by RMSE
        final_candidates = candidates_raw[:N_MAX]  # Take top N_MAX (3)
