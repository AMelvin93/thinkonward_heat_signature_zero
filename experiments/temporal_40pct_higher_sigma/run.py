#!/usr/bin/env python3
"""
Run script for Temporal 40% + Higher Sigma experiment.

Tests the hypothesis that higher sigma combined with 40% temporal fidelity
can achieve better accuracy within budget.

Based on W2's successful temporal fidelity optimizer with polish.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run Temporal 40% + Higher Sigma experiment')
    ...
    # Sigma (KEY PARAMETER TO TUNE)
    parser.add_argument('--sigma0-1src', type=float, default=0.25)
    parser.add_argument('--sigma0-2src', type=float, default=0.30)
    # Temporal fidelity
    ...
    print(f"\nTemporal 40% + Higher Sigma Optimizer")
    print(f"=" * 70)
    ...
    if not args.no_mlflow:
        ...
        mlflow.set_experiment('temporal_40pct_higher_sigma')

        run_name = args.run_name or f"sigma_{args.sigma0_1src}_{args.sigma0_2src}_{datetime.now().strftime('%H%M%S')}"
