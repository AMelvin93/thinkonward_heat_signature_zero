"""
Progressive Perturbation Scale Optimizer

Tests different perturbation scales and progressive reduction.
Basin hopping research shows scale tuning is critical.
"""
...
class ProgressivePerturbationOptimizer:
    """Optimizer with configurable perturbation scale."""
    ...
    def __init__(
        ...
        perturbation_scale: float = 0.05,  # Can be overridden
    ...
    def _weighted_centroid_init(self, sample, n_sources):
        ...
        if n_sources == 1:
            ...
            return np.array([centroid[0] - offset, centroid[1], centroid[0] + offset, centroid[1]])
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                result = minimize(objective_coarse, pos_params, method='Nelder-Mead',
        ...
        if self.enable_perturbation:
            ...
            best_pos, best_rmse, best_init = refined_solutions[0]

            for _ in range(self.n_perturbations):
                perturbed = self._generate_perturbation(best_pos, lb, ub)
                result = minimize(objective_coarse, perturbed, method='Nelder-Mead',
                    options={'maxiter': self.perturb_nm_iters, 'xatol': 0.01, 'fatol': 0.001})
                if result.fun < best_rmse:
                    refined_solutions.append((result.x, result.fun, 'perturbed_improved'))
                    best_pos = result.x
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        candidates_raw, n_sims = self._run_single_optimization(
            sample, meta, q_range, solver_coarse, solver_fine, primary_inits, n_sources, nt_reduced)
        ...
        if best_rmse_initial > threshold:
            fallback_inits = [
                (self._weighted_centroid_init(sample, n_sources), 'centroid'),
                (self._random_init_positions(n_sources), 'random')]
            fallback_candidates, fallback_sims = self._run_single_optimization(
                sample, meta, q_range, solver_coarse, solver_fine, fallback_inits, n_sources, nt_reduced)
        ...
        results = [CandidateResult(params=c[1], rmse=c[2], init_type=c[3],
