"""
Run script for progressive_perturbation_scale experiment.

Tests different perturbation scales:
1. Large scale (0.10) - broader exploration
2. Medium scale (0.05) - baseline
3. Small scale (0.02) - fine refinement
"""
...
from optimizer import ProgressivePerturbationOptimizer
...
def process_sample(args):
    ...
    optimizer = ProgressivePerturbationOptimizer(**config)
    try:
...
def run_experiment(config, config_name, data):
    ...
    print(f"Parameters: perturbation_scale={config.get('perturbation_scale', 0.05)}")
...
def main():
    ...
    configs = [
        ({**base_config, 'perturbation_scale': 0.10}, 'scale_010_large'),
        ({**base_config, 'perturbation_scale': 0.05}, 'scale_005_medium'),
        ({**base_config, 'perturbation_scale': 0.02}, 'scale_002_small'),
    ]
