#!/usr/bin/env python3
"""
Run script for Weighted Sensor Loss optimizer.

Tests the hypothesis that weighting sensors by informativeness (variance, mean temp)
can improve CMA-ES convergence.
"""
...
from optimizer import WeightedSensorLossOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = WeightedSensorLossOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Weighted Sensor Loss optimizer')
    ...
    # Weighting strategy
    parser.add_argument('--weighting-strategy', type=str, default='variance',
                        choices=['uniform', 'variance', 'mean_temp', 'gradient', 'sqrt_variance'])
    # Shared parameters
    ...
    print(f"\nWeighted Sensor Loss Optimizer")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Weighting strategy: {args.weighting_strategy}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Weighted Sensor Loss ({args.weighting_strategy})")
    print(f"{'='*70}")
    ...
    run_name = args.mlflow_run_name or f"weighted_sensor_{args.weighting_strategy}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        mlflow.log_param("experiment_id", "EXP_WEIGHTED_LOSS_001")
        mlflow.log_param("worker", "W1")
        mlflow.log_param("algorithm", "weighted_sensor_loss")
        mlflow.log_param("platform", "wsl")
        mlflow.log_param("weighting_strategy", args.weighting_strategy)
