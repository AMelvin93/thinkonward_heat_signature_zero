#!/usr/bin/env python
"""Run A12: Guaranteed Diversity Generator experiment."""
...
from experiments.diversity_guaranteed.optimizer import DiversityGuaranteedOptimizer, extract_enhanced_features, N_MAX
...
def process_sample(sample, meta, config, history_1src, history_2src):
    optimizer = DiversityGuaranteedOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Guaranteed Diversity Optimizer')
    ...
    print("=" * 70)
    print("A12: GUARANTEED DIVERSITY GENERATOR")
    print("=" * 70)
    ...
    print()
    print("KEY INNOVATION: Structurally diverse initializations")
    print("  - Triangulation (physics-based)")
    print("  - Left-Right split (horizontal separation)")
    print("  - Top-Bottom split (vertical separation)")
    print("  - Transfer learning from history")
    print("=" * 70)
    ...
    # Count init types across all candidates
    init_type_counts = {}
    for r in all_results:
    ...
    if is_g4dn and not args.max_samples:
        ...
        run_name = f"diversity_guaranteed_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param("optimizer", "DiversityGuaranteedOptimizer")
