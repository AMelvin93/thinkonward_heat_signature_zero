#!/usr/bin/env python3
"""Run script for CMA-ES with Best Solution Injection on Restart."""
...
from optimizer import CMAESRestartInjectBestOptimizer
...
def process_single_sample(args):
    ...
    optimizer = CMAESRestartInjectBestOptimizer(
        ...
        inject_best_on_fallback=config['inject_best_on_fallback'],
    )
    start = time.time()
    try:
        ...
        has_injected = 'injected' in init_types
        return {
            ...
            'has_injected': has_injected, 'success': True,
        }
        ...
        return {
            ...
            'has_injected': False, 'success': False, 'error': str(e),
            'traceback': traceback.format_exc(),
        }


def main():
    parser = argparse.ArgumentParser(description='Run CMA-ES with Best Solution Injection')
    ...
    parser.add_argument('--no-inject', action='store_true',
                       help='Disable injection (for comparison)')
    ...
    print(f"Inject best on fallback: {not args.no_inject}")
    print(f"=" * 60)
    ...
    config = {
        ...
        'inject_best_on_fallback': not args.no_inject,
    }
    ...
    # Count how often injection was used
    n_injected = sum(1 for r in results if r['success'] and r.get('has_injected', False))
    print(f"Injection used: {n_injected}/{len([r for r in results if r['success']])}")
    ...
    if not args.no_mlflow:
        try:
            ...
            run_name = f"cmaes_restart_inject_best_run1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            with mlflow.start_run(run_name=run_name) as run:
                ...
                mlflow.log_metric("n_injected", n_injected)

                mlflow.log_param("experiment_id", "EXP_CMAES_WITH_RESTART_BEST_001")
                mlflow.log_param("worker", "W1")
                mlflow.log_param("tuning_run", 1)
                mlflow.log_param("optimizer", "cmaes_restart_inject_best")
                ...
                mlflow.log_param("inject_best_on_fallback", not args.no_inject)
