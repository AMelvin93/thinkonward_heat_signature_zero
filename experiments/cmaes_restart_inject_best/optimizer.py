"""
CMA-ES with Best Solution Injection on Restart

Key modification: When triggering fallback (due to poor primary results),
inject the best solution found so far into the new CMA-ES population.

This may help by:
- Seeding the new search with known good starting point
...
class CMAESRestartInjectBestOptimizer:
    """
    Optimizer that injects best solution when doing fallback restart.
    """
    ...
    def __init__(
        ...
        # NEW: Injection control
        inject_best_on_fallback: bool = True,
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.inject_best_on_fallback = inject_best_on_fallback
    ...
    def _run_optimization_with_injection(self, sample, meta, q_range, solver_coarse, solver_fine,
                                         initializations, n_sources, nt_reduced,
                                         inject_solution=None):
        """
        Run optimization pass with optional solution injection.

        Args:
            inject_solution: If provided, inject this solution into the CMA-ES population
        """
        ...
        for init_params, init_type in initializations:
            ...
            # NEW: Inject best solution if provided
            if inject_solution is not None and len(inject_solution) == len(init_params):
                try:
                    # Inject solution with fitness evaluation
                    inject_fit = objective_coarse(inject_solution)
                    es.inject([inject_solution.tolist()], force=True)
                    all_solutions.append((inject_solution.copy(), inject_fit, 'injected'))
                except Exception as e:
                    pass  # Injection failed, continue normally
        ...
        return candidates_raw, n_sims[0], all_solutions[0][0] if all_solutions else None
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run primary optimization (no injection)
        candidates_raw, n_sims, best_pos_params = self._run_optimization_with_injection(
            sample, meta, q_range, solver_coarse, solver_fine,
            primary_inits, n_sources, nt_reduced, inject_solution=None
        )
        ...
        if best_rmse_initial > threshold:
            # Fallback: try alternative initializations WITH best solution injection
            fallback_inits = []
            ...
            # NEW: Inject best position params from primary run
            inject_solution = best_pos_params if self.inject_best_on_fallback else None

            fallback_candidates, fallback_sims, _ = self._run_optimization_with_injection(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, nt_reduced, inject_solution=inject_solution
            )
