#!/usr/bin/env python3
"""
Run script for Adaptive Population Size CMA-ES optimizer.

Tests the hypothesis that starting with large popsize (exploration)
then reducing (exploitation) improves convergence.
"""
...
from optimizer import AdaptivePopsizeOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = AdaptivePopsizeOptimizer(
        ...
        popsize_phase1=config['popsize_phase1'],
        popsize_phase2=config['popsize_phase2'],
...
def main():
    parser = argparse.ArgumentParser(description='Run Adaptive Population Size CMA-ES optimizer')
    ...
    # Adaptive popsize parameters
    parser.add_argument('--popsize-phase1', type=int, default=12)
    parser.add_argument('--popsize-phase2', type=int, default=6)
    parser.add_argument('--phase1-fraction', type=float, default=0.60)
    # Shared parameters
    ...
    print(f"Popsize: Phase1={args.popsize_phase1} ({args.phase1_fraction:.0%}), Phase2={args.popsize_phase2}")
    ...
    config = {
        ...
        'popsize_phase1': args.popsize_phase1,
        'popsize_phase2': args.popsize_phase2,
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Adaptive Popsize (P1={args.popsize_phase1}/{args.phase1_fraction:.0%}, P2={args.popsize_phase2})")
    print(f"{'='*70}")
    ...
    run_name = args.mlflow_run_name or f"adaptive_popsize_p{args.popsize_phase1}_{args.popsize_phase2}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        mlflow.log_param("experiment_id", "EXP_ADAPTIVE_POPSIZE_001")
        mlflow.log_param("worker", "W1")
        mlflow.log_param("algorithm", "adaptive_popsize_cmaes")
        mlflow.log_param("platform", "wsl")
        mlflow.log_param("popsize_phase1", args.popsize_phase1)
        mlflow.log_param("popsize_phase2", args.popsize_phase2)
        mlflow.log_param("phase1_fraction", args.phase1_fraction)
