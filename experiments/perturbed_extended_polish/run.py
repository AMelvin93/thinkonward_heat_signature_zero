"""
Run script for perturbed_extended_polish experiment.

Building on perturbed_local_restart (1.1452 @ 47.7 min) with extended polish
to use remaining 12+ min budget.
...
def process_single_sample(args):
    ...
    optimizer = PerturbedLocalRestartOptimizer(
        ...
        refine_maxiter=config.get('refine_maxiter', 10),
...
def main():
    ...
    parser.add_argument('--refine-maxiter', type=int, default=10, help='NM polish iterations')
    parser.add_argument('--sigma0-1src', type=float, default=0.15, help='Initial sigma for 1-source')
    parser.add_argument('--sigma0-2src', type=float, default=0.20, help='Initial sigma for 2-source')
    ...
    print(f"\n{'='*60}")
    print(f"PERTURBED EXTENDED POLISH EXPERIMENT")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Perturbed Extended Polish")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (perturbed_local_restart): 1.1452 @ 47.7 min")
    print(f"This run:                           {score:.4f} @ {projected_400:.1f} min")
    print(f"Delta vs baseline:                  {score - 1.1452:+.4f} score, {projected_400 - 47.7:+.1f} min")
    print()
    ...
    if state_path.exists():
        ...
        state = {
            'experiment': 'perturbed_extended_polish',
            'experiment_id': 'EXP_PERTURBED_EXTENDED_POLISH_001',
            'claimed_by': 'W1',
    ...
    if not args.no_mlflow:
        ...
        run_name = f"perturbed_extended_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param('experiment_name', 'perturbed_extended_polish')
            mlflow.log_param('experiment_id', 'EXP_PERTURBED_EXTENDED_POLISH_001')
