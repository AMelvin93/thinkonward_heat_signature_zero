"""
Expanded Candidate Pool via Perturbation

Hypothesis: Generate more candidates by perturbing top-K from CMA-ES,
then select best subset from the expanded pool.

Different from baseline which perturbs only top 1 with multiple perturbations.
This approach perturbs top 3, each once, for broader exploration.
"""
...
class ExpandedPoolOptimizer:
    """
    Optimizer that expands candidate pool via perturbation of top-K.
    """
    ...
    def __init__(
        ...
        sigma0_1src: float = 0.18,  # Use proven higher sigma
        sigma0_2src: float = 0.22,
        use_triangulation: bool = True,
        n_candidates: int = N_MAX,
        candidate_pool_size: int = 10,
        refine_maxiter: int = 8,  # Standard NM polish
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.4,
        rmse_threshold_2src: float = 0.5,
        timestep_fraction: float = 0.40,  # 40% temporal fidelity
        # Expanded pool parameters
        expand_top_n: int = 3,  # How many candidates to perturb
        n_perturbations_per_candidate: int = 1,  # Perturbations per candidate
        perturbation_scale: float = 0.05,  # Position perturbation magnitude
        perturb_nm_iters: int = 4,  # NM iterations for perturbed points
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.n_perturbations_per_candidate = n_perturbations_per_candidate
    ...
    def _generate_perturbations(self, pos_params, n_sources, lb, ub):
        ...
        perturbations = []
        for _ in range(self.n_perturbations_per_candidate):
            # Random direction
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run CMA-ES optimization with expanded candidate pool via perturbation."""
        ...
        # Refine top N solutions with standard NM polish (using COARSE grid)
        refined_solutions = []
        ...
        for idx in range(n_to_perturb):
            ...
            for perturbed_pos in perturbations:
                ...
                expanded_candidates.append((result.x, result.fun, f'perturbed_from_{idx}'))

        # Add expanded candidates to pool
        refined_solutions.extend(expanded_candidates)

        # Convert to full candidates (with intensities) using FINE grid
        candidates_raw = []
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Expanding pool: top {self.expand_top_n} candidates, {self.n_perturbations_per_candidate} perturbations each")
        ...
        # Run optimization with expanded pool
