...
Key experiment: Combine higher sigma (0.20/0.25) from W1's finding
with proven baseline thresholds (0.35/0.45).

Hypothesis: Higher sigma improves exploration while baseline thresholds
catch outliers more aggressively than W1's 0.40/0.50.
"""
...
class SigmaBaselineOptimizer:
    """
    Optimizer combining higher sigma with baseline thresholds.

    Key parameters from queue:
    - sigma0_1src: 0.20 (higher than baseline 0.15)
    - sigma0_2src: 0.25 (higher than baseline 0.20)
    ...
    def __init__(
        ...
        sigma0_1src: float = 0.20,      # Higher than baseline (0.15)
        sigma0_2src: float = 0.25,      # Higher than baseline (0.20)
        ...
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.35,  # Baseline threshold
        rmse_threshold_2src: float = 0.45,  # Baseline threshold
        fallback_fevals: int = 18,          # Fallback budget
    ):
        self.Lx = Lx
        self.Ly = Ly
