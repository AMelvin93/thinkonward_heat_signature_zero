#!/usr/bin/env python
"""Run Better 2-Source Init Optimizer experiment."""
...
from experiments.better_2src_init.optimizer import (
    Better2SrcOptimizer,
...
def process_sample(sample, meta, config, history_1src, history_2src):
    optimizer = Better2SrcOptimizer(
        ...
        use_advanced_init=config.get('use_advanced_init', True),
    )
...
def main():
    parser = argparse.ArgumentParser(description='Run Better 2-Source Init Optimizer')
    ...
    parser.add_argument('--no-advanced-init', action='store_true')
    ...
    config = {
        ...
        'k_similar': 1,
        'use_advanced_init': not args.no_advanced_init,
    }
    ...
    print("=" * 70)
    print("BETTER 2-SOURCE INIT OPTIMIZER (A5)")
    print("=" * 70)
    ...
    print(f"Advanced init: {'ON' if config['use_advanced_init'] else 'OFF'}")
    print("=" * 70)
    ...
    for batch_idx in range(n_batches):
        ...
        for result in batch_results:
            ...
            init_type = result['best_init_type']
    ...
    if is_g4dn_simulation and not args.max_samples:
        ...
        run_name = f"better_2src_init_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for init_type, count in sorted(init_type_counts.items(), key=lambda x: -x[1]):
        print(f"  {init_type}: {count} ({100*count/n_samples:.1f}%)")
    print()
