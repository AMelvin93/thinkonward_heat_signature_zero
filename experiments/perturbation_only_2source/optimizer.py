...
class TabuMemory:
    """Tabu memory for tracking explored positions."""

    def __init__(self, tabu_distance: float = 0.03):
        self.explored_positions = []
        self.tabu_distance = tabu_distance

    def add(self, pos_params):
        """Add a position to the tabu list."""
        self.explored_positions.append(pos_params.copy())

    def is_tabu(self, pos_params):
        """Check if position is too close to any explored position."""
        for explored in self.explored_positions:
            dist = np.linalg.norm(pos_params - explored)
            if dist < self.tabu_distance:
                return True
        return False
...
class TabuBasinHoppingOptimizer:
    ...
    def __init__(
        ...
        perturb_only_2src: bool = False,  # Only apply perturbation for 2-source problems
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _generate_tabu_aware_perturbation(self, pos_params, lb, ub, tabu_memory):
        """Generate a perturbation that avoids tabu regions."""
        for attempt in range(self.max_tabu_attempts):
            # Random direction
            ...
            # Check if this position is not tabu
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # === TABU BASIN HOPPING ===
        # Only apply perturbation if enabled and (n_sources == 2 or not perturb_only_2src)
        should_perturb = self.enable_tabu_hopping and (n_sources == 2 or not self.perturb_only_2src)
        if should_perturb:
