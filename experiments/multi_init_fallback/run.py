#!/usr/bin/env python3
"""
Run script for Multi-Init Fallback optimizer.
"""
...
from optimizer import MultiInitFallbackOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = MultiInitFallbackOptimizer(
        ...
        fallback_fevals_1src=config.get('fallback_fevals_1src', 12),
        ...
        n_fallback_inits=config.get('n_fallback_inits', 2),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Multi-Init Fallback optimizer')
    ...
    parser.add_argument('--n-fallback-inits', type=int, default=2)
    ...
    print(f"Fallback inits: {args.n_fallback_inits}")
    print(f"=" * 60)
    ...
    config = {
        ...
        'n_fallback_inits': args.n_fallback_inits,
    }
