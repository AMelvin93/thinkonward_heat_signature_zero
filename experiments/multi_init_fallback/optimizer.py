"""
Multi-Init Fallback Optimizer.

Key idea: If the initial run with triangulation + smart inits has high RMSE,
add additional random initializations and rerun. This provides more exploration
opportunities for difficult samples.
"""
...
class MultiInitFallbackOptimizer:
    """
    Optimizer with multi-init fallback strategy.

    Primary run uses triangulation + smart inits.
    If RMSE is high, fallback adds random inits with larger sigma.
    """
    ...
    def __init__(
        ...
        fallback_sigma_1src: float = 0.30,
        fallback_sigma_2src: float = 0.35,
        ...
        fallback_fevals_1src: int = 12,
        fallback_fevals_2src: int = 20,
        n_fallback_inits: int = 2,
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.fallback_sigma_1src = fallback_sigma_1src
        self.fallback_sigma_2src = fallback_sigma_2src
        ...
        self.fallback_fevals_1src = fallback_fevals_1src
        self.fallback_fevals_2src = fallback_fevals_2src
        self.n_fallback_inits = n_fallback_inits
    ...
    def _run_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                          initializations, n_sources, early_frac, max_fevals, sigma0,
                          do_refine=True):
        ...
        # Refine top N on coarse grid
        refined_solutions = []
        refine_n = self.refine_top_n if do_refine else 0

        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:refine_n]):
        ...
        start_idx = refine_n
        for pos_params, rmse_coarse, init_type in all_solutions[start_idx:self.candidate_pool_size]:
            refined_solutions.append((pos_params, rmse_coarse, init_type))
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Fallback run parameters
        fallback_sigma = self.fallback_sigma_1src if n_sources == 1 else self.fallback_sigma_2src
        fallback_fevals = self.fallback_fevals_1src if n_sources == 1 else self.fallback_fevals_2src

        # Build primary initializations
        primary_inits = []
        ...
        candidates_raw, n_sims = self._run_optimization(
            ...
            primary_inits, n_sources, early_frac, max_fevals, sigma0,
            do_refine=True
        )
        ...
        if best_rmse_initial > threshold:
            # Fallback: add random inits and run with larger sigma
            fallback_inits = []
            for i in range(self.n_fallback_inits):
                rand_init = self._random_init_positions(n_sources)
                fallback_inits.append((rand_init, f'random_{i}'))

            # Also add perturbed versions of current best
            best_pos = candidates_raw[0][1][:2*n_sources]  # Get position params
            perturbed = best_pos + np.random.normal(0, 0.1 * self.Lx, size=best_pos.shape)
            perturbed = np.clip(perturbed,
                               [0.05 * self.Lx, 0.05 * self.Ly] * n_sources,
                               [0.95 * self.Lx, 0.95 * self.Ly] * n_sources)
            fallback_inits.append((perturbed, 'perturbed'))

            fallback_candidates, fallback_sims = self._run_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, early_frac, fallback_fevals, fallback_sigma,
                do_refine=False
            )
