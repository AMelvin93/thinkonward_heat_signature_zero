...
Key idea: Use aggressive thresholds to catch outliers early, but keep
the fallback run lean (fewer fevals) with larger sigma for exploration.

This allows using the best thresholds (0.30/0.38) from sigma_fallback
while staying within budget by reducing fallback computational cost.

Config:
- Primary: 36 fevals with sigma=0.20
- Fallback: 20 fevals with sigma=0.35 (lean but exploratory)
- Thresholds: 0.30/0.38 (aggressive to catch outliers)
"""
...
class LeanSigmaFallbackOptimizer:
    """
    Optimizer with lean sigma-based fallback strategy.

    Uses aggressive thresholds to catch outliers but keeps fallback
    computationally lean by using fewer fevals with larger sigma.
    """
    ...
    def __init__(
        ...
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.30,
        rmse_threshold_2src: float = 0.38,
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _run_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Refine top N on coarse grid (only for primary run)
        refined_solutions = []
        ...
        # Add remaining solutions (no refinement for fallback)
        start_idx = refine_n
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run primary optimization with normal sigma
        ...
        if best_rmse_initial > threshold:
            # Lean fallback: fewer fevals, larger sigma, no refinement
            fallback_candidates, fallback_sims = self._run_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                primary_inits, n_sources, early_frac, fallback_fevals, fallback_sigma,
                do_refine=False  # Skip refinement to save time
            )
