...
in inverse problems. Based on the temporal fidelity optimizer with added
popsize parameter.

Research source: Oxford Academic notes that 'convergence can be further improved
by using larger population sizes' for inverse problems.
"""
...
class LargerPopulationOptimizer:
    """
    Optimizer with configurable CMA-ES population size.

    Key hypothesis: Larger populations improve covariance estimation
    and convergence quality in inverse problems.

    Default CMA-ES popsize:
    - 1-source (dim=2): ~6
    - 2-source (dim=4): ~8

    This optimizer allows testing larger values (12, 16, 20).
    """
    ...
    def __init__(
        ...
        # NEW: Population size parameters
        popsize_1src: int = None,  # None = use CMA-ES default
        popsize_2src: int = None,  # None = use CMA-ES default
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.popsize_1src = popsize_1src
        self.popsize_2src = popsize_2src
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        popsize = self.popsize_1src if n_sources == 1 else self.popsize_2src
        ...
        for init_params, init_type in initializations:
            ...
            opts['tolfun'] = 1e-6
            opts['tolx'] = 1e-6
            # NEW: Set population size if specified
