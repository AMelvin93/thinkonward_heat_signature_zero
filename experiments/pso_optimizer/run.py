#!/usr/bin/env python3
"""
Run script for PSO-based optimizer.

Tests Particle Swarm Optimization as alternative to CMA-ES.
"""
...
def process_single_sample(args):
    ...
    optimizer = PSOOptimizer(
        ...
        n_particles=config.get('n_particles', 8),
        pso_iterations=config.get('pso_iterations', 8),
...
def main():
    parser = argparse.ArgumentParser(description='Run PSO-based optimizer')
    ...
    parser.add_argument('--pso-iterations', type=int, default=8)
    ...
    print(f"Particles: {args.n_particles}, Iterations: {args.pso_iterations}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: PSO-based Optimizer")
    print(f"{'='*70}")
    ...
    print()
    print(f"CMA-ES Baseline: 1.1247 @ 57.2 min")
    print(f"This PSO run:    {score:.4f} @ {projected_400:.1f} min")
    print(f"Delta:           {score - 1.1247:+.4f} score, {projected_400 - 57.2:+.1f} min")
    print()
    if projected_400 > 60:
        print("STATUS: OVER BUDGET")
    elif score >= 1.10:
        print("STATUS: PROMISING (score >= 1.10 target)")
    elif score >= 0.95:
        print("STATUS: MEDIOCRE (above abort threshold)")
    else:
        print("STATUS: ABORT (score < 0.95)")
    print(f"{'='*70}\n")
