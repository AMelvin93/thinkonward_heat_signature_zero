#!/usr/bin/env python3
"""
Run script for W1 High Sigma Less Fallback optimizer.

EXP005: Testing higher thresholds (0.42/0.52) with reduced fallback_fevals (14).
Goal: Fit within 60 min budget while keeping higher sigma benefits.
Config:
- sigma0_1src: 0.20
- sigma0_2src: 0.25
- threshold_1src: 0.42
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 High Sigma Less Fallback optimizer')
    ...
    parser.add_argument('--threshold-1src', type=float, default=0.42)
    parser.add_argument('--threshold-2src', type=float, default=0.52)
    parser.add_argument('--fallback-fevals', type=int, default=14)
    ...
    print(f"\nW1 High Sigma Less Fallback Optimizer (EXP005)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 High Sigma Less Fallback (EXP005)")
    print(f"{'='*70}")
