"""
W3 Asymmetric Budget Optimizer.

Key idea: Allocate fewer fevals to easier 1-source problems and more fevals
to harder 2-source problems. This should improve 2-source accuracy without
significantly impacting time.

Hypothesis:
- 1-source problems are easier to converge (simpler search space)
- 2-source problems need more exploration (60% of samples, higher variance)
- Asymmetric allocation can improve overall score within time budget
"""
...
class AsymmetricBudgetOptimizer:
    """
    Optimizer with asymmetric function evaluation budget.

    Allocates fewer fevals to 1-source problems (easier) and more to
    2-source problems (harder, higher variance).
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        max_fevals_1src: int = 16,    # Reduced from 20
        max_fevals_2src: int = 40,    # Increased from 36
        ...
        fallback_fevals_1src: int = 14,   # Fallback budget for 1-src
        fallback_fevals_2src: int = 20,   # Fallback budget for 2-src
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if best_rmse_initial > threshold:
            # Fallback: try alternative initializations with asymmetric budget
            fallback_inits = []
            ...
            # Use asymmetric fallback budget
            ...
            fallback_candidates, fallback_sims = self._run_single_optimization(
                ...
                max_fevals_override=fallback_fevals
            )
