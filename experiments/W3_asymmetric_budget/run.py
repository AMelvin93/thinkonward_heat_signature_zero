#!/usr/bin/env python3
"""
Run script for W3 Asymmetric Budget optimizer.

Key experiment: Test asymmetric function evaluation allocation:
- Fewer fevals for 1-source problems (easier to converge)
- More fevals for 2-source problems (harder, higher variance)
"""
...
from optimizer import AsymmetricBudgetOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = AsymmetricBudgetOptimizer(
        ...
        fallback_fevals_1src=config.get('fallback_fevals_1src', 14),
        fallback_fevals_2src=config.get('fallback_fevals_2src', 20),
    )
    start = time.time()
    try:
...
def main():
    ...
    parser.add_argument('--fallback-fevals-1src', type=int, default=14)
    ...
    print(f"Fallback fevals: 1-src={args.fallback_fevals_1src}, 2-src={args.fallback_fevals_2src}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            flag = " [FB]" if result['best_rmse'] > 0.35 and result['n_sources'] == 1 else ""
            flag = " [FB]" if result['best_rmse'] > 0.45 and result['n_sources'] == 2 else flag
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W3 Asymmetric Budget")
    print(f"{'='*70}")
