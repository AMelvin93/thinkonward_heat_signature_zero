"""
Asymmetric NM Iterations Optimizer

Tests different NM polish iteration counts for 1-source vs 2-source problems.

Hypothesis: 2-source problems have more complex landscapes and may benefit
from more NM polish iterations.
"""
...
class AsymmetricNMIterationsOptimizer:
    """
    Optimizer with asymmetric NM iterations for 1-src vs 2-src problems.
    """
    ...
    def __init__(
        ...
        self.nm_iters_1src = nm_iters_1src
        self.nm_iters_2src = nm_iters_2src
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Select NM iterations based on n_sources
        nm_iters = self.nm_iters_1src if n_sources == 1 else self.nm_iters_2src
        ...
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if nm_iters > 0:
                result = minimize(
                    ...
                    method='Nelder-Mead',
                    options={
                        'maxiter': nm_iters,  # Use asymmetric iterations
                        'xatol': 0.01,
                        'fatol': 0.001,
                    }
                )
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  NM iterations: {nm_iters} for {n_sources}-source")
