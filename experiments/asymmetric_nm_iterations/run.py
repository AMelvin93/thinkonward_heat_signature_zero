"""
Run script for asymmetric_nm_iterations experiment.

Tests different NM polish iterations for 1-src vs 2-src problems.
"""
...
from optimizer import AsymmetricNMIterationsOptimizer
...
def process_single_sample(args):
    ...
    optimizer = AsymmetricNMIterationsOptimizer(
        nm_iters_1src=config['nm_iters_1src'],
        nm_iters_2src=config['nm_iters_2src'],
...
def main():
    ...
    parser.add_argument('--nm-iters-1src', type=int, default=8)
    parser.add_argument('--nm-iters-2src', type=int, default=8)
    ...
    print(f"\n{'='*60}")
    print(f"ASYMMETRIC NM ITERATIONS EXPERIMENT")
    print(f"{'='*60}")
    print(f"Samples: {n_samples} ({n_1src} 1-source, {n_2src} 2-source)")
    print(f"Workers: {args.workers}")
    print(f"NM iters: {args.nm_iters_1src} for 1-src, {args.nm_iters_2src} for 2-src")
    print(f"{'='*60}")

    config = {
        'nm_iters_1src': args.nm_iters_1src,
        'nm_iters_2src': args.nm_iters_2src,
        'max_fevals_1src': 20,
        'max_fevals_2src': 36,
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Asymmetric NM Iterations")
    print(f"{'='*70}")
