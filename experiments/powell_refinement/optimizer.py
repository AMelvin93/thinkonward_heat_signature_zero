"""
Multi-Fidelity with Powell Refinement.

Powell's method is a conjugate direction method that's efficient for
smooth functions without needing derivatives. May be more efficient
than Nelder-Mead for our heat source problem.
"""
...
class PowellRefinementOptimizer:
    """
    Multi-fidelity optimizer with Powell refinement on coarse grid.

    Powell's method is a conjugate direction method that may be more
    efficient than Nelder-Mead for smooth functions like our heat objective.
    """
    ...
    def __init__(
        ...
        refine_maxfev: int = 15,  # Max function evals for Powell
        ...
        self.refine_maxfev = refine_maxfev
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # ===== Powell refinement on top N solutions on COARSE grid =====
        refined_solutions = []
        bounds_scipy = list(zip(lb, ub))

        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxfev > 0:
                result = minimize(
                    objective_coarse,
                    pos_params,
                    method='Powell',
                    bounds=bounds_scipy,
                    options={
                        'maxfev': self.refine_maxfev,
                        'ftol': 1e-4,
                        'xtol': 1e-3,
                    }
                )
                if result.fun < rmse_coarse:
                    refined_solutions.append((result.x, result.fun, 'powell_refined'))
                else:
