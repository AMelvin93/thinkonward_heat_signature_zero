#!/usr/bin/env python3
"""
Run script for Sigma Ladder experiment.

Tests: Start with high sigma (0.30/0.35) and reduce to low sigma (0.15/0.20)
during CMA-ES optimization, rather than letting CMA-ES adapt naturally.
"""
...
from optimizer import SigmaLadderOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = SigmaLadderOptimizer(
        ...
        sigma_start_1src=config['sigma_start_1src'],
        sigma_start_2src=config['sigma_start_2src'],
        sigma_end_1src=config['sigma_end_1src'],
        sigma_end_2src=config['sigma_end_2src'],
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Sigma Ladder Experiment')
    ...
    parser.add_argument('--sigma-start-1src', type=float, default=0.30)
    parser.add_argument('--sigma-start-2src', type=float, default=0.35)
    parser.add_argument('--sigma-end-1src', type=float, default=0.15)
    parser.add_argument('--sigma-end-2src', type=float, default=0.20)
    ...
    config = {
        ...
        'sigma_start_1src': args.sigma_start_1src,
        'sigma_start_2src': args.sigma_start_2src,
        'sigma_end_1src': args.sigma_end_1src,
        'sigma_end_2src': args.sigma_end_2src,
    }

    print(f"\nSigma Ladder Experiment")
    print(f"=" * 60)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"CMA-ES fevals: {args.max_fevals_1src}/{args.max_fevals_2src}")
    print(f"Sigma schedule: {args.sigma_start_1src}/{args.sigma_start_2src} -> {args.sigma_end_1src}/{args.sigma_end_2src}")
    ...
    run_name = f"sigma_ladder_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        mlflow.log_param("experiment_id", "EXP_SIGMA_LADDER_001")
        ...
        mlflow.log_param("sigma_start_1src", args.sigma_start_1src)
        mlflow.log_param("sigma_start_2src", args.sigma_start_2src)
        mlflow.log_param("sigma_end_1src", args.sigma_end_1src)
        mlflow.log_param("sigma_end_2src", args.sigma_end_2src)
        ...
        print(f"\n{'='*70}")
        print(f"RESULTS - Sigma Ladder ({args.sigma_start_1src}/{args.sigma_start_2src} -> {args.sigma_end_1src}/{args.sigma_end_2src})")
        print(f"{'='*70}")
        ...
        print()
        print(f"Baseline:   1.1688 @ 58.4 min (fixed sigma 0.15/0.20)")
