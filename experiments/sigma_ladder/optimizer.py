"""
Sigma Ladder Optimizer

Same as TemporalFidelityWithPolishOptimizer but implements a sigma ladder:
- Start with high sigma (0.30/0.35) for broad exploration
- Reduce to low sigma (0.15/0.20) for fine-tuning

This tests whether manual sigma scheduling can improve on CMA-ES's natural adaptation.
"""
...
class SigmaLadderOptimizer:
    """
    Sigma Ladder optimizer with decreasing sigma schedule.

    Key difference: Starts with high sigma (0.30/0.35) for broad exploration,
    then reduces to low sigma (0.15/0.20) for fine-tuning.
    """
    ...
    def __init__(
        ...
        # Sigma ladder parameters
        sigma_start_1src: float = 0.30,  # Start high
        sigma_start_2src: float = 0.35,
        sigma_end_1src: float = 0.15,    # End at baseline
        sigma_end_2src: float = 0.20,
        ...
        self.sigma_start_1src = sigma_start_1src
        self.sigma_start_2src = sigma_start_2src
        self.sigma_end_1src = sigma_end_1src
        self.sigma_end_2src = sigma_end_2src
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run optimization with sigma ladder schedule."""
        ...
        sigma_start = self.sigma_start_1src if n_sources == 1 else self.sigma_start_2src
        sigma_end = self.sigma_end_1src if n_sources == 1 else self.sigma_end_2src
        ...
        for init_params, init_type in initializations:
            eval_count[0] = 0  # Reset for each init
            ...
            # Start with HIGH sigma
            es = cma.CMAEvolutionStrategy(init_params.tolist(), sigma_start, opts)
            ...
            while not es.stop():
                ...
                # Apply sigma ladder: linear decay from sigma_start to sigma_end
                # based on fraction of fevals used
                frac_done = eval_count[0] / fevals_per_init
                target_sigma = sigma_start + (sigma_end - sigma_start) * frac_done

                # Scale sigma towards target (smooth transition)
                if es.sigma > target_sigma:
                    # Only reduce, don't increase (let CMA-ES increase if needed)
                    decay_factor = target_sigma / es.sigma
                    # Apply partial decay to avoid too aggressive reduction
                    es.sigma *= max(0.85, decay_factor)
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run optimization with sigma ladder
