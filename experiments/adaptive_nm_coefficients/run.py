#!/usr/bin/env python3
"""
Run script for Adaptive NM Coefficients optimizer.

Tests scipy's adaptive=True option for Nelder-Mead polish.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run Adaptive NM Coefficients optimizer')
    ...
    print(f"\nAdaptive NM Coefficients Optimizer")
    print(f"=" * 60)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Timestep fraction: {args.timestep_fraction:.0%}")
    print(f"Final polish: {args.final_polish_maxiter} NM iters with adaptive=True")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Adaptive NM Coefficients (adaptive=True)")
    print(f"{'='*70}")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"adaptive_nm_coefficients_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_ADAPTIVE_NM_COEFFICIENTS_001")
            mlflow.log_param("worker", "W2")
            mlflow.log_param("nm_adaptive", True)
