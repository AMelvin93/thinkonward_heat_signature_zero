...
Tests scipy's adaptive=True option for Nelder-Mead, which uses
dimension-dependent coefficients for reflection, expansion, and contraction.

Hypothesis: Default NM coefficients may not be optimal for 2-4D problems.
Scipy's adaptive option scales coefficients based on dimension count.
"""
...
class TemporalFidelityWithPolishOptimizer:
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                result = minimize(
                    ...
                    pos_params,
                    method='Nelder-Mead',
                    options={'maxiter': self.refine_maxiter, 'xatol': 0.01, 'fatol': 0.001, 'adaptive': True}
                )
        ...
        if self.final_polish_maxiter > 0 and candidates_raw:
            ...
            # Polish with full timesteps - using adaptive coefficients
            result = minimize(
                objective_fine_full,
                best_pos_params,
                method='Nelder-Mead',
                options={'maxiter': self.final_polish_maxiter, 'xatol': 0.005, 'fatol': 0.0005, 'adaptive': True}
            )
