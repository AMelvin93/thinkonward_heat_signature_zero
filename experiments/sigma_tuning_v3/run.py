"""
Sigma tuning experiment using the new best config (fevals_22_40) as baseline.

Current best: fevals_22_40 @ 1.1640, 57.1 min
- sigma0_1src: 0.18
- sigma0_2src: 0.22

Testing:
- tighter_sigma (0.16/0.20): Faster convergence, less exploration
- looser_sigma (0.20/0.25): More exploration, may find better basins
- asymmetric (0.18/0.25): Keep 1src same, explore more for 2src
"""
...
def run_experiment(config, config_name, data):
    ...
    print(f"Parameters:")
    print(f"  sigma0_1src={config.get('sigma0_1src')}")
    print(f"  sigma0_2src={config.get('sigma0_2src')}")
...
def main():
    ...
    # Base config from successful fevals_22_40
    base_config = {
    ...
    configs = [
        ({**base_config, 'sigma0_1src': 0.16, 'sigma0_2src': 0.20}, 'tighter_sigma'),
        ({**base_config, 'sigma0_1src': 0.20, 'sigma0_2src': 0.25}, 'looser_sigma'),
        ({**base_config, 'sigma0_1src': 0.18, 'sigma0_2src': 0.25}, 'asymmetric_sigma'),
    ]
    ...
    print("fevals_22_40 (sigma 0.18/0.22): 1.1640 @ 57.1 min")
