...
Test if higher fevals (22/48) can improve accuracy within budget.

Prior finding:
- 3-pert @ fevals 20/44: 1.1475 @ 55.7 min (4.3 min buffer)

Hypothesis: With 4.3 min budget remaining, higher fevals might improve accuracy.
"""
...
def main():
    ...
    BASELINE = 1.1475  # Validated 3-pert baseline

    # Higher fevals config: 22/48 instead of 20/44
    config = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'max_fevals_1src': 22,          # INCREASED from 20
        'max_fevals_2src': 48,          # INCREASED from 44
    ...
    print("="*60)
    print("HIGHER FEVALS: 22/48 with 3 perturbations")
    print("="*60)
    print(f"3-pert baseline: {BASELINE} @ 55.7 min")
    print(f"Budget remaining: ~4.3 min")
    print(f"Goal: Use remaining budget for higher fevals")

    results = []

    # Run 3 validation tests
    for run_num in range(1, 4):
        result = run_single_test(run_num, config, "3pert_fevals_22_48", data)
        results.append(result)
    ...
    print("\n" + "="*60)
    print("SUMMARY: Higher Fevals (22/48) with 3 perturbations")
    print("="*60)
    print(f"\nRun Results:")
    for r in results:
    ...
    print(f"  vs 3-pert baseline: {mean_score - BASELINE:+.4f}")

    in_budget = mean_time <= 60.0
    print(f"\nBudget Status: {'IN BUDGET' if in_budget else 'OVER BUDGET'}")

    if in_budget and mean_score > BASELINE:
        print("\nCONCLUSION: SUCCESS - Higher fevals improves score within budget!")
    elif in_budget:
        print("\nCONCLUSION: IN BUDGET but no improvement over baseline")
    else:
