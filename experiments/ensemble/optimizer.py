"""
Ensemble/Refinement Optimizer for Heat Source Identification.

Key Innovation: Two-stage optimization
1. Stage 1: Standard CMA-ES for global exploration
2. Stage 2: Local refinement with smaller sigma around best solution

This leverages the idea that CMA-ES finds good regions but may not
fully converge due to limited fevals.
"""
...
class EnsembleOptimizer:
    """
    Two-stage CMA-ES optimizer with local refinement.

    Stage 1: Global exploration with standard sigma
    Stage 2: Local refinement with smaller sigma around best solution
    """
    ...
    def __init__(
        ...
        nx: int = 100,
        ny: int = 50,
        # Stage 1 fevals (exploration)
        stage1_fevals_1src: int = 10,
        stage1_fevals_2src: int = 18,
        # Stage 2 fevals (refinement)
        stage2_fevals_1src: int = 4,
        stage2_fevals_2src: int = 6,
        # Sigma values
        sigma0_1src: float = 0.15,
        sigma0_2src: float = 0.20,
        refine_sigma_factor: float = 0.3,  # Smaller sigma for refinement
        ...
        self.stage2_fevals_1src = stage2_fevals_1src
        self.stage2_fevals_2src = stage2_fevals_2src
        self.sigma0_1src = sigma0_1src
        self.sigma0_2src = sigma0_2src
        self.refine_sigma_factor = refine_sigma_factor
    ...
    def estimate_sources(
        ...
        """Two-stage optimization with local refinement."""
        ...
        stage1_fevals = self.stage1_fevals_1src if n_sources == 1 else self.stage1_fevals_2src
        ...
        stage1_best_sol = all_solutions[0][0]
        stage1_best_rmse = all_solutions[0][1]

        if verbose:
            print(f"  Stage 1: RMSE={stage1_best_rmse:.4f}")

        # === STAGE 2: LOCAL REFINEMENT ===
        stage2_fevals = self.stage2_fevals_1src if n_sources == 1 else self.stage2_fevals_2src
        refine_sigma = sigma0 * self.refine_sigma_factor

        opts2 = cma.CMAOptions()
        opts2['maxfevals'] = stage2_fevals
        opts2['bounds'] = [lb, ub]
        opts2['verbose'] = -9
        opts2['tolfun'] = 1e-7
        opts2['tolx'] = 1e-7

        es2 = cma.CMAEvolutionStrategy(stage1_best_sol.tolist(), refine_sigma, opts2)

        while not es2.stop():
            solutions = es2.ask()
            fitness = [objective(s) for s in solutions]
            es2.tell(solutions, fitness)

            for sol, fit in zip(solutions, fitness):
                all_solutions.append((np.array(sol), fit, best_init_type + '_refined'))
        ...
        if final_candidates:
            ...
            best_positions = stage1_best_sol
