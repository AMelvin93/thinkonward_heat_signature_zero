"""
A6 Ensemble/Fusion Optimizer V2 for Heat Source Identification.

Key Innovation: Source-Type Specialized Strategy
- 1-source: Minimal fevals (already solved well)
- 2-source: Maximum fevals (main bottleneck)
- Combines adaptive budget + parallel init evaluation

Strategy:
1. Detect difficulty based on init RMSE
2. Use fewer fevals for easy 1-src (save time)
3. Use more fevals for 2-src (main bottleneck)
4. Run top 2 inits in parallel for 2-src (hedge bets)
"""
...
class EnsembleOptimizerV2:
    """
    A6 Source-Type Specialized Optimizer.

    Key Strategy:
    - 1-source: Minimal fevals (already well-solved, 0.17 RMSE typical)
    - 2-source: Maximum fevals (main bottleneck, 0.30+ RMSE typical)
    - For 2-source: Run top 2 inits with partial fevals each, take best
    """
    ...
    def __init__(
        ...
        nx: int = 100,
        ny: int = 50,
        # 1-source: minimize time (already good accuracy)
        fevals_1src: int = 10,
        # 2-source: maximize accuracy (main bottleneck)
        fevals_2src: int = 24,
        # For 2-source, run top N inits in parallel
        n_parallel_inits_2src: int = 1,  # 1 = standard, 2 = hedge bets
        ...
        self.n_parallel_inits_2src = n_parallel_inits_2src
    ...
    def _run_cmaes(self, init_params: np.ndarray, n_sources: int, max_fevals: int,
                   ...
                   init_type: str) -> List[Tuple]:
        """Run CMA-ES and return all solutions."""
    ...
    def estimate_sources(
        ...
        """Estimate sources with source-type specialized strategy."""
        ...
        # === SOURCE-TYPE SPECIALIZED OPTIMIZATION ===
        if n_sources == 1:
            # 1-source: Use standard approach with minimal fevals
            best_init_params, best_init_type, _ = init_evaluations[0]
            all_solutions = self._run_cmaes(
                best_init_params, n_sources, self.fevals_1src,
                Y_observed, solver, dt, nt, T0, sensors_xy, q_range, best_init_type
            )
        else:
            # 2-source: Main bottleneck - give maximum resources
            # Option to run multiple inits in parallel
            n_inits_to_run = min(self.n_parallel_inits_2src, len(init_evaluations))
            fevals_per_init = self.fevals_2src // n_inits_to_run

            all_solutions = []
            for i in range(n_inits_to_run):
                init_params, init_type, _ = init_evaluations[i]
                solutions = self._run_cmaes(
                    init_params, n_sources, fevals_per_init,
                    Y_observed, solver, dt, nt, T0, sensors_xy, q_range, init_type
                )
        ...
        if final_candidates:
            ...
            best_positions = init_evaluations[0][0]
        ...
        results = [
            CandidateResult(
                ...
                rmse=c[2],
                init_type=c[3],
                n_evals=len(all_solutions) // len(final_candidates) if final_candidates else len(all_solutions)
            )
