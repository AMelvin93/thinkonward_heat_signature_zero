...
from experiments.ensemble.optimizer import (
    EnsembleOptimizer,
...
def process_sample(sample, meta, config, history_1src, history_2src):
    optimizer = EnsembleOptimizer(
        ...
        stage2_fevals_1src=config['stage2_fevals_1src'],
        stage2_fevals_2src=config['stage2_fevals_2src'],
        sigma0_1src=config.get('sigma0_1src', 0.15),
        sigma0_2src=config.get('sigma0_2src', 0.20),
        refine_sigma_factor=config.get('refine_sigma_factor', 0.3),
...
def main():
    parser = argparse.ArgumentParser(description='Run Ensemble Optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--batch-size', type=int, default=20)
    parser.add_argument('--max-samples', type=int, default=None)
    # Stage 1 fevals
    parser.add_argument('--stage1-fevals-1src', type=int, default=10)
    parser.add_argument('--stage1-fevals-2src', type=int, default=18)
    # Stage 2 fevals
    parser.add_argument('--stage2-fevals-1src', type=int, default=4)
    parser.add_argument('--stage2-fevals-2src', type=int, default=6)
    parser.add_argument('--refine-sigma', type=float, default=0.3)
    ...
    config = {
        ...
        'stage2_fevals_1src': args.stage2_fevals_1src,
        'stage2_fevals_2src': args.stage2_fevals_2src,
        'refine_sigma_factor': args.refine_sigma,
    ...
    total_1src = config['stage1_fevals_1src'] + config['stage2_fevals_1src']
    total_2src = config['stage1_fevals_2src'] + config['stage2_fevals_2src']
    ...
    print("=" * 70)
    print("ENSEMBLE/REFINEMENT OPTIMIZER (A6)")
    print("=" * 70)
    ...
    print(f"1-src: Stage1={config['stage1_fevals_1src']} + Stage2={config['stage2_fevals_1src']} = {total_1src}")
    print(f"2-src: Stage1={config['stage1_fevals_2src']} + Stage2={config['stage2_fevals_2src']} = {total_2src}")
    print(f"Refine sigma factor: {config['refine_sigma_factor']}")
    print("=" * 70)
    ...
    if is_g4dn_simulation and not args.max_samples:
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param("optimizer", "EnsembleOptimizer")
            mlflow.log_param("stage1_fevals_1src", config['stage1_fevals_1src'])
            mlflow.log_param("stage1_fevals_2src", config['stage1_fevals_2src'])
            mlflow.log_param("stage2_fevals_1src", config['stage2_fevals_1src'])
            mlflow.log_param("stage2_fevals_2src", config['stage2_fevals_2src'])
