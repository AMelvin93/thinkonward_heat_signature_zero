#!/usr/bin/env python3
"""
Run script for Cluster-Enhanced Robust optimizer.
"""
...
from optimizer import ClusterRobustOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = ClusterRobustOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Cluster-Enhanced Robust optimizer')
    ...
    print(f"Fallback type: cluster+onset (2-src), centroid+random (1-src)")
    print(f"=" * 60)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            flag = ""
            if result['best_rmse'] > 0.35 and result['n_sources'] == 1:
                flag = " [FB]"
            elif result['best_rmse'] > 0.45 and result['n_sources'] == 2:
                flag = " [FB]"
