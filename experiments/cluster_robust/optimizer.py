"""
Cluster-Enhanced Robust Optimizer.

Combines robust fallback strategy with cluster-based initialization
for improved 2-source accuracy.

Key change from robust_fallback:
- Primary: tri + smart (same)
- Fallback: cluster + onset (instead of centroid + random)
"""
...
class ClusterRobustOptimizer:
    """
    Robust optimizer with cluster-based fallback.

    Primary: tri + smart inits
    Fallback: cluster + onset inits (for 2-source)
             centroid + random (for 1-source)
    """
    ...
    def _onset_init_2src(self, sample):
        """Onset time-based initialization for 2-source problems."""
        readings = sample['Y_noisy']
        sensors = np.array(sample['sensors_xy'])
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if best_rmse_initial > threshold:
            # Fallback: use cluster + onset for 2-source, centroid + random for 1-source
            fallback_inits = []

            if n_sources == 2:
                # Use cluster-based and onset-based inits for 2-source
                ...
                # Use centroid + random for 1-source
