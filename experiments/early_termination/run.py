#!/usr/bin/env python
"""
Run Early CMA-ES Termination Optimizer experiment.

Key Innovation: Stop CMA-ES early when converged, saving time for hard samples.

Hypothesis:
    - Easy samples converge in 10 fevals, no need for 22+
    - Early termination could save 20-30% time on easy samples
    - Should maintain accuracy while improving efficiency

Target: Score >= 1.0116 AND time < 58.6 min (beat current best)

Usage:
    # Quick validation (10 samples, ~2 min)
    uv run python experiments/early_termination/run.py --workers 7 --shuffle --max-samples 10

    # Full run (80 samples)
    uv run python experiments/early_termination/run.py --workers 7 --shuffle
"""
...
from experiments.early_termination.optimizer import (
    EarlyTerminationOptimizer,
...
def process_sample(sample, meta, config, history_1src, history_2src):
    """Process a single sample with early termination optimizer."""
    optimizer = EarlyTerminationOptimizer(
    ...
    start = time.time()
    (candidates, best_rmse, results, features, best_positions,
     n_transferred, early_stopped, fevals_used) = optimizer.estimate_sources(
    ...
    max_fevals = config['max_fevals_1src'] if sample['n_sources'] == 1 else config['max_fevals_2src']
    fevals_saved = max_fevals - fevals_used if early_stopped else 0
...
def main():
    parser = argparse.ArgumentParser(description='Run Early Termination Optimizer')
    ...
    parser.add_argument('--stagnation-threshold', type=float, default=1e-4,
                        help='Improvement threshold for stagnation detection')
    parser.add_argument('--stagnation-patience', type=int, default=3,
                        help='Number of stagnant generations before early stop')
    ...
    if args.max_samples and args.max_samples < len(samples):
        # Shuffle first if requested, then limit
        if args.shuffle:
    ...
    # Print header
    print("=" * 70)
    print("EARLY CMA-ES TERMINATION OPTIMIZER")
    print("=" * 70)
    ...
    print()
    print("KEY INNOVATION: Early termination for converged samples")
    print(f"  - Stagnation threshold: {config['stagnation_threshold']}")
    print(f"  - Stagnation patience: {config['stagnation_patience']} generations")
    print(f"  - Expected: 20-30% time savings on easy samples")
    print()
    print(f"Config:")
    ...
    for batch_idx in range(n_batches):
        ...
        batch_early_stops = sum(1 for r in batch_results if r['early_stopped'])
        batch_fevals_saved = sum(r['fevals_saved'] for r in batch_results)
        batch_times = [r['time'] for r in batch_results]
        print(f"  -> Batch RMSE: {np.mean(batch_rmses):.4f}, "
              f"Early stops: {batch_early_stops}/{len(batch_results)}, "
              f"Fevals saved: {batch_fevals_saved}, "
    ...
    all_early_stops = [r['early_stopped'] for r in all_results]
    all_fevals_saved = [r['fevals_saved'] for r in all_results]
    ...
    n_early_stopped = sum(all_early_stops)
    early_stop_rate = n_early_stopped / len(all_results) * 100
    total_fevals_saved = sum(all_fevals_saved)
    avg_fevals_saved = np.mean(all_fevals_saved)
    ...
    early_stop_1src = sum(early_stops_by_nsources.get(1, [])) / max(1, len(early_stops_by_nsources.get(1, []))) * 100
    early_stop_2src = sum(early_stops_by_nsources.get(2, [])) / max(1, len(early_stops_by_nsources.get(2, []))) * 100
    ...
    if is_g4dn_simulation and not is_quick_validation:
        ...
        run_name = f"early_term_{config['max_fevals_1src']}_{config['max_fevals_2src']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_metric("early_stop_rate", early_stop_rate)
            mlflow.log_metric("total_fevals_saved", total_fevals_saved)

            mlflow.log_param("optimizer", "EarlyTerminationOptimizer")
            mlflow.log_param("max_fevals_1src", config['max_fevals_1src'])
            mlflow.log_param("max_fevals_2src", config['max_fevals_2src'])
            mlflow.log_param("stagnation_threshold", config['stagnation_threshold'])
            mlflow.log_param("stagnation_patience", config['stagnation_patience'])
            ...
            with open(output_path, 'wb') as f:
                pickle.dump({
                    ...
                    'early_stop_stats': {
                        ...
                        'total_fevals_saved': total_fevals_saved,
                    }
                }, f)
            try:
    ...
    print("RESULTS")
    print("=" * 70)
    print(f"Config:           {config['max_fevals_1src']}/{config['max_fevals_2src']} fevals (early termination)")
    ...
    print()
    print("Early Termination Stats:")
    print(f"  Early stopped:  {n_early_stopped}/{len(all_results)} samples ({early_stop_rate:.1f}%)")
    print(f"  Total fevals saved: {total_fevals_saved}")
    print(f"  Avg fevals saved: {avg_fevals_saved:.1f}")
    print()
    ...
    for n_src in sorted(rmse_by_nsources.keys()):
        ...
        early_stops = early_stops_by_nsources[n_src]
        early_pct = sum(early_stops) / len(early_stops) * 100 if early_stops else 0
        print(f"  {n_src}-source: RMSE={np.mean(rmses):.6f} +/- {np.std(rmses):.6f}, "
              f"sims={np.mean(evals):.1f}, time={np.mean(times):.2f}s, "
              f"early_stop={early_pct:.1f}% (n={len(rmses)})")
    print()
    ...
    print(f"Current Best (SmartInit 12/22): Score {baseline_score:.4f} @ {baseline_time:.1f} min")
    print(f"Early Termination ({config['max_fevals_1src']}/{config['max_fevals_2src']}): Score {final_score:.4f} @ {projected_400:.1f} min")
    print()
