"""
Early CMA-ES Termination Optimizer for Heat Source Identification.

Key Innovation: Stop CMA-ES early when converged, saving time for hard samples.

Observation: Some samples converge in 10 fevals, others need 25.
- Fixed feval budget wastes compute on easy samples
- Early termination can save 20-30% on easy samples
- Reallocate saved compute to harder 2-source samples (if needed)

Stagnation Detection:
- Monitor improvement rate during CMA-ES
- If improvement < threshold for N iterations, stop early
- Typically converged when improvement < 1e-4 for 3 generations
"""
...
class EarlyTerminationOptimizer:
    """
    CMA-ES optimizer with EARLY TERMINATION for converged samples.

    Key Innovation: Stop CMA-ES early when stagnated, saving time for hard samples.

    Stagnation Detection:
        - Track best fitness across generations
        - If improvement < stagnation_threshold for stagnation_patience generations, stop
        - Report how many fevals were saved

    Benefits:
        - Easy samples converge quickly (~10 fevals), no need for 22+ fevals
        - Time saved can be used for harder 2-source samples
        - Should improve overall efficiency without sacrificing accuracy
    """
    ...
    def _evaluate_init(self, init_params: np.ndarray, n_sources: int,
        ...
        """Quickly evaluate an init by computing its RMSE (1 or 2 sims)."""
        if n_sources == 1:
            x, y = init_params
    ...
    def estimate_sources(
        ...
        verbose: bool = False,
    ) -> Tuple[List[List[Tuple]], float, List[CandidateResult], np.ndarray, np.ndarray, int, bool, int]:
        """
        Estimate sources with EARLY TERMINATION for converged samples.
        ...
        Returns:
            ...
            - early_stopped: whether CMA-ES terminated early
            - fevals_used: actual number of fevals used (may be < max)
        """
        ...
        # Evaluate each init quickly (1 or 2 sims per init)
        init_evaluations = []
        n_selection_sims = 0
        ...
        # Run CMA-ES with early termination
        opts = cma.CMAOptions()
        ...
        while not es.stop():
            ...
            fevals_used = es.result.evaluations
            ...
            improvement = abs(prev_best - current_best)
            ...
            if improvement < self.stagnation_threshold:
                ...
                if stagnation_count >= self.stagnation_patience:
                    early_stopped = True
                    if verbose:
                        print(f"  Early termination at {fevals_used} fevals (stagnated for {stagnation_count} gens)")
                    break
            else:
                stagnation_count = 0
        ...
        return candidate_sources, best_rmse, results, features, best_positions, n_transferred, early_stopped, fevals_used
