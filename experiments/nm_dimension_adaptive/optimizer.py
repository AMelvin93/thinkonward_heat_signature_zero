"""
NM Dimension-Adaptive Optimizer - Uses scipy's adaptive NM parameters

scipy's 'adaptive=True' option for Nelder-Mead implements dimension-dependent
parameters from Gao and Han (2012): "Implementing the Nelder-Mead simplex
algorithm with adaptive parameters"

Standard NM uses fixed parameters:
- reflection: 1.0, expansion: 2.0, contraction: 0.5, shrink: 0.5

Adaptive NM adjusts these based on dimension n:
- reflection: 1, expansion: 1 + 2/n, contraction: 0.75 - 1/(2n), shrink: 1 - 1/n

For our problem:
- 1-src (n=2): expansion=2.0, contraction=0.5 (standard)
- 2-src (n=4): expansion=1.5, contraction=0.625 (more conservative)

The adaptive parameters are designed to improve stability in higher dimensions.
"""
...
class NMDimensionAdaptiveOptimizer:
    """
    Optimizer that uses adaptive Nelder-Mead parameters for polish.

    Uses scipy's adaptive=True option which implements dimension-dependent
    NM parameters from Gao and Han (2012).
    """
    ...
    def __init__(
        ...
        # NEW: Adaptive NM option
        use_adaptive_nm: bool = True,
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.use_adaptive_nm = use_adaptive_nm
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Refine top N solutions with ADAPTIVE NM
        refined_solutions = []
            ...
            if self.refine_maxiter > 0:
                result = minimize(
                    ...
                    options={
                        ...
                        'xatol': 0.01,
                        'fatol': 0.001,
                        'adaptive': self.use_adaptive_nm  # KEY: Use adaptive parameters
                    }
                )
        ...
        # Final polish with ADAPTIVE NM on best candidate
