#!/usr/bin/env python3
"""
Run script for Sigma Fallback optimizer.
"""
...
from optimizer import SigmaFallbackOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = SigmaFallbackOptimizer(
        max_fevals_1src=config['max_fevals_1src'],
        max_fevals_2src=config['max_fevals_2src'],
        sigma0_1src=config.get('sigma0_1src', 0.15),
        sigma0_2src=config.get('sigma0_2src', 0.20),
        fallback_sigma_1src=config.get('fallback_sigma_1src', 0.25),
        fallback_sigma_2src=config.get('fallback_sigma_2src', 0.30),
...
def main():
    parser = argparse.ArgumentParser(description='Run Sigma Fallback optimizer')
    ...
    parser.add_argument('--fallback-sigma-1src', type=float, default=0.25)
    parser.add_argument('--fallback-sigma-2src', type=float, default=0.30)
