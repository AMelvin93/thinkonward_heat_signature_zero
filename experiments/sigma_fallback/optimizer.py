"""
Sigma Fallback Optimizer.

Key idea: If first run has high RMSE, restart with larger sigma
to encourage more exploration. This should help escape local minima
that trap the initial CMA-ES run.

Fallback strategy:
- sigma0 = 0.20 for initial run
- sigma0 = 0.30 for fallback (more exploration)
"""
...
class SigmaFallbackOptimizer:
    """
    Optimizer with sigma-based fallback strategy.
    """
    ...
    def __init__(
        ...
        fallback_sigma_1src: float = 0.25,
        fallback_sigma_2src: float = 0.30,
    ...
    def _run_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                          initializations, n_sources, early_frac, sigma0):
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        candidates_raw, n_sims = self._run_optimization(
            ...
            primary_inits, n_sources, early_frac, sigma0
        )
        ...
        if best_rmse_initial > threshold:
            # Fallback: run again with LARGER sigma for more exploration
            fallback_candidates, fallback_sims = self._run_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                primary_inits, n_sources, early_frac, fallback_sigma
            )
