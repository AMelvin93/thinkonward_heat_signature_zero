...
class TabuBasinHoppingOptimizer:
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # NO dissimilarity filtering - just take top N_MAX by RMSE
        # (testing hypothesis that top candidates by RMSE are naturally distinct)
        candidates_sorted = sorted(candidates_raw, key=lambda c: c[2])  # Sort by RMSE
        final_candidates = candidates_sorted[:N_MAX]
