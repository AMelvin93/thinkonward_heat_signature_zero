#!/usr/bin/env python3
"""
Run script for W2 Balanced Timing optimizer.

EXP006: balanced_timing_config
Config:
- sigma0_1src: 0.20 (higher)
- sigma0_2src: 0.25 (higher)
- threshold_1src: 0.40 (W1 proven)
- threshold_2src: 0.50 (W1 proven)
- fallback_fevals: 16 (reduced)

Hypothesis: W1's 0.40/0.50 thresholds with reduced fallback_fevals may balance score and time.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Balanced Timing optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP006 specific
    parser.add_argument('--sigma0-1src', type=float, default=0.20)
    parser.add_argument('--sigma0-2src', type=float, default=0.25)
    parser.add_argument('--threshold-1src', type=float, default=0.40)  # W1 proven
    parser.add_argument('--threshold-2src', type=float, default=0.50)  # W1 proven
    parser.add_argument('--fallback-fevals', type=int, default=16)    # Reduced
    # Other parameters
    ...
    print(f"\n[W2] Balanced Timing Optimizer")
    print(f"=" * 70)
    print(f"EXP006: balanced_timing_config")
    print(f"-" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Balanced Timing (EXP006)")
    print(f"{'='*70}")
