#!/usr/bin/env python3
"""
Run script for W1 Very Reduced Primary Fevals optimizer.

EXP011: High sigma (0.20/0.25) with very reduced primary fevals (16/30).
Testing if more aggressive fevals reduction can save enough time for budget.
...
from optimizer import VeryReducedFevalsOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = VeryReducedFevalsOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Very Reduced Primary Fevals optimizer')
    ...
    parser.add_argument('--sigma0-1src', type=float, default=0.20)  # High sigma
    parser.add_argument('--sigma0-2src', type=float, default=0.25)  # High sigma
    ...
    print(f"\nW1 Very Reduced Primary Fevals Optimizer (EXP011)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Very Reduced Primary Fevals (EXP011)")
    print(f"{'='*70}")
    ...
    print(f"EXP002:   1.1362 @ 69.2 min (20/36 fevals)")
    print(f"EXP008:   1.1273 @ 66.6 min (18/32 fevals)")
    print(f"This run: {score:.4f} @ {projected_400:.1f} min (16/30 fevals)")
    print(f"Delta vs baseline: {score - 1.1247:+.4f} score, {projected_400 - 57.2:+.1f} min")
    print(f"Delta vs EXP008:   {score - 1.1273:+.4f} score, {projected_400 - 66.6:+.1f} min")
    print()
    if projected_400 > 60:
        print("OVER BUDGET")
    elif score > 1.1247:
