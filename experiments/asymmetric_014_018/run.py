"""
Run script for asymmetric_014_018 experiment.

Tests tighter 2-source sigma (0.18 vs 0.19) with optimal 1-source sigma (0.14).

Hypothesis: Since sigma 0.19 is optimal for 2-source, maybe 0.18 is even better.
(Similar to how 0.14 was found to be optimal for 1-source)
"""
...
def main():
    ...
    print(f"\nHypothesis: Test tighter 2-source sigma (0.18 vs 0.19)")
    print(f"Prior best: sigma 0.14/0.19 + scale 0.06 = ~1.167")
    ...
    # Configs to test - 2-source sigma sweep
    configs = [
        # Baseline: sigma 0.14/0.19 (current optimal)
        {**base_config, 'sigma0_1src': 0.14, 'sigma0_2src': 0.19},
        # Test: sigma 0.14/0.18 (tighter 2-source)
        {**base_config, 'sigma0_1src': 0.14, 'sigma0_2src': 0.18},
        # Test: sigma 0.14/0.20 (looser 2-source)
        {**base_config, 'sigma0_1src': 0.14, 'sigma0_2src': 0.20},
    ]
    config_names = ['baseline_014_019', 'test_014_018', 'test_014_020']
    ...
    # Analysis
    print("\n" + "="*80)
    print("EXPERIMENT RESULTS: 2-SOURCE SIGMA TUNING")
    print("="*80)
    print(f"{'Config':<20} {'Sigma':>12} {'Score':>8} {'Proj 400':>10} {'RMSE 1src':>10} {'RMSE 2src':>10}")
    print("-"*80)
    for r in results:
        sigma = f"{r['sigma0_1src']}/{r['sigma0_2src']}"
        print(f"{r['config']:<20} {sigma:>12} {r['score']:>8.4f} {r['projected_400_min']:>9.1f}m {r['rmse_1src']:>10.4f} {r['rmse_2src']:>10.4f}")
    ...
    # Analyze 2-source sigma effect
    print("\n--- 2-SOURCE SIGMA ANALYSIS ---")
    baseline = next((r for r in results if '019' in r['config']), None)
    for r in results:
        if baseline:
            delta = r['score'] - baseline['score']
            rmse_2src_delta = r['rmse_2src'] - baseline['rmse_2src']
            print(f"{r['config']}: {delta:+.4f} score, 2src RMSE: {rmse_2src_delta:+.4f}")
            if delta > 0.002:
                print(f"  → sigma 0.{int(r['sigma0_2src']*100)} is BETTER for 2-source")
            elif delta < -0.002:
                print(f"  → sigma 0.{int(r['sigma0_2src']*100)} is WORSE for 2-source")
            else:
                print(f"  → Within run variance")
