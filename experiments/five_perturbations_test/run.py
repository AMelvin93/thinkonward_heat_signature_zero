"""
Experiment: five_perturbations_test
Test 5 perturbations to see if the improvement trend continues.

Current best: sigma 0.18/0.22 + fevals 20/44 + 4 perturbations = 1.1437 @ 42.2 min
"""
...
def run_experiment(config, run_num, data):
    ...
    n_perturb = config.get('n_perturbations')
    print(f"\n=== Run {run_num}: {n_perturb} perturbations ===")
...
def main():
    ...
    # Run with 5 perturbations (3 runs for validation)
    for run_num in range(1, 4):
        config = {**base_config, 'n_perturbations': 5}
    ...
    print("\n" + "="*60)
    print("SUMMARY: 5 Perturbations Test")
    print("="*60)
    print(f"Current best (4 perturb): 1.1437 @ 42.2 min")
    print()
    for r in results:
        delta = r['score'] - CURRENT_BEST
        in_budget = "IN BUDGET" if r['projected_400_min'] < 60 else "OVER BUDGET"
        print(f"  Run {r['run']}: {r['score']:.4f} @ {r['projected_400_min']:.1f} min ({delta:+.4f}) [{in_budget}]")
    ...
    if np.mean(scores) > CURRENT_BEST and np.mean(times) < 60:
        improvement = (np.mean(scores) - CURRENT_BEST) / CURRENT_BEST * 100
        print(f"NEW BEST CANDIDATE! {improvement:.2f}% improvement")
    else:
        print("5 perturbations did not improve on 4")
