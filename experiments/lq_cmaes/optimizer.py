"""
A7: lq-CMA-ES (Linear-Quadratic Surrogate) Optimizer.

Key Innovation: Uses pycma's linear-quadratic surrogate model to accelerate CMA-ES.
The surrogate is built from evaluated points and used to pre-screen candidates,
potentially achieving 6-20x faster convergence.

Based on research:
- https://cma-es.github.io/lq-cma
- Quadratic surrogate shows 6-20x faster convergence on benchmark functions
"""
...
class LQCMAESOptimizer:
    """
    CMA-ES optimizer with Linear-Quadratic Surrogate acceleration.

    Uses pycma's lq-CMA-ES which builds a quadratic surrogate model
    to pre-screen candidates, potentially achieving 6-20x faster convergence.

    Based on research:
    - https://cma-es.github.io/lq-cma
    - Claims linear convergence 6-20x faster than standard CMA-ES
    """
    ...
    def _run_lq_cmaes(self, init_params: np.ndarray, n_sources: int, max_fevals: int,
        ...
        """Run lq-CMA-ES (linear-quadratic surrogate) and return all solutions."""
        ...
        if n_sources == 1:
            def objective(xy_params):
                ...
                q, Y_pred, rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver, dt, nt, T0, sensors_xy, q_range)
                all_solutions.append((np.array([x, y]), rmse, init_type))
                return rmse
        else:
            def objective(xy_params):
                ...
                (q1, q2), Y_pred, rmse = compute_optimal_intensity_2src(
                    x1, y1, x2, y2, Y_observed, solver, dt, nt, T0, sensors_xy, q_range)
                all_solutions.append((np.array([x1, y1, x2, y2]), rmse, init_type))
                return rmse

        # Use lq-CMA-ES (linear-quadratic surrogate)
        opts = {
        ...
        try:
            # Try to use lq_surr if available
            result = cma.fmin_lq_surr(objective, init_params.tolist(), sigma0, opts)
            # result is (xbest, es) tuple
        except AttributeError:
            # Fall back to standard CMA-ES if lq_surr not available
    ...
    def estimate_sources(
        ...
        """Estimate sources using lq-CMA-ES with smart init selection."""
        ...
        # === RUN LQ-CMA-ES ON BEST INIT ===
        max_fevals = self.max_fevals_1src if n_sources == 1 else self.max_fevals_2src
        all_solutions = self._run_lq_cmaes(
            best_init_params, n_sources, max_fevals,
