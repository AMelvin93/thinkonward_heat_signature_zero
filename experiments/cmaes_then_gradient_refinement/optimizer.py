"""
CMA-ES + L-BFGS-B Hybrid Optimizer for Inverse Heat Source Problem

Uses CMA-ES for global search (same as baseline), then replaces Nelder-Mead polish
with L-BFGS-B polish. The hypothesis is that L-BFGS-B's gradient-based refinement
converges faster in the final basin than NM's simplex method.

Key insight: Near the optimum, gradient methods have quadratic convergence while
NM has only linear convergence. This might allow fewer polish iterations.
"""
...
class HybridCMAESLBFGSBOptimizer:
    """
    Hybrid optimizer that uses:
    - CMA-ES with 40% timesteps for global search (same as baseline)
    - L-BFGS-B instead of Nelder-Mead for final polish

    The hypothesis is that L-BFGS-B converges faster near the optimum due to
    gradient information, allowing fewer polish iterations.
    """
    ...
    def __init__(
        ...
        # Polish configuration - L-BFGS-B instead of NM
        polish_maxiter: int = 3,  # L-BFGS-B iterations (vs NM's 8)
        polish_top_n: int = 2,
        polish_method: str = 'L-BFGS-B',  # or 'Nelder-Mead' for comparison
        # Temporal fidelity
    ...
    def _polish_lbfgsb(self, params, objective_fn, lb, ub):
    ...
    def _polish_nm(self, params, objective_fn, lb, ub):
        """Polish using Nelder-Mead (for comparison)."""
        def bounded_objective(p):
            clipped = np.clip(p, lb, ub)
        ...
        return np.clip(result.x, lb, ub), result.fun, result.nfev

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, nt_reduced):
        """Run CMA-ES optimization with reduced timesteps, then polish with L-BFGS-B."""
        ...
        # Polish top N solutions using L-BFGS-B or NM
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.polish_top_n]):
            if self.polish_maxiter > 0:
                if self.polish_method == 'L-BFGS-B':
                    polished_params, polished_rmse, nfev = self._polish_lbfgsb(
                        pos_params, objective_coarse, lb, ub)
                else:
                    polished_params, polished_rmse, nfev = self._polish_nm(
                        pos_params, objective_coarse, lb, ub)

                if polished_rmse < rmse_coarse:
                    refined_solutions.append((polished_params, polished_rmse, f'{init_type}_polished'))
                else:
                    refined_solutions.append((pos_params, rmse_coarse, init_type))
            else:
                refined_solutions.append((pos_params, rmse_coarse, init_type))

        # Add remaining top solutions (unpolished)
        for pos_params, rmse_coarse, init_type in all_solutions[self.polish_top_n:self.candidate_pool_size]:
            refined_solutions.append((pos_params, rmse_coarse, init_type))
