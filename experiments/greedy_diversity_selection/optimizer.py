"""
Greedy Diversity Selection Optimizer

Same as baseline but uses greedy selection to maximize combined score
(accuracy + diversity) instead of just sorting by RMSE.
"""
...
LAMBDA = 0.3  # Diversity weight in scoring formula
...
def calculate_score(rmse_list, n_candidates, lambda_=LAMBDA, n_max=N_MAX):
    """Calculate the submission score for a set of candidates."""
    if n_candidates == 0:
        return 0.0
    accuracy_term = sum(1.0 / (1.0 + rmse) for rmse in rmse_list) / n_candidates
    diversity_term = lambda_ * (n_candidates / n_max)
    return accuracy_term + diversity_term


def greedy_select_candidates(candidates, tau=TAU, n_max=N_MAX):
    """
    Greedy selection that maximizes combined score (accuracy + diversity).

    Instead of just sorting by RMSE, this considers:
    1. Pick best RMSE first
    2. For subsequent picks, select the candidate that:
       - Passes dissimilarity check (distance >= tau from all selected)
       - Maximizes the total score when added

    The scoring formula is: score = mean(1/(1+RMSE)) + 0.3*(n/3)
    """
    if not candidates:
        return []

    # candidates is list of (sources, rmse) tuples
    candidates = list(candidates)  # Make a copy

    selected = []
    selected_rmses = []

    # Greedy selection
    while len(selected) < n_max and candidates:
        best_candidate = None
        best_score_gain = -float('inf')
        best_idx = -1

        for i, (sources, rmse) in enumerate(candidates):
            # Check dissimilarity with already selected candidates
            if selected:
                min_dist = min(candidate_distance(sources, sel[0]) for sel in selected)
                if min_dist < tau:
                    continue  # Skip candidates too similar to selected ones

            # Calculate score if we add this candidate
            test_rmses = selected_rmses + [rmse]
            new_score = calculate_score(test_rmses, len(test_rmses))

            # Calculate current score
            if selected_rmses:
                current_score = calculate_score(selected_rmses, len(selected_rmses))
            else:
                current_score = 0.0

            score_gain = new_score - current_score

            # For first candidate, also consider RMSE directly
            # (ensures we pick a good anchor)
            if not selected:
                # Prioritize low RMSE for first candidate
                effective_gain = score_gain + 0.1 * (1.0 / (1.0 + rmse))
            else:
                effective_gain = score_gain

            if effective_gain > best_score_gain:
                best_score_gain = effective_gain
                best_candidate = (sources, rmse)
                best_idx = i

        if best_candidate is None:
            # No more candidates pass the dissimilarity check
            break

        selected.append(best_candidate)
        selected_rmses.append(best_candidate[1])
...
class GreedyDiversityOptimizer:
    """
    Optimizer with greedy diversity selection that maximizes combined score.
    """
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # GREEDY DIVERSITY SELECTION instead of simple sort+filter
        filtered = greedy_select_candidates(
            [(c[0], c[2]) for c in candidates_raw],
            tau=self.tau,
            n_max=N_MAX
        )
