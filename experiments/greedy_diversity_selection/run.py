#!/usr/bin/env python3
"""Run script for Greedy Diversity Selection experiment."""
...
from optimizer import GreedyDiversityOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = GreedyDiversityOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Greedy Diversity Selection')
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Greedy Diversity Selection")
    print(f"{'='*70}")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"greedy_diversity_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param('experiment_id', 'EXP_GREEDY_DIVERSITY_001')
            mlflow.log_param('worker', 'W2')
            mlflow.log_param('optimizer', 'GreedyDiversityOptimizer')
            ...
            mlflow.log_param('selection_strategy', 'greedy')
