#!/usr/bin/env python3
"""
Run script for W1 Sigma 0.18/0.22 with lower thresholds optimizer.

EXP023: Testing sigma 0.18/0.22 with lower thresholds (0.32/0.42).
Goal: Lower thresholds may trigger more fallbacks on hard cases and improve score.

Config:
- sigma0_1src: 0.18
- sigma0_2src: 0.22
- threshold_1src: 0.32 (lower than standard 0.35)
- threshold_2src: 0.42 (lower than standard 0.45)
- fevals_1src: 20
- fevals_2src: 36
- fallback_fevals: 18
"""
...
from optimizer import Sigma018_022Optimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = Sigma018_022Optimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Sigma 0.18/0.22 with lower thresholds (EXP023)')
    ...
    parser.add_argument('--threshold-1src', type=float, default=0.32)  # Lower threshold!
    parser.add_argument('--threshold-2src', type=float, default=0.42)  # Lower threshold!
    ...
    print(f"\nW1 Sigma 0.18/0.22 + Lower Thresholds (EXP023)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Sigma 0.18/0.22 + Lower Thresholds (EXP023)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline:      1.1247 @ 57.2 min (sigma 0.15/0.20, thresh 0.35/0.45)")
    print(f"EXP018 ref:    1.1271 @ 67.7 min (sigma 0.18/0.22, thresh 0.35/0.45)")
    print(f"EXP019 ref:    1.1296 @ 64.0 min (sigma 0.18/0.22, thresh 0.38/0.48)")
    print(f"This run:      {score:.4f} @ {projected_400:.1f} min (sigma 0.18/0.22, thresh 0.32/0.42)")
