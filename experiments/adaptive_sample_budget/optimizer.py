...
Key idea: Track CMA-ES convergence metrics and terminate early when converged.
This saves budget on easy samples, allowing more compute for hard samples.

Two-pass approach:
1. First pass: Run all samples with base budget + early termination
2. Identify hard samples (high RMSE after first pass)
3. Second pass: Re-run only hard samples with bonus budget (saved from easy samples)
"""
...
class ConvergenceInfo:
    ...
    reason: str  # 'budget_exhausted', 'sigma_converged', 'fitness_stagnated'
...
class AdaptiveBudgetOptimizer:
    """
    Optimizer with adaptive budget allocation based on convergence tracking.

    Key improvements over baseline:
    1. Early termination when CMA-ES converges (sigma < threshold)
    2. Early termination when fitness stagnates (no improvement for N gens)
    3. Track budget savings per sample
    4. Re-run hard samples with bonus budget (two-pass mode)
    """
    ...
    def __init__(
        ...
        sigma_converge_thresh: float = 0.01,  # Stop if sigma < this
        stagnation_gens: int = 3,  # Stop if no improvement for N gens
        stagnation_tol: float = 1e-4,  # Improvement threshold
        # Bonus budget for hard samples
        bonus_fevals_1src: int = 10,  # Extra fevals for hard samples
        bonus_fevals_2src: int = 18,  # Extra fevals for hard samples
        rmse_threshold_1src: float = 0.25,  # Use bonus if RMSE > this
        rmse_threshold_2src: float = 0.35,  # Use bonus if RMSE > this
        # Standard parameters
        ...
        self.sigma_converge_thresh = sigma_converge_thresh
        self.stagnation_gens = stagnation_gens
        self.stagnation_tol = stagnation_tol
        self.bonus_fevals_1src = bonus_fevals_1src
        self.bonus_fevals_2src = bonus_fevals_2src
    ...
    def _run_adaptive_cmaes(self, objective_fn, init_params, sigma0, max_fevals, lb, ub):
        """
        Run CMA-ES with adaptive early termination.

        Returns: (best_solution, best_fitness, convergence_info)
        """
        opts = cma.CMAOptions()
        ...
        while not es.stop():
            solutions = es.ask()
            fitness = [objective_fn(s) for s in solutions]
            ...
            # Track improvement
            if gen_best < best_fitness_ever - self.stagnation_tol:
            ...
            # Check 1: Sigma convergence
            if current_sigma < self.sigma_converge_thresh:
                termination_reason = 'sigma_converged'
                break

            # Check 2: Fitness stagnation
            if gens_without_improvement >= self.stagnation_gens:
                termination_reason = 'fitness_stagnated'
                break

        # Get best solution
        all_solutions.sort(key=lambda x: x[1])
        best_sol, best_fit = all_solutions[0] if all_solutions else (init_params, float('inf'))

        conv_info = ConvergenceInfo(
            terminated_early=(termination_reason != 'budget_exhausted'),
            fevals_used=fevals_used,
            fevals_budget=max_fevals,
            final_sigma=es.sigma,
            fitness_history=fitness_history,
            reason=termination_reason
        )

        return all_solutions, conv_info

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, early_frac, max_fevals):
        """Run optimization with given budget."""
        ...
        for init_params, init_type in initializations:
            solutions, conv_info = self._run_adaptive_cmaes(
                objective_coarse, init_params, sigma0, fevals_per_init, lb, ub
            )
        ...
        return candidates_raw, n_sims[0], convergence_infos

    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False,
                         allow_bonus: bool = True):
        """
        Estimate heat sources with adaptive budget allocation.

        Args:
            allow_bonus: If True, use bonus budget for hard samples
        """
        ...
        # Base budget
        base_fevals = self.max_fevals_1src if n_sources == 1 else self.max_fevals_2src

        # Run primary optimization with adaptive early termination
        candidates_raw, n_sims, conv_infos = self._run_single_optimization(
            sample, meta, q_range, solver_coarse, solver_fine,
            primary_inits, n_sources, early_frac, base_fevals
        )

        # Check if result needs bonus budget
        ...
        used_bonus = False
        if allow_bonus and best_rmse_initial > threshold:
            # Use bonus budget with different initializations
            bonus_inits = []
            centroid_init = self._weighted_centroid_init(sample, n_sources)
            bonus_inits.append((centroid_init, 'centroid'))

            bonus_fevals = self.bonus_fevals_1src if n_sources == 1 else self.bonus_fevals_2src

            bonus_candidates, bonus_sims, bonus_conv = self._run_single_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                bonus_inits, n_sources, early_frac, bonus_fevals
            )
            n_sims += bonus_sims
            candidates_raw.extend(bonus_candidates)
            conv_infos.extend(bonus_conv)
            used_bonus = True
        ...
        # Compute budget efficiency metrics
        total_fevals_budget = base_fevals + (
            (self.bonus_fevals_1src if n_sources == 1 else self.bonus_fevals_2src) if used_bonus else 0
        )
        early_terminated = any(ci.terminated_early for ci in conv_infos)
        fevals_saved = sum(
            ci.fevals_budget - ci.fevals_used for ci in conv_infos if ci.terminated_early
        )

        extra_info = {
            'early_terminated': early_terminated,
            'fevals_saved': fevals_saved,
            'used_bonus': used_bonus,
            'convergence_reasons': [ci.reason for ci in conv_infos],
            'final_sigmas': [ci.final_sigma for ci in conv_infos],
        }

        return candidate_sources, best_rmse, results, n_sims, extra_info
