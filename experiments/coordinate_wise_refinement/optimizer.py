"""
Coordinate-wise refinement after NM polish.

After standard CMA-ES + NM polish, do coordinate-wise line search
along each parameter axis. May find improvements that NM's simplex
geometry misses.

Very low overhead: 2*n_params evaluations per candidate.
"""
...
class CoordRefineOptimizer:
    """
    Optimizer with coordinate-wise refinement after NM polish.
    No perturbation - just CMA-ES + NM + coord refine.
    """
    ...
    def __init__(
        ...
        refine_maxiter: int = 7,  # Using 7 to stay in budget
        refine_top_n: int = 2,
        ...
        coord_step: float = 0.01,  # Step size for coordinate search
        coord_refine_top_n: int = 1,  # Only refine top N candidates
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _coordinate_refine(self, pos_params, objective, lb, ub, n_sims_counter):
        ...
        Tests +/- step on each coordinate and accepts improvements.
        """
        ...
        for coord_idx in range(len(pos_params)):
            ...
            if test_rmse < best_rmse:
                ...
                best_rmse = test_rmse
                continue  # Skip negative test, we improved
