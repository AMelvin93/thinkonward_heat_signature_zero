"""
Multi-Fidelity with COARSE-Grid Coordinate Descent Refinement.

Key insight: Use coordinate descent on coarse grid (faster than Nelder-Mead),
only use fine grid for final candidate evaluation.
"""
...
class CoarseCoordDescentOptimizer:
    """
    Multi-fidelity optimizer with Coordinate Descent refinement on COARSE grid.

    Key insight: Coordinate descent is simpler and potentially faster than Nelder-Mead.
    Do refinement on coarse grid (~4x faster than fine), only use fine grid for final eval.
    """
    ...
    def __init__(
        ...
        refine_cycles: int = 2,   # Coordinate descent cycles on COARSE grid
        refine_top_n: int = 3,    # Refine top N solutions
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # ===== KEY CHANGE: Coordinate Descent refinement on COARSE grid =====
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_cycles > 0:
                # Coordinate descent: optimize each coordinate in sequence
                refined_params = pos_params.copy()
                for cycle in range(self.refine_cycles):
                    for dim in range(len(refined_params)):
                        def line_objective(val):
                            test_params = refined_params.copy()
                            test_params[dim] = val
                            return objective_coarse(test_params)
                        ...
                        refined_params[dim] = result.x

                refined_rmse = objective_coarse(refined_params)
                if refined_rmse < rmse_coarse:
                    refined_solutions.append((refined_params, refined_rmse, 'refined_cd'))
                else:
