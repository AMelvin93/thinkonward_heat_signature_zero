#!/usr/bin/env python
"""Run A10: Alternating Optimization experiment."""
...
from experiments.alternating_opt.optimizer import (
...
def main():
    parser = argparse.ArgumentParser(description='Run Alternating Optimizer')
    ...
    print("=" * 70)
    print("A10: Alternating Optimization")
    print("=" * 70)
    ...
    print(f"N rounds: {config['n_rounds']}")
    print("=" * 70)
    ...
    if is_g4dn_simulation and not args.max_samples:
        ...
        run_name = f"alternating_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print()
    print(f"Init type distribution:")
    for init_type, count in sorted(init_counts.items(), key=lambda x: -x[1]):
