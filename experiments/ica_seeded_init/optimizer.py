"""
ICA-Seeded Temporal Fidelity Optimizer
...
Using ICA only for initialization (~5-10s) should provide better
starting positions for CMA-ES while staying within time budget.
"""
...
def fast_ica_decompose_2source(
    ...
    Key speedup: max_iter=50 instead of 500 (10x faster, ~1-2 sec per sample).
    """
    info = {
        'success': False,
        'method': 'fast_ica',
...
class TemporalFidelityWithPolishOptimizer:
    """
    Temporal fidelity optimizer with final NM polish using full timesteps.

    Key difference: After CMA-ES with truncated timesteps, do a final
    NM refinement on the BEST candidate using FULL timesteps for accuracy.
    """
    ...
    def __init__(
        ...
        # NEW: Final polish parameters
        final_polish_maxiter: int = 5,  # NM iterations on best candidate with full timesteps
        # NEW: ICA initialization parameters
        use_ica: bool = True,  # Enable ICA initialization for 2-source problems
        ica_max_iter: int = 50,  # FastICA iterations (50 = ~1-2 sec per sample)
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _ica_init_positions(self, sample):
        """
        Get position initialization from FastICA for 2-source problems.

        Uses the mixing matrix from ICA to estimate source positions via
        weighted centroid of sensor locations.
        """
        if not self.use_ica or sample['n_sources'] != 2:
            return None, {'used': False, 'reason': 'ICA only for 2-source'}
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # 1. ICA initialization (2-source only) - THE KEY INNOVATION
        if n_sources == 2:
            ica_init, ica_info = self._ica_init_positions(sample)
            if ica_init is not None and ica_info.get('success', False):
                primary_inits.append((ica_init, 'ica'))
