#!/usr/bin/env python3
"""
Run script for W2 Sigma 0.18/0.22 + Reduced Fevals optimizer.

EXP028: sigma_0.18_0.22_reduced_fevals
Config:
- sigma0_1src: 0.18 (proven to work)
- sigma0_2src: 0.22 (proven to work)
- threshold_1src: 0.35 (standard - proven optimal)
- threshold_2src: 0.45 (standard - proven optimal)
- fevals_1src: 16 (reduced from 20)
- fevals_2src: 30 (reduced from 36)
- fallback_fevals: 15 (reduced from 18)

Hypothesis: Reduce fevals to bring sigma 0.18/0.22 from 67.7 min into budget (~55 min).
Based on: EXP018 got 1.1271 @ 67.7 min. Need 20% time reduction - try 20% fewer fevals.
"""
...
def process_single_sample(args):
    ...
    optimizer = SigmaBaselineOptimizer(
        ...
        max_fevals_1src=config.get('max_fevals_1src', 16),
        max_fevals_2src=config.get('max_fevals_2src', 30),
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Sigma 0.18/0.22 + Reduced Fevals')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP028 specific
    ...
    parser.add_argument('--fallback-fevals', type=int, default=15)  # Reduced
    # Reduced primary fevals
    parser.add_argument('--max-fevals-1src', type=int, default=16)  # Reduced from 20
    ...
    print(f"\n[W2] Sigma 0.18/0.22 + Reduced Fevals Optimizer")
    print(f"=" * 70)
    print(f"EXP028: sigma_0.18_0.22_reduced_fevals")
    print(f"-" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Sigma: 1-src={args.sigma0_1src}, 2-src={args.sigma0_2src} (proven to work)")
    ...
    print(f"Fallback fevals: {args.fallback_fevals} (REDUCED)")
    print(f"=" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Sigma 0.18/0.22 + Reduced Fevals (EXP028)")
    print(f"{'='*70}")
    ...
    print(f"EXP018 (sigma 0.18/0.22, fevals 20/36): 1.1271 @ 67.7 min")
    print(f"Target: ~55-58 min with score >= EXP018 (1.1271)")
    print(f"This run: {score:.4f} @ {projected_400:.1f} min")
    print(f"Delta vs baseline: {score - 1.1247:+.4f} score, {projected_400 - 57.2:+.1f} min")
    print(f"Delta vs EXP018: {score - 1.1271:+.4f} score, {projected_400 - 67.7:+.1f} min")
    print()
    if projected_400 <= 60:
        if score > 1.1247:
