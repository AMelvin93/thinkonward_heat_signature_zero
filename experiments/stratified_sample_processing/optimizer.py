"""
Stratified Sample Processing Optimizer

Uses different optimization settings for 1-source vs 2-source problems:
- 1-source: fewer fevals (15), more polish (10 iter) - simpler landscape, refinement helps
- 2-source: more fevals (36), less polish (6 iter) - complex landscape, exploration matters
...
class StratifiedOptimizer:
    """
    Stratified optimizer with different settings for 1-source vs 2-source.

    Key differences from baseline:
    - 1-source: fewer fevals (15), more polish (10 iter)
    - 2-source: standard fevals (36), less polish (6 iter)
    """
    ...
    def __init__(
        ...
        ny_coarse: int = 25,
        # STRATIFIED settings
        max_fevals_1src: int = 15,  # Reduced from 20
        max_fevals_2src: int = 36,  # Same as baseline
        sigma0_1src: float = 0.15,
        sigma0_2src: float = 0.20,
        # STRATIFIED polish settings
        polish_maxiter_1src: int = 10,  # Increased from 8
        polish_maxiter_2src: int = 6,   # Reduced from 8
        # Standard settings
        ...
        self.polish_maxiter_1src = polish_maxiter_1src
        self.polish_maxiter_2src = polish_maxiter_2src
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # STRATIFIED: use different settings based on n_sources
        max_fevals = self.max_fevals_1src if n_sources == 1 else self.max_fevals_2src
        sigma0 = self.sigma0_1src if n_sources == 1 else self.sigma0_2src
        polish_maxiter = self.polish_maxiter_1src if n_sources == 1 else self.polish_maxiter_2src
        ...
        # STRATIFIED POLISH: use different maxiter based on n_sources
        if polish_maxiter > 0 and candidates_raw:
            ...
            result = minimize(
                ...
                best_pos_params,
                method='Nelder-Mead',
                options={'maxiter': polish_maxiter, 'xatol': 0.005, 'fatol': 0.0005}
            )
