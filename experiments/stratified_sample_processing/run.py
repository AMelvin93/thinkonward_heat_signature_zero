#!/usr/bin/env python3
"""Run script for Stratified Sample Processing with MLflow logging."""
...
from optimizer import StratifiedOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = StratifiedOptimizer(
        max_fevals_1src=config['max_fevals_1src'],
        max_fevals_2src=config['max_fevals_2src'],
        polish_maxiter_1src=config['polish_maxiter_1src'],
        polish_maxiter_2src=config['polish_maxiter_2src'],
...
def main():
    parser = argparse.ArgumentParser(description='Run Stratified Sample Processing')
    ...
    parser.add_argument('--polish-maxiter-1src', type=int, default=10)
    parser.add_argument('--polish-maxiter-2src', type=int, default=6)
    ...
    print(f"\nStratified Sample Processing")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Timestep fraction: {args.timestep_fraction:.0%}")
    print(f"1-source: fevals={args.max_fevals_1src}, polish={args.polish_maxiter_1src} iter")
    print(f"2-source: fevals={args.max_fevals_2src}, polish={args.polish_maxiter_2src} iter")
    print(f"=" * 70)

    config = {
        'max_fevals_1src': args.max_fevals_1src,
        'max_fevals_2src': args.max_fevals_2src,
        'polish_maxiter_1src': args.polish_maxiter_1src,
        'polish_maxiter_2src': args.polish_maxiter_2src,
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Stratified Sample Processing")
    print(f"{'='*70}")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"stratified_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param('experiment_id', 'EXP_SAMPLE_STRATIFIED_001')
            mlflow.log_param('worker', 'W2')
            mlflow.log_param('optimizer', 'StratifiedOptimizer')
            ...
            mlflow.log_param('polish_maxiter_1src', args.polish_maxiter_1src)
            mlflow.log_param('polish_maxiter_2src', args.polish_maxiter_2src)
