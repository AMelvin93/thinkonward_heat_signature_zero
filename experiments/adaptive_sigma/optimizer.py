"""
Adaptive Sigma CMA-ES for Heat Source Identification.

Key hypothesis: Higher initial sigma (0.30/0.35) provides broader exploration.
...
Comparison:
- Baseline sigma: 0.15/0.20 -> Score 1.1247 @ 57.2 min
- High sigma: 0.20/0.25 -> Score 1.1362 @ 69.2 min (over budget)
- This experiment: 0.30/0.35 -> ? (hypothesis: better exploration, same feval budget)
"""
...
class AdaptiveSigmaOptimizer:
    """
    High initial sigma optimizer: starts with broad exploration.

    Key change from robust_fallback:
    - sigma0_1src: 0.15 -> 0.30 (2x higher)
    - sigma0_2src: 0.20 -> 0.35 (1.75x higher)

    CMA-ES's internal sigma adaptation will reduce step size over time,
    potentially giving benefits of high sigma exploration without full time cost.
    """
    ...
    def __init__(
        ...
        sigma0_1src: float = 0.30,  # HIGH initial sigma
        sigma0_2src: float = 0.35,  # HIGH initial sigma
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run a single optimization pass with high initial sigma."""
        ...
        # Refine top N solutions on COARSE grid with Nelder-Mead
        refined_solutions = []
