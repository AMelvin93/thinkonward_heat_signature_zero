#!/usr/bin/env python3
"""
Run script for W2 Sigma 0.17/0.21 + Reduced Fevals optimizer.

EXP015: sigma_0.17_0.21_reduced_fevals
Config:
- sigma0_1src: 0.17 (slightly higher than baseline)
- sigma0_2src: 0.21 (slightly higher than baseline)
...
Hypothesis: Combine sigma 0.17/0.21 with reduced fevals to target ~58 min with better score.
Based on: Timing model suggests sigma 0.17/0.21 ~ 60 min. Reduced fevals saves ~2-3 min.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Sigma 0.17/0.21 + Reduced Fevals optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP015 specific
    parser.add_argument('--sigma0-1src', type=float, default=0.17)  # Slightly higher
    parser.add_argument('--sigma0-2src', type=float, default=0.21)  # Slightly higher
    ...
    print(f"\n[W2] Sigma 0.17/0.21 + Reduced Fevals Optimizer")
    print(f"=" * 70)
    print(f"EXP015: sigma_0.17_0.21_reduced_fevals")
    print(f"-" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Sigma 0.17/0.21 + Reduced Fevals (EXP015)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Config: sigma={args.sigma0_1src}/{args.sigma0_2src}, fevals={args.max_fevals_1src}/{args.max_fevals_2src} (reduced)")
    print(f"Baseline: 1.1247 @ 57.2 min")
    print(f"Target: ~58 min with score improvement")
