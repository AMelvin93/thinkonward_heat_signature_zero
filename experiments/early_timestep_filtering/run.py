#!/usr/bin/env python3
"""
Run script for Temporal Fidelity optimizer.

Tests the hypothesis that using reduced timesteps (25-50%) for CMA-ES
evaluation can provide 2-4x speedup while maintaining accuracy.
"""
...
def process_single_sample(args):
    ...
    optimizer = TemporalFidelityOptimizer(
        ...
        timestep_fraction=config['timestep_fraction'],  # KEY PARAMETER
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Temporal Fidelity optimizer')
    ...
    # KEY: Temporal fidelity parameter
    parser.add_argument('--timestep-fraction', type=float, default=0.25,
                        help='Fraction of timesteps to use for CMA-ES (0.25=4x speedup)')
    ...
    print(f"\nTemporal Fidelity Optimizer")
    print(f"=" * 60)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Timestep fraction: {args.timestep_fraction:.0%} (expected {1/args.timestep_fraction:.1f}x speedup)")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Temporal Fidelity ({args.timestep_fraction:.0%} timesteps)")
    print(f"{'='*70}")
    ...
    if projected_400 > 60:
        print("OVER BUDGET")
    elif score >= 1.12 and projected_400 <= 45:
        print("SUCCESS! Meets criteria: score >= 1.12, time <= 45 min (20% speedup)")
    elif score >= 1.12:
        print("PARTIAL: Good score but minimal time improvement")
    elif projected_400 <= 45:
        print("PARTIAL: Good speedup but score dropped")
    else:
