#!/usr/bin/env python3
"""
Run script for Sensor Subset Diversity optimizer.

Key Experiment:
    Use different sensor subsets to generate naturally diverse candidates.
    Each subset gives a different "view" of the problem.

Usage:
    uv run python experiments/sensor_subset_diversity/run.py --workers 7 --shuffle
    uv run python experiments/sensor_subset_diversity/run.py --workers 7 --max-samples 10
"""
...
def process_single_sample(args):
    ...
    optimizer = SensorSubsetDiversityOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Sensor Subset Diversity optimizer')
    ...
    parser.add_argument('--n-subsets', type=int, default=3,
                        help='Number of sensor subsets (default: 3)')
    ...
    # Calculate score using per-sample scoring (same as analytical_intensity)
    def calculate_sample_score(rmse, n_candidates, lambda_=0.3, n_max=3):
        """Calculate competition score for a sample."""
        if n_candidates == 0:
            return 0.0
    ...
    # MLflow logging
    if MLFLOW_AVAILABLE:
        mlflow.set_experiment("sensor_subset_diversity")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        run_name = f"subset_{args.max_fevals_1src}_{args.max_fevals_2src}_{timestamp}"
