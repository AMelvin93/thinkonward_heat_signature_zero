...
Key idea: Run CMA-ES multiple times with different random seeds
for each sample and take the best result. This reduces variance
from CMA-ES's stochastic nature.

Trade-off: More runs = more time, but can distribute fevals across runs.
"""
...
class MultiSeedOptimizer:
    """
    Runs CMA-ES with multiple seeds and takes the best result.
    """
    ...
    def __init__(
        ...
        n_seeds: int = 2,  # Number of CMA-ES runs with different seeds
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Distribute fevals across inits and seeds
        # With 2 inits and 2 seeds = 4 runs total
        n_runs = len(initializations) * self.n_seeds
        fevals_per_run = max(5, max_fevals // n_runs)
        ...
        for seed_offset in range(self.n_seeds):
            for init_params, init_type in initializations:
                ...
                opts['tolfun'] = 1e-6
                opts['tolx'] = 1e-6
                opts['seed'] = 42 + seed_offset * 1000  # Different seed per run
                ...
                while not es.stop():
                    ...
                    for sol, fit in zip(solutions, fitness):
                        all_solutions.append((np.array(sol), fit, f"{init_type}_s{seed_offset}"))
