"""
Double NM Polish Optimizer

Based on solution_verification_pass but replaces gradient verification with
double NM polish rounds:

1. Standard CMA-ES optimization
2. First NM polish (6 iterations) - coarse refinement
3. Perturb simplex vertices slightly (scale 0.02)
4. Second NM polish (4 iterations) - fine refinement
5. Keep best result from each candidate

Hypothesis: First NM polish may get stuck in local basin. Second round from
different simplex may escape.
"""
...
class DoubleNMPolishOptimizer:
    """
    Optimizer with double Nelder-Mead polish rounds.

    Key addition: After CMA-ES, run two rounds of NM polish:
    - First round: 6 iterations for coarse refinement
    - Perturb simplex vertices
    - Second round: 4 iterations for fine refinement
    """
    ...
    def __init__(
        ...
        # Double polish parameters
        polish1_maxiter: int = 6,  # First polish iterations
        polish2_maxiter: int = 4,  # Second polish iterations
        perturb_scale: float = 0.02,  # Simplex perturbation scale
        refine_top_n: int = 2,
        ...
        self.polish1_maxiter = polish1_maxiter
        self.polish2_maxiter = polish2_maxiter
    ...
    def _double_nm_polish(self, pos_params, objective_fn, lb, ub):
        """
        Run two rounds of NM polish:
        1. First round with more iterations
        2. Perturb solution slightly
        3. Second round for fine-tuning

        Returns: (best_pos, best_rmse, n_extra_evals)
        """
        n_params = len(pos_params)
        n_evals = [0]

        best_pos = pos_params.copy()
        best_rmse = objective_fn(pos_params)
        n_evals[0] += 1

        # === First NM Polish Round ===
        if self.polish1_maxiter > 0:
            result1 = minimize(
                objective_fn,
                pos_params,
                method='Nelder-Mead',
                options={
                    'maxiter': self.polish1_maxiter,
                    'xatol': 0.01,
                    'fatol': 0.001,
                }
            )
            n_evals[0] += result1.nfev

            if result1.fun < best_rmse:
                best_pos = result1.x
                best_rmse = result1.fun

        # === Perturb Solution ===
        perturbed_pos = best_pos + np.random.randn(n_params) * self.perturb_scale
        # Clip to bounds
        perturbed_pos = np.clip(perturbed_pos, lb, ub)

        # === Second NM Polish Round ===
        if self.polish2_maxiter > 0:
            result2 = minimize(
                objective_fn,
                perturbed_pos,
                method='Nelder-Mead',
                options={
                    'maxiter': self.polish2_maxiter,
                    'xatol': 0.005,  # Tighter tolerance for fine polish
                    'fatol': 0.0005,
                }
            )
            n_evals[0] += result2.nfev

            if result2.fun < best_rmse:
                best_pos = result2.x
                best_rmse = result2.fun

        return best_pos, best_rmse, n_evals[0]

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, nt_reduced):
        """Run CMA-ES optimization with double NM polish."""
        ...
        # === DOUBLE NM POLISH for top N solutions ===
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            # Apply double NM polish on COARSE grid with reduced timesteps
            polished_pos, polished_rmse, n_extra = self._double_nm_polish(
                pos_params, objective_coarse, lb, ub
            )

            if polished_rmse < rmse_coarse:
                refined_solutions.append((polished_pos, polished_rmse, 'double_polished'))
            else:
                refined_solutions.append((pos_params, rmse_coarse, init_type))

        # Add remaining top solutions (no polish)
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Double NM polish: {self.polish1_maxiter} + {self.polish2_maxiter} iterations")
            print(f"  Perturbation scale: {self.perturb_scale}")
