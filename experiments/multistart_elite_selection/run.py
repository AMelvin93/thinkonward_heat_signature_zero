#!/usr/bin/env python3
"""Run script for Elite Selection Multi-Start experiment with MLflow logging."""
...
from optimizer import EliteSelectionOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = EliteSelectionOptimizer(
        ...
        n_initial_runs=config['n_initial_runs'],
        eval_generations=config['eval_generations'],
        top_n_continue=config['top_n_continue'],
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Elite Selection Multi-Start Experiment')
    ...
    # Elite selection parameters
    parser.add_argument('--n-initial-runs', type=int, default=4)
    parser.add_argument('--eval-generations', type=int, default=3)
    parser.add_argument('--top-n-continue', type=int, default=2)
    ...
    print(f"\nElite Selection Multi-Start Experiment")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Timestep fraction: {args.timestep_fraction:.0%}")
    print(f"Elite selection: {args.n_initial_runs} runs, eval after {args.eval_generations} gens, "
          f"continue top {args.top_n_continue}")
    ...
    config = {
        ...
        'n_initial_runs': args.n_initial_runs,
        'eval_generations': args.eval_generations,
        'top_n_continue': args.top_n_continue,
    }
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(f"[{len(results):3d}/{n_samples}] Sample {result['idx']:3d}: "
                  ...
                  f"time={result['elapsed']:.1f}s [{status}]{polished}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Elite Selection Multi-Start")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (fixed 8 NM): 1.1688 @ 58.4 min (documented)")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"elite_selection_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_MULTISTART_ELITE_001")
            ...
            mlflow.log_param("n_initial_runs", args.n_initial_runs)
            mlflow.log_param("eval_generations", args.eval_generations)
            mlflow.log_param("top_n_continue", args.top_n_continue)
