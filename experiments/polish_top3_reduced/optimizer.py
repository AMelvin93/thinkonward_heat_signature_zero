"""
Polish Top-3 Optimizer - Polish ALL top-3 candidates instead of just best

Hypothesis: Improving accuracy of 2nd/3rd candidates will increase the average
accuracy component of the score since the scoring formula averages accuracy
over all N candidates: score = (1/N) * sum(1/(1+RMSE_i)) + 0.3*(N/3)

Key change: Polish all top-3 candidates with 4 NM iterations each (12 total)
instead of just best with 8 iterations.
"""
...
class PolishTop3Optimizer:
    """
    Optimizer that polishes ALL top-3 candidates instead of just the best.

    Key difference from baseline:
    - Baseline: 8 NM iterations on best candidate only
    - This: 4 NM iterations on each of top-3 candidates (12 total)

    Hypothesis: Improving 2nd/3rd candidate accuracy increases average score.
    """
    ...
    def __init__(
        ...
        # NEW: Polish parameters for top-3
        polish_iters_per_candidate: int = 4,  # NM iterations per candidate
        n_candidates_to_polish: int = 3,  # How many top candidates to polish
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.polish_iters_per_candidate = polish_iters_per_candidate
        self.n_candidates_to_polish = n_candidates_to_polish
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run optimization with top-3 candidate polishing."""
        ...
        # FINAL POLISH: NM refinement on TOP-N candidates using FULL timesteps
        if self.polish_iters_per_candidate > 0 and candidates_raw:
            # Sort by RMSE to find top candidates
            candidates_raw.sort(key=lambda x: x[2])

            # Polish top N candidates
            n_to_polish = min(self.n_candidates_to_polish, len(candidates_raw))
            for idx in range(n_to_polish):
                pos_params = candidates_raw[idx][4]
                best_rmse = candidates_raw[idx][2]
                ...
                result = minimize(
                    ...
                    pos_params,
                    method='Nelder-Mead',
                    options={'maxiter': self.polish_iters_per_candidate, 'xatol': 0.005, 'fatol': 0.0005}
                )

                if result.fun < best_rmse:
                    # Update candidate with polished result
                    if n_sources == 1:
                        x, y = result.x
                        q, _, final_rmse = compute_optimal_intensity_1src(
                            x, y, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
                    ...
                    candidates_raw[idx] = (sources, full_params, final_rmse, 'polished', result.x)
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run optimization with top-3 polish
