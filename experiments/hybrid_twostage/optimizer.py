"""
Hybrid Two-Stage Optimizer: Fast CMA-ES + L-BFGS-B Refinement.

Key idea:
- Stage 1: Quick CMA-ES (8-12 fevals) with large sigma for broad exploration
- Stage 2: L-BFGS-B refinement on top candidates for precise local convergence

This combines global exploration of CMA-ES with fast local refinement of L-BFGS-B.
"""
...
class HybridTwoStageOptimizer:
    """
    Hybrid Two-Stage Optimizer.

    Stage 1: Fast CMA-ES exploration with high sigma and few evaluations
    Stage 2: L-BFGS-B local refinement on the most promising candidates

    Hypothesis: CMA-ES quickly identifies promising regions.
    L-BFGS-B converges faster for local refinement than more CMA-ES iterations.
    """
    ...
    def __init__(
        ...
        stage1_fevals_1src: int = 8,
        stage1_fevals_2src: int = 12,
        stage1_sigma: float = 0.25,  # Higher sigma for exploration
        # Stage 2: L-BFGS-B refinement
        stage2_lbfgs_maxiter: int = 15,
        n_candidates_to_refine: int = 3,
        # Fallback thresholds
        fallback_threshold_1src: float = 0.40,
        fallback_threshold_2src: float = 0.50,
        # Common settings
        ...
        # Stage 1
        self.stage1_fevals_1src = stage1_fevals_1src
        self.stage1_fevals_2src = stage1_fevals_2src
        self.stage1_sigma = stage1_sigma

        # Stage 2
        self.stage2_lbfgs_maxiter = stage2_lbfgs_maxiter
        self.n_candidates_to_refine = n_candidates_to_refine

        # Fallback
        self.fallback_threshold_1src = fallback_threshold_1src
        self.fallback_threshold_2src = fallback_threshold_2src
    ...
    def _onset_init_positions(self, sample, n_sources):
        """Init based on earliest temperature rise."""
        readings = sample['Y_noisy']
        sensors = np.array(sample['sensors_xy'])

        # Find when each sensor first shows significant temperature rise
        baseline = readings[0]
        onset_times = []
        for i in range(len(sensors)):
            rise = readings[:, i] - baseline[i]
            significant = np.where(rise > 0.05 * np.max(rise))[0]
            onset_times.append(significant[0] if len(significant) > 0 else len(readings))

        # Select sensors with earliest onset
        onset_order = np.argsort(onset_times)
        selected = []
        for idx in onset_order:
            if len(selected) >= n_sources:
                break
            if all(np.linalg.norm(sensors[idx] - sensors[p]) >= 0.25 for p in selected):
                selected.append(idx)
    ...
    def _run_stage1_cmaes(self, objective, init_params, n_sources):
        """Stage 1: Quick CMA-ES exploration."""
        max_fevals = self.stage1_fevals_1src if n_sources == 1 else self.stage1_fevals_2src
        ...
        es = cma.CMAEvolutionStrategy(init_params.tolist(), self.stage1_sigma, opts)
    ...
    def _run_stage2_lbfgsb(self, objective, candidates, n_sources):
        """Stage 2: L-BFGS-B refinement on top candidates."""
        lb, ub = self._get_position_bounds(n_sources)
        bounds = list(zip(lb, ub))

        refined = []
        for pos_params, rmse in candidates[:self.n_candidates_to_refine]:
            result = minimize(
                objective,
                pos_params,
                method='L-BFGS-B',
                bounds=bounds,
                options={
                    'maxiter': self.stage2_lbfgs_maxiter,
                    'ftol': 1e-6,
                    'gtol': 1e-6,
                }
            )
            if result.fun < rmse:
                refined.append((result.x, result.fun, 'lbfgsb_refined'))
            else:
                refined.append((pos_params, rmse, 'cmaes'))

        # Also keep some unrefined candidates for diversity
        for pos_params, rmse in candidates[self.n_candidates_to_refine:self.candidate_pool_size]:
            refined.append((pos_params, rmse, 'cmaes'))
    ...
    def _run_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Stage 1: Quick CMA-ES exploration from each initialization
        all_cmaes_solutions = []
        for init_params, init_type in initializations:
            solutions = self._run_stage1_cmaes(objective_coarse, init_params, n_sources)
            for sol, fit in solutions:
                all_cmaes_solutions.append((sol, fit))

        # Deduplicate and sort
        all_cmaes_solutions.sort(key=lambda x: x[1])

        # Stage 2: L-BFGS-B refinement on top candidates
        refined_solutions = self._run_stage2_lbfgsb(objective_coarse, all_cmaes_solutions, n_sources)
        ...
        candidates_raw = []
        for pos_params, rmse_coarse, stage_type in refined_solutions[:self.candidate_pool_size]:
            if n_sources == 1:
                x, y = pos_params
            ...
            candidates_raw.append((sources, full_params, final_rmse, stage_type))
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        threshold = self.fallback_threshold_1src if n_sources == 1 else self.fallback_threshold_2src

        if best_rmse_initial > threshold:
            # Fallback: try more initializations
            fallback_inits = []
            onset_init = self._onset_init_positions(sample, n_sources)
            fallback_inits.append((onset_init, 'onset'))
