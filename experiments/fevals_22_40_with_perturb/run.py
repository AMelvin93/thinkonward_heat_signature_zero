"""
Test combining higher fevals (22/40) with full perturbation config (8 NM, 2 perturb).

Based on two successful findings:
1. hopping_no_tabu: 1.1689 @ 58.18 min (8 NM, 2 perturb, 20/36 fevals)
2. fevals_22_40: 1.1640 @ 57.10 min (4 NM, 1 perturb, 22/40 fevals)

Testing: What if we combine 8 NM + 2 perturb + higher fevals?
"""
...
# Import the optimizer using importlib
...
def main():
    ...
    # Uses: 8 NM, 2 perturb, sigma 0.18/0.22, 40% temporal, 20/36 fevals
    base_config = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'timestep_fraction': 0.40,
        'refine_maxiter': 8,  # Full NM polish (best known)
        'enable_tabu_hopping': True,
        'n_perturbations': 2,  # 2 perturbations (best known)
        'perturbation_scale': 0.05,
        'perturb_nm_iters': 3,
        'tabu_distance': 0.0,  # No tabu checking (simpler)
    }

    # Test combining higher fevals with full perturbation
    configs = [
        ({**base_config, 'max_fevals_1src': 22, 'max_fevals_2src': 40}, 'nm8_perturb2_fevals22_40'),
        ({**base_config, 'max_fevals_1src': 24, 'max_fevals_2src': 42}, 'nm8_perturb2_fevals24_42'),
        ({**base_config, 'refine_maxiter': 6, 'max_fevals_1src': 22, 'max_fevals_2src': 40}, 'nm6_perturb2_fevals22_40'),
    ]
    ...
    print("="*70)
    print(f"{'Config':<30} {'Score':>8} {'Time':>8} {'In Budget':>10} {'RMSE 1src':>10} {'RMSE 2src':>10}")
    print("-"*70)
    for r in results:
        in_budget_str = "YES" if r['in_budget'] else "NO"
        print(f"{r['config']:<30} {r['score']:>8.4f} {r['time_min']:>7.1f}m {in_budget_str:>10} {r['rmse_1src']:>10.6f} {r['rmse_2src']:>10.6f}")

    # Compare with baselines
    print("\n--- Comparison with baselines ---")
    print("hopping_no_tabu (8 NM, 2 perturb, 20/36): 1.1689 @ 58.18 min")
    print("fevals_22_40 (4 NM, 1 perturb, 22/40): 1.1640 @ 57.10 min")
