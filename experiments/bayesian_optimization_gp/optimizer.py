"""
Bayesian Optimization with Gaussian Process Surrogate for Heat Source Identification.

Key idea: Use GP-based Bayesian Optimization with Expected Improvement acquisition function
for sample-efficient optimization. Collect all evaluated points as candidate pool for diversity.

Unlike lq-CMA-ES which only returned one solution, BO naturally tracks all evaluated points.
"""
...
class BayesianOptimizer:
    ...
    Uses scikit-optimize's gp_minimize with Expected Improvement acquisition.
    Collects all evaluated points as candidate pool for diversity scoring.
    """
    ...
    def __init__(
        ...
        self.n_calls_1src = n_calls_1src
        self.n_calls_2src = n_calls_2src
    ...
    def _smart_init_position(self, sample, n_sources):
        """Get smart initialization from hottest sensors."""
    ...
    def _run_bo_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                             n_sources, early_frac):
        """Run Bayesian Optimization and collect all evaluated points."""
        ...
        n_sims = [0]
        all_solutions = []  # Collect all evaluated points
        ...
        if n_sources == 1:
            space = [
                Real(margin * self.Lx, (1 - margin) * self.Lx, name='x'),
                Real(margin * self.Ly, (1 - margin) * self.Ly, name='y'),
            ]
            ...
            def objective(params):
                ...
                # Store this solution
                all_solutions.append(([x, y], q, rmse_early))
                return rmse_early

            n_calls = self.n_calls_1src
        else:
            space = [
                Real(margin * self.Lx, (1 - margin) * self.Lx, name='x1'),
                Real(margin * self.Ly, (1 - margin) * self.Ly, name='y1'),
                Real(margin * self.Lx, (1 - margin) * self.Lx, name='x2'),
                Real(margin * self.Ly, (1 - margin) * self.Ly, name='y2'),
            ]
            ...
            def objective(params):
                ...
                (q1, q2), Y_pred, rmse_early, rmse_full = compute_optimal_intensity_2src(
                    ...
                    early_fraction=early_frac)
                # Store this solution
                all_solutions.append(([x1, y1, x2, y2], (q1, q2), rmse_early))
                return rmse_early

            n_calls = self.n_calls_2src

        # Get smart initialization for x0
        smart_init = self._smart_init_position(sample, n_sources)
        tri_init = self._triangulation_init_positions(sample, meta, n_sources, q_range)

        x0 = [smart_init.tolist()]
        if tri_init is not None:
            x0.append(tri_init.tolist())
        ...
        try:
            result = gp_minimize(
                ...
                space,
                n_calls=n_calls,
                n_initial_points=max(1, self.n_initial_points - len(x0)),
                x0=x0,
                acq_func=self.acquisition_function,
                random_state=42,
                verbose=False,
                n_jobs=1,  # No parallelism within BO
            )
        except Exception as e:
            # Fallback: just use the smart init
            print(f"BO failed: {e}, using smart init")
            objective(smart_init.tolist())

        # Sort solutions by RMSE and take top candidates
        all_solutions.sort(key=lambda x: x[2])
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run Bayesian Optimization
        bo_solutions, n_sims = self._run_bo_optimization(
        ...
        # Refine top solutions on coarse grid with Nelder-Mead
        refined_solutions = []
        for pos, q_vals, rmse_coarse in bo_solutions[:self.refine_top_n]:
            if n_sources == 1:
                def obj_refine(xy):
                    x, y = xy
                    q, _, rmse_early, _ = compute_optimal_intensity_1src(
                        x, y, Y_observed, solver_coarse, dt, nt, T0, sensors_xy, q_range,
                        early_fraction=early_frac)
                    return rmse_early
                ...
                if result.fun < rmse_coarse:
                    n_sims += 1
                    q, _, _, rmse = compute_optimal_intensity_1src(
                        result.x[0], result.x[1], Y_observed, solver_coarse, dt, nt, T0, sensors_xy, q_range,
                        early_fraction=early_frac)
                    refined_solutions.append((result.x.tolist(), q, result.fun))
                else:
                    refined_solutions.append((pos, q_vals, rmse_coarse))
            else:
                def obj_refine(params):
                    x1, y1, x2, y2 = params
                    (q1, q2), _, rmse_early, _ = compute_optimal_intensity_2src(
                        x1, y1, x2, y2, Y_observed, solver_coarse, dt, nt, T0, sensors_xy, q_range,
                        early_fraction=early_frac)
                    return rmse_early
                ...
                if result.fun < rmse_coarse:
                    n_sims += 2
                    (q1, q2), _, _, rmse = compute_optimal_intensity_2src(
                        result.x[0], result.x[1], result.x[2], result.x[3],
                        Y_observed, solver_coarse, dt, nt, T0, sensors_xy, q_range,
                        early_fraction=early_frac)
                    refined_solutions.append((result.x.tolist(), (q1, q2), result.fun))
                else:
                    refined_solutions.append((pos, q_vals, rmse_coarse))

        # Add remaining top solutions
        for sol in bo_solutions[self.refine_top_n:self.candidate_pool_size]:
            refined_solutions.append(sol)

        # Evaluate candidates on FINE grid
        candidates_raw = []
        for pos, q_vals, rmse_coarse in refined_solutions:
            if n_sources == 1:
                x, y = pos
            ...
            candidates_raw.append((sources, full_params, final_rmse, 'bo'))
