"""
Targeted High Sigma for 2-Source Problems.

Key idea: 2-source problems are much harder (RMSE 0.23 vs 0.14 for 1-src).
Use conservative settings for easy 1-source problems (fast),
use aggressive settings for hard 2-source problems (more exploration).
...
class Targeted2SrcSigmaOptimizer:
    """
    Optimizer with targeted high sigma for 2-source problems.

    1-source: Conservative baseline settings (fast, reliable)
    2-source: Aggressive higher sigma (better exploration for hard cases)
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        # 1-source settings (conservative/fast)
        max_fevals_1src: int = 20,
        sigma0_1src: float = 0.15,
        # 2-source settings (aggressive/thorough)
        max_fevals_2src: int = 36,
        sigma0_2src: float = 0.25,  # Higher sigma for better exploration
