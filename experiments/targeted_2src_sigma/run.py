#!/usr/bin/env python3
"""
Run script for Targeted 2-Source High Sigma optimizer.

Tests whether targeting higher sigma specifically for 2-source problems
improves accuracy without excessive time penalty.
"""
...
from optimizer import Targeted2SrcSigmaOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = Targeted2SrcSigmaOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Targeted 2-Source High Sigma optimizer')
    ...
    print(f"\nTargeted 2-Source High Sigma Optimizer")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"1-source: sigma={args.sigma0_1src}, fevals={args.max_fevals_1src}")
    print(f"2-source: sigma={args.sigma0_2src}, fevals={args.max_fevals_2src}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: Targeted 2-Source High Sigma")
    print(f"{'='*70}")
