#!/usr/bin/env python3
"""
Run script for IPOP-CMA-ES with Temporal Fidelity optimizer.

IPOP-CMA-ES restarts with larger populations to escape local optima.
Combined with 40% timesteps for fast evaluation.
"""
...
from optimizer import IPOPTemporalOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = IPOPTemporalOptimizer(
        ...
        popsize_factor=config.get('popsize_factor', 2),
        initial_popsize=config.get('initial_popsize', 4),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run IPOP-CMA-ES Temporal optimizer')
    ...
    parser.add_argument('--max-restarts', type=int, default=2,
                        help='Maximum number of IPOP restarts (0=no restarts)')
    parser.add_argument('--popsize-factor', type=int, default=2,
                        help='Population increase factor per restart')
    parser.add_argument('--initial-popsize', type=int, default=4,
    ...
    print(f"IPOP: max_restarts={args.max_restarts}, popsize_factor={args.popsize_factor}")
    print(f"Initial popsize: {args.initial_popsize}")
    ...
    config = {
        ...
        'popsize_factor': args.popsize_factor,
        'initial_popsize': args.initial_popsize,
    }
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - IPOP-CMA-ES with Temporal Fidelity")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (8 NM polish): 1.1688 @ 58.4 min")
    ...
    if projected_400 > 60:
        ...
        print("SUCCESS! Beats baseline (1.1688)")
    elif score >= 1.14:
        print("PARTIAL: Near baseline but not better")
    else:
