"""
IPOP-CMA-ES with Temporal Fidelity

IPOP-CMA-ES (Increasing Population CMA-ES) restarts with larger populations
to escape local optima. Combined with temporal fidelity (40% timesteps)
for fast evaluation.

Key features:
- IPOP restart strategy: population doubles after convergence
- 40% timesteps during CMA-ES (temporal fidelity speedup)
- Final NM polish using FULL timesteps
- Up to 2-3 restarts per initialization
"""
...
class IPOPTemporalOptimizer:
    """
    IPOP-CMA-ES with temporal fidelity and final polish.

    IPOP (Increasing Population) strategy:
    - Start with base population size
    - After convergence, restart with 2x population
    ...
    def __init__(
        ...
        # IPOP-specific parameters
        max_restarts: int = 2,  # Maximum number of IPOP restarts
        popsize_factor: int = 2,  # Population increase factor per restart
        initial_popsize: int = 4,  # Initial population size (None = default)
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _run_ipop_cmaes(self, objective, init_params, sigma0, lb, ub, max_fevals, n_sources):
        """
        Run IPOP-CMA-ES: multiple restarts with increasing population.

        Returns list of (solution, fitness, restart_num) tuples.
        """
        all_solutions = []
        best_solution = None
        ...
        popsize = self.initial_popsize

        for restart_num in range(self.max_restarts + 1):
            if remaining_fevals <= 0:
                break

            # Allocate fevals for this restart
            restart_fevals = min(remaining_fevals, max_fevals // (self.max_restarts + 1))
            if restart_num == self.max_restarts:
                restart_fevals = remaining_fevals  # Use all remaining
            ...
            # Start from best found or random restart
            if restart_num > 0 and best_solution is not None:
                # IPOP: restart from random, not best (to explore)
                init_for_restart = self._random_init_positions(n_sources, margin=0.1)
            else:
                init_for_restart = init_params
            ...
            while not es.stop():
                ...
                for sol, fit in zip(solutions, fitness):
                    all_solutions.append((np.array(sol), fit, f'restart_{restart_num}'))
                    if fit < best_fitness:
                        best_fitness = fit
            ...
            if popsize is not None:
                popsize = popsize * self.popsize_factor
            else:
                # If using default, get the actual popsize and increase it
                popsize = es.popsize * self.popsize_factor
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run IPOP-CMA-ES optimization with final polish."""
        ...
        # Run IPOP-CMA-ES for each initialization
        for init_params, init_type in initializations:
            solutions = self._run_ipop_cmaes(
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run IPOP optimization with final polish
