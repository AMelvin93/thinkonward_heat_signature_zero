#!/usr/bin/env python3
"""
Run script for W1 Sweet Spot Faster Fallback optimizer.

EXP007: Same config as EXP001 (1.1330 score) but with reduced fallback_fevals (14 vs 18).
Testing if fewer fallback fevals can reduce time while keeping EXP001's good score.
Note: Based on EXP005 finding, fallback fevals likely don't affect overall timing much.
...
- threshold_1src: 0.38
- threshold_2src: 0.48
- fallback_fevals: 14 (reduced from 18)
"""
...
from optimizer import SigmaSweetSpotOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = SigmaSweetSpotOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Sweet Spot Faster Fallback optimizer')
    ...
    parser.add_argument('--threshold-1src', type=float, default=0.38)
    parser.add_argument('--threshold-2src', type=float, default=0.48)
    parser.add_argument('--fallback-fevals', type=int, default=14)  # Reduced from 18
    ...
    print(f"\nW1 Sweet Spot Faster Fallback Optimizer (EXP007)")
    print(f"=" * 60)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            # Flag if this looks like a fallback was triggered
            flag = ""
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Sweet Spot Faster Fallback (EXP007)")
    print(f"{'='*70}")
    ...
    print(f"EXP001:   1.1330 @ 68.1 min (same thresholds, 18 fallback_fevals)")
    print(f"This run: {score:.4f} @ {projected_400:.1f} min")
    print(f"Delta vs baseline: {score - 1.1247:+.4f} score, {projected_400 - 57.2:+.1f} min")
    print(f"Delta vs EXP001:   {score - 1.1330:+.4f} score, {projected_400 - 68.1:+.1f} min")
    print()
    if projected_400 > 60:
        print("OVER BUDGET")
    elif score > 1.1247:
