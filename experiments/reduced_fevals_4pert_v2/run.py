...
Test reduced CMA-ES fevals with 4 perturbations to find budget-fitting config.

Baseline:
- no_perturb @ 20/44 fevals: 56.3 min, score 1.1367
- 4pert_nm2 @ 20/44 fevals: 70.9 min, score 1.1546

Need to reduce total time by ~15 min to fit 4-pert in budget.
CMA-ES portion is ~56 min for no-perturb, so need ~30% reduction in fevals.
"""
...
def main():
    ...
    # Configs to test - progressively reduced fevals
    configs = {
        'fevals_16_32_4pert': {
            'sigma0_1src': 0.18,
            'sigma0_2src': 0.22,
            'max_fevals_1src': 16,  # Reduced from 20
            'max_fevals_2src': 32,  # Reduced from 44
        ...
        'fevals_14_28_4pert': {
            'sigma0_1src': 0.18,
            'sigma0_2src': 0.22,
            'max_fevals_1src': 14,  # Further reduced
            'max_fevals_2src': 28,  # Further reduced
            'timestep_fraction': 0.40,
            'refine_maxiter': 8,
            'enable_tabu_hopping': True,
            'n_perturbations': 4,
    ...
    print("=" * 70)
    print("EXPERIMENT: reduced_fevals_4pert_v2")
    print("=" * 70)
    print("Testing reduced CMA-ES fevals with 4 perturbations")
    print("\nBaselines:")
    print("  no_perturb @ 20/44: 56.3 min, score 1.1367")
    print("  4pert_nm2 @ 20/44:  70.9 min, score 1.1546")
    print("\nTarget: <60 min with 4 perturbations")
    ...
    for config_name, config in configs.items():
        ...
        # Early exit if we found an in-budget config
        if result['in_budget']:
            print(f"\n*** FOUND IN-BUDGET CONFIG! ***")

    print("\n" + "=" * 70)
    print("SUMMARY")
    print("=" * 70)
    print(f"{'Config':<25} {'Score':>8} {'Time':>10} {'Status':>12}")
    print("-" * 70)
    print(f"{'no_perturb (base)':<25} {'1.1367':>8} {'56.3 min':>10} {'IN':>12}")
    print(f"{'4pert_nm2 (base)':<25} {'1.1546':>8} {'70.9 min':>10} {'OVER':>12}")
    print("-" * 70)

    for r in results:
        status = "✓ IN" if r['in_budget'] else "✗ OVER"
        print(f"{r['config_name']:<25} {r['score']:>8.4f} {r['projected_400_min']:>10.1f} {status:>12}")
    ...
    if in_budget:
        ...
        print(f"    Improvement over no_perturb: +{best['score'] - 1.1367:.4f}")
    else:
