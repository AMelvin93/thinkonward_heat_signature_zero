#!/usr/bin/env python3
"""
Run IPOP-CMA-ES optimizer experiment.

IPOP-CMA-ES uses restart strategy with increasing population to escape local minima.

Usage:
    uv run python experiments/ipop_cmaes/run.py --workers 7 --shuffle
    uv run python experiments/ipop_cmaes/run.py --workers 7 --max-samples 10  # Quick test
"""
...
from optimizer import IPOPCMAESOptimizer, N_MAX
...
def process_sample(args):
    ...
    optimizer = IPOPCMAESOptimizer(**optimizer_config)
...
def main():
    ...
    parser.add_argument('--n-restarts', type=int, default=2,
                        help='Number of restarts for 2-source problems')
    parser.add_argument('--popsize-inc', type=float, default=2.0,
                        help='Population size increase factor per restart')
    parser.add_argument('--no-mlflow', action='store_true')
    args = parser.parse_args()

    print("=" * 70)
    print("IPOP-CMA-ES Optimizer Experiment")
    print("=" * 70)
    ...
    print(f"  N Restarts (2-src): {args.n_restarts}")
    print(f"  Popsize Inc: {args.popsize_inc}")
    ...
    optimizer_config = {
        ...
        'n_restarts_2src': args.n_restarts,
        'popsize_inc': args.popsize_inc,
    }
    ...
    if not args.no_mlflow:
        mlflow.set_experiment("ipop_cmaes")
    ...
    results_path = os.path.join(
        ...
        f'ipop_cmaes_{args.max_fevals_1src}_{args.max_fevals_2src}_{timestamp}_results.pkl'
    )
