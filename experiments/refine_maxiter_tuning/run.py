"""
Experiment: refine_maxiter_tuning
Test different refine_maxiter (6, 10) vs baseline 8.

Current best: all params + refine_maxiter=8 = 1.1437 @ 42.2 min
"""
...
def run_experiment(config, config_name, data):
    ...
    maxiter = config.get('refine_maxiter')
    print(f"\nConfig: {config_name} (refine_maxiter={maxiter})")
...
def main():
    ...
    # Test different refine_maxiter
    for maxiter in [6, 10, 8]:
        config = {**base_config, 'refine_maxiter': maxiter}
        name = f"maxiter_{maxiter}"
        result = run_experiment(config, name, data)
        results.append(result)

    # Summary
    print("\n" + "="*60)
    print("SUMMARY: Refine Maxiter Tuning")
    print("="*60)
    print(f"Current best (refine_maxiter=8): 1.1437 @ 42.2 min")
    print()
    for r in results:
        delta = r['score'] - CURRENT_BEST
        in_budget = "IN BUDGET" if r['projected_400_min'] < 60 else "OVER BUDGET"
        print(f"  maxiter={r['refine_maxiter']}: {r['score']:.4f} @ {r['projected_400_min']:.1f} min ({delta:+.4f}) [{in_budget}]")

    best = max(results, key=lambda x: x['score'] if x['projected_400_min'] < 60 else 0)
    print()
    print(f"Best in-budget: maxiter={best['refine_maxiter']} = {best['score']:.4f}")
