"""
Pure Nelder-Mead Multistart Optimizer.

Uses Nelder-Mead simplex method from multiple starting points,
with analytical intensity computation to reduce dimensionality.

Key idea: If initialization is good enough, Nelder-Mead's local search
might be sufficient without expensive global search (CMA-ES).
"""
...
class NelderMeadMultistartOptimizer:
    """
    Pure Nelder-Mead from multiple starting points with analytical intensity.
    """
    ...
    def __init__(
        ...
        self.nx = nx
        self.ny = ny
        self.n_starts_1src = n_starts_1src
        self.n_starts_2src = n_starts_2src
    ...
    def _onset_based_init(self, sample, n_sources):
        """Init using temperature onset times."""
        readings = sample['Y_noisy']
        sensors = np.array(sample['sensors_xy'])

        n_sensors = len(sensors)
        onset_times = np.zeros(n_sensors)
        max_temps = np.max(readings, axis=0)

        for i in range(n_sensors):
            signal = readings[:, i]
            thresh = 0.1 * max_temps[i]
            above = np.where(signal > thresh)[0]
            if len(above) > 0:
                onset_times[i] = above[0]
            else:
                onset_times[i] = len(signal)

        earliest = np.argsort(onset_times)
    ...
    def _gradient_based_init(self, sample, n_sources):
        """Init using temperature gradient direction."""
        readings = sample['Y_noisy']
        sensors = np.array(sample['sensors_xy'])

        # Find peak temperatures
        peak_temps = np.max(readings, axis=0)
        hot_idx = np.argsort(peak_temps)[::-1]

        # For the hottest sensor, estimate source direction from gradient
        if n_sources == 1:
            hottest = hot_idx[0]
            x, y = sensors[hottest]
            ...
            # For 2 sources, use two hottest well-separated sensors
            selected = []
            for idx in hot_idx:
                if len(selected) >= n_sources:
                    break
                if all(np.linalg.norm(sensors[idx] - sensors[s]) >= 0.3 for s in selected):
                    selected.append(idx)
            while len(selected) < n_sources:
                for idx in hot_idx:
                    if idx not in selected:
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        n_sims = [0]
        n_starts = self.n_starts_1src if n_sources == 1 else self.n_starts_2src

        # Objective function with bounds enforcement
        margin = 0.05
        def clip_to_bounds(params):
            clipped = []
            for i in range(0, len(params), 2):
                x = np.clip(params[i], margin * self.Lx, (1 - margin) * self.Lx)
                y = np.clip(params[i+1], margin * self.Ly, (1 - margin) * self.Ly)
                clipped.extend([x, y])
            return np.array(clipped)

        if n_sources == 1:
            def objective(xy_params):
                xy_params = clip_to_bounds(xy_params)
                x, y = xy_params
                n_sims[0] += 1
        ...
        # Priority order: triangulation, smart, onset, gradient, random
        tri_init = self._triangulation_init_positions(sample, meta, n_sources, q_range)
        if tri_init is not None:
            init_points.append((tri_init, 'triangulation'))

        init_points.append((self._smart_init_positions(sample, n_sources), 'smart'))
        init_points.append((self._onset_based_init(sample, n_sources), 'onset'))
        init_points.append((self._gradient_based_init(sample, n_sources), 'gradient'))

        # Add random inits to fill remaining slots
        while len(init_points) < n_starts:
            init_points.append((self._random_init_positions(n_sources), f'random_{len(init_points)}'))

        all_results = []
        best_so_far = float('inf')

        for init_params, init_type in init_points[:n_starts]:
            try:
                result = minimize(
                    objective,
                    init_params,
                    method='Nelder-Mead',
                    options={
                        'maxiter': self.max_iter_per_start,
                        'xatol': self.xatol,
                        'fatol': self.fatol,
                    }
                )
                ...
                if result.fun < best_so_far:
                    best_so_far = result.fun

                # Early termination if we find a good solution
                if result.fun < self.early_termination_threshold:
                    break

            except Exception as e:
                if verbose:
                    print(f"Nelder-Mead failed from {init_type}: {e}")
                continue
        ...
        for pos_params, rmse, init_type in all_results:
            pos_params = clip_to_bounds(pos_params)
            if n_sources == 1:
                x, y = pos_params
