"""
Run script for multi_candidate_perturbation experiment.

Tests perturbing top 2 candidates instead of just top 1.
"""
...
def process_single_sample(args):
    ...
    optimizer = PerturbedLocalRestartOptimizer(
        ...
        perturb_top_n=config.get('perturb_top_n', 2),  # Perturb top 2
        n_perturbations=config.get('n_perturbations', 1),  # 1 each = 2 total
...
def main():
    ...
    parser.add_argument('--perturb-top-n', type=int, default=2)
    parser.add_argument('--n-perturbations', type=int, default=1)
    ...
    print(f"\n{'='*60}")
    print(f"MULTI-CANDIDATE PERTURBATION EXPERIMENT")
    print(f"{'='*60}")
    print(f"Samples: {n_samples} ({n_1src} 1-source, {n_2src} 2-source)")
    print(f"Workers: {args.workers}")
    print(f"Perturb top {args.perturb_top_n} with {args.n_perturbations} perturbation(s) each")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Multi-Candidate Perturbation")
    print(f"{'='*70}")
