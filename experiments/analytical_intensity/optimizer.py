"""
Analytical Intensity Optimizer for Heat Source Identification.

Key Innovation: Exploits the LINEARITY of the heat equation to compute
optimal intensities analytically, eliminating intensity from optimization.

Physics Insight:
    The heat equation is LINEAR in source intensity q:
    T(x,t) = q × T_unit(x,t)   where T_unit is temperature from q=1 source

    This means for given positions (x, y), optimal q minimizes:
    ||q × T_unit - T_observed||²

    Solution (least squares):
    q_optimal = (T_unit · T_observed) / (T_unit · T_unit)

Benefits:
1. Reduces 1-source from 3 params → 2 params
2. Reduces 2-source from 6 params → 4 params
3. Exact intensity (no approximation error)
4. Faster CMA-ES convergence (fewer dimensions)
5. No intensity polish needed

For 2-source, we solve a 2x2 linear system:
    [Y1·Y1  Y1·Y2] [q1]   [Y1·Yobs]
    [Y2·Y1  Y2·Y2] [q2] = [Y2·Yobs]

And use linearity: Y_pred = q1*Y1 + q2*Y2 (no extra simulation!)
"""
...
def extract_enhanced_features(sample: Dict, meta: Dict = None) -> np.ndarray:
    ...
    11 features: basic(5) + spatial(3) + temporal(2) + correlation(1)
    """
    Y = sample['Y_noisy']
...
def compute_optimal_intensity_1src(
    ...
    Returns:
        q_optimal: Optimal intensity (clipped to q_range)
        Y_pred: Predicted sensor readings
        rmse: RMSE with optimal intensity
    """
    # Simulate with q=1.0
...
def compute_optimal_intensity_2src(
    ...
    Uses linearity: T_total = q1*T1 + q2*T2
    Solves 2x2 linear system for optimal (q1, q2).

    Returns:
        (q1, q2): Optimal intensities (clipped to q_range)
        Y_pred: Predicted sensor readings
        rmse: RMSE with optimal intensities
    """
    # Simulate each source with q=1.0 (2 simulations)
    ...
    try:
        # Add small regularization to handle ill-conditioned systems
    ...
    # Compute prediction using linearity (NO extra simulation!)
    Y_pred = q1 * Y1 + q2 * Y2
    rmse = np.sqrt(np.mean((Y_pred - Y_observed) ** 2))
...
class AnalyticalIntensityOptimizer:
    """
    CMA-ES optimizer with analytical intensity computation.

    Key Innovation: Exploits linearity of heat equation to compute
    optimal intensities analytically, reducing parameter space:
    - 1-source: 3 params → 2 params (x, y)
    - 2-source: 6 params → 4 params (x1, y1, x2, y2)
    """
    ...
    def __init__(
        ...
        nx: int = 100,
        ny: int = 50,
        max_fevals_1src: int = 25,  # Can use more since fewer params
        ...
        """
        Initialize the analytical intensity optimizer.

        Note: No intensity_polish parameters - intensity is computed analytically!
        """
        self.Lx = Lx
        self.Ly = Ly
        self.nx = nx
    ...
    def _triangulation_init_positions(self, sample: Dict, meta: Dict,
        ...
        try:
            ...
            for i in range(n_sources):
                positions.extend([full_init[i*3], full_init[i*3 + 1]])  # x, y
    ...
    def estimate_sources(
        ...
        """
        Estimate sources with analytical intensity computation.

        Key difference: CMA-ES optimizes positions only, intensities
        are computed analytically at each evaluation.
        """
        ...
        # Transfer learning (positions only)
        ...
        if verbose:
            print(f"  Transfer: {n_transferred} from history of {len(history)}")
        ...
        # 3. Transfer learning (positions only)
        ...
        # === CMA-ES OPTIMIZATION (positions only) ===
