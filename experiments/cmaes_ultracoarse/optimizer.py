"""
Ultra-Coarse Grid CMA-ES for Heat Source Identification.

Key hypothesis: 30x15 coarse grid is 2.8x faster than 50x25.
If CMA-ES can still converge with noisier objective, total time
reduces by ~10+ min, allowing higher sigma within budget.

Grid comparison:
- Current coarse: 50x25 = 1250 cells
- Ultra-coarse: 30x15 = 450 cells (2.8x fewer)
- Fine: 100x50 = 5000 cells (final evaluation only)
"""
...
class UltraCoarseOptimizer:
    """
    Ultra-coarse grid optimizer: 30x15 for exploration, 100x50 for evaluation.

    Key change from robust_fallback:
    - nx_coarse: 50 -> 30
    - ny_coarse: 25 -> 15

    This reduces simulation time by ~2.8x during exploration phase.
    """
    ...
    def __init__(
        ...
        nx_fine: int = 100,
        ny_fine: int = 50,
        nx_coarse: int = 30,  # CHANGED: 50 -> 30
        ny_coarse: int = 15,  # CHANGED: 25 -> 15
        max_fevals_1src: int = 20,
        max_fevals_2src: int = 36,
        sigma0_1src: float = 0.20,  # Higher sigma to leverage faster grid
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Objective using COARSE grid (30x15 ultra-coarse)
        if n_sources == 1:
        ...
        # Evaluate candidates on FINE grid (100x50)
        candidates_raw = []
