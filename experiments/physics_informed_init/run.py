#!/usr/bin/env python3
"""
Run script for Physics-Informed Initialization experiment.

Tests the hypothesis that gradient-based initialization provides better
starting points for CMA-ES optimization than hottest-sensor approach.
"""
...
from optimizer import PhysicsInformedInitOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = PhysicsInformedInitOptimizer(
        ...
        use_gradient_init=config.get('use_gradient_init', True),
...
def main():
    parser = argparse.ArgumentParser(description='Run Physics-Informed Initialization experiment')
    ...
    # Gradient init
    parser.add_argument('--no-gradient-init', action='store_true', help='Disable gradient init for A/B test')
    # Other
    ...
    use_gradient = not args.no_gradient_init
    init_mode = "gradient+tri+smart" if use_gradient else "tri+smart"
    ...
    config = {
        ...
        'use_gradient_init': use_gradient,
    ...
    print()
    print(f"W2 Baseline (40% temporal + polish): 1.1688 @ 58.4 min")
    ...
    if projected_400 > 60:
        ...
        print("PARTIAL SUCCESS - Met target (1.14)")
    else:
    ...
    if not args.no_mlflow:
        ...
        mlflow.set_experiment('physics_informed_init')

        run_name = args.run_name or f"gradient_init_{datetime.now().strftime('%H%M%S')}"
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param('use_gradient_init', use_gradient)
            ...
            # Log init type stats
            for it, count in init_type_counts.items():
                mlflow.log_metric(f'init_type_{it}', count)
