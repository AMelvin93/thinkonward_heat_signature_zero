"""
Ensemble on 40% Temporal Baseline Optimizer

Combines the 1.1688 baseline (40% timesteps + 8 NM polish) with ensemble averaging.

Based on:
- experiments/early_timestep_filtering/optimizer_with_polish.py (1.1688 baseline)
- experiments/lightweight_ensemble_postprocessing/optimizer.py (ensemble code)

Key config from 1.1688 baseline:
- timestep_fraction = 0.40
- final_polish_maxiter = 8 (this is what gave 1.1688 vs 1.1557 with 5 iters)

Ensemble addition:
- After fine-grid evaluation, compute weighted average of top-5 candidates
- Add ensemble solution to candidate pool BEFORE final polish
"""
...
class EnsembleOn40PctTemporalOptimizer:
    """
    40% temporal baseline (1.1688) + ensemble averaging.

    Key differences from production:
    - Uses 8 NM polish iterations (vs 5 in some other experiments)
    - Adds ensemble step after fine-grid evaluation, before final polish
    """
    ...
    def __init__(
        ...
        # 8 NM polish iterations (key to 1.1688 baseline)
    ...
    def _compute_ensemble_solution(self, candidates_raw, n_sources, Y_observed,
        ...
        Returns (ensemble_result, n_sims) or (None, 0) if not enough candidates.
        """
        ...
        if n_sources == 1:
            positions = [c[4] for c in sorted_candidates]
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # ENSEMBLE STEP (after fine-grid eval, before final polish)
        ...
        # FINAL POLISH: 8 NM iterations on best candidate (key to 1.1688)
