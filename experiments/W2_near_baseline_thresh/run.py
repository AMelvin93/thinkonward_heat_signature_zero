#!/usr/bin/env python3
"""
Run script for W2 Near-Baseline Threshold optimizer.

EXP004: sigma_threshold_near_baseline
Config:
- sigma0_1src: 0.20 (higher)
- sigma0_2src: 0.25 (higher)
- threshold_1src: 0.36 (slightly above baseline 0.35)
- threshold_2src: 0.46 (slightly above baseline 0.45)
- fallback_fevals: 18

Hypothesis: Middle ground between EXP001 (0.38/0.48) and EXP002 (0.35/0.45).
Should give good score with faster time than EXP002.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Near-Baseline optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP004 specific
    parser.add_argument('--sigma0-1src', type=float, default=0.20)
    parser.add_argument('--sigma0-2src', type=float, default=0.25)
    parser.add_argument('--threshold-1src', type=float, default=0.36)  # Slightly above baseline
    parser.add_argument('--threshold-2src', type=float, default=0.46)  # Slightly above baseline
    ...
    print(f"\n[W2] Near-Baseline Threshold Optimizer")
    print(f"=" * 70)
    print(f"EXP004: sigma_threshold_near_baseline")
    print(f"-" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Near-Baseline Threshold (EXP004)")
    print(f"{'='*70}")
