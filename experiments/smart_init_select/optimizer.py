"""
Smart Init Selection Optimizer.

Key idea:
1. Generate multiple initialization options (triangulation, smart, cluster, onset)
2. Evaluate all inits with a single cheap evaluation
3. Pick the best 2 inits
4. Run full CMA-ES only on those 2 inits

This gives diversity benefit without spreading fevals too thin.
"""
...
class SmartInitSelectOptimizer:
    """
    Smart init selection optimizer.

    For 2-source problems:
    1. Generate 4 init options (triangulation, smart, cluster, onset)
    2. Evaluate all with single simulation
    3. Pick best 2
    4. Run CMA-ES on those 2

    This should be faster than running all 4 while getting better accuracy.
    """
    ...
    def __init__(
        ...
        refine_top_n: int = 2,
        n_inits_to_use: int = 2,  # How many inits to run CMA-ES on
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.n_inits_to_use = n_inits_to_use
    ...
    def _cluster_init_2src(self, sample):
        """K-means clustering based init for 2-source."""
    ...
    def _onset_init_2src(self, sample):
        """Onset-time based init for 2-source."""
        readings = sample['Y_noisy']
        sensors = np.array(sample['sensors_xy'])
        ...
        earliest_idx = np.argsort(onset_times)[:10]
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Build ALL initialization options
        all_inits = []

        tri_init = self._triangulation_init_positions(sample, meta, n_sources, q_range)
        if tri_init is not None:
            all_inits.append((tri_init, 'triangulation'))

        smart_init = self._smart_init_positions(sample, n_sources)
        all_inits.append((smart_init, 'smart'))

        if n_sources == 2:
            cluster_init = self._cluster_init_2src(sample)
            all_inits.append((cluster_init, 'cluster'))

            onset_init = self._onset_init_2src(sample)
            all_inits.append((onset_init, 'onset'))

        # Evaluate all inits with single simulation each
        init_scores = []
        for init_params, init_type in all_inits:
            score = objective_coarse(init_params)
            init_scores.append((init_params, init_type, score))

        # Sort by score and pick best N
        init_scores.sort(key=lambda x: x[2])
        selected_inits = init_scores[:self.n_inits_to_use]

        # Run CMA-ES on selected inits
        max_fevals = self.max_fevals_1src if n_sources == 1 else self.max_fevals_2src
        # Subtract fevals used for init evaluation
        remaining_fevals = max_fevals - len(all_inits) * (1 if n_sources == 1 else 2)
        sigma0 = self.sigma0_1src if n_sources == 1 else self.sigma0_2src
        lb, ub = self._get_position_bounds(n_sources)
        fevals_per_init = max(5, remaining_fevals // len(selected_inits))

        all_solutions = []

        for init_params, init_type, _ in selected_inits:
            opts = cma.CMAOptions()
