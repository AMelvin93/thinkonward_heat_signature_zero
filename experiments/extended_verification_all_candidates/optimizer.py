"""
Extended Verification All Candidates Optimizer

Extends solution_verification_pass to verify ALL top-N candidates, not just the best.

Hypothesis: Solution verification improved best candidate by +1%. Applying to all
candidates may improve overall score since scoring averages accuracy over candidates.

Key changes from solution_verification_pass:
1. After NM polish and fine-grid evaluation, apply gradient verification to EACH candidate
2. For each candidate, if gradient is large, take a step in negative gradient direction
3. Keep better of (original, verified) for EACH candidate
"""
...
class ExtendedVerificationOptimizer:
    """
    Optimizer with gradient verification applied to ALL top candidates.

    Key addition: After CMA-ES + NM polish, compute gradient for EACH candidate.
    If gradient is large, take a step in negative gradient direction.
    """
    ...
    def __init__(
        ...
        verify_top_n: int = 3,  # NEW: Number of candidates to verify
        gradient_eps: float = 0.02,
    ...
    def _verify_single_candidate(self, pos_params, n_sources, objective_fn, lb, ub):
        """
        Compute gradient and optionally take a step if gradient is large.

        Returns: (new_pos_params, new_rmse, n_extra_evals, improved)
        """
        n_params = len(pos_params)
        n_extra_evals = [0]

        # Compute gradient via finite differences
        gradient = np.zeros(n_params)
        f0 = objective_fn(pos_params)
        n_extra_evals[0] += 1

        for i in range(n_params):
            pos_plus = pos_params.copy()
            pos_plus[i] = min(ub[i], pos_params[i] + self.gradient_eps)
            f_plus = objective_fn(pos_plus)
            n_extra_evals[0] += 1
            gradient[i] = (f_plus - f0) / self.gradient_eps

        grad_norm = np.linalg.norm(gradient)

        if grad_norm < self.gradient_threshold:
            # Gradient is small - solution is at local minimum
            return pos_params, f0, n_extra_evals[0], False

        # Take a step in negative gradient direction
        step = -gradient / grad_norm * self.step_size
        new_pos = pos_params + step

        # Clip to bounds
        new_pos = np.clip(new_pos, lb, ub)

        # Evaluate new position
        f_new = objective_fn(new_pos)
        n_extra_evals[0] += 1

        if f_new < f0:
            return new_pos, f_new, n_extra_evals[0], True
        else:
            return pos_params, f0, n_extra_evals[0], False

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, nt_reduced):
        """Run CMA-ES optimization with verification pass on ALL candidates."""
        ...
        # === NEW: GRADIENT VERIFICATION PASS ON ALL TOP CANDIDATES ===
        if self.enable_verification:
            # Sort by RMSE
            candidates_raw.sort(key=lambda x: x[2])

            # Verify top N candidates (configurable)
            n_to_verify = min(self.verify_top_n, len(candidates_raw))
            verified_improvements = []

            for idx in range(n_to_verify):
                cand = candidates_raw[idx]
                pos = cand[4]  # Position params

                new_pos, new_rmse, n_extra, improved = self._verify_single_candidate(
                    pos, n_sources, objective_fine, lb, ub
                )
                n_sims[0] += n_extra

                if improved:
                    # Create new candidate with improved position
                    if n_sources == 1:
                        x, y = new_pos
                        q, _, final_rmse = compute_optimal_intensity_1src(
                            x, y, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
                    ...
                    verified_improvements.append((sources, full_params, final_rmse, 'verified', new_pos))

            # Add verified improvements to candidate pool
            candidates_raw.extend(verified_improvements)

        # Remove position params from output
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Gradient verification: {'ENABLED' if self.enable_verification else 'DISABLED'}")
            if self.enable_verification:
                print(f"  Verifying top {self.verify_top_n} candidates")
