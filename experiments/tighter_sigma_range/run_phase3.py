"""
Run script for tighter_sigma_range experiment - Phase 3.

Phase 2 showed sigma 0.16/0.20 is NEW BEST (1.1699).
Phase 3: Test even tighter sigma to see if we can push higher.
"""
...
def main():
    ...
    configs = [
        # Even tighter sigma
        {**base_config, 'sigma0_1src': 0.15, 'sigma0_2src': 0.19},
        # Intermediate
        {**base_config, 'sigma0_1src': 0.14, 'sigma0_2src': 0.18},
    ]

    config_names = ['sigma_015_019', 'sigma_014_018']
    ...
    for config, name in zip(configs, config_names):
        ...
        with open('run_phase3_output.json', 'w') as f:
            json.dump(results, f, indent=2, default=str)

    # Print summary
    print("\n" + "="*70)
    print("PHASE 3 EXPERIMENT SUMMARY (EVEN TIGHTER SIGMA)")
    print("="*70)
    ...
    print(f"\nPhase 2 Best: sigma_016_020_with_perturb = 1.1699 @ 49.0 min projected")

    best = max(results, key=lambda x: x['score'])
    print(f"\nBest Phase 3: {best['config']} with score {best['score']:.4f}")

    if best['score'] > 1.1699:
        print(f"*** NEW BEST! +{best['score'] - 1.1699:.4f} vs Phase 2 ***")
    else:
        print(f"Delta vs Phase 2 best: {best['score'] - 1.1699:+.4f}")
