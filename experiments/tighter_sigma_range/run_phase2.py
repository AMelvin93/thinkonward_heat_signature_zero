"""
Run script for tighter_sigma_range experiment - Phase 2.

Tests tighter sigma WITH perturbation enabled.
Phase 1 showed sigma 0.16/0.20 is best without perturbation.
Phase 2 tests if tighter sigma + perturbation beats 1.1689 baseline.
"""
...
def run_experiment(config, config_name, data):
    ...
    print(f"Perturbation: ENABLED (enable_tabu_hopping={config.get('enable_tabu_hopping')})")
    print(f"{'='*60}")
...
def main():
    ...
    # Base config WITH perturbation enabled
    base_config = {
    ...
    # Test configurations
    configs = [
        # Run 4: Tighter sigma WITH perturbation
        {**base_config, 'sigma0_1src': 0.16, 'sigma0_2src': 0.20},
        # Run 5: Baseline WITH perturbation
        {**base_config, 'sigma0_1src': 0.18, 'sigma0_2src': 0.22},
    ]

    config_names = ['sigma_016_020_with_perturb', 'sigma_018_022_with_perturb']
    ...
    for config, name in zip(configs, config_names):
        ...
        with open('run_phase2_output.json', 'w') as f:
            json.dump(results, f, indent=2, default=str)

    # Print summary
    print("\n" + "="*70)
    print("PHASE 2 EXPERIMENT SUMMARY (WITH PERTURBATION)")
    print("="*70)
    ...
    baseline_score = 1.1689
    baseline_time = 58.2
    print(f"\nBaseline (hopping_with_tabu_memory no_tabu): {baseline_score:.4f} @ {baseline_time:.1f} min")

    # Find best
    best = max(results, key=lambda x: x['score'])
    print(f"\nBest Phase 2: {best['config']} with score {best['score']:.4f} @ {best['time_min']:.1f} min")

    if best['score'] > baseline_score:
        print(f"*** NEW BEST! +{best['score'] - baseline_score:.4f} vs baseline ***")
    else:
        print(f"Delta vs baseline: {best['score'] - baseline_score:+.4f}")
