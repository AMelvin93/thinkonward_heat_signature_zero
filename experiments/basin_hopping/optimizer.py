"""
Basin Hopping Optimizer with L-BFGS-B local refinement.

Key idea: Use scipy.optimize.basinhopping to combine global search
with fast L-BFGS-B local minimization. This may avoid local minima
while being faster than CMA-ES.

Basin hopping works by:
1. Start from initial position (triangulation or smart init)
2. Local minimization with L-BFGS-B
...
class BasinHoppingOptimizer:
    """
    Basin Hopping optimizer using scipy.optimize.basinhopping.

    Combines global search with L-BFGS-B local refinement.
    """
    ...
    def _onset_based_init(self, sample, n_sources):
        """Alternative init using temperature onset times."""
        readings = sample['Y_noisy']
        sensors = np.array(sample['sensors_xy'])

        # Find when each sensor first responds significantly
        ...
        # Earliest onset sensors are likely closest to source
    ...
    def _run_basin_hopping(self, x0, objective, bounds, niter, stepsize, T):
        """Run basin hopping from a single starting point."""
        minimizer_kwargs = {
            'method': 'L-BFGS-B',
            'bounds': bounds,
            'options': {'maxiter': self.minimizer_maxiter, 'disp': False}
        }

        # Custom step function that respects bounds
        class BoundedStep:
        ...
        take_step = BoundedStep(stepsize, bounds)
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        init_points.append((smart_init, 'smart'))

        # Onset-based init
        onset_init = self._onset_based_init(sample, n_sources)
        init_points.append((onset_init, 'onset'))

        # Run basin hopping from multiple starting points
        all_results = []

        for init_params, init_type in init_points[:self.n_restarts]:
            try:
                best_x, best_rmse, n_iters = self._run_basin_hopping(
                    init_params, objective, bounds,
                    niter=self.niter, stepsize=self.stepsize, T=self.T
                )
                all_results.append((best_x, best_rmse, init_type))
            except Exception as e:
                if verbose:
                    print(f"Basin hopping failed from {init_type}: {e}")
                continue

        # Check if best result is bad - try more random restarts
        if all_results:
            ...
            if best_rmse > threshold:
                ...
                for i in range(2):
                    ...
                    try:
                        best_x, rmse, n_iters = self._run_basin_hopping(
                            random_init, objective, bounds,
                            niter=self.niter, stepsize=self.stepsize, T=self.T
                        )
                        all_results.append((best_x, rmse, f'random_{i}'))
                    except:
                        continue

        if not all_results:
            # Fallback: simple L-BFGS-B from smart init
            smart_init = self._smart_init_positions(sample, n_sources)
            result = minimize(objective, smart_init, method='L-BFGS-B', bounds=bounds)
            all_results.append((result.x, result.fun, 'fallback'))

        # Build candidate list with full parameters (including q)
        candidates_raw = []
