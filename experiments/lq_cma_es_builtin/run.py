"""
Run script for lq-CMA-ES experiment with MLflow logging.

This script tests pycma's built-in lq-CMA-ES (linear-quadratic surrogate)
to see if it can reduce function evaluations and runtime.
"""
...
def run_experiment(
    ...
    if verbose:
        print(f"[W1] Running lq-CMA-ES experiment with {n_samples} samples, {N_WORKERS} workers")
        print(f"Config: max_fevals_1src={max_fevals_1src}, max_fevals_2src={max_fevals_2src}")
    ...
    for idx, candidate_sources, best_rmse, n_sims, error in results_list:
        ...
        if error:
            ...
            # candidate_sources is already a list of candidates (each candidate is a list of (x,y,q) tuples)
            # So we don't wrap it in another list
    ...
    # Create prediction dataset for scoring
    # score_submission expects a list of dicts with 'sample_id' and 'estimated_sources'
    pred_dataset = []
    for idx, sample in enumerate(samples):
        pred_sample = {
            'sample_id': sample['sample_id'],
            'estimated_sources': predictions[idx]
        }
        pred_dataset.append(pred_sample)

    # Score submission
    try:
        score = score_submission(
            data, pred_dataset,
            N_max=3, lambda_=0.3, tau=0.2,
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"RESULTS: lq-CMA-ES Experiment")
        print(f"{'='*60}")
...
def main():
    ...
    run_name = f"lq_cmaes_run1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        # Log parameters
        mlflow.log_param("experiment_id", "EXP_LQ_CMAES_001")
        mlflow.log_param("worker", "W1")
        mlflow.log_param("optimizer", "lq-CMA-ES")
        ...
        if results['in_budget']:
            if state['best_in_budget'] is None or results['score'] > state['best_in_budget']['score']:
