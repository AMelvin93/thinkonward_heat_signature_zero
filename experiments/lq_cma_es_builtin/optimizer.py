"""
lq-CMA-ES Optimizer using pycma's built-in linear-quadratic surrogate model.

Key idea: Use cma.fmin_lq_surr() which builds a global quadratic surrogate
to skip some objective function evaluations, potentially achieving 2-6x speedup.

Based on:
- https://cma-es.github.io/lq-cma
- GECCO 2019 paper on surrogate-assisted CMA-ES
"""
...
class LqCmaEsOptimizer:
    """
    Optimizer using pycma's built-in lq-CMA-ES (linear-quadratic surrogate).

    Uses cma.fmin_lq_surr() which builds a global quadratic surrogate model
    to skip some objective evaluations when the surrogate is reliable.
    """
    ...
    def _run_lq_cmaes_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                    initializations, n_sources, early_frac):
        """Run lq-CMA-ES optimization (surrogate-assisted)."""
        ...
        for init_params, init_type in initializations:
            ...
            try:
                # Use lq-CMA-ES with surrogate model
                xbest, es = cma.fmin_lq_surr(
                    objective_coarse,
                    init_params.tolist(),
                    sigma0,
                    opts
                ...
                # Use xfavorite as recommended (not es.best which uses surrogate values)
                best_x = es.result.xfavorite if hasattr(es.result, 'xfavorite') else xbest

                # Get best solution fitness by evaluating it
                best_fit = objective_coarse(best_x)
                all_solutions.append((np.array(best_x), best_fit, init_type))

            except Exception as e:
                # Fallback to standard CMA-ES if lq-CMA-ES fails
                print(f"lq-CMA-ES failed with {e}, using standard CMA-ES")
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        candidates_raw, n_sims = self._run_lq_cmaes_optimization(
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims = self._run_lq_cmaes_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
