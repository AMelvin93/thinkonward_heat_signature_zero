#!/usr/bin/env python
"""Run Multi-Start CMA-ES Optimizer experiment."""
...
from experiments.multi_start.optimizer import (
    MultiStartOptimizer,
...
def process_sample(sample, meta, config, history_1src, history_2src):
    optimizer = MultiStartOptimizer(
        n_restarts_1src=config['n_restarts_1src'],
        n_restarts_2src=config['n_restarts_2src'],
        fevals_per_restart_1src=config['fevals_per_restart_1src'],
        fevals_per_restart_2src=config['fevals_per_restart_2src'],
        refine_fevals_1src=config['refine_fevals_1src'],
        refine_fevals_2src=config['refine_fevals_2src'],
        ...
        n_random_inits=config.get('n_random_inits', 2),
    )
...
def main():
    parser = argparse.ArgumentParser(description='Run Multi-Start Optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--batch-size', type=int, default=20)
    parser.add_argument('--max-samples', type=int, default=None)
    # Restart config
    parser.add_argument('--n-restarts-1src', type=int, default=2)
    parser.add_argument('--n-restarts-2src', type=int, default=3)
    parser.add_argument('--fevals-per-restart-1src', type=int, default=6)
    parser.add_argument('--fevals-per-restart-2src', type=int, default=8)
    parser.add_argument('--refine-fevals-1src', type=int, default=4)
    parser.add_argument('--refine-fevals-2src', type=int, default=6)
    parser.add_argument('--n-random-inits', type=int, default=2)
    ...
    config = {
        ...
        'fevals_per_restart_1src': args.fevals_per_restart_1src,
        'fevals_per_restart_2src': args.fevals_per_restart_2src,
    ...
    # Calculate total fevals
    total_1src = config['n_restarts_1src'] * config['fevals_per_restart_1src'] + config['refine_fevals_1src']
    total_2src = config['n_restarts_2src'] * config['fevals_per_restart_2src'] + config['refine_fevals_2src']
    ...
    print("=" * 70)
    print("MULTI-START CMA-ES OPTIMIZER (A4)")
    print("=" * 70)
    ...
    print(f"1-src: {config['n_restarts_1src']} restarts × {config['fevals_per_restart_1src']} fevals + "
          f"{config['refine_fevals_1src']} refine = ~{total_1src} total")
    print(f"2-src: {config['n_restarts_2src']} restarts × {config['fevals_per_restart_2src']} fevals + "
          f"{config['refine_fevals_2src']} refine = ~{total_2src} total")
    print("=" * 70)
    ...
    if is_g4dn_simulation and not args.max_samples:
        ...
        run_name = f"multi_start_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param("optimizer", "MultiStartOptimizer")
            mlflow.log_param("n_restarts_1src", config['n_restarts_1src'])
            mlflow.log_param("n_restarts_2src", config['n_restarts_2src'])
            mlflow.log_param("total_fevals_1src", total_1src)
            mlflow.log_param("total_fevals_2src", total_2src)
