"""
Combined Coordinate Refinement + Perturbation Optimizer

Combines two orthogonal improvement techniques:
1. Coordinate refinement: Fine-tunes within current basin
2. Perturbation: Explores other nearby basins

Flow:
1. CMA-ES exploration
2. NM polish (reduced iterations to fit budget)
3. Coordinate refinement on best
4. Perturbation + brief NM on perturbed positions
5. Final dissimilarity filtering
"""
...
class CoordRefineAndPerturbOptimizer:
    """
    Optimizer combining coordinate refinement AND perturbation.
    """
    ...
    def __init__(
        ...
        # Coord refine params
        self.enable_coord_refine = enable_coord_refine
        self.coord_step = coord_step
        self.coord_refine_top_n = coord_refine_top_n
        # Perturbation params
    ...
    def _coordinate_refine(self, pos_params, objective, lb, ub, n_sims_counter):
        """
        Coordinate-wise line search refinement.
        """
        best_params = pos_params.copy()
        best_rmse = objective(best_params)
        n_sims_counter[0] += 1

        for coord_idx in range(len(pos_params)):
            # Test positive step
            test_params = best_params.copy()
            test_params[coord_idx] += self.coord_step
            test_params[coord_idx] = np.clip(test_params[coord_idx], lb[coord_idx], ub[coord_idx])
            test_rmse = objective(test_params)
            n_sims_counter[0] += 1
    ...
    def _generate_perturbation(self, pos_params, lb, ub):
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run CMA-ES + NM + coord refine + perturbation."""
        ...
        if self.enable_coord_refine:
            refined_solutions.sort(key=lambda x: x[1])

            coord_refined = []
            for i, (pos_params, rmse, init_type) in enumerate(refined_solutions[:self.coord_refine_top_n]):
                new_params, new_rmse = self._coordinate_refine(
                    pos_params, objective_coarse, lb, ub, n_sims
                )
                if new_rmse < rmse:
                    coord_refined.append((new_params, new_rmse, 'coord_refined'))
                else:
                    coord_refined.append((pos_params, rmse, init_type))

            # Replace top N with coord-refined versions
            refined_solutions = coord_refined + refined_solutions[self.coord_refine_top_n:]

        # === PERTURBATION (Basin Hopping) ===
        if self.enable_perturbation:
            ...
            for _ in range(self.n_perturbations):
                ...
                perturbed = self._generate_perturbation(best_pos, lb, ub)
                ...
                if result.fun < best_rmse:
                    new_candidates.append((result.x, result.fun, 'perturb_improved'))
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Coord refine: {'ENABLED' if self.enable_coord_refine else 'DISABLED'}")
            print(f"  Perturbation: {'ENABLED' if self.enable_perturbation else 'DISABLED'} (n={self.n_perturbations})")
