#!/usr/bin/env python3
"""
Run script for More Initializations Select Best optimizer.

Tests running more CMA-ES initializations and selecting best by RMSE.
"""
...
from optimizer import MoreInitsSelectBestOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = MoreInitsSelectBestOptimizer(
        n_inits=config['n_inits'],
        max_fevals_1src=config['max_fevals_1src'],
        max_fevals_2src=config['max_fevals_2src'],
        keep_top_n=config.get('keep_top_n', 5),
...
def main():
    parser = argparse.ArgumentParser(description='More Inits Select Best optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--n-inits', type=int, default=6)
    parser.add_argument('--max-fevals-1src', type=int, default=20)
    parser.add_argument('--max-fevals-2src', type=int, default=36)
    parser.add_argument('--keep-top-n', type=int, default=5)
    ...
    print(f"\nMore Inits Select Best Optimizer")
    print(f"=" * 60)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"N Inits: {args.n_inits}, Fevals: {args.max_fevals_1src}/{args.max_fevals_2src}")
    print(f"Keep Top N: {args.keep_top_n}, Polish: {args.final_polish_maxiter} iters")
    print(f"Sigma: {args.sigma0_1src}/{args.sigma0_2src}, Timestep: {args.timestep_fraction:.0%}")
    print(f"=" * 60)
    ...
    run_name = f"more_inits_{args.n_inits}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    with mlflow.start_run(run_name=run_name) as run:
        # Log parameters
        mlflow.log_param("experiment_id", "EXP_MORE_INITS_SELECT_BEST_001")
        mlflow.log_param("worker", "W2")
        mlflow.log_param("n_inits", args.n_inits)
        mlflow.log_param("max_fevals_1src", args.max_fevals_1src)
        mlflow.log_param("max_fevals_2src", args.max_fevals_2src)
        mlflow.log_param("keep_top_n", args.keep_top_n)
        ...
        print(f"\n{'='*70}")
        print(f"RESULTS - More Inits ({args.n_inits} inits)")
        print(f"{'='*70}")
