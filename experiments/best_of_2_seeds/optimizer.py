"""
Best of 2 Seeds Optimizer

Runs 2 independent CMA-ES instances with different random seeds.
Each gets half the normal budget. Select best before polish.

Hypothesis: CMA-ES can get stuck in suboptimal basins.
Running 2 instances may find better global optimum.
"""
...
class BestOf2SeedsOptimizer:
    """
    Optimizer that runs 2 independent CMA-ES with different seeds.
    Selects best before polish.
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        max_fevals_1src: int = 20,  # Total budget (split between 2 seeds)
        ...
        n_seeds: int = 2,  # Number of seeds to run
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _run_cmaes_with_seed(self, objective_fn, init_params, sigma0, lb, ub,
                              max_fevals, seed, init_type):
        """Run a single CMA-ES optimization with specific seed."""
        opts = cma.CMAOptions()
        ...
        while not es.stop():
            ...
            for sol, fit in zip(solutions, fitness):
                all_solutions.append((np.array(sol), fit, f'{init_type}_seed{seed}'))

        return all_solutions

    def _run_dual_seed_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                     initializations, n_sources, nt_reduced):
        """Run CMA-ES with 2 different seeds, select best before polish."""
        ...
        total_max_fevals = self.max_fevals_1src if n_sources == 1 else self.max_fevals_2src
        sigma0 = self.sigma0_1src if n_sources == 1 else self.sigma0_2src
        lb, ub = self._get_position_bounds(n_sources)

        # Split budget between seeds
        fevals_per_seed = total_max_fevals // self.n_seeds
        fevals_per_init_per_seed = max(3, fevals_per_seed // len(initializations))

        all_solutions = []

        # Run each seed
        for seed_idx in range(self.n_seeds):
            seed = 42 + seed_idx * 1000  # Different seeds

            for init_params, init_type in initializations:
                solutions = self._run_cmaes_with_seed(
                    objective_coarse, init_params, sigma0, lb, ub,
                    fevals_per_init_per_seed, seed, init_type
                )
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Running {self.n_seeds} independent seeds")
        ...
        # Run dual-seed optimization
        candidates_raw, n_sims = self._run_dual_seed_optimization(
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims = self._run_dual_seed_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
