"""
Source-Count Based NM Iterations Optimizer (v2)

Simplified approach: use different FIXED iteration counts for 1-source vs 2-source.
Based on Run 1 data:
- 1-source: avg 6.7 iters needed
- 2-source: avg 8.0 iters needed, some need 10-12

This avoids the overhead of multiple minimize() calls.
"""
...
class SourceCountBasedPolishOptimizer:
    """
    Optimizer with different NM polish iterations for 1-source vs 2-source.

    Key idea: Use fewer iterations for 1-source (simpler, converges faster)
    and more for 2-source (needs more refinement).
    """
    ...
    def __init__(
        ...
        polish_iters_1src: int = 6,   # Fewer for 1-source
        polish_iters_2src: int = 10,  # More for 2-source
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.polish_iters_1src = polish_iters_1src
        self.polish_iters_2src = polish_iters_2src
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Select polish iterations based on source count
        polish_iters = self.polish_iters_1src if n_sources == 1 else self.polish_iters_2src
        ...
        # NM POLISH with source-count-based iterations (single minimize call)
        if polish_iters > 0 and candidates_raw:
            ...
            result = minimize(
                ...
                best_pos_params,
                method='Nelder-Mead',
                options={'maxiter': polish_iters, 'xatol': 0.005, 'fatol': 0.0005}
            )
        ...
        return candidates_raw, n_sims[0], polish_iters
