#!/usr/bin/env python3
"""Run script for Source-Count Based NM Iterations (v2) with MLflow logging."""
...
from optimizer_v2 import SourceCountBasedPolishOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = SourceCountBasedPolishOptimizer(
        ...
        polish_iters_1src=config['polish_iters_1src'],
        polish_iters_2src=config['polish_iters_2src'],
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Source-Count Based NM Iterations')
    ...
    # Source-count based polish iterations
    parser.add_argument('--polish-iters-1src', type=int, default=6)
    parser.add_argument('--polish-iters-2src', type=int, default=10)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--no-mlflow', action='store_true', help='Skip MLflow logging')
    parser.add_argument('--tuning-run', type=int, default=2, help='Tuning run number')
    ...
    print(f"\nSource-Count Based NM Iterations (v2)")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Timestep fraction: {args.timestep_fraction:.0%}")
    print(f"Polish iterations: 1-src={args.polish_iters_1src}, 2-src={args.polish_iters_2src}")
    print(f"=" * 70)
    ...
    config = {
        ...
        'polish_iters_1src': args.polish_iters_1src,
        'polish_iters_2src': args.polish_iters_2src,
    }
    ...
    polish_iters_1src_used = []
    polish_iters_2src_used = []
        ...
        for future in as_completed(futures):
            ...
            if result['success']:
                if result['n_sources'] == 1:
                    polish_iters_1src_used.append(iters)
                else:
                    polish_iters_2src_used.append(iters)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Source-Count Based NM Iterations")
    print(f"{'='*70}")
    ...
    print()
    if rmses_1src:
        print(f"  1-source: RMSE={np.mean(rmses_1src):.4f} (n={len(rmses_1src)}), polish={args.polish_iters_1src}")
    if rmses_2src:
        print(f"  2-source: RMSE={np.mean(rmses_2src):.4f} (n={len(rmses_2src)}), polish={args.polish_iters_2src}")
    print()
    ...
    if not args.no_mlflow:
        ...
        run_name = f"source_count_nm_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            ...
            mlflow.log_param("polish_iters_1src", args.polish_iters_1src)
            mlflow.log_param("polish_iters_2src", args.polish_iters_2src)
            mlflow.log_param("approach", "source_count_based")
