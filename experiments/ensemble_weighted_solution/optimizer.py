"""
Ensemble Weighted Solution Optimizer

Takes top-N solutions from CMA-ES and creates an ensemble solution by weighted averaging.
Hypothesis: Averaging multiple good solutions may reduce noise and yield lower RMSE.

Key challenges:
- For 2-source, need to align sources across solutions (permutation ambiguity)
- If solutions are in different local minima, averaging may produce worse results
- This is a postprocessing step, so minimal overhead
"""
...
class EnsembleWeightedOptimizer:
    """
    CMA-ES optimizer with ensemble weighted averaging postprocessing.

    After CMA-ES, takes top-N solutions and creates an ensemble by weighted averaging.
    """
    ...
    def _compute_ensemble_position(self, solutions_with_rmse, n_sources):
        """
        Compute weighted average of top solutions.

        solutions_with_rmse: list of (position_params, rmse) tuples
        Returns: weighted average position params
        """
        if len(solutions_with_rmse) < 2:
            return solutions_with_rmse[0][0] if solutions_with_rmse else None

        # Sort by RMSE and take top N
        sorted_sols = sorted(solutions_with_rmse, key=lambda x: x[1])[:self.ensemble_top_n]

        if n_sources == 1:
            # Simple case: average positions weighted by 1/RMSE
            positions = [sol[0] for sol in sorted_sols]
            rmses = [sol[1] for sol in sorted_sols]
            ...
            return np.array([avg_s1[0], avg_s1[1], avg_s2[0], avg_s2[1]])

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, max_fevals_override=None):
        ...
        # Refine top N solutions with Nelder-Mead
        refined_solutions = []
        ...
        # === ENSEMBLE STEP: Create weighted average solution ===
        ensemble_input = [(sol[0], sol[1]) for sol in refined_solutions[:self.ensemble_top_n]]
        ensemble_params = self._compute_ensemble_position(ensemble_input, n_sources)
        ...
        if ensemble_params is not None:
            if n_sources == 1:
                x, y = ensemble_params
                q, _, _, ensemble_rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_fine, dt, nt, T0, sensors_xy, q_range,
                    early_fraction=1.0)
                n_sims[0] += 1
                ensemble_full = np.array([x, y, q])
                ensemble_sources = [(float(x), float(y), float(q))]
            else:
                x1, y1, x2, y2 = ensemble_params
                (q1, q2), _, _, ensemble_rmse = compute_optimal_intensity_2src(
                    x1, y1, x2, y2, Y_observed, solver_fine, dt, nt, T0, sensors_xy, q_range,
                    early_fraction=1.0)
                n_sims[0] += 2
                ensemble_full = np.array([x1, y1, q1, x2, y2, q2])
                ensemble_sources = [(float(x1), float(y1), float(q1)),
                                   (float(x2), float(y2), float(q2))]

            candidates_raw.append((ensemble_sources, ensemble_full, ensemble_rmse, 'ensemble', ensemble_params))
        ...
        # Remove the pos_params from candidates
