#!/usr/bin/env python3
"""
Run script for Lower Sigma experiment.

Tests the hypothesis that lower sigma (0.15/0.18) may converge faster
and more precisely than baseline (0.15/0.20).
"""
...
from experiments.lower_sigma_baseline.optimizer import LowerSigmaOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = LowerSigmaOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Lower Sigma optimizer')
    ...
    parser.add_argument('--sigma0-1src', type=float, default=0.15,
                        help='Initial sigma for 1-source (baseline: 0.15)')
    parser.add_argument('--sigma0-2src', type=float, default=0.18,
                        help='Initial sigma for 2-source (baseline: 0.20, test: 0.18)')
    ...
    print(f"Refine: {args.refine_maxiter} NM iterations")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Lower Sigma (sigma={args.sigma0_1src}/{args.sigma0_2src})")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline:  1.1688 @ 58.4 min (sigma=0.15/0.20)")
    ...
    if projected_400 > 60:
        ...
        print("SUCCESS! Meets target: score >= 1.17, time <= 60 min")
    elif score > 1.1688:
        print("IMPROVED: Score better than baseline")
    else:
