"""
Powell Polish Optimizer - Uses Powell's method instead of Nelder-Mead for final polish

Powell's method performs coordinate-wise line searches, which may be more efficient
for the 2-4D search space than NM's simplex operations.
"""
...
class PowellPolishOptimizer:
    """
    Optimizer using Powell's method for final polish instead of Nelder-Mead.
    """
    ...
    def __init__(
        ...
        # Polish method: 'powell' or 'nelder-mead'
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Refine top N solutions using POWELL or NM
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                # KEY CHANGE: Use specified polish method
                result = minimize(
                    objective_coarse,
                    pos_params,
                    method=self.polish_method,  # 'powell' or 'nelder-mead'
                    options={
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Polish method: {self.polish_method}")
