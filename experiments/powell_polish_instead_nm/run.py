#!/usr/bin/env python3
"""
Run script for Powell Polish Optimizer.

Tests whether Powell's method (coordinate-wise line search) is more efficient
than Nelder-Mead for final polish in the 2-4D search space.
"""
...
from optimizer import PowellPolishOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = PowellPolishOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Powell Polish optimizer')
    ...
    # Polish method
    parser.add_argument('--polish-method', type=str, default='powell', choices=['powell', 'nelder-mead'])
    # Shared parameters
    ...
    print(f"\nPowell Polish Optimizer")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Polish method: {args.polish_method}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - {args.polish_method.upper()} Polish")
    print(f"{'='*70}")
    ...
    run_name = args.mlflow_run_name or f"powell_polish_{args.polish_method}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        mlflow.log_param("experiment_id", "EXP_POWELL_POLISH_001")
        mlflow.log_param("worker", "W1")
        mlflow.log_param("algorithm", "powell_polish")
