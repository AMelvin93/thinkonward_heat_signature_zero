"""
Run script for hopping_with_tabu_memory experiment.

Tests:
1. Tabu hopping with standard params
2. Tabu hopping with larger tabu distance (more exploration)
3. No tabu hopping (baseline)
"""
...
def run_experiment(config, config_name, data):
    ...
    print(f"Parameters: enable_tabu_hopping={config.get('enable_tabu_hopping', True)}")
    print(f"            tabu_distance={config.get('tabu_distance', 0.03)}")
...
def main():
    ...
    # Configuration 1: Tabu hopping with standard distance
    config1 = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
    ...
    # Configuration 2: Tabu hopping with larger distance (more exploration)
    config2 = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'timestep_fraction': 0.40,
    ...
    # Configuration 3: No tabu hopping (baseline)
    config3 = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'timestep_fraction': 0.40,
    ...
    configs = [
        (config1, 'tabu_dist_003'),
        (config2, 'tabu_dist_005'),
        (config3, 'no_tabu_baseline'),
    ]
