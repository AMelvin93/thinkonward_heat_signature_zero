#!/usr/bin/env python3
"""
Run script for W1 Sigma 0.18/0.22 optimizer - Re-verification.

EXP018: Re-verifying sigma 0.18/0.22 which was the ONLY higher sigma config
that beat baseline (EXP010: 1.1337 @ 63.5 min).
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Sigma 0.18/0.22 optimizer (EXP018)')
    ...
    print(f"\nW1 Sigma 0.18/0.22 Optimizer (EXP018 - Re-verification)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Sigma 0.18/0.22 (EXP018 - Re-verification)")
    print(f"{'='*70}")
    ...
    print(f"This run:      {score:.4f} @ {projected_400:.1f} min (sigma 0.18/0.22)")
    print(f"Delta vs baseline: {score - 1.1247:+.4f} score, {projected_400 - 57.2:+.1f} min")
    print(f"Delta vs EXP010:   {score - 1.1337:+.4f} score, {projected_400 - 63.5:+.1f} min")
    print()
    if projected_400 <= 60:
        if score > 1.1247:
