"""
Experiment: W2_sigma_scale_tuning - Phase 2

Phase 1 found: sigma_020_024 with 1.1526 @ 56.4 min
Gap to Top 10: only 0.0059!

Phase 2:
1. Validate sigma_020_024 with 3 runs
2. Try even higher sigma (0.22/0.26)
3. Try combining best sigma with best config options
"""
...
def main():
    ...
    # Best config from Phase 1 (sigma_020_024)
    best_config = {
        'sigma0_1src': 0.20,
        'sigma0_2src': 0.24,
        'max_fevals_1src': 20,
    ...
    print("=" * 70)
    print("EXPERIMENT: W2_sigma_scale_tuning - PHASE 2")
    print("=" * 70)
    print(f"Phase 1 best: sigma_020_024 = 1.1526 @ 56.4 min")
    print(f"Gap to Top 10: 0.0059")
    print()

    # Validation: Run sigma_020_024 three times
    print("=" * 70)
    print("VALIDATION: Running sigma_020_024 three times")
    print("=" * 70)
    ...
    for run_num in range(1, 4):
        ...
        run_data = {
            ...
            'config_name': f'sigma_020_024_validation_run{run_num}',
            'config': {'sigma0_1src': 0.20, 'sigma0_2src': 0.24},
            'score': result['score'],
    ...
    # Now try even higher sigma
    print("\n" + "=" * 70)
    print("EXPLORATORY: Testing sigma 0.22/0.26")
    print("=" * 70)

    higher_sigma_config = {**best_config, 'sigma0_1src': 0.22, 'sigma0_2src': 0.26}
    result = run_full_test('sigma_022_026', higher_sigma_config, samples, meta)

    status = "IN BUDGET" if result['in_budget'] else "OVER BUDGET"
    delta = result['score'] - np.mean(scores)
    print(f"\nResult: Score={result['score']:.4f} ({delta:+.4f} vs validation mean), Time={result['projected_400_min']:.1f} min [{status}]")

    run_data = {
        'run': len(state['tuning_runs']) + 1,
        'config_name': 'sigma_022_026',
        'config': {'sigma0_1src': 0.22, 'sigma0_2src': 0.26},
        'score': result['score'],
        'time_min': result['projected_400_min'],
        'in_budget': result['in_budget'],
        'timestamp': datetime.now().isoformat()
    ...
    print(f"\nConfig: sigma_020_024 + 3pert_nm3_refine10")
    print(f"  sigma0_1src: 0.20")
    print(f"  sigma0_2src: 0.24")
    ...
    print(f"  perturbation_scale: 0.05")
