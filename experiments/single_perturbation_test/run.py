"""
Test if single perturbation (n=1) is sufficient vs double (n=2).

Uses the hopping_with_tabu_memory optimizer with:
- enable_tabu_hopping=False (no tabu memory)
- Tests n_perturbations = 0, 1, 2

Goal: Determine if perturbation helps and if so, how many are needed.
"""
...
from optimizer import TabuBasinHoppingOptimizer as TabuHoppingOptimizer
...
def process_sample(args):
    ...
    optimizer = TabuHoppingOptimizer(**config)
    try:
...
def run_experiment(config, config_name, data):
    ...
    print(f"  perturbation_scale={config.get('perturbation_scale', 0.05)}")
    print(f"  enable_tabu_hopping={config.get('enable_tabu_hopping', False)}")
    print(f"{'='*60}")
...
def main():
    ...
    base_config = {
        ...
        'refine_maxiter': 8,
        'enable_tabu_hopping': True,  # Enable to use perturbation
        'tabu_distance': 0.0,  # Set to 0 to disable tabu checking
        'perturbation_scale': 0.05,
        'perturb_nm_iters': 3,
    }

    # Test different perturbation counts
    configs = [
        ({**base_config, 'enable_tabu_hopping': False}, 'no_perturbation'),
        ({**base_config, 'n_perturbations': 1}, 'perturbation_n1'),
        ({**base_config, 'n_perturbations': 2}, 'perturbation_n2'),
    ]
