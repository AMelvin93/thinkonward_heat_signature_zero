"""
Two-Phase CMA-ES Optimizer.

Phase 1: CMA-ES with early_fraction=0.3 for position localization
Phase 2: CMA-ES with early_fraction=1.0 starting from Phase 1 best

The idea is that early timesteps help find approximate position,
then full timesteps refine for final accuracy.
"""
...
class TwoPhaseCMAESOptimizer:
    """
    Two-Phase CMA-ES:
    Phase 1: Early timestep objective (0.3) for exploration
    Phase 2: Full timestep objective (1.0) for refinement
    """
    ...
    def __init__(
        ...
        phase1_fraction: float = 0.7,  # fraction of fevals for phase 1
        sigma0_1src: float = 0.15,
        sigma0_2src: float = 0.20,
        sigma0_phase2: float = 0.05,  # smaller sigma for phase 2 refinement
        use_triangulation: bool = True,
        n_candidates: int = N_MAX,
        candidate_pool_size: int = 10,
        early_fraction_phase1: float = 0.3,
        early_fraction_phase2: float = 1.0,
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.sigma0_phase2 = sigma0_phase2
        self.use_triangulation = use_triangulation
        self.n_candidates = min(n_candidates, N_MAX)
        self.candidate_pool_size = candidate_pool_size
        self.early_fraction_phase1 = early_fraction_phase1
        self.early_fraction_phase2 = early_fraction_phase2
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        def make_objective(solver, early_fraction):
            if n_sources == 1:
                def obj(xy_params):
                    x, y = xy_params
                    n_sims[0] += 1
                    ...
                    return rmse_early if early_fraction < 1.0 else rmse_full
                return obj
            else:
                def obj(xy_params):
        ...
        objective_phase1 = make_objective(solver_coarse, self.early_fraction_phase1)
        objective_phase2 = make_objective(solver_coarse, self.early_fraction_phase2)
        ...
        # Phase 1: Early timestep optimization
        fevals_phase1 = int(max_fevals * self.phase1_fraction)
        fevals_phase2 = max_fevals - fevals_phase1
        fevals_per_init = max(5, fevals_phase1 // len(initializations))
        ...
        for init_params, init_type in initializations:
            ...
            while not es.stop():
                solutions = es.ask()
                fitness = [objective_phase1(s) for s in solutions]
                es.tell(solutions, fitness)
                for sol, fit in zip(solutions, fitness):
                    all_solutions.append((np.array(sol), fit, init_type))

        # Sort by phase 1 fitness and take top candidates for phase 2
        all_solutions.sort(key=lambda x: x[1])
        top_for_phase2 = all_solutions[:3]  # Top 3 for phase 2

        # Phase 2: Full timestep refinement from top positions
        phase2_solutions = []
        fevals_per_top = max(3, fevals_phase2 // len(top_for_phase2))

        for pos_params, _, init_type in top_for_phase2:
            opts = cma.CMAOptions()
            opts['maxfevals'] = fevals_per_top
            opts['bounds'] = [lb, ub]
            opts['verbose'] = -9
            opts['tolfun'] = 1e-6
            opts['tolx'] = 1e-6

            es = cma.CMAEvolutionStrategy(pos_params.tolist(), self.sigma0_phase2, opts)

            while not es.stop():
                solutions = es.ask()
                fitness = [objective_phase2(s) for s in solutions]
                es.tell(solutions, fitness)
                for sol, fit in zip(solutions, fitness):
                    phase2_solutions.append((np.array(sol), fit, 'phase2'))

        # Combine and sort
        all_solutions = phase2_solutions + all_solutions[3:]  # Phase2 results + remaining phase1
        all_solutions.sort(key=lambda x: x[1])
