#!/usr/bin/env python3
"""
Run script for Two-Phase CMA-ES experiment.

Phase 1: Early timestep optimization for position localization
Phase 2: Full timestep optimization for refinement
"""
...
from optimizer import TwoPhaseCMAESOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = TwoPhaseCMAESOptimizer(
        max_fevals_1src=config['max_fevals_1src'],
        max_fevals_2src=config['max_fevals_2src'],
        phase1_fraction=config['phase1_fraction'],
        sigma0_2src=config.get('sigma0_2src', 0.20),
        sigma0_phase2=config.get('sigma0_phase2', 0.05),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Two-Phase CMA-ES')
    ...
    parser.add_argument('--phase1-fraction', type=float, default=0.7)
    parser.add_argument('--sigma0-2src', type=float, default=0.20)
    parser.add_argument('--sigma0-phase2', type=float, default=0.05)
    ...
    print(f"Phase 1: {args.phase1_fraction*100:.0f}% fevals (early=0.3)")
    print(f"Phase 2: {(1-args.phase1_fraction)*100:.0f}% fevals (early=1.0)")
    print(f"=" * 60)
    ...
    config = {
        ...
        'sigma0_phase2': args.sigma0_phase2,
    }
