"""
Coarse-to-Fine Temporal Fidelity Optimizer

Multi-fidelity approach:
1. Initial CMA-ES search with 20% timesteps (very fast exploration)
2. Refine top candidates with 40% timesteps (medium fidelity)
3. Final evaluation with full timesteps (accurate ranking)

Hypothesis: Using very coarse temporal resolution for exploration allows more
iterations, while progressive refinement maintains accuracy for final selection.
"""
...
class CoarseToFineTemporalOptimizer:
    """
    Multi-fidelity temporal optimizer with progressive refinement.

    Key parameters:
    - coarse_fraction: Timestep fraction for initial CMA-ES (default 0.20)
    - medium_fraction: Timestep fraction for refinement (default 0.40)
    - refine_top_n: Number of candidates to refine at medium fidelity
    """
    ...
    def __init__(
        ...
        refine_top_n: int = 5,  # Number of candidates to refine at medium fidelity
        rmse_threshold_1src: float = 0.4,
        rmse_threshold_2src: float = 0.5,
        # Multi-fidelity parameters
        coarse_fraction: float = 0.20,  # 20% timesteps for exploration
        medium_fraction: float = 0.40,  # 40% timesteps for refinement
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.medium_fraction = medium_fraction
    ...
    def _run_coarse_search(self, sample, meta, q_range, solver_coarse,
                           initializations, n_sources, nt_coarse):
        """
        Phase 1: Very coarse CMA-ES search (20% timesteps).
        Returns top candidate positions for refinement.
        """
        ...
        # Objective using COARSE grid with VERY COARSE timesteps
        if n_sources == 1:
        ...
        # Sort by coarse fitness and return top candidates
        all_solutions.sort(key=lambda x: x[1])
        return all_solutions[:self.candidate_pool_size], n_sims[0]

    def _refine_at_medium_fidelity(self, candidates, sample, meta, q_range,
                                    solver_coarse, n_sources, nt_medium):
        """
        Phase 2: Refine top candidates with medium fidelity (40% timesteps).
        Uses Nelder-Mead polish at higher temporal resolution.
        """
        sensors_xy = np.array(sample['sensors_xy'])
        Y_observed = sample['Y_noisy']
        dt = meta['dt']
        ...
        if n_sources == 1:
            def objective_medium(xy_params):
                x, y = xy_params
                n_sims[0] += 1
                q, Y_pred, rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_coarse, dt, nt_medium, T0, sensors_xy, q_range)
                return rmse
        else:
            def objective_medium(xy_params):
                x1, y1, x2, y2 = xy_params
                n_sims[0] += 2
                (q1, q2), Y_pred, rmse = compute_optimal_intensity_2src(
                    x1, y1, x2, y2, Y_observed, solver_coarse, dt, nt_medium, T0, sensors_xy, q_range)
                return rmse

        refined = []
        for pos_params, rmse_coarse, init_type in candidates[:self.refine_top_n]:
            # NM polish at medium fidelity
            result = minimize(
                objective_medium,
                pos_params,
                method='Nelder-Mead',
            ...
            if result.fun < rmse_coarse:
                refined.append((result.x, result.fun, 'refined_medium'))
            else:
                # Re-evaluate at medium fidelity
                rmse_medium = objective_medium(pos_params)
                refined.append((pos_params, rmse_medium, init_type))

        # Add remaining candidates (re-evaluated at medium fidelity)
        for pos_params, rmse_coarse, init_type in candidates[self.refine_top_n:]:
            rmse_medium = objective_medium(pos_params)
            refined.append((pos_params, rmse_medium, init_type))

        return refined, n_sims[0]
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Compute timesteps for each fidelity level
        nt_coarse = max(10, int(nt_full * self.coarse_fraction))
        nt_medium = max(20, int(nt_full * self.medium_fraction))

        if verbose:
            print(f"  Coarse: {nt_coarse}/{nt_full} ({self.coarse_fraction*100:.0f}%)")
            print(f"  Medium: {nt_medium}/{nt_full} ({self.medium_fraction*100:.0f}%)")
        ...
        # Phase 1: Coarse search (20% timesteps)
        coarse_candidates, n_sims_coarse = self._run_coarse_search(
            sample, meta, q_range, solver_coarse, primary_inits, n_sources, nt_coarse
        )

        # Check if we need fallback
        best_rmse_coarse = coarse_candidates[0][1] if coarse_candidates else float('inf')
        threshold = self.rmse_threshold_1src if n_sources == 1 else self.rmse_threshold_2src

        if best_rmse_coarse > threshold * 1.5:  # More lenient threshold for coarse
            # Fallback with additional initializations
            fallback_inits = []
            ...
            fallback_candidates, n_sims_fallback = self._run_coarse_search(
                sample, meta, q_range, solver_coarse, fallback_inits, n_sources, nt_coarse
            )
            n_sims_coarse += n_sims_fallback

            # Merge and re-sort
            coarse_candidates.extend(fallback_candidates)
            coarse_candidates.sort(key=lambda x: x[1])
            coarse_candidates = coarse_candidates[:self.candidate_pool_size]

        # Phase 2: Medium fidelity refinement (40% timesteps)
        medium_candidates, n_sims_medium = self._refine_at_medium_fidelity(
            coarse_candidates, sample, meta, q_range, solver_coarse, n_sources, nt_medium
        )

        # Sort by medium fidelity RMSE
        medium_candidates.sort(key=lambda x: x[1])

        # Phase 3: Final evaluation at full fidelity
        candidates_raw = []
        n_sims_final = 0
        for pos_params, rmse_medium, init_type in medium_candidates:
            if n_sources == 1:
                x, y = pos_params
        ...
        total_sims = n_sims_coarse + n_sims_medium + n_sims_final
