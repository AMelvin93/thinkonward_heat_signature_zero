"""
Niching CMA-ES Optimizer for Diversity

Approach: Run INDEPENDENT CMA-ES instances that each find their own local optimum.
Instead of pooling all solutions and filtering, explicitly maintain diversity by:
1. Running multiple independent CMA-ES populations
2. Taking the best from each population as a distinct candidate
3. This ensures diversity by design rather than post-hoc filtering

Key insight: The scoring formula rewards diversity (0.3 * N_valid/3).
"""
...
class NichingCMAESOptimizer:
    """
    Niching CMA-ES optimizer that maintains population diversity.

    Key changes from baseline:
    1. Run INDEPENDENT CMA-ES populations (not pooled)
    2. Each population finds its own local optimum
    3. Take best from each as a distinct candidate
    4. Add "taboo" regions to prevent convergence to same solution
    """
    ...
    def __init__(
        ...
        # Niching parameters
        n_independent_runs: int = 3,  # Number of independent CMA-ES populations
        taboo_radius: float = 0.15,  # Radius of exclusion zone around found optima
        taboo_penalty: float = 10.0,  # Penalty for entering taboo region
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.taboo_radius = taboo_radius
        self.taboo_penalty = taboo_penalty
    ...
    def _run_independent_cmaes(self, init_params, init_type, objective, max_fevals,
                                sigma0, lb, ub, taboo_centers, n_sources):
        """Run a single independent CMA-ES with taboo regions."""

        def objective_with_taboo(xy_params):
            base_rmse = objective(xy_params)

            # Add penalty for being in taboo regions
            penalty = 0.0
            if taboo_centers:
                # Compute minimum distance to any taboo center
                for center in taboo_centers:
                    if n_sources == 1:
                        dist = np.linalg.norm(xy_params - center)
                    else:
                        # For 2-source, use minimum over source pairings
                        pos1 = np.array([xy_params[0], xy_params[1]])
                        pos2 = np.array([xy_params[2], xy_params[3]])
                        c_pos1 = np.array([center[0], center[1]])
                        c_pos2 = np.array([center[2], center[3]])
                        dist = min(
                            np.linalg.norm(pos1 - c_pos1) + np.linalg.norm(pos2 - c_pos2),
                            np.linalg.norm(pos1 - c_pos2) + np.linalg.norm(pos2 - c_pos1)
                        ) / 2

                    if dist < self.taboo_radius:
                        penalty += self.taboo_penalty * (1 - dist / self.taboo_radius)
        ...
        while not es.stop():
            solutions = es.ask()
            fitness = [objective_with_taboo(s) for s in solutions]
            es.tell(solutions, fitness)

            # Track best (using base RMSE, not penalized)
            for sol in solutions:
                base_rmse = objective(sol)
                if base_rmse < best_rmse:
                    best_rmse = base_rmse
                    best_solution = np.array(sol)

        return best_solution, best_rmse, init_type

    def _run_niching_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                   n_sources, nt_reduced, nt_full):
        """Run niching optimization with independent CMA-ES populations."""
        ...
        # Distribute fevals across independent runs
        fevals_per_run = max(5, max_fevals // self.n_independent_runs)
        ...
        # Pad with random initializations if needed
        while len(initializations) < self.n_independent_runs:
            random_init = self._random_init_positions(n_sources)
            initializations.append((random_init, 'random'))

        # Run INDEPENDENT CMA-ES populations with taboo regions
        niche_solutions = []
        taboo_centers = []

        for i in range(min(self.n_independent_runs, len(initializations))):
            init_params, init_type = initializations[i]

            best_pos, best_rmse, best_type = self._run_independent_cmaes(
                init_params, init_type, objective_coarse, fevals_per_run,
                sigma0, lb, ub, taboo_centers, n_sources
            )

            if best_pos is not None:
                niche_solutions.append((best_pos, best_rmse, best_type))
                taboo_centers.append(best_pos)  # Add to taboo for next runs

        # Refine top solutions with NM
        refined_solutions = []
        for pos_params, rmse_coarse, init_type in niche_solutions:
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run niching optimization
        candidates_raw, n_sims = self._run_niching_optimization(
        ...
        if best_rmse_initial > threshold:
            # Fallback with more random restarts
            fallback_candidates, fallback_sims = self._run_niching_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                n_sources, nt_reduced, nt_full
            )
