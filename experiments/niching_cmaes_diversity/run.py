#!/usr/bin/env python3
"""Run script for Niching CMA-ES diversity optimizer."""
...
from optimizer import NichingCMAESOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = NichingCMAESOptimizer(
        ...
        n_independent_runs=config.get('n_independent_runs', 3),
        taboo_radius=config.get('taboo_radius', 0.15),
        taboo_penalty=config.get('taboo_penalty', 10.0),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Niching CMA-ES Diversity Optimizer')
    ...
    parser.add_argument('--n-independent-runs', type=int, default=3)
    parser.add_argument('--taboo-radius', type=float, default=0.15)
    ...
    print(f"\nNiching CMA-ES Diversity Optimizer")
    print(f"=" * 60)
    ...
    print(f"Independent runs: {args.n_independent_runs}")
    print(f"Taboo radius: {args.taboo_radius}")
    print(f"=" * 60)
    ...
    config = {
        ...
        'taboo_radius': args.taboo_radius,
    }
    ...
    n_cands = [r['n_candidates'] for r in results if r['success']]
    projected_400 = (total_time / n_samples) * 400 / 60

    print(f"\n{'='*70}")
    print(f"RESULTS - Niching CMA-ES Diversity")
    print(f"{'='*70}")
    print(f"Submission Score: {score:.4f}")
    print(f"Projected (400):  {projected_400:.1f} min")
    print(f"Avg N_valid:      {np.mean(n_cands):.2f}")
    if rmses_1src:
        print(f"  1-source: RMSE={np.mean(rmses_1src):.4f} (n={len(rmses_1src)})")
    if rmses_2src:
        print(f"  2-source: RMSE={np.mean(rmses_2src):.4f} (n={len(rmses_2src)})")
    print()
    print(f"Baseline (polish): 1.1688 @ 58.4 min")
    print(f"This run:          {score:.4f} @ {projected_400:.1f} min")
    print(f"Delta:             {score - 1.1688:+.4f} score, {projected_400 - 58.4:+.1f} min")
    print(f"{'='*70}\n")

    # Log candidate count distribution
    cand_dist = {}
    for r in results:
        n = r['n_candidates']
        cand_dist[n] = cand_dist.get(n, 0) + 1
    print("Candidate count distribution:")
    for n in sorted(cand_dist.keys()):
        print(f"  {n} candidates: {cand_dist[n]} samples ({cand_dist[n]/len(results)*100:.1f}%)")
