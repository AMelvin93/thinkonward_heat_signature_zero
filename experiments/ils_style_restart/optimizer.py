"""
Iterated Local Search (ILS) Style Optimizer

Classic ILS approach from optimization literature:
1. Run standard CMA-ES + NM polish to find initial candidates
2. For best candidate, iteratively:
   a. Perturb position
   b. Run local search (NM) from perturbed position
   c. If improved, accept; else keep original
   d. Repeat up to max_ils_iterations times

Key insight: ILS keeps iterating on the BEST solution found so far,
rather than just checking each perturbation once.
"""
...
class ILSStyleOptimizer:
    """
    Optimizer using Iterated Local Search (ILS) approach.

    After standard CMA-ES + NM polish, applies ILS to the best candidate:
    - Perturb position
    - Run NM from perturbed position
    - If improved, accept new solution
    - Repeat up to max_ils_iterations times
    """
    ...
    def __init__(
        ...
        max_ils_iterations: int = 3,  # Max ILS iterations
        ils_nm_iters: int = 4,  # NM iterations per ILS step
        ils_perturbation_scale: float = 0.05,  # Perturbation magnitude
        ils_apply_to_top_n: int = 1,  # Apply ILS to top N candidates
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        # ILS params
        self.enable_ils = enable_ils
        self.max_ils_iterations = max_ils_iterations
        self.ils_nm_iters = ils_nm_iters
        self.ils_perturbation_scale = ils_perturbation_scale
        self.ils_apply_to_top_n = ils_apply_to_top_n
    ...
    def _perturb_position(self, pos_params, lb, ub):
        """Generate a random perturbation of position parameters."""
        direction = np.random.randn(len(pos_params))
        direction = direction / (np.linalg.norm(direction) + 1e-8)
        perturbed = pos_params + direction * self.ils_perturbation_scale
        return np.clip(perturbed, lb, ub)

    def _run_ils(self, pos_params, rmse, objective_fn, lb, ub, n_sims_counter):
        """
        Run Iterated Local Search on a single candidate.

        Returns: (best_pos, best_rmse, ils_improved, ils_iterations_run)
        """
        best_pos = pos_params.copy()
        best_rmse = rmse
        ils_improved = False

        for iteration in range(self.max_ils_iterations):
            # Perturb best position
            perturbed = self._perturb_position(best_pos, lb, ub)
            ...
            if result.fun < best_rmse:
                best_pos = result.x.copy()
                best_rmse = result.fun
                ils_improved = True
            else:
                # No improvement - ILS often terminates on no improvement
                # But we continue for remaining iterations
                pass

        return best_pos, best_rmse, ils_improved, self.max_ils_iterations

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, nt_reduced):
        """Run CMA-ES + NM polish + ILS optimization."""
        ...
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                ...
                if result.fun < rmse_coarse:
                    refined_solutions.append((result.x, result.fun, 'nm_polished'))
                else:
        ...
        if self.enable_ils:
            ...
            # Apply ILS to top N candidates
            n_to_ils = min(self.ils_apply_to_top_n, len(refined_solutions))

            for idx in range(n_to_ils):
                pos_params, rmse, init_type = refined_solutions[idx]

                # Run ILS
                new_pos, new_rmse, improved, _ = self._run_ils(
                    pos_params, rmse, objective_coarse, lb, ub, n_sims
                )

                if improved:
                    refined_solutions[idx] = (new_pos, new_rmse, 'ils_improved')
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  ILS: {'ENABLED' if self.enable_ils else 'DISABLED'}")
            if self.enable_ils:
                print(f"       max_iterations={self.max_ils_iterations}, nm_iters={self.ils_nm_iters}")
