"""
Multi-Fidelity with Coordinate Descent Refinement on Coarse Grid.

Key insight from research (arXiv:1803.01454):
- Coordinate descent is efficient for smooth, low-dimensional problems
- Each cycle optimizes one coordinate at a time via line search
- For 4D: 4 line searches per cycle (vs ~5 evals/iter for Nelder-Mead)
"""
...
def coordinate_descent_refinement(params, objective, bounds, n_cycles=2):
    """
    Optimize each coordinate in sequence via bounded line search.

    For 4D: 4 line searches per cycle
    More efficient than Nelder-Mead for smooth objectives.
    """
    ...
    for cycle in range(n_cycles):
        for i in range(len(params)):
            ...
            result = minimize_scalar(
                ...
                method='bounded',
                options={'maxiter': 5}  # Quick search
            )
...
class CoordDescentRefineOptimizer:
    """
    Multi-fidelity optimizer with Coordinate Descent refinement on COARSE grid.

    Key insight: Coordinate descent is more efficient than Nelder-Mead for
    smooth, low-dimensional problems like heat source localization.
    """
    ...
    def __init__(
        ...
        refine_cycles: int = 2,  # Coordinate descent cycles
        refine_top_n: int = 2,   # Refine top N solutions
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # ===== KEY: Coordinate Descent refinement on COARSE grid =====
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_cycles > 0:
                refined_params, refined_rmse = coordinate_descent_refinement(
                    pos_params, objective_coarse, (lb, ub), n_cycles=self.refine_cycles
                )
                if refined_rmse < rmse_coarse:
                    refined_solutions.append((refined_params, refined_rmse, 'refined'))
                else:
                    refined_solutions.append((pos_params, rmse_coarse, init_type))
            else:
                refined_solutions.append((pos_params, rmse_coarse, init_type))

        # Add remaining solutions without refinement
