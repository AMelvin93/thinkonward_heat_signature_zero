"""
BIPOP-CMA-ES with Temporal Fidelity

BIPOP-CMA-ES (Bi-Population CMA-ES) alternates between large and small
population restarts, unlike IPOP which only increases population.

Key features:
- BIPOP restart strategy: alternates large/small populations
- Large population: global exploration (random restart)
- Small population: local exploitation (best-so-far restart)
- 40% timesteps during CMA-ES (temporal fidelity speedup)
- Final NM polish using FULL timesteps

Research: BIPOP can solve problems IPOP misses (Gallagher, Katsuuras functions)
because small populations can be more efficient for local refinement.
"""
...
class BIPOPTemporalOptimizer:
    """
    BIPOP-CMA-ES with temporal fidelity and final polish.

    BIPOP (Bi-Population) strategy:
    - Alternates between large population (global search) and small population (local refinement)
    - Large pop: random restart with 2x population
    - Small pop: restart from best-so-far with default/small population
    - This differs from IPOP which only increases population
    """
    ...
    def __init__(
        ...
        max_restarts: int = 2,  # Maximum number of restarts
        large_popsize_factor: int = 2,  # Factor for large population
        small_popsize: int = 4,  # Small population size
        use_bipop: bool = True,  # Enable BIPOP vs standard CMA-ES
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.large_popsize_factor = large_popsize_factor
        self.small_popsize = small_popsize
        self.use_bipop = use_bipop
    ...
    def _run_bipop_cmaes(self, objective, init_params, sigma0, lb, ub, max_fevals, n_sources):
        """
        Run BIPOP-CMA-ES: alternating large and small population restarts.
        
        Strategy:
        - First run: default population from init_params
        - Large pop restart: random init with 2x population (global search)
        - Small pop restart: best-so-far init with small population (local refinement)
        
        Returns list of (solution, fitness, restart_info) tuples.
        """
        all_solutions = []
        best_solution = None
        best_fitness = float('inf')
        
        remaining_fevals = max_fevals
        
        # Track budget for large/small regimes
        large_budget_used = 0
        small_budget_used = 0
        
        # First run with default population
        restart_num = 0
        init_fevals = min(remaining_fevals, max(5, max_fevals // 3))
        
        opts = cma.CMAOptions()
        opts['maxfevals'] = init_fevals
        opts['bounds'] = [lb, ub]
        opts['verbose'] = -9
        opts['tolfun'] = 1e-6
        opts['tolx'] = 1e-6
        ...
        while not es.stop():
            ...
            for sol, fit in zip(solutions, fitness):
                all_solutions.append((np.array(sol), fit, 'initial'))
                if fit < best_fitness:
                    best_fitness = fit
                    best_solution = np.array(sol)
        
        remaining_fevals -= fevals_used
        default_popsize = es.popsize
        
        # BIPOP restarts
        for restart_num in range(1, self.max_restarts + 1):
            if remaining_fevals <= 3:
                break
                
            # Allocate fevals for this restart
            restart_fevals = min(remaining_fevals, max(3, max_fevals // (self.max_restarts + 1)))
            
            if self.use_bipop:
                # BIPOP: alternate between large and small population
                if restart_num % 2 == 1:
                    # Large population regime: random restart for global exploration
                    popsize = default_popsize * self.large_popsize_factor
                    init_for_restart = self._random_init_positions(n_sources, margin=0.1)
                    regime = 'large'
                else:
                    # Small population regime: best-so-far for local refinement
                    popsize = self.small_popsize
                    if best_solution is not None:
                        # Add small random perturbation
                        init_for_restart = best_solution + np.random.randn(len(best_solution)) * 0.05
                        init_for_restart = np.clip(init_for_restart, lb, ub)
                    else:
                        init_for_restart = self._random_init_positions(n_sources, margin=0.1)
                    regime = 'small'
            else:
                # Standard restart (not BIPOP)
                popsize = default_popsize
                init_for_restart = self._random_init_positions(n_sources, margin=0.1)
                regime = 'standard'
            
            opts = cma.CMAOptions()
            opts['maxfevals'] = restart_fevals
            opts['bounds'] = [lb, ub]
            opts['verbose'] = -9
            opts['tolfun'] = 1e-6
            opts['tolx'] = 1e-6
            ...
            es = cma.CMAEvolutionStrategy(init_for_restart.tolist(), sigma0, opts)
            fevals_used = 0
            ...
            while not es.stop():
                ...
                for sol, fit in zip(solutions, fitness):
                    all_solutions.append((np.array(sol), fit, f'restart_{restart_num}_{regime}'))
                    if fit < best_fitness:
                        best_fitness = fit
                        best_solution = np.array(sol)
            ...
            if regime == 'large':
                large_budget_used += fevals_used
            else:
                small_budget_used += fevals_used
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run BIPOP-CMA-ES optimization with final polish."""
        ...
        # Run BIPOP-CMA-ES for each initialization
        for init_params, init_type in initializations:
            solutions = self._run_bipop_cmaes(
                objective_coarse, init_params, sigma0, lb, ub, fevals_per_init, n_sources
            )
            for sol, fit, restart_info in solutions:
                all_solutions.append((sol, fit, f'{init_type}_{restart_info}'))

        # Sort by coarse fitness
        all_solutions.sort(key=lambda x: x[1])

        # Refine top N solutions on COARSE grid with reduced timesteps
        refined_solutions = []
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run BIPOP optimization with final polish
