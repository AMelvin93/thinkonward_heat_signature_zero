#!/usr/bin/env python3
"""
Run script for BIPOP-CMA-ES optimizer.

Tests the hypothesis that BIPOP (alternating large/small populations) can
solve problems that IPOP (increasing population only) misses.
"""
...
from optimizer import BIPOPTemporalOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = BIPOPTemporalOptimizer(
        ...
        large_popsize_factor=config.get('large_popsize_factor', 2),
        small_popsize=config.get('small_popsize', 4),
        use_bipop=config.get('use_bipop', True),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run BIPOP-CMA-ES optimizer')
    ...
    parser.add_argument('--large-popsize-factor', type=int, default=2)
    parser.add_argument('--small-popsize', type=int, default=4)
    parser.add_argument('--disable-bipop', action='store_true', help='Use standard CMA-ES instead')
    # MLflow
    ...
    print(f"BIPOP enabled: {not args.disable_bipop}")
    print(f"Max restarts: {args.max_restarts}")
    print(f"Large popsize factor: {args.large_popsize_factor}x")
    print(f"Small popsize: {args.small_popsize}")
    ...
    config = {
        ...
        'large_popsize_factor': args.large_popsize_factor,
        'small_popsize': args.small_popsize,
        'use_bipop': not args.disable_bipop,
    }
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - BIPOP-CMA-ES ({args.timestep_fraction:.0%} timesteps)")
    print(f"{'='*70}")
    ...
    if projected_400 > 60:
        print("OVER BUDGET")
        status = "OVER_BUDGET"
    elif score >= 1.17 and projected_400 <= 60:
        print("SUCCESS! Score >= 1.17 AND time <= 60 min")
        status = "SUCCESS"
    elif score >= 1.16:
        print("PARTIAL: Good score but may not beat baseline")
        status = "PARTIAL"
    else:
        print("FAILED: Score below baseline")
        status = "FAILED"
    print(f"{'='*70}\n")

    # Count diversity statistics
    n_3cands = sum(1 for r in results if r['n_candidates'] == 3)
    n_2cands = sum(1 for r in results if r['n_candidates'] == 2)
    n_1cands = sum(1 for r in results if r['n_candidates'] == 1)
    print(f"Diversity: {n_3cands} samples with 3 cands, {n_2cands} with 2, {n_1cands} with 1")
    ...
    if not args.no_mlflow:
        ...
        run_name = args.run_name or f"bipop_cmaes_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_BIPOP_CMAES_001")
            mlflow.log_param("worker", "W1")
            mlflow.log_param("tuning_run", args.tuning_run)
            mlflow.log_param("platform", "wsl")
            mlflow.log_param("optimizer", "bipop_cmaes_temporal")
            ...
            mlflow.log_param("max_restarts", args.max_restarts)
            mlflow.log_param("large_popsize_factor", args.large_popsize_factor)
            mlflow.log_param("small_popsize", args.small_popsize)
            mlflow.log_param("use_bipop", not args.disable_bipop)
            ...
            mlflow.log_metric("n_3_candidates", n_3cands)
            mlflow.log_metric("n_2_candidates", n_2cands)
            mlflow.log_metric("n_1_candidates", n_1cands)
