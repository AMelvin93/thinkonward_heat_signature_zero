#!/usr/bin/env python3
"""
Run script for W2 Intermediate Sigma optimizer.

EXP010: intermediate_sigma
Config:
...
Hypothesis: Intermediate sigma (0.18/0.22) may give some exploration benefit while being faster than 0.20/0.25.
Based on: Sigma 0.15/0.20 = 57 min, sigma 0.20/0.25 = 68 min. Try middle ground.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Intermediate Sigma optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP010 specific (intermediate sigma)
    ...
    print(f"\n[W2] Intermediate Sigma Optimizer")
    print(f"=" * 70)
    print(f"EXP010: intermediate_sigma")
    print(f"-" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Intermediate Sigma (EXP010)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Config: sigma={args.sigma0_1src}/{args.sigma0_2src} (INTERMEDIATE), thresh={args.threshold_1src}/{args.threshold_2src}")
    print(f"Baseline: 1.1247 @ 57.2 min (sigma 0.15/0.20)")
    print(f"Best ever: 1.1362 @ 69.2 min (sigma 0.20/0.25)")
