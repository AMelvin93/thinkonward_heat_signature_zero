"""
BOBYQA/COBYLA Polish Optimizer

Uses COBYLA (Constrained Optimization BY Linear Approximations) instead of
Nelder-Mead for the final polish step. COBYLA builds a linear approximation
of the objective and constraints, potentially converging faster than the
simplex-based NM for smooth landscapes.

Unlike Powell (line search overhead) or L-BFGS-B (gradient overhead), COBYLA
uses local approximations without line search.
"""
...
class BOBYQAPolishOptimizer:
    """
    Optimizer that uses COBYLA for final polish instead of Nelder-Mead.
    
    COBYLA uses local linear approximations of the objective and constraints.
    Unlike Powell (line search) or L-BFGS-B (gradients), it should have
    similar per-iteration cost to NM but potentially faster convergence
    ...
    def __init__(
        ...
        # Polish parameters
        polish_method: str = 'COBYLA',  # COBYLA or trust-constr
    ...
    def _polish_with_cobyla(self, objective, pos_params, n_sources, margin=0.05):
        """Polish using COBYLA with bounds as constraints."""
        # Build bound constraints for COBYLA
        # COBYLA uses constraint functions where c(x) >= 0 means constraint is satisfied
        lb, ub = self._get_position_bounds(n_sources, margin)
        
        constraints = []
        for i in range(len(pos_params)):
            # Lower bound: x[i] - lb[i] >= 0
            constraints.append({
                'type': 'ineq',
                'fun': lambda x, i=i, lb=lb: x[i] - lb[i]
            })
            # Upper bound: ub[i] - x[i] >= 0
            constraints.append({
                'type': 'ineq',
                'fun': lambda x, i=i, ub=ub: ub[i] - x[i]
            })
    ...
    def _polish_with_trust_constr(self, objective, pos_params, n_sources, margin=0.05):
        """Polish using trust-constr with bounds."""
        lb, ub = self._get_position_bounds(n_sources, margin)
        bounds = [(lb[i], ub[i]) for i in range(len(pos_params))]
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Refine top N with coarse NM (quick)
        refined_solutions = []
        ...
        # FINAL POLISH: Use COBYLA/trust-constr instead of NM
        if self.polish_maxiter > 0 and candidates_raw:
            ...
            # Polish with selected method
            if self.polish_method == 'COBYLA':
                result = self._polish_with_cobyla(objective_fine, best_pos_params, n_sources)
            elif self.polish_method == 'trust-constr':
                result = self._polish_with_trust_constr(objective_fine, best_pos_params, n_sources)
            else:
                # Fallback to NM
                result = minimize(
                    objective_fine,
