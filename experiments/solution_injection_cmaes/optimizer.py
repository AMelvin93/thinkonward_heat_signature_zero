"""
Solution Injection CMA-ES Optimizer - Share Best Solutions Between Inits

Hypothesis: Injecting best solutions from early CMA-ES inits into later inits
can improve convergence. Based on GloMPO (Globally Managed Parallel Optimization).

Key changes from baseline:
1. Run CMA-ES inits sequentially to share solutions
2. After first init completes, inject its best solution into subsequent inits
3. Use pycma's inject() API to add solutions to the population
"""
...
class SolutionInjectionOptimizer:
    """
    CMA-ES optimizer that injects best solutions from previous inits into subsequent ones.
    """
    ...
    def __init__(
        ...
        inject_best_n: int = 1,  # Number of best solutions to inject
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.inject_best_n = inject_best_n
    ...
    def _get_smart_inits(self, readings, sensors, n_sources):
        ...
        if n_sources == 1:
            ...
            # For 2-source, need 4 params - duplicate hottest with offset
    ...
    def _run_cmaes_with_injection(self, objective, init_params, sigma, fevals, bounds,
                                  solutions_to_inject=None):
        """
        Run CMA-ES with optional solution injection.

        Args:
            objective: Fitness function
            init_params: Initial position
            sigma: Initial step size
            fevals: Max function evaluations
            bounds: Search bounds
            solutions_to_inject: List of solutions to inject (or None)

        Returns:
            (best_x, best_f, all_evaluated) tuple
        """
        opts = {
        ...
        while not es.stop():
            # Inject solutions if available
            if solutions_to_inject:
                for sol in solutions_to_inject[:self.inject_best_n]:
                    es.inject([sol])
        ...
        result = es.result
        return result.xbest, result.fbest, all_evaluated
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=True):
        ...
        # Run CMA-ES with solution injection
        all_solutions = []
        best_solutions_so_far = []  # Solutions to inject into subsequent runs

        for idx, (init_params, init_type) in enumerate(inits):
            # Run CMA-ES, injecting best solutions from previous runs
            best_x, best_f, _ = self._run_cmaes_with_injection(
                objective, init_params, sigma, fevals, bounds,
                solutions_to_inject=best_solutions_so_far if idx > 0 else None
            )

            all_solutions.append((best_x, best_f, init_type))

            # Update best solutions for injection into next runs
            # Keep top N solutions across all runs so far
            all_so_far = sorted(all_solutions, key=lambda x: x[1])
            best_solutions_so_far = [sol[0] for sol in all_so_far[:self.inject_best_n]]
            ...
            for init_params, init_type in inits:
                try:
                    best_x, best_f, _ = self._run_cmaes_with_injection(
                        ...
                        solutions_to_inject=best_solutions_so_far
                    )
