"""Quick timing test - mixed 1/2 source samples."""
...
def main():
    ...
    print(f"Full dataset: {len(src1_indices)} 1-source, {len(src2_indices)} 2-source")

    # Test with 10 of each
    test_indices = src1_indices[:10] + src2_indices[:10]
    test_samples = [(i, samples[i]) for i in test_indices]
    ...
    print("\nMixed timing test: 10 1-source + 10 2-source, 4-pert + nm2")
    print("-" * 60)
    ...
    # Calculate true projection accounting for distribution
    # Full dataset: 32 1-src (40%), 48 2-src (60%) out of 80
    n_1src_full = 32
    n_2src_full = 48
    avg_time_1src = np.mean(times_1src) if times_1src else 0
    avg_time_2src = np.mean(times_2src) if times_2src else 0

    # Projected time for 80 samples
    ...
    print(f"2-source/1-source ratio: {avg_time_2src/avg_time_1src:.2f}x")
    print(f"\nProjected for full 80 samples (32 1-src + 48 2-src):")
    print(f"  Serial sum: {avg_time_1src * n_1src_full + avg_time_2src * n_2src_full:.1f}s")
