"""Full 80-sample test of production config."""
...
def main():
    ...
    # 4-pert + nm2 config (validated best)
    config = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'max_fevals_1src': 20,
    ...
    print("="*70)
    print("FULL 80-SAMPLE TEST: 4-pert + nm2")
    print("="*70)
    ...
    print(f"  2-source/1-source ratio: {np.mean(times_2src)/np.mean(times_1src):.2f}x")
    ...
    with open('full_test_output.json', 'w') as f:
        json.dump({
            ...
            'time_1src_mean': float(np.mean(times_1src)),
            'time_2src_mean': float(np.mean(times_2src)),
            'avg_candidates': float(avg_cands),
            'in_budget': projected_400 <= 60
        }, f, indent=2)

    print(f"\nResults saved to full_test_output.json")
