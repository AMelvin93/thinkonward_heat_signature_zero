#!/usr/bin/env python3
"""
Run script for W1 Sigma 0.17/0.21 optimizer.

EXP013: Testing sigma 0.17/0.21 - intermediate between baseline and 0.18/0.22.
Target: ~60 min timing with score improvement over baseline.
...
from optimizer import Sigma017021Optimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = Sigma017021Optimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Sigma 0.17/0.21 optimizer')
    ...
    parser.add_argument('--sigma0-1src', type=float, default=0.17)
    parser.add_argument('--sigma0-2src', type=float, default=0.21)
    ...
    print(f"\nW1 Sigma 0.17/0.21 Optimizer (EXP013)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Sigma 0.17/0.21 (EXP013)")
    print(f"{'='*70}")
    ...
    print(f"EXP010:    1.1337 @ 63.5 min (sigma 0.18/0.22)")
    print(f"Target:    ~60 min (interpolated from baseline and EXP010)")
    print(f"This run:  {score:.4f} @ {projected_400:.1f} min (sigma 0.17/0.21)")
