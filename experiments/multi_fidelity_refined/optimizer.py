"""
Multi-Fidelity + Minimal Refinement Optimizer.

Combines:
- Coarse grid (50x25) for CMA-ES exploration
- Fine grid (100x50) for final evaluation
- Optional Nelder-Mead refinement on top solution
"""
...
class MultiFidelityRefinedOptimizer:
    """
    Multi-fidelity optimizer with optional Nelder-Mead refinement.
    """
    ...
    def __init__(
        ...
        refine_maxiter: int = 3,  # Minimal refinement iterations
        refine_1src_only: bool = False,  # Only refine 1-source
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.refine_1src_only = refine_1src_only
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Sort by coarse fitness and take top solutions
        all_solutions.sort(key=lambda x: x[1])
        top_solutions = all_solutions[:self.candidate_pool_size]

        # Refine the best solution with Nelder-Mead on fine grid
        best_pos = top_solutions[0][0]
        best_init_type = top_solutions[0][2]

        should_refine = self.refine_maxiter > 0
        if self.refine_1src_only and n_sources > 1:
            should_refine = False
        ...
        if should_refine:
            ...
            if result.fun < objective_fine(best_pos):
                best_pos = result.x
                best_init_type = 'refined'

        # Evaluate top candidates on FINE grid
        candidates_raw = []

        # First add the refined best
        if n_sources == 1:
            x, y = best_pos
        ...
        candidates_raw.append((sources, full_params, final_rmse, best_init_type))

        # Add remaining top solutions (without refinement)
        for pos_params, rmse_coarse, init_type in top_solutions[1:]:
            if n_sources == 1:
                x, y = pos_params
                q, _, rmse_early, final_rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_fine, dt, nt, T0, sensors_xy, q_range,
