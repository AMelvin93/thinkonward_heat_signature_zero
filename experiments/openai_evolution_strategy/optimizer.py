"""
OpenAI Evolution Strategy Optimizer for Inverse Heat Source Problem

Implements the OpenAI ES algorithm (Salimans et al. 2017) as an alternative to CMA-ES.

Key differences from CMA-ES:
- Uses diagonal covariance (no full matrix adaptation)
- Uses antithetic sampling (mirrored perturbations)
- Gradient-based update with natural gradient
- Fixed sigma (no step-size adaptation)

Note: OpenAI ES is designed for high-dimensional problems where covariance
matrix computation is expensive. For our 2-4D problem, CMA-ES's covariance
adaptation may actually be beneficial.
"""
...
class OpenAIESOptimizer:
    """
    OpenAI Evolution Strategy optimizer for inverse heat source identification.

    Key features:
    - Diagonal covariance (no full matrix)
    - Antithetic sampling
    ...
    def __init__(
        ...
        ny_coarse: int = 25,
        # ES parameters
        population_size: int = 10,  # Number of perturbations
        sigma: float = 0.2,  # Noise standard deviation
        learning_rate: float = 0.1,
        max_generations: int = 10,  # Generations per init
        # Search parameters
        use_triangulation: bool = True,
        n_candidates: int = N_MAX,
        candidate_pool_size: int = 10,
        refine_maxiter: int = 8,  # NM polish
        refine_top_n: int = 2,
    ...
    def _run_openai_es(self, init_params, objective_fn, lb, ub, n_sims_counter):
        """
        Run OpenAI ES from a single starting point.

        Uses antithetic sampling (mirror perturbations) for variance reduction.
        """
        n_params = len(init_params)
        mean = init_params.copy()
        best_params = mean.copy()
        best_fitness = objective_fn(mean)
        n_sims_counter[0] += 1

        all_solutions = [(mean.copy(), best_fitness)]

        for gen in range(self.max_generations):
            # Generate perturbations (antithetic sampling)
            epsilon = np.random.randn(self.population_size // 2, n_params)
            epsilon = np.vstack([epsilon, -epsilon])  # Mirror

            # Evaluate all perturbed solutions
            fitness = []
            for i in range(len(epsilon)):
                params = self._clip_to_bounds(mean + self.sigma * epsilon[i], lb, ub)
                f = objective_fn(params)
                n_sims_counter[0] += 1
                fitness.append(f)
                all_solutions.append((params.copy(), f))

                if f < best_fitness:
                    best_fitness = f
                    best_params = params.copy()

            # Normalize fitness for ranking
            fitness = np.array(fitness)
            # Use fitness shaping (rank-based)
            ranks = np.argsort(np.argsort(fitness))  # Double argsort gives ranks
            n = len(fitness)
            weights = np.maximum(0, np.log(n/2 + 1) - np.log(ranks + 1))
            weights /= weights.sum()

            # Compute gradient estimate (natural gradient with fitness shaping)
            grad = np.zeros(n_params)
            for i in range(len(epsilon)):
                grad += weights[i] * epsilon[i]

            # Update mean
            mean = self._clip_to_bounds(mean + self.learning_rate * self.sigma * grad, lb, ub)

            # Early stopping if converged
            if np.std(fitness) < 1e-6:
                break

        return all_solutions, best_params, best_fitness

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, nt_reduced):
        """Run OpenAI ES optimization with reduced timesteps, then polish with NM."""
        ...
        # Run OpenAI ES from each starting point
        for init_params, init_type in initializations:
            solutions, best_params, best_fitness = self._run_openai_es(
                init_params, objective_coarse, lb, ub, n_sims
            )
            for params, fitness in solutions:
                all_solutions.append((params, fitness, init_type))
