"""
W1 Baseline Sigma Better Thresholds Optimizer.

EXP009: Baseline sigma (0.15/0.20) with aggressive thresholds (0.35/0.45).
Hypothesis: Baseline sigma runs at ~57 min. Aggressive thresholds (from EXP002's
best score) may improve score while staying in budget.

Config:
- sigma0_1src: 0.15  (baseline)
- sigma0_2src: 0.20  (baseline)
- threshold_1src: 0.35  (aggressive from EXP002)
- threshold_2src: 0.45  (aggressive from EXP002)
- fallback_fevals: 18
"""
...
class BaselineSigmaBetterThreshOptimizer:
    """
    Optimizer with baseline sigma and aggressive thresholds.

    EXP009 Config:
    - sigma0_1src: 0.15 (baseline - proven to run at ~57 min)
    - sigma0_2src: 0.20 (baseline - proven to run at ~57 min)
    - threshold_1src: 0.35 (aggressive from EXP002's best score)
    - threshold_2src: 0.45 (aggressive from EXP002's best score)
    - fallback_fevals: 18

    Hypothesis: Baseline sigma keeps timing in budget, aggressive thresholds
    trigger fewer but more effective fallbacks for outliers.
    """
    ...
    def __init__(
        ...
        sigma0_1src: float = 0.15,      # Baseline sigma
        sigma0_2src: float = 0.20,      # Baseline sigma
        ...
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.35,  # Aggressive threshold from EXP002
        rmse_threshold_2src: float = 0.45,  # Aggressive threshold from EXP002
