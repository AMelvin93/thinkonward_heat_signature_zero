#!/usr/bin/env python3
"""
Run script for W1 Baseline Sigma Better Thresholds optimizer.

EXP009: Baseline sigma (0.15/0.20) with aggressive thresholds (0.35/0.45).
Testing if baseline timing (~57 min) with EXP002's best thresholds can beat baseline score.
...
from optimizer import BaselineSigmaBetterThreshOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = BaselineSigmaBetterThreshOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Baseline Sigma Better Thresholds optimizer')
    ...
    print(f"\nW1 Baseline Sigma Better Thresholds Optimizer (EXP009)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Baseline Sigma Better Thresholds (EXP009)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline: 1.1247 @ 57.2 min (sigma 0.15/0.20, threshold 0.35/0.45)")
    print(f"EXP002:   1.1362 @ 69.2 min (sigma 0.20/0.25, threshold 0.35/0.45)")
