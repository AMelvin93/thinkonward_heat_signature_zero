"""
Final Gradient Verification Optimizer

Based on perturbed_extended_polish baseline, adds a lightweight gradient verification
step at the end on only the final best candidate.

Steps:
1. Run standard CMA-ES + NM + perturbation (same as baseline)
2. After selecting final candidates, run L-BFGS-B on ONLY the best candidate
3. Accept if improved, else keep original
"""
...
class FinalGradientVerificationOptimizer:
    """
    Optimizer with final gradient verification step on best candidate.

    Same as perturbed_extended_polish, but adds a lightweight L-BFGS-B
    verification step at the end on only the best candidate.
    """
    ...
    def __init__(
        ...
        refine_maxiter: int = 8,  # Standard NM polish iterations
        refine_top_n: int = 2,
        ...
        perturb_top_n: int = 1,  # Number of candidates to perturb
        n_perturbations: int = 2,  # Number of perturbations per candidate
        perturbation_scale: float = 0.05,  # Position perturbation magnitude
        perturb_nm_iters: int = 3,  # NM iterations for perturbed points
        # Final gradient verification parameters
        enable_gradient_verify: bool = True,
        gradient_verify_maxiter: int = 3,  # L-BFGS-B iterations (very lightweight)
        gradient_verify_use_fine_grid: bool = True,  # Use fine grid for verification
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.perturb_top_n = perturb_top_n
        self.n_perturbations = n_perturbations
        self.perturbation_scale = perturbation_scale
        self.perturb_nm_iters = perturb_nm_iters
        # Gradient verification params
        self.enable_gradient_verify = enable_gradient_verify
        self.gradient_verify_maxiter = gradient_verify_maxiter
        self.gradient_verify_use_fine_grid = gradient_verify_use_fine_grid
    ...
    def _generate_perturbations(self, pos_params, n_sources, lb, ub):
        """Generate perturbations of the position parameters."""
        perturbations = []
        for _ in range(self.n_perturbations):
            # Random direction
            direction = np.random.randn(len(pos_params))
            direction = direction / (np.linalg.norm(direction) + 1e-8)

            # Apply perturbation
            perturbed = pos_params + direction * self.perturbation_scale
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run CMA-ES optimization with perturbation + local restart."""
        ...
        # Refine top N solutions with NM polish (using COARSE grid for polish)
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                result = minimize(
                    objective_coarse,  # Polish on COARSE grid (fast)
                    pos_params,
                    method='Nelder-Mead',
                    options={
        ...
        # === PERTURBATION + LOCAL RESTART (on COARSE grid) ===
        if self.enable_perturbation:
            # Sort by coarse RMSE
            refined_solutions.sort(key=lambda x: x[1])

            # Apply perturbation to top N candidates
            n_to_perturb = min(self.perturb_top_n, len(refined_solutions))
            new_candidates = []

            for idx in range(n_to_perturb):
                pos_params, rmse, init_type = refined_solutions[idx]

                # Generate perturbations
                perturbations = self._generate_perturbations(pos_params, n_sources, lb, ub)

                for perturbed_pos in perturbations:
                    # Run brief NM polish from perturbed position (on COARSE grid)
                    result = minimize(
                        objective_coarse,  # Use COARSE grid for speed
                        perturbed_pos,
                        method='Nelder-Mead',
                        options={
                    ...
                    if result.fun < rmse:
                        new_candidates.append((result.x, result.fun, 'perturbed'))

            # Add improved candidates
            refined_solutions.extend(new_candidates)

        # Convert to full candidates (with intensities)
        candidates_raw = []
        for pos_params, rmse_fine, init_type in refined_solutions:
            if n_sources == 1:
                x, y = pos_params
        ...
        return candidates_raw, n_sims[0], objective_fine

    def _gradient_verify_best(self, best_candidate, objective_fine, n_sources, lb, ub, q_range,
                               Y_observed, solver_fine, dt, nt_full, T0, sensors_xy):
        """Run gradient verification on the best candidate using L-BFGS-B."""
        sources, full_params, rmse_before, init_type = best_candidate

        # Extract position parameters
        if n_sources == 1:
            pos_params = np.array([full_params[0], full_params[1]])
        else:
            pos_params = np.array([full_params[0], full_params[1], full_params[3], full_params[4]])

        # Run L-BFGS-B verification
        result = minimize(
            objective_fine,
            pos_params,
            method='L-BFGS-B',
            ...
            options={
                'maxiter': self.gradient_verify_maxiter,
                'ftol': 1e-6,
                'gtol': 1e-6,
            }
        )
        ...
        # Check if improved
        if result.fun < rmse_before:
            # Reconstruct full candidate with new positions
            verified_pos = result.x
            if n_sources == 1:
                x, y = verified_pos
                q, _, final_rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
                full_params_new = np.array([x, y, q])
                sources_new = [(float(x), float(y), float(q))]
            else:
                x1, y1, x2, y2 = verified_pos
                (q1, q2), _, final_rmse = compute_optimal_intensity_2src(
                    x1, y1, x2, y2, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
                full_params_new = np.array([x1, y1, q1, x2, y2, q2])
                sources_new = [(float(x1), float(y1), float(q1)),
                              (float(x2), float(y2), float(q2))]

            return (sources_new, full_params_new, final_rmse, 'gradient_verified'), result.nfev, True
        else:
            return best_candidate, result.nfev, False
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Perturbation: {'ENABLED' if self.enable_perturbation else 'DISABLED'}")
            print(f"  Gradient verify: {'ENABLED' if self.enable_gradient_verify else 'DISABLED'}")
        ...
        candidates_raw, n_sims, objective_fine = self._run_single_optimization(
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims, _ = self._run_single_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # === FINAL GRADIENT VERIFICATION (on best candidate only) ===
        gradient_verify_improved = False
        if self.enable_gradient_verify and final_candidates:
            # Sort to find best
            ...
            verified_candidate, verify_evals, improved = self._gradient_verify_best(
                best_candidate, objective_fine, n_sources, lb, ub, q_range,
                Y_observed, solver_fine, dt, nt_full, T0, sensors_xy
            )
            n_sims += verify_evals

            if improved:
                final_candidates[0] = verified_candidate
                gradient_verify_improved = True
                if verbose:
                    print(f"  Gradient verification IMPROVED: {best_candidate[2]:.6f} -> {verified_candidate[2]:.6f}")
            elif verbose:
                print(f"  Gradient verification: no improvement")
