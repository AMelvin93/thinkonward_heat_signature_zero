"""
Adjusted Dissimilarity Threshold Optimizer

Same as TemporalFidelityWithPolishOptimizer but with configurable tau parameter
for dissimilarity filtering.
"""
...
def filter_dissimilar(candidates, tau, n_max=N_MAX):
    """Filter candidates by dissimilarity with configurable tau."""
    if not candidates:
        return []
...
class AdjustedTauOptimizer:
    """
    Temporal fidelity optimizer with configurable tau for dissimilarity filtering.
    """
    ...
    def __init__(
        ...
        # NEW: Configurable tau parameter
        tau: float = 0.2,
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.tau = tau  # NEW: Store tau parameter
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Dissimilarity filtering with configurable tau
        filtered = filter_dissimilar([(c[0], c[2]) for c in candidates_raw], tau=self.tau)
