#!/usr/bin/env python3
"""
Run script for COBYLA Refinement optimizer.

Tests COBYLA (trust region) instead of Nelder-Mead for refinement.
"""
...
from optimizer import COBYLARefineOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = COBYLARefineOptimizer(
        ...
        refine_maxiter=config.get('refine_maxiter', 20),
        refine_top_n=config.get('refine_top_n', 2),
        cobyla_rhobeg=config.get('cobyla_rhobeg', 0.1),
...
def main():
    parser = argparse.ArgumentParser(description='Run COBYLA Refinement optimizer')
    ...
    parser.add_argument('--refine-maxiter', type=int, default=20)
    parser.add_argument('--refine-top', type=int, default=2)
    parser.add_argument('--cobyla-rhobeg', type=float, default=0.1)
    ...
    print(f"COBYLA: maxiter={args.refine_maxiter}, rhobeg={args.cobyla_rhobeg}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            cobyla_refined = "[C]" if 'cobyla_refined' in result['init_types'] else "   "
            print(f"[{len(results):3d}/{n_samples}] Sample {result['idx']:3d}: "
                  ...
                  f"time={result['elapsed']:.1f}s [{status}]{fb_flag}{cobyla_refined}")
    ...
    # Count COBYLA refinements
    cobyla_count = sum(1 for r in results if 'cobyla_refined' in r.get('init_types', []))

    print(f"\n{'='*70}")
    print(f"RESULTS: COBYLA Refinement Optimizer")
    print(f"{'='*70}")
    ...
    print()
    print(f"COBYLA improved: {cobyla_count}/{n_samples} samples")
    print()
    print(f"Baseline (Nelder-Mead): 1.1247 @ 57.2 min")
    print(f"This COBYLA run:        {score:.4f} @ {projected_400:.1f} min")
    print(f"Delta:                  {score - 1.1247:+.4f} score, {projected_400 - 57.2:+.1f} min")
    print()
    if projected_400 > 60:
        print("STATUS: OVER BUDGET")
    elif score > 1.1247 and projected_400 < 57.2 * 0.9:  # 10% faster
        print("STATUS: SUCCESS (same/better score, 10%+ faster)")
    elif score > 1.1247:
        print("STATUS: IMPROVED (better score)")
    elif projected_400 < 57.2 * 0.9:
        print("STATUS: PARTIAL (faster but worse score)")
    else:
