...
Uses COBYLA (Constrained Optimization BY Linear Approximation) instead of
Nelder-Mead for the refinement step. COBYLA builds a local linear model
and uses a trust region approach, which may be more efficient.
...
class COBYLARefineOptimizer:
    """
    Optimizer using COBYLA for refinement instead of Nelder-Mead.

    COBYLA (Constrained Optimization BY Linear Approximation) builds
    a local linear approximation and uses trust region method.
    """
    ...
    def __init__(
        ...
        refine_top_n: int = 2,
        cobyla_rhobeg: float = 0.1,  # Initial trust region radius
        ...
        self.cobyla_rhobeg = cobyla_rhobeg
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Refine top N solutions using COBYLA instead of Nelder-Mead
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                # Build constraints for COBYLA (bounds as inequality constraints)
                constraints = []
                for j in range(len(lb)):
                    # x[j] >= lb[j] => x[j] - lb[j] >= 0
                    constraints.append({'type': 'ineq', 'fun': lambda x, j=j: x[j] - lb[j]})
                    # x[j] <= ub[j] => ub[j] - x[j] >= 0
                    constraints.append({'type': 'ineq', 'fun': lambda x, j=j: ub[j] - x[j]})
                ...
                result = minimize(
                    ...
                    options={
                        ...
                        'rhobeg': self.cobyla_rhobeg,
                    }
                )
                if result.fun < rmse_coarse:
                    refined_solutions.append((result.x, result.fun, 'cobyla_refined'))
                else:
