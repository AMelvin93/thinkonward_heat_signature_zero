"""
Adaptive Timestep Fraction Optimizer

Uses multi-fidelity approach: start with fewer timesteps (25%) for fast exploration,
increase to more timesteps (40%) as CMA-ES converges for better accuracy.

Key hypothesis: Early CMA-ES iterations need rough landscape estimate (25% timesteps fine).
Later iterations benefit from more accurate signal (40% timesteps).
"""
...
class AdaptiveTimestepOptimizer:
    """
    Optimizer with adaptive timestep fraction during CMA-ES.

    Multi-fidelity approach:
    - First half of fevals: use lower timestep fraction (25%) for fast exploration
    - Second half of fevals: use higher timestep fraction (40%) for accuracy
    - Final polish: use full timesteps
    """
    ...
    def __init__(
        ...
        low_fidelity_fraction: float = 0.25,  # Early exploration
        high_fidelity_fraction: float = 0.40,  # Later convergence
        switch_at_fraction: float = 0.50,  # Switch from low to high at 50% of fevals
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.low_fidelity_fraction = low_fidelity_fraction
        self.high_fidelity_fraction = high_fidelity_fraction
        self.switch_at_fraction = switch_at_fraction
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, nt_low, nt_high, nt_full, max_fevals):
        """Run CMA-ES with adaptive timestep fraction."""
        ...
        n_sims = [0]
        feval_count = [0]  # Track fevals to switch fidelity
        switch_at = int(max_fevals * self.switch_at_fraction)

        # Adaptive objective function - switches fidelity based on feval count
        if n_sources == 1:
            def objective_adaptive(xy_params):
                x, y = xy_params
                # Choose timestep based on feval count
                if feval_count[0] < switch_at:
                    nt = nt_low
                else:
                    nt = nt_high
                n_sims[0] += 1
            ...
            def objective_adaptive(xy_params):
                ...
                if feval_count[0] < switch_at:
                    ...
                    nt = nt_high
                n_sims[0] += 2
                feval_count[0] += 1  # Count as 1 feval (for 1 candidate)
        ...
        for init_params, init_type in initializations:
            feval_count[0] = 0  # Reset for each init
            switch_at = int(fevals_per_init * self.switch_at_fraction)
            ...
            while not es.stop():
                solutions = es.ask()
                fitness = [objective_adaptive(s) for s in solutions]
        ...
        # Refine top N solutions on high-fidelity (40% timesteps)
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                # Use high fidelity for refinement
                if n_sources == 1:
                    def obj_high(xy):
                        n_sims[0] += 1
                        q, _, rmse = compute_optimal_intensity_1src(
                            xy[0], xy[1], Y_observed, solver_coarse, dt, nt_high, T0, sensors_xy, q_range)
                        return rmse
                else:
                    def obj_high(xy):
                        n_sims[0] += 2
                        (q1, q2), _, rmse = compute_optimal_intensity_2src(
                            xy[0], xy[1], xy[2], xy[3], Y_observed, solver_coarse, dt, nt_high, T0, sensors_xy, q_range)
                        return rmse
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Compute timesteps for low and high fidelity
        nt_low = max(10, int(nt_full * self.low_fidelity_fraction))
        nt_high = max(10, int(nt_full * self.high_fidelity_fraction))
        ...
        # Run adaptive optimization with final polish
        candidates_raw, n_sims = self._run_single_optimization(
            sample, meta, q_range, solver_coarse, solver_fine,
            primary_inits, n_sources, nt_low, nt_high, nt_full, max_fevals
        )
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims = self._run_single_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, nt_low, nt_high, nt_full, max_fevals
            )
