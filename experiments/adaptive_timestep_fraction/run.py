#!/usr/bin/env python3
"""
Run script for Adaptive Timestep Fraction optimizer.

Uses multi-fidelity approach: start with 25% timesteps, switch to 40% timesteps
as CMA-ES converges.
"""
...
from optimizer import AdaptiveTimestepOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = AdaptiveTimestepOptimizer(
        ...
        low_fidelity_fraction=config['low_fidelity_fraction'],
        high_fidelity_fraction=config['high_fidelity_fraction'],
        switch_at_fraction=config.get('switch_at_fraction', 0.50),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Adaptive Timestep optimizer')
    ...
    # Adaptive timestep parameters
    parser.add_argument('--low-fidelity', type=float, default=0.25,
                        help='Timestep fraction for early exploration (e.g., 0.25=25%)')
    parser.add_argument('--high-fidelity', type=float, default=0.40,
                        help='Timestep fraction for later convergence (e.g., 0.40=40%)')
    parser.add_argument('--switch-at', type=float, default=0.50,
                        help='Switch from low to high fidelity at this fraction of fevals')
    ...
    print(f"Adaptive fidelity: {args.low_fidelity:.0%} -> {args.high_fidelity:.0%} at {args.switch_at:.0%} of fevals")
    print(f"Fevals: {args.max_fevals_1src}/{args.max_fevals_2src}")
    print(f"Final polish: {args.final_polish_maxiter} iterations (full timesteps)")
    print(f"=" * 60)
    ...
    config = {
        ...
        'low_fidelity_fraction': args.low_fidelity,
        'high_fidelity_fraction': args.high_fidelity,
        'switch_at_fraction': args.switch_at,
    }
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Adaptive Timestep ({args.low_fidelity:.0%}->{args.high_fidelity:.0%})")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (fixed 40%):  1.1688 @ 58.4 min")
    ...
    if projected_400 > 60:
        ...
        print("SUCCESS! Better than baseline and faster")
    elif score >= 1.13 and projected_400 <= 35:
        print("SUCCESS! Meets criteria: score >= 1.13, time <= 35 min")
    elif score >= 1.16:
        print("PARTIAL: Near baseline accuracy")
    else:
        print("FAILED: Below baseline")
    print(f"{'='*70}\n")
