#!/usr/bin/env python3
"""
Run script for Reduced Fevals + More Polish experiment.

Tests trading CMA-ES fevals (15/28 instead of 20/36) for extended NM polish (12 instead of 8).
"""
...
# Import baseline optimizer with polish from early_timestep_filtering
...
def main():
    parser = argparse.ArgumentParser(description='Run Reduced Fevals + More Polish Experiment')
    ...
    # KEY: Extended polish
    parser.add_argument('--final-polish-maxiter', type=int, default=12)
    ...
    print(f"\nReduced Fevals + More Polish Experiment")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"CMA-ES fevals: {args.max_fevals_1src}/{args.max_fevals_2src} (baseline: 20/36)")
    print(f"NM polish iters: {args.final_polish_maxiter} (baseline: 8)")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Reduced Fevals ({args.max_fevals_1src}/{args.max_fevals_2src}) + Extended Polish ({args.final_polish_maxiter})")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (20/36 fevals, 8 polish): 1.1688 @ 58.4 min")
    print(f"This run ({args.max_fevals_1src}/{args.max_fevals_2src} fevals, {args.final_polish_maxiter} polish): {score:.4f} @ {projected_400:.1f} min")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"feval_polish_trade_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_FEVAL_POLISH_TRADE_001")
