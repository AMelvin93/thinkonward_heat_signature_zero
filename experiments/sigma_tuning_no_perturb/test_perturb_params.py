"""
Test: Perturbation parameter variations

Current best uses: nm_iters=3, scale=0.05
Test: nm_iters=4, scale=0.06 for potential improvement
"""
...
def main():
    ...
    configs = {
        'nm4_scale05': {**base_config, 'perturb_nm_iters': 4, 'perturbation_scale': 0.05},
        'nm3_scale06': {**base_config, 'perturb_nm_iters': 3, 'perturbation_scale': 0.06},
    }

    print("=" * 70)
    print("TEST: Perturbation parameter variations")
    print("=" * 70)
    print("Current best (nm3_scale05): 1.1425 @ 51.3 min")
    ...
    print("-" * 70)
    print(f"{'nm3_scale05 (best)':<20} {'1.1425':>8} {'---':>8} {'51.3 min':>10} {'IN':>8}")
    print("-" * 70)

    for r in results:
        status = "IN" if r['in_budget'] else "OVER"
        delta = r['score'] - 1.1425
        print(f"{r['config_name']:<20} {r['score']:>8.4f} {delta:>+8.4f} {r['projected_400_min']:>8.1f} m {status:>8}")
    ...
    if in_budget:
        ...
        if best['score'] > 1.1425:
            print(f"\n*** POTENTIAL NEW BEST: {best['config_name']} ***")
            print(f"    Score: {best['score']:.4f} (+{best['score'] - 1.1425:.4f})")
            print(f"    Needs validation with 3 runs!")
        else:
            print(f"\n*** No improvement over current best ***")

    with open('test_perturb_params_output.json', 'w') as f:
