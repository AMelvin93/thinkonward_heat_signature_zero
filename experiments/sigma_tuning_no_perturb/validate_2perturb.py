"""
VALIDATION: 2perturb_018_022 (3 runs)

CRITICAL FINDING: 2 perturbations fits in budget on this system!
Single run: 1.1427 @ 52.0 min (vs baseline 1.1386 @ 49.3 min)

This needs validation to confirm improvement.
"""
...
def main():
    ...
    print("=" * 70)
    print("VALIDATION: 2perturb_018_022 (3 runs)")
    print("=" * 70)
    print("Config: sigma 0.18/0.22, 2 perturbations, nm_iters=3")
    print("Single run result: 1.1427 @ 52.0 min")
    ...
    print("\n" + "=" * 70)
    print("VALIDATION SUMMARY: 2perturb_018_022 (3 runs)")
    print("=" * 70)
    print(f"\nRun Results:")
    for r in results:
    ...
    print(f"\nComparison:")
    print(f"  vs no_perturb baseline (1.1386): {mean_score - 1.1386:+.4f}")
    print(f"  Gap to Top 10 (1.1585): {1.1585 - mean_score:.4f}")

    best_score = max(scores)
    print(f"\n  Best single run: {best_score:.4f} (gap to Top 10: {1.1585 - best_score:.4f})")

    if runs_in_budget == 3 and mean_score > 1.1386:
        print("\n*** VALIDATED: IMPROVEMENT over baseline! ***")
        print(f"*** Recommend 2perturb_018_022 for submission ***")
    elif runs_in_budget == 3:
        print("\n*** All runs in budget (no significant improvement) ***")
    else:
        print(f"\n*** WARNING: {3-runs_in_budget} run(s) over budget ***")

    with open('validate_2perturb_output.json', 'w') as f:
        json.dump({
            'config': '2perturb_018_022',
            'mean_score': float(mean_score),
            'std_score': float(std_score),
            'mean_time': float(mean_time),
            'std_time': float(std_time),
            'runs_in_budget': runs_in_budget,
            'improvement_vs_baseline': float(mean_score - 1.1386),
            'gap_to_top10': float(1.1585 - mean_score),
            'runs': results
        }, f, indent=2)

    print(f"\nResults saved to validate_2perturb_output.json")
