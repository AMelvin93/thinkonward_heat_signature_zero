"""
VALIDATION: nm5_scale05 (3 runs)

Single run: 1.1547 @ 58.1 min - only 0.0038 from Top 10!
BUT timing is tight - 58.1 min vs 60 min budget.

Need to validate both score AND timing reliability.
"""
...
def main():
    ...
    config = {
        ...
        'perturb_nm_iters': 5,  # INCREASED FROM 4
        'tabu_distance': 0.04,
        'max_tabu_attempts': 10,
    }

    print("=" * 70)
    print("VALIDATION: nm5_scale05 (3 runs)")
    print("=" * 70)
    print("Config: 2 perturbations, nm_iters=5, scale=0.05")
    print("Single run result: 1.1547 @ 58.1 min")
    print("Gap to Top 10: only 0.0038!")
    print("\nWARNING: Timing is tight (58.1 min vs 60 min budget)")
    ...
    print("\n" + "=" * 70)
    print("VALIDATION SUMMARY: nm5_scale05 (3 runs)")
    print("=" * 70)
    print(f"\nRun Results:")
    for r in results:
    ...
    # Compare
    print(f"\nComparison:")
    print(f"  vs nm4_scale05 (1.1487): {mean_score - 1.1487:+.4f}")
    print(f"  Gap to Top 10: {1.1585 - mean_score:.4f}")

    best_score = max(scores)
    print(f"\n  Best single run: {best_score:.4f} (gap: {1.1585 - best_score:.4f})")

    # Timing analysis
    print(f"\nTiming Analysis:")
    print(f"  Max time: {max(times):.1f} min")
    if max(times) > 60:
        print(f"  *** WARNING: Max time exceeds budget! ***")
    print(f"  Buffer from 60 min: {60 - max(times):.1f} min")

    if runs_in_budget == 3:
        if mean_score > 1.1487:
            print("\n*** VALIDATED: nm5_scale05 is NEW BEST CONFIG ***")
        else:
            print("\n*** nm5_scale05 validated but not better than nm4 ***")
    else:
        print(f"\n*** TIMING RISK: {3-runs_in_budget} run(s) over budget ***")
        print(f"*** nm4_scale05 remains safer choice ***")

    with open('validate_nm5_output.json', 'w') as f:
        json.dump({
            'config': 'nm5_scale05',
    ...
    print(f"\nResults saved to validate_nm5_output.json")
