"""
LRA-CMA-ES Optimizer - Learning Rate Adapted CMA-ES

Uses the cmaes library's lr_adapt=True option which maintains constant
signal-to-noise ratio in learning. May improve convergence on difficult
samples where standard CMA-ES struggles.

Based on early_timestep_filtering baseline (best current approach).
"""
...
import numpy as np
from cmaes import CMA  # Use cmaes library (not pycma) for lr_adapt support
...
class LRACMAESOptimizer:
    """
    Optimizer using LRA-CMA-ES (Learning Rate Adapted CMA-ES).

    Uses the cmaes library with lr_adapt=True which maintains constant
    signal-to-noise ratio in learning, potentially improving convergence
    on difficult samples.

    Based on early_timestep_filtering baseline with 40% temporal fidelity.
    """
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """
        Run a single optimization pass using LRA-CMA-ES with REDUCED timesteps.
        """
        ...
        bounds = np.column_stack([lb, ub])  # cmaes format: (n_dim, 2)
        ...
        for init_params, init_type in initializations:
            ...
            # Use cmaes library with lr_adapt option
            optimizer = CMA(
            ...
            while evals_this_init < fevals_per_init:
                ...
                evals_this_init += popsize

                # Tell results to optimizer - cmaes expects list of (solution, fitness) tuples
                tell_list = [(sol, fit) for sol, fit in zip(solutions, fitness)]
                optimizer.tell(tell_list)

                # Store all solutions for later selection
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            print(f"  Using {nt_reduced}/{nt_full} timesteps ({self.timestep_fraction*100:.0f}%) for LRA-CMA-ES")
            print(f"  lr_adapt={self.lr_adapt}")
