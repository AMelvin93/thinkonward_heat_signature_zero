#!/usr/bin/env python3
"""
Run script for LRA-CMA-ES (Learning Rate Adapted CMA-ES) Optimizer.

Tests whether LRA-CMA-ES improves convergence on difficult samples compared
to standard CMA-ES by maintaining constant signal-to-noise ratio in learning.
"""
...
from optimizer import LRACMAESOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = LRACMAESOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run LRA-CMA-ES optimizer')
    ...
    # LRA option
    parser.add_argument('--lr-adapt', action='store_true', default=True)
    parser.add_argument('--no-lr-adapt', action='store_false', dest='lr_adapt')
    # Shared parameters
    ...
    print(f"\nLRA-CMA-ES Optimizer")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"LRA (Learning Rate Adaptation): {args.lr_adapt}")
    ...
    print(f"Polish: {args.refine_maxiter} iters on top-{args.refine_top}")
    print(f"=" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - LRA-CMA-ES (lr_adapt={args.lr_adapt})")
    print(f"{'='*70}")
    ...
    run_name = args.mlflow_run_name or f"lra_cmaes_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        mlflow.log_param("experiment_id", "EXP_LRA_CMAES_001")
        mlflow.log_param("worker", "W1")
        mlflow.log_param("algorithm", "lra_cmaes")
        mlflow.log_param("platform", "wsl")
        mlflow.log_param("lr_adapt", args.lr_adapt)
