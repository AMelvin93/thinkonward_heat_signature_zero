...
Key insight: Do Nelder-Mead refinement on coarse grid (fast),
only use fine grid for final candidate evaluation.

This should give us refinement benefits without the timing penalty.
"""
...
class MultiFidelityCoarseRefineOptimizer:
    """
    Multi-fidelity optimizer with Nelder-Mead refinement on COARSE grid.

    Key insight: Refinement on fine grid is too expensive.
    Do refinement on coarse grid (~4x faster), only use fine grid for final eval.
    """
    ...
    def __init__(
        ...
        refine_maxiter: int = 5,  # Nelder-Mead iterations on COARSE grid
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # ===== KEY CHANGE: Refine top N solutions on COARSE grid =====
        refined_solutions = []
