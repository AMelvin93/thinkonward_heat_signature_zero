...
Test 3 perturbations + tabu_distance=0.04 as a compromise between score and budget.

Prior findings:
- 4 pert + tabu 0.04: 1.1535 @ 61.3 min (over budget but best score)
- 4 pert + tabu 0.03: 1.1437 @ 42.2 min (in budget)
- 2 pert + tabu 0.04: 1.1496 @ 55.4 min (in budget)

Hypothesis: 3 perturbations should give ~55-58 min with good score.
"""
...
def run_single_test(run_num, config, config_name, data):
    ...
    print(f"vs Baseline: {score - BASELINE:+.4f}")
...
def main():
    ...
    BASELINE = 1.1337
    BEST_4PERT_TABU04 = 1.1535

    # 3 perturbations + tabu_distance=0.04
    config = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
        'max_fevals_1src': 20,
        ...
        'n_perturbations': 3,          # Compromise: 3 instead of 4
        'perturb_nm_iters': 3,
        'perturbation_scale': 0.05,
        'tabu_distance': 0.04,          # Keep improved tabu distance
        'max_tabu_attempts': 10,
    }

    print("="*60)
    print("3 PERTURBATIONS + TABU_DISTANCE=0.04")
    print("="*60)
    print(f"Baseline: {BASELINE}")
    print(f"4pert+tabu04 (over budget): {BEST_4PERT_TABU04} @ 61.3 min")
    print(f"Expected: ~1.15 @ ~55 min")
    ...
    print("\n" + "="*60)
    print("SUMMARY: 3 Perturbations + tabu 0.04")
    print("="*60)
    print(f"\nRun Results:")
    for r in results:
    ...
    print(f"  vs 4pert+tabu04: {mean_score - BEST_4PERT_TABU04:+.4f}")
