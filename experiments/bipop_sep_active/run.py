#!/usr/bin/env python3
"""
Run script for BIPOP + sep-CMA-ES + Active CMA-ES experiment.

Key innovations:
- CMA_active=True: Negative update from worst (up to 2x speedup)
- CMA_diagonal=True: Separable CMA-ES for near-separable problems
- Multi-fidelity with coarse grid refinement

Usage:
    uv run python experiments/bipop_sep_active/run.py --workers 7 --shuffle
"""
...
from optimizer import BIPOPSepActiveOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = BIPOPSepActiveOptimizer(
        ...
        use_active=config.get('use_active', True),
        use_diagonal=config.get('use_diagonal', True),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run BIPOP + sep-CMA-ES + Active')
    ...
    parser.add_argument('--use-active', action='store_true', default=True)
    parser.add_argument('--no-active', dest='use_active', action='store_false')
    parser.add_argument('--use-diagonal', action='store_true', default=True)
    parser.add_argument('--no-diagonal', dest='use_diagonal', action='store_false')
    ...
    print(f"\nBIPOP + sep-CMA-ES + Active CMA-ES")
    print(f"=" * 60)
    ...
    print(f"Active CMA-ES: {args.use_active}, Diagonal: {args.use_diagonal}")
    print(f"=" * 60)
