"""
BIPOP-CMA-ES with Separable (Diagonal) Mode and Active CMA-ES.

Key Innovations:
1. BIPOP restart strategy - alternates large (global) and small (local) restarts
2. sep-CMA-ES - diagonal covariance for faster convergence on near-separable problems
3. Active CMA-ES - negative update from worst individuals (2x speedup potential)

Based on research:
- Hansen 2005 BIPOP paper
- Ros & Hansen 2008 sep-CMA-ES paper
- CMA-ES official documentation on active updates
"""
...
class BIPOPSepActiveOptimizer:
    """
    Multi-fidelity optimizer with BIPOP-CMA-ES, sep-CMA-ES, and Active updates.

    Key features:
    1. CMA_active=True: Uses negative update from worst individuals (up to 2x speedup)
    2. CMA_diagonal=True: Separable CMA-ES for faster convergence on near-separable
    3. Multiple restarts with different sigma0 (simulating BIPOP alternation)
    4. Coarse grid for exploration, fine grid for final evaluation
    """
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # CMA-ES on coarse grid with BIPOP-style restarts
        ...
        for init_params, init_type in initializations:
            # Configure CMA-ES with active update and diagonal mode
            opts = cma.CMAOptions()
            ...
            # KEY: Enable Active CMA-ES (negative update from worst)
            if self.use_active:
                opts['CMA_active'] = True

            # KEY: Enable Diagonal/Separable CMA-ES
