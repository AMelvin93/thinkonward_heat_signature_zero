#!/usr/bin/env python3
"""
Run script for Dual-Track CMA-ES optimizer.
"""
...
from optimizer import DualTrackCMAESOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = DualTrackCMAESOptimizer(
        max_fevals_1src=config['max_fevals_1src'],
        max_fevals_2src=config['max_fevals_2src'],
        sigma0_1src=config.get('sigma0_1src', 0.15),
        sigma0_2src=config.get('sigma0_2src', 0.20),
        sigma_explore_1src=config.get('sigma_explore_1src', 0.25),
        sigma_explore_2src=config.get('sigma_explore_2src', 0.30),
        fevals_split=config.get('fevals_split', 0.5),
...
def main():
    parser = argparse.ArgumentParser(description='Run Dual-Track CMA-ES optimizer')
    ...
    parser.add_argument('--sigma-explore-1src', type=float, default=0.25)
    parser.add_argument('--sigma-explore-2src', type=float, default=0.30)
    parser.add_argument('--fevals-split', type=float, default=0.5)
    ...
    print(f"Fevals: {args.max_fevals_1src}/{args.max_fevals_2src}, Split: {args.fevals_split:.0%}/{1-args.fevals_split:.0%}")
    print(f"Sigma exploit: 1-src={args.sigma0_1src}, 2-src={args.sigma0_2src}")
    print(f"Sigma explore: 1-src={args.sigma_explore_1src}, 2-src={args.sigma_explore_2src}")
    print(f"=" * 60)
    ...
    config = {
        ...
        'sigma_explore_1src': args.sigma_explore_1src,
        'sigma_explore_2src': args.sigma_explore_2src,
        'fevals_split': args.fevals_split,
