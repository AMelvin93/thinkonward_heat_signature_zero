"""
Dual-Track CMA-ES Optimizer.

Key idea: Run two CMA-ES tracks in parallel with different sigma values:
- Track 1: Normal sigma (0.20) for local exploitation
- Track 2: Larger sigma (0.30) for global exploration

This avoids sequential fallback overhead while getting benefits of both strategies.
"""
...
class DualTrackCMAESOptimizer:
    """
    Dual-track optimizer running two CMA-ES with different sigmas in parallel.

    Track 1: Normal sigma for exploitation
    Track 2: Larger sigma for exploration

    Combines results from both tracks for better coverage.
    """
    ...
    def __init__(
        ...
        sigma_explore_1src: float = 0.25,
        sigma_explore_2src: float = 0.30,
        fevals_split: float = 0.5,  # Fraction of fevals for exploit track
        ...
        self.sigma_explore_1src = sigma_explore_1src
        self.sigma_explore_2src = sigma_explore_2src
        self.fevals_split = fevals_split
    ...
    def _run_cmaes_track(self, init_params, sigma, max_fevals, objective, lb, ub, init_type):
        """Run a single CMA-ES track."""
        all_solutions = []
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        sigma_exploit = self.sigma0_1src if n_sources == 1 else self.sigma0_2src
        sigma_explore = self.sigma_explore_1src if n_sources == 1 else self.sigma_explore_2src
        ...
        # Split fevals between tracks
        fevals_exploit = int(max_fevals * self.fevals_split)
        fevals_explore = max_fevals - fevals_exploit
        fevals_per_init_exploit = max(5, fevals_exploit // len(inits))
        fevals_per_init_explore = max(5, fevals_explore // len(inits))

        all_solutions = []

        # Track 1: Exploitation with normal sigma
        for init_params, init_type in inits:
            track_solutions = self._run_cmaes_track(
                init_params, sigma_exploit, fevals_per_init_exploit,
                objective_coarse, lb, ub, f'{init_type}_exploit'
            )
            all_solutions.extend(track_solutions)

        # Track 2: Exploration with larger sigma
        for init_params, init_type in inits:
            track_solutions = self._run_cmaes_track(
                init_params, sigma_explore, fevals_per_init_explore,
                objective_coarse, lb, ub, f'{init_type}_explore'
            )
            all_solutions.extend(track_solutions)
