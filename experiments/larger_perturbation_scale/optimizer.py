"""
Perturbed Local Restart Optimizer

Basin hopping concept: After NM polish, perturb position slightly and re-run
local optimizer to escape shallow local minima.

Steps:
1. Run standard CMA-ES to find candidates
2. NM polish on top candidates
3. For each polished candidate, generate perturbations
4. Run brief NM from perturbed positions
5. Keep best of (original, perturbed) for each candidate
"""
...
class PerturbedLocalRestartOptimizer:
    """
    Optimizer with perturbation + local restart (basin hopping concept).
    """
    ...
    def __init__(
        ...
        perturb_top_n: int = 3,  # Number of candidates to perturb
        n_perturbations: int = 3,  # Number of perturbations per candidate
