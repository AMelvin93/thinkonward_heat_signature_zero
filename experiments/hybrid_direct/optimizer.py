"""
Hybrid Direct Solution Optimizer for Heat Source Identification.

BREAKTHROUGH APPROACH (A1): Combine multiple fast methods to get near-optimal
solutions directly, then do minimal refinement.

Key Innovation:
- ICA decomposition gives direct 2-source position estimates (not just init)
- If ICA estimate is good (RMSE < threshold), skip CMA-ES entirely
- If ICA estimate is poor, fall back to triangulation + CMA-ES
- Result: ICA-quality accuracy with SmartInit-speed runtime

Target: Score 1.15+ (close gap to top teams)

Why This Could Work:
- ICA achieves 1.0422 but uses too many fevals
- If ICA estimate is accurate enough, we don't need many fevals
- Adaptive decision: skip CMA-ES for "easy" samples, use it for "hard" ones
"""
...
# =============================================================================
# ICA DECOMPOSITION FOR 2-SOURCE
# =============================================================================
...
def ica_decompose_2source(
    ...
    Returns position array [x1, y1, x2, y2] or None if failed.
    """
    if Y_obs.shape[1] < 2:
        return None
    ...
    try:
        ...
        S = ica.fit_transform(Y_obs)  # (time, 2)
        A = ica.mixing_  # (n_sensors, 2)
...
def pca_decompose_2source(
    ...
    """
    Alternative: PCA decomposition for 2-source position estimation.
    Faster than ICA but less accurate for non-Gaussian sources.
    """
    if Y_obs.shape[1] < 2:
        return None
    ...
    try:
        ...
        A = eigenvectors[:, idx]  # (n_sensors, 2)
...
class HybridDirectOptimizer:
    """
    Hybrid Direct Solution for heat source identification.

    BREAKTHROUGH APPROACH:
    1. Generate multiple direct position estimates (ICA, PCA, triangulation, smart)
    2. Evaluate each with analytical intensity (1-2 sims per estimate)
    3. Pick the best one
    4. Only do CMA-ES refinement if best RMSE > threshold (adaptive)

    For "easy" samples where direct estimates are accurate:
    - Skip CMA-ES entirely → huge time savings
    - Direct estimate is the final answer

    For "hard" samples where direct estimates are poor:
    - Use best direct estimate as CMA-ES init
    - Run minimal CMA-ES to refine
    """
    ...
    def __init__(
        ...
        max_fevals_2src: int = 18,  # Reduced since we have better inits
        sigma0_1src: float = 0.15,
        sigma0_2src: float = 0.15,  # Smaller sigma since inits are better
        use_triangulation: bool = True,
        n_candidates: int = N_MAX,
        candidate_pool_size: int = 10,
        k_similar: int = 1,
        use_enhanced_features: bool = True,
        # Hybrid direct parameters
        skip_cmaes_threshold_1src: float = 0.12,  # Skip CMA-ES if RMSE < this
        skip_cmaes_threshold_2src: float = 0.20,  # Skip CMA-ES if RMSE < this
        use_ica: bool = True,
        use_pca: bool = True,
    ):
        self.Lx = Lx
        ...
        self.skip_cmaes_threshold_1src = skip_cmaes_threshold_1src
        self.skip_cmaes_threshold_2src = skip_cmaes_threshold_2src
    ...
    def _evaluate_init(self, init_params: np.ndarray, n_sources: int,
                       ...
                       q_range: Tuple[float, float]) -> Tuple[float, np.ndarray]:
        """Evaluate an init by computing RMSE and full params."""
        if n_sources == 1:
            x, y = init_params
    ...
    def estimate_sources(
        ...
        verbose: bool = False,
    ) -> Tuple[List[List[Tuple]], float, List[CandidateResult], np.ndarray, np.ndarray, int, bool]:
        """
        Estimate sources using Hybrid Direct approach.

        Returns:
            - candidate_sources: list of source candidates
            - best_rmse: best RMSE achieved
            - results: list of CandidateResult
            - features: sample features
            - best_positions: best position params for history
            - n_transferred: number of transferred inits used
            - skipped_cmaes: whether CMA-ES was skipped
        """
        ...
        # === PHASE 1: GENERATE DIRECT ESTIMATES ===
        direct_estimates = []

        # 1. Triangulation
        tri_init = self._triangulation_init_positions(sample, meta, n_sources, q_range)
        if tri_init is not None:
            direct_estimates.append(('triangulation', tri_init))

        # 2. Smart (hottest sensor)
        smart_init = self._smart_init_positions(sample, n_sources)
        direct_estimates.append(('smart', smart_init))

        # 3. Transfer learning
        lb, ub = self._get_position_bounds(n_sources)
        for i, sol in enumerate(similar_solutions):
            sol_clipped = np.clip(sol, lb, ub)
            direct_estimates.append((f'transfer_{i}', sol_clipped))
        ...
        if n_sources == 2 and self.use_ica:
            ...
            if ica_init is not None:
                direct_estimates.append(('ica', ica_init))
        ...
        if n_sources == 2 and self.use_pca:
            ...
            if pca_init is not None:
                direct_estimates.append(('pca', pca_init))

        # === PHASE 2: EVALUATE ALL DIRECT ESTIMATES ===
        evaluated = []
        n_sims = [0]

        for init_type, init_params in direct_estimates:
            rmse, full_params = self._evaluate_init(
                init_params, n_sources, Y_observed, solver, dt, nt, T0, sensors_xy, q_range
            )
            n_sims[0] += (1 if n_sources == 1 else 2)
            evaluated.append((init_params, full_params, rmse, init_type))

        # Sort by RMSE
        evaluated.sort(key=lambda x: x[2])
        best_init_params, best_full_params, best_init_rmse, best_init_type = evaluated[0]

        if verbose:
            print(f"  Direct estimates ({len(evaluated)} methods):")
            for _, full_p, rmse, itype in evaluated[:5]:
                marker = " <-- BEST" if itype == best_init_type else ""
                print(f"    {itype}: RMSE={rmse:.4f}{marker}")

        # === PHASE 3: DECIDE WHETHER TO RUN CMA-ES ===
        skip_threshold = self.skip_cmaes_threshold_1src if n_sources == 1 else self.skip_cmaes_threshold_2src
        skipped_cmaes = (best_init_rmse < skip_threshold)

        all_solutions = [(best_init_params.copy(), best_init_rmse, best_init_type)]

        if not skipped_cmaes:
            # Run CMA-ES refinement from best init
        ...
        # === PHASE 4: BUILD CANDIDATES FROM TOP SOLUTIONS ===
        ...
        results = [
            CandidateResult(
                ...
                used_cmaes=not skipped_cmaes,
            )
            for c in final_candidates
        ]

        return candidate_sources, best_rmse, results, features, best_positions, n_transferred, skipped_cmaes
