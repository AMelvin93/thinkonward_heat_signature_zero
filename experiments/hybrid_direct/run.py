#!/usr/bin/env python
"""
Run Hybrid Direct Solution Optimizer experiment.

BREAKTHROUGH APPROACH (A1): Combine multiple fast methods to get near-optimal
solutions directly, then do minimal refinement.

Key Innovation:
- Generate multiple direct position estimates (ICA, PCA, triangulation, smart)
- Evaluate each with analytical intensity (1-2 sims per estimate)
- If best estimate RMSE < threshold, skip CMA-ES entirely
- Only run CMA-ES for "hard" samples that need refinement

Expected Benefits:
    - Skip CMA-ES on ~40% of samples → major time savings
    - ICA/PCA give better 2-source position estimates
    - More time budget for hard samples

Target: Score 1.15+ to be competitive with top 5 teams

Usage:
    # Quick validation (10 samples)
    uv run python experiments/hybrid_direct/run.py --workers 7 --shuffle --max-samples 10

    # Full run
    uv run python experiments/hybrid_direct/run.py --workers 7 --shuffle
"""
...
from experiments.hybrid_direct.optimizer import (
    HybridDirectOptimizer,
...
def process_sample(sample, meta, config, history_1src, history_2src):
    """Process a single sample with hybrid direct optimizer."""
    optimizer = HybridDirectOptimizer(
        ...
        skip_cmaes_threshold_1src=config['skip_cmaes_threshold_1src'],
        skip_cmaes_threshold_2src=config['skip_cmaes_threshold_2src'],
    ...
    start = time.time()
    candidates, best_rmse, results, features, best_positions, n_transferred, skipped_cmaes = optimizer.estimate_sources(
...
def main():
    parser = argparse.ArgumentParser(description='Run Hybrid Direct Solution Optimizer')
    ...
    # Fevals - reduced since we have better inits
    ...
    parser.add_argument('--max-fevals-2src', type=int, default=18,
                        help='Max CMA-ES evaluations for 2-source (reduced)')
    parser.add_argument('--sigma0-1src', type=float, default=0.15,
                        help='Initial step size for 1-source')
    parser.add_argument('--sigma0-2src', type=float, default=0.15,
                        help='Initial step size for 2-source (smaller due to better init)')
    ...
    # Hybrid direct specific parameters
    parser.add_argument('--skip-threshold-1src', type=float, default=0.12,
                        help='Skip CMA-ES if 1-source RMSE below this')
    parser.add_argument('--skip-threshold-2src', type=float, default=0.20,
                        help='Skip CMA-ES if 2-source RMSE below this')
    ...
    config = {
        ...
        'skip_cmaes_threshold_1src': args.skip_threshold_1src,
        'skip_cmaes_threshold_2src': args.skip_threshold_2src,
    ...
    # Print header
    print("=" * 70)
    print("HYBRID DIRECT SOLUTION OPTIMIZER (A1 Breakthrough)")
    print("=" * 70)
    ...
    print()
    print("BREAKTHROUGH APPROACH:")
    print("  - Generate direct estimates: ICA, PCA, triangulation, smart")
    print("  - Evaluate each with analytical intensity (1-2 sims)")
    print("  - Skip CMA-ES if best direct RMSE < threshold")
    print("  - Only refine 'hard' samples with CMA-ES")
    print()
    print(f"Config:")
    print(f"  max_fevals: 1-src={config['max_fevals_1src']}, 2-src={config['max_fevals_2src']}")
    print(f"  skip_threshold: 1-src={config['skip_cmaes_threshold_1src']}, 2-src={config['skip_cmaes_threshold_2src']}")
    ...
    for batch_idx in range(n_batches):
        ...
        batch_skipped = sum(1 for r in batch_results if r['skipped_cmaes'])
        batch_times = [r['time'] for r in batch_results]
        print(f"  -> Batch RMSE: {np.mean(batch_rmses):.4f}, "
              f"Skipped CMA-ES: {batch_skipped}/{len(batch_results)}, "
    ...
    all_skipped = [r['skipped_cmaes'] for r in all_results]
    ...
    # Track skipped by n_sources
    skipped_1src = sum(1 for r in all_results if r['n_sources'] == 1 and r['skipped_cmaes'])
    skipped_2src = sum(1 for r in all_results if r['n_sources'] == 2 and r['skipped_cmaes'])
    total_1src = sum(1 for r in all_results if r['n_sources'] == 1)
    total_2src = sum(1 for r in all_results if r['n_sources'] == 2)
    ...
    skip_rate = sum(all_skipped) / len(all_skipped) * 100
    ...
    if is_g4dn_simulation and args.max_samples is None:
        ...
        run_name = f"hybrid_direct_{config['max_fevals_1src']}_{config['max_fevals_2src']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_metric("cmaes_skip_rate_pct", skip_rate)
            mlflow.log_metric("transfer_benefit_pct", transfer_benefit_pct)
            mlflow.log_metric("total_time_sec", total_time)

            mlflow.log_param("optimizer", "HybridDirectOptimizer")
            mlflow.log_param("max_fevals_1src", config['max_fevals_1src'])
            mlflow.log_param("max_fevals_2src", config['max_fevals_2src'])
            mlflow.log_param("skip_threshold_1src", config['skip_cmaes_threshold_1src'])
            mlflow.log_param("skip_threshold_2src", config['skip_cmaes_threshold_2src'])
    ...
    print("RESULTS")
    print("=" * 70)
    print(f"Config:           {config['max_fevals_1src']}/{config['max_fevals_2src']} fevals (hybrid direct)")
    ...
    print(f"CMA-ES Skip Rate: {skip_rate:.1f}%")
    ...
    print()
    print("CMA-ES skip breakdown:")
    print(f"  1-source: {skipped_1src}/{total_1src} skipped ({skipped_1src/max(total_1src,1)*100:.1f}%)")
    print(f"  2-source: {skipped_2src}/{total_2src} skipped ({skipped_2src/max(total_2src,1)*100:.1f}%)")
    print()
    ...
    print(f"Hybrid Direct ({config['max_fevals_1src']}/{config['max_fevals_2src']}):     Score {final_score:.4f} @ {projected_400:.1f} min")
    print()
