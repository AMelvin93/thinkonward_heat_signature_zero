...
Test if reducing fevals allows 4 perturbations to fit within budget.

Prior findings:
- 4 pert + tabu 0.04 @ fevals 20/44: 1.1555 @ 61.9 min (OVER BUDGET by 1.9 min)
- 3 pert + tabu 0.04 @ fevals 20/44: 1.1478 @ 58.4 min (IN BUDGET)

Hypothesis: Reducing fevals by 10% might save 2-3 min, allowing 4 pert to fit budget.
"""
...
def run_single_test(run_num, config, config_name, data):
    ...
    print(f"vs 4-pert @ full fevals (1.1555): {score - BEST_4PERT:+.4f}")
    print(f"vs 3-pert @ full fevals (1.1478): {score - BEST_3PERT:+.4f}")
...
def main():
    ...
    # Config 1: 4 pert with reduced fevals (18/40 instead of 20/44)
    config_reduced = {
        'sigma0_1src': 0.18,
        'sigma0_2src': 0.22,
    ...
    print("="*60)
    print("4 PERTURBATIONS WITH REDUCED FEVALS")
    print("="*60)
    print(f"True Baseline: {BASELINE}")
    print(f"4-pert @ fevals 20/44: {BEST_4PERT} @ 61.9 min (OVER BUDGET)")
    print(f"3-pert @ fevals 20/44: {BEST_3PERT} @ 58.4 min (IN BUDGET)")
    print(f"Goal: 4-pert with reduced fevals to fit budget")

    results = []

    # Run 3 validation tests
    for run_num in range(1, 4):
        result = run_single_test(run_num, config_reduced, "4pert_fevals_18_40", data)
        results.append(result)
    ...
    print("\n" + "="*60)
    print("SUMMARY: 4 Perturbations with Reduced Fevals (18/40)")
    print("="*60)
    print(f"\nRun Results:")
    for r in results:
    ...
    print(f"  vs 4-pert full fevals: {mean_score - BEST_4PERT:+.4f}")
    print(f"  vs 3-pert full fevals: {mean_score - BEST_3PERT:+.4f}")

    in_budget = mean_time <= 60.0
    print(f"\nBudget Status: {'IN BUDGET' if in_budget else 'OVER BUDGET'}")

    if in_budget and mean_score > BEST_3PERT:
        print("\nCONCLUSION: SUCCESS - Better than 3-pert and within budget!")
    elif in_budget:
        print("\nCONCLUSION: IN BUDGET but not better than 3-pert")
    else:
        print("\nCONCLUSION: STILL OVER BUDGET - fevals reduction not enough")
    ...
    with open('run_output.json', 'w') as f:
        json.dump({
            ...
            'vs_3pert': mean_score - BEST_3PERT,
            'in_budget': in_budget,
            'runs': results
        }, f, indent=2)
