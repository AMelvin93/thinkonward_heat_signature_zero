"""
Run script for simple_position_average_best2 experiment.
...
from optimizer import SimplePositionAverageOptimizer
...
def process_single_sample(args):
    ...
    optimizer = SimplePositionAverageOptimizer(
        enable_position_averaging=config['enable_averaging'],
        n_to_average=config['n_to_average'],
    ...
    try:
        ...
        # Check if averaged candidate was selected
        avg_selected = any(r.init_type == 'averaged_top2' and abs(r.rmse - best_rmse) < 1e-10
                          for r in results)
...
def main():
    ...
    parser.add_argument('--no-average', action='store_true', help='Disable averaging (baseline comparison)')
    parser.add_argument('--n-to-average', type=int, default=2, help='Number of top candidates to average')
    ...
    enable_averaging = not args.no_average

    n_1src = sum(1 for s in samples_to_process if s['n_sources'] == 1)
    n_2src = n_samples - n_1src

    print(f"\n{'='*60}")
    print(f"SIMPLE POSITION AVERAGE (TOP-{args.n_to_average}) EXPERIMENT")
    print(f"{'='*60}")
    print(f"Samples: {n_samples} ({n_1src} 1-source, {n_2src} 2-source)")
    print(f"Workers: {args.workers}")
    print(f"Position averaging: {'ENABLED' if enable_averaging else 'DISABLED'}")
    print(f"N to average: {args.n_to_average}")
    print(f"{'='*60}")

    config = {
        'enable_averaging': enable_averaging,
        'n_to_average': args.n_to_average,
        'max_fevals_1src': 20,
        'max_fevals_2src': 36,
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            avg_flag = " [AVG]" if result['avg_selected'] else ""
            print(f"[{len(results):3d}/{n_samples}] Sample {result['idx']:3d}: "
                  f"{result['n_sources']}-src RMSE={result['best_rmse']:.4f} "
                  f"cands={result['n_candidates']} time={result['elapsed']:.1f}s [{status}]{avg_flag}")
    ...
    avg_chosen = sum(1 for r in results if r['avg_selected'])

    projected_400 = (total_time / n_samples) * 400 / 60

    print(f"\n{'='*70}")
    print(f"RESULTS - Simple Position Average (Top-{args.n_to_average})")
    print(f"{'='*70}")
    ...
    print()
    print(f"Averaged candidate selected: {avg_chosen}/{n_samples} ({100*avg_chosen/n_samples:.1f}%)")
    print()
    print(f"Baseline:  1.1688 @ 58.4 min (early_timestep_filtering)")
    ...
    if projected_400 > 60:
        ...
        print("STATUS: SUCCESS - Beats baseline!")
    else:
    ...
    results_summary = {
        ...
        'avg_chosen_count': avg_chosen,
        'avg_chosen_pct': 100 * avg_chosen / n_samples,
    }
    ...
    if not args.no_mlflow:
        ...
        run_name = f"simple_pos_avg_top{args.n_to_average}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param('experiment_name', 'simple_position_average_best2')
            mlflow.log_param('experiment_id', 'EXP_SIMPLE_POSITION_AVERAGE_001')
            mlflow.log_param('worker', 'W1')
            mlflow.log_param('enable_averaging', enable_averaging)
            mlflow.log_param('n_to_average', args.n_to_average)
            ...
            mlflow.log_metric('avg_chosen_count', results_summary['avg_chosen_count'])
            mlflow.log_metric('avg_chosen_pct', results_summary['avg_chosen_pct'])
            ...
            if state_path.exists():
                ...
                state['tuning_runs'].append({
                    ...
                    'results': {
                        ...
                        'avg_chosen_count': results_summary['avg_chosen_count'],
                        'avg_chosen_pct': results_summary['avg_chosen_pct'],
