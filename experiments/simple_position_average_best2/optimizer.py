"""
Simple Position Average (Top-2) Optimizer

Key change from baseline:
- After CMA-ES, take top-2 candidates by RMSE
- Average their positions: (x1+x2)/2, (y1+y2)/2
- Compute optimal intensity for averaged position
- Add to candidate pool before dissimilarity filtering

Hypothesis: Averaging top candidates may find a better local minimum.
"""
...
class SimplePositionAverageOptimizer:
    """
    Optimizer with simple top-2 position averaging.

    Key insight: After CMA-ES finds multiple candidates, averaging the positions
    of top-2 candidates and recomputing optimal intensity may yield a better solution.
    """
    ...
    def __init__(
        ...
        # NEW: Position averaging parameters
        enable_position_averaging: bool = True,
        n_to_average: int = 2,  # Average top-N candidates
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.enable_position_averaging = enable_position_averaging
        self.n_to_average = n_to_average
    ...
    def _average_top_n_positions_1src(self, candidates_raw, n_to_average, Y_observed,
                                       solver_fine, dt, nt_full, T0, sensors_xy, q_range):
        """Average positions of top-N 1-source candidates."""
        if len(candidates_raw) < n_to_average:
            return None

        # Sort by RMSE
        sorted_candidates = sorted(candidates_raw, key=lambda c: c[2])
        top_n = sorted_candidates[:n_to_average]

        # Average positions
        avg_x = np.mean([c[0][0][0] for c in top_n])  # c[0] = sources list, [0] = first source, [0] = x
        avg_y = np.mean([c[0][0][1] for c in top_n])

        # Clip to valid bounds
        margin = 0.05
        avg_x = np.clip(avg_x, margin * self.Lx, (1 - margin) * self.Lx)
        avg_y = np.clip(avg_y, margin * self.Ly, (1 - margin) * self.Ly)
        ...
        return (sources, full_params, rmse, 'averaged_top2')

    def _average_top_n_positions_2src(self, candidates_raw, n_to_average, Y_observed,
                                       solver_fine, dt, nt_full, T0, sensors_xy, q_range):
        """Average positions of top-N 2-source candidates."""
        if len(candidates_raw) < n_to_average:
            return None
        ...
        # For 2-source, we need to carefully handle the source matching
        # Each candidate has sources: [(x1, y1, q1), (x2, y2, q2)]
        # We need to align sources before averaging (by position proximity)

        # Use first candidate as reference
        ref_sources = top_n[0][0]

        # Collect aligned positions
        all_positions = []  # Will be list of [(x1, y1), (x2, y2)] tuples

        for cand in top_n:
            sources = cand[0]
            s1, s2 = sources[0], sources[1]

            # Check which assignment minimizes distance to reference
            dist_same = (np.sqrt((s1[0] - ref_sources[0][0])**2 + (s1[1] - ref_sources[0][1])**2) +
                        np.sqrt((s2[0] - ref_sources[1][0])**2 + (s2[1] - ref_sources[1][1])**2))
            dist_swap = (np.sqrt((s1[0] - ref_sources[1][0])**2 + (s1[1] - ref_sources[1][1])**2) +
                        np.sqrt((s2[0] - ref_sources[0][0])**2 + (s2[1] - ref_sources[0][1])**2))

            if dist_same <= dist_swap:
                all_positions.append([(s1[0], s1[1]), (s2[0], s2[1])])
            else:
                all_positions.append([(s2[0], s2[1]), (s1[0], s1[1])])

        # Average positions
        avg_x1 = np.mean([p[0][0] for p in all_positions])
        avg_y1 = np.mean([p[0][1] for p in all_positions])
        avg_x2 = np.mean([p[1][0] for p in all_positions])
        avg_y2 = np.mean([p[1][1] for p in all_positions])

        # Clip to valid bounds
        margin = 0.05
        avg_x1 = np.clip(avg_x1, margin * self.Lx, (1 - margin) * self.Lx)
        avg_y1 = np.clip(avg_y1, margin * self.Ly, (1 - margin) * self.Ly)
        avg_x2 = np.clip(avg_x2, margin * self.Lx, (1 - margin) * self.Lx)
        avg_y2 = np.clip(avg_y2, margin * self.Ly, (1 - margin) * self.Ly)

        # Compute optimal intensities
        (q1, q2), _, rmse = compute_optimal_intensity_2src(
            avg_x1, avg_y1, avg_x2, avg_y2, Y_observed,
            solver_fine, dt, nt_full, T0, sensors_xy, q_range
        )
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run primary optimization with reduced timesteps
        ...
        # ========== NEW: TOP-2 POSITION AVERAGING ==========
        if self.enable_position_averaging and len(candidates_raw) >= self.n_to_average:
            if n_sources == 1:
                avg_candidate = self._average_top_n_positions_1src(
                    candidates_raw, self.n_to_average, Y_observed,
                    solver_fine, dt, nt_full, T0, sensors_xy, q_range
                )
            else:
                avg_candidate = self._average_top_n_positions_2src(
                    candidates_raw, self.n_to_average, Y_observed,
                    solver_fine, dt, nt_full, T0, sensors_xy, q_range
                )

            if avg_candidate is not None:
                n_sims += (1 if n_sources == 1 else 2)
                candidates_raw.append(avg_candidate)
                if verbose:
                    print(f"  Added averaged candidate with RMSE: {avg_candidate[2]:.4f}")
        # ===================================================
