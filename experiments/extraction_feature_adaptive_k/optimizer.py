"""
Enhanced Transfer Learning Optimizer with Improved Feature Extraction and Adaptive k.

Key Improvements over base Transfer Learning:
1. Enhanced Feature Extraction:
   - Basic: max_temp, mean_temp, std_temp, kappa, n_sensors (existing)
   - Spatial: centroid_x, centroid_y, spatial_spread (NEW)
   - Temporal: onset_mean, onset_std (NEW)
   - Correlation: avg_sensor_correlation (NEW)

2. Adaptive k Selection:
   - Hard samples (2-source, low SNR) get k=2
   - Easy samples (1-source, high SNR) get k=1
   - This focuses transfer learning where it helps most

Expected Impact:
- Better similarity matching -> more relevant transfers
- More transfer benefit for hard samples
- Potential score improvement: +0.02-0.05
"""
...
def extract_enhanced_features(sample: Dict, meta: Dict = None) -> np.ndarray:
    """
    ENHANCED feature extraction for better similarity matching.

    Features (11 total):
    - Basic (5): max_temp, mean_temp, std_temp, kappa, n_sensors
    - Spatial (3): centroid_x, centroid_y, spatial_spread
    - Temporal (2): onset_mean, onset_std
    - Correlation (1): avg_sensor_correlation

    All features normalized to ~[0, 1] range for balanced distance calculations.

    Fast: O(n) in data size, ~0.5ms per sample.
    """
    Y = sample['Y_noisy']
    ...
    basic = [
        ...
        kappa * 10,                  # Diffusivity (scaled up)
        len(sensors) / 10.0,        # Sensor count (normalized)
    ]

    # === SPATIAL FEATURES (NEW) ===
    # Which region is hottest - encodes approximate source location
    max_temps_per_sensor = Y.max(axis=0)
    weights = max_temps_per_sensor / (max_temps_per_sensor.sum() + 1e-8)

    # Weighted centroid (where temperature is concentrated)
    centroid_x = np.average(sensors[:, 0], weights=weights) / 2.0  # Normalized by Lx
    centroid_y = np.average(sensors[:, 1], weights=weights)        # Normalized by Ly

    # Spatial spread (how distributed the heat is)
    ...
    # Onset timing patterns - encodes source distance from sensors
    onset_times = []
    ...
    onset_mean = np.mean(onset_times) / 100.0  # Normalized
    onset_std = np.std(onset_times) / 50.0      # Normalized

    temporal = [onset_mean, onset_std]

    # === CORRELATION FEATURE (NEW) ===
    # How similar are sensor responses - encodes problem structure
    if Y.shape[1] > 1:
...
def extract_basic_features(sample: Dict) -> np.ndarray:
    """
    Original basic feature extraction (for comparison/fallback).
    """
    Y = sample['Y_noisy']
    ...
    return np.array([
        ...
        np.mean(Y) / 5.0,
        np.std(Y) / 2.0,
        sample['sample_metadata']['kappa'] * 10,
        len(sample['sensors_xy']) / 10.0,
    ])


def compute_sample_difficulty(sample: Dict) -> Tuple[float, dict]:
    """
    Compute difficulty score for a sample.

    Returns:
        difficulty: Float score (higher = harder)
        metrics: Dict with individual difficulty metrics
    """
    Y = sample['Y_noisy']
    n_sources = sample['n_sources']

    # Signal-to-noise ratio (higher = easier)
    snr = Y.max() / (Y.std() + 1e-8)

    # Temperature range (higher = easier to identify)
    temp_range = Y.max() - Y.min()

    # Onset time variance (higher variance = harder, multiple sources confuse timing)
    onset_times = []
    for i in range(Y.shape[1]):
        signal = Y[:, i]
        threshold = 0.1 * (signal.max() + 1e-8)
        onset_idx = np.argmax(signal > threshold)
        onset_times.append(onset_idx)
    onset_std = np.std(onset_times)

    # Compute overall difficulty
    difficulty = 0.0

    # 2-source is inherently harder (permutation symmetry)
    if n_sources == 2:
        difficulty += 0.5
    ...
    # High onset variance suggests overlapping/confusing signals
    if onset_std > 20:
        difficulty += 0.2

    # Low temperature range is harder to fit
    if temp_range < 1.0:
        difficulty += 0.1
...
def get_adaptive_k(sample: Dict, history_size: int, k_easy: int = 1, k_hard: int = 2) -> int:
    """
    Adaptively select k based on sample difficulty.

    Hard samples benefit more from transfer learning because:
    - They're more likely to get stuck in local minima
    - Similar solutions provide better escape routes

    Easy samples don't need as much transfer because:
    - Optimizer finds good solutions independently
    - More transfers = more compute time without benefit

    Args:
        sample: Sample data
        history_size: Size of available history
        k_easy: k for easy samples (default 1)
        k_hard: k for hard samples (default 2)
    ...
    difficulty, metrics = compute_sample_difficulty(sample)

    # Hard threshold: difficulty >= 0.5
    if difficulty >= 0.5:
        return min(k_hard, history_size)
    else:
        return min(k_easy, history_size)
...
class EnhancedTransferOptimizer:
    """
    CMA-ES optimizer with enhanced transfer learning.

    Key Improvements:
    1. Enhanced feature extraction (11 features vs 5)
    2. Adaptive k selection based on sample difficulty
    3. Better similarity matching for more relevant transfers
    """
    ...
    def __init__(
        ...
        """
        Initialize the enhanced transfer learning optimizer.
        ...
        Args:
            ...
            k_easy: k for easy samples
            k_hard: k for hard samples
            use_adaptive_k: Enable adaptive k selection
            use_enhanced_features: Use enhanced (11) vs basic (5) features
        """
        self.Lx = Lx
        self.Ly = Ly
        self.nx = nx
    ...
    def estimate_sources(
        ...
        verbose: bool = False,
    ) -> Tuple[List[List[Tuple]], float, List[CandidateResult], np.ndarray, np.ndarray, int, float]:
        """
        Estimate sources with enhanced transfer learning.
        ...
        # === COMPUTE DIFFICULTY ===
        difficulty, diff_metrics = compute_sample_difficulty(sample)
        ...
        if self.use_adaptive_k:
            k = get_adaptive_k(sample, len(history), self.k_easy, self.k_hard)
        else:
            k = min(self.k_easy, len(history))  # Use k_easy as fixed k

        similar_solutions = find_similar_solutions(features, history, k=k)
        n_transferred = len(similar_solutions)

        if verbose:
            print(f"  Difficulty: {difficulty:.2f} (SNR={diff_metrics['snr']:.1f}, "
                  f"onset_std={diff_metrics['onset_std']:.1f})")
            print(f"  Adaptive k={k}, Transfer: {n_transferred} from history of {len(history)}")
        ...
        return candidate_sources, best_rmse, results, features, best_params, n_transferred, difficulty
