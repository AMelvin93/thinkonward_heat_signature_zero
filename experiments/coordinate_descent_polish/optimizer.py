...
Uses coordinate descent (Powell's method) instead of Nelder-Mead for the final polish step.

Hypothesis: For 2-4D problems, coordinate descent may converge faster than simplex methods
because it exploits the axis-aligned nature of the bounded parameter space.
"""
...
class CoordinateDescentPolishOptimizer:
    """
    Same as TemporalFidelityWithPolishOptimizer but uses coordinate descent (Powell)
    instead of Nelder-Mead for the final polish.

    Powell's method is a coordinate descent approach that optimizes one direction at a time.
    For bounded 2-4D problems, this may converge faster than simplex methods.
    """
    ...
    def __init__(
        ...
        polish_method: str = 'Powell',  # 'Powell' or 'Nelder-Mead' for comparison
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run optimization with final polish using coordinate descent."""
        ...
        # Quick refinement on coarse grid (unchanged)
        refined_solutions = []
        ...
        # FINAL POLISH: Using selected method (Powell for coordinate descent)
        if self.final_polish_maxiter > 0 and candidates_raw:
            ...
            # Polish with coordinate descent (Powell) or Nelder-Mead
            if self.polish_method == 'Powell':
                result = minimize(
                    objective_fine_full,
                    best_pos_params,
                    method='Powell',
                ...
                # Default to Nelder-Mead for comparison
                result = minimize(
                    objective_fine_full,
                    best_pos_params,
                    method='Nelder-Mead',
            ...
            polished_params = np.clip(result.x, lb, ub)
