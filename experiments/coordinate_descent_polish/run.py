#!/usr/bin/env python3
"""
Run script for Coordinate Descent Polish optimizer.

Tests the hypothesis that Powell's coordinate descent method may converge
faster than Nelder-Mead for the final polish step in 2-4D problems.
"""
...
from optimizer import CoordinateDescentPolishOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = CoordinateDescentPolishOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Coordinate Descent Polish optimizer')
    ...
    parser.add_argument('--polish-method', type=str, default='Powell',
    ...
    print(f"\nCoordinate Descent Polish Optimizer")
    print(f"=" * 60)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Timestep fraction: {args.timestep_fraction:.0%}")
    print(f"Polish method: {args.polish_method} ({args.final_polish_maxiter} iters)")
    ...
    n_polished = sum(1 for r in results if 'polished' in r.get('init_types', []))

    print(f"\n{'='*70}")
    print(f"RESULTS - Coordinate Descent Polish ({args.polish_method})")
    print(f"{'='*70}")
    print(f"RMSE:             {rmse_mean:.6f}")
    print(f"Submission Score: {score:.4f}")
    print(f"Projected (400):  {projected_400:.1f} min")
    print(f"Polished samples: {n_polished}/{n_samples}")
    print()
    if rmses_1src:
    ...
    print()
    print(f"Baseline (NM):  1.1688 @ 58.4 min")
    ...
    if projected_400 > 60:
        ...
        status = "SUCCESS - Better than baseline!"
    elif score >= 1.1688 - 0.005:
        if projected_400 < 58.4 - 2:
            status = "PARTIAL - Similar score, faster time"
        else:
            status = "SIMILAR - No significant change"
    else:
    ...
    if not args.no_mlflow:
        ...
        run_name = f"coord_descent_polish_{args.polish_method}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_COORDINATE_POLISH_001")
            ...
            mlflow.log_metric("rmse_1src", np.mean(rmses_1src) if rmses_1src else 0)
            mlflow.log_metric("rmse_2src", np.mean(rmses_2src) if rmses_2src else 0)
            mlflow.log_metric("n_polished", n_polished)
