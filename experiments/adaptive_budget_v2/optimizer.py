"""
Adaptive Budget Optimizer V2.

Key idea: Use a quick probe phase to estimate sample difficulty,
then allocate more fevals to harder samples and fewer to easy ones.

This should improve overall accuracy by focusing compute where it helps most.
"""
...
class AdaptiveBudgetV2Optimizer:
    """
    Adaptive budget optimizer that allocates fevals based on sample difficulty.

    1. Quick probe phase to estimate difficulty
    2. Easy samples: fewer fevals (faster)
    3. Hard samples: more fevals + fallback inits
    """
    ...
    def __init__(
        ...
        ny_coarse: int = 25,
        # Base fevals
        base_fevals_1src: int = 20,
        base_fevals_2src: int = 36,
        # Easy samples get fewer fevals
        easy_fevals_1src: int = 15,
        easy_fevals_2src: int = 28,
        # Hard samples get more fevals + fallback
        hard_fevals_1src: int = 24,
        hard_fevals_2src: int = 44,
        # Thresholds for difficulty
        easy_threshold_1src: float = 0.18,  # If probe RMSE < this, easy
        easy_threshold_2src: float = 0.28,
        hard_threshold_1src: float = 0.35,  # If probe RMSE > this, hard
        hard_threshold_2src: float = 0.50,
        # Standard params
        ...
        self.base_fevals_1src = base_fevals_1src
        self.base_fevals_2src = base_fevals_2src
        self.easy_fevals_1src = easy_fevals_1src
        self.easy_fevals_2src = easy_fevals_2src
        self.hard_fevals_1src = hard_fevals_1src
        self.hard_fevals_2src = hard_fevals_2src
        self.easy_threshold_1src = easy_threshold_1src
        self.easy_threshold_2src = easy_threshold_2src
        self.hard_threshold_1src = hard_threshold_1src
        self.hard_threshold_2src = hard_threshold_2src
    ...
    def _probe_difficulty(self, sample, meta, q_range, solver_coarse, n_sources, early_frac):
        """Quick probe to estimate sample difficulty using just the smart init."""
    ...
    def _run_cmaes(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Refine top solutions on coarse grid
        refined_solutions = []
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Quick probe to estimate difficulty
        probe_rmse, probe_sims = self._probe_difficulty(
            sample, meta, q_range, solver_coarse, n_sources, early_frac
        )

        # Determine difficulty and fevals
        if n_sources == 1:
            easy_thresh = self.easy_threshold_1src
            hard_thresh = self.hard_threshold_1src
            easy_fevals = self.easy_fevals_1src
            base_fevals = self.base_fevals_1src
            hard_fevals = self.hard_fevals_1src
        else:
            easy_thresh = self.easy_threshold_2src
            hard_thresh = self.hard_threshold_2src
            easy_fevals = self.easy_fevals_2src
            base_fevals = self.base_fevals_2src
            hard_fevals = self.hard_fevals_2src

        if probe_rmse < easy_thresh:
            difficulty = 'easy'
            max_fevals = easy_fevals
            use_fallback = False
        elif probe_rmse > hard_thresh:
            difficulty = 'hard'
            max_fevals = hard_fevals
            use_fallback = True
        else:
            difficulty = 'medium'
            max_fevals = base_fevals
            use_fallback = False
