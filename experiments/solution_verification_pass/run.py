"""
Run script for solution_verification_pass experiment.
...
def process_single_sample(args):
    ...
    try:
        ...
        # Check if verified candidate was selected
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"SOLUTION VERIFICATION PASS EXPERIMENT")
    print(f"{'='*60}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            ver_flag = " [VERIFIED]" if result['verified_improved'] else ""
    ...
    n_verified = sum(1 for r in results if r['verified_improved'])

    projected_400 = (total_time / n_samples) * 400 / 60

    print(f"\n{'='*70}")
    print(f"RESULTS - Solution Verification Pass")
    print(f"{'='*70}")
    ...
    print()
    print(f"Verification improved: {n_verified}/{n_samples} ({100*n_verified/n_samples:.1f}%)")
    print()
    ...
    results_summary = {
        ...
        'pct_verified_improved': 100 * n_verified / n_samples,
    }
    ...
    if not args.no_mlflow:
        ...
        run_name = f"solution_verification_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param('experiment_name', 'solution_verification_pass')
            mlflow.log_param('experiment_id', 'EXP_FINAL_SOLUTION_VERIFICATION_001')
            ...
            mlflow.log_metric('n_verified_improved', results_summary['n_verified_improved'])
            mlflow.log_metric('pct_verified_improved', results_summary['pct_verified_improved'])
            ...
            if state_path.exists():
                ...
                state['tuning_runs'].append({
                    ...
                    'results': {
                        ...
                        'n_verified_improved': results_summary['n_verified_improved'],
                        'pct_verified_improved': results_summary['pct_verified_improved'],
