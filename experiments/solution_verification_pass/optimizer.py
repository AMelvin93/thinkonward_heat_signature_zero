"""
Solution Verification Pass Optimizer

Adds a gradient verification step after optimization:
...
3. If gradient magnitude is large (solution not at local minimum), take a step in negative gradient direction
4. Re-evaluate and keep the better solution

Hypothesis: Some CMA-ES solutions may terminate before reaching true local minimum.
Quick gradient check can identify and fix these cases.
"""
...
class SolutionVerificationOptimizer:
    """
    Optimizer with gradient verification pass.

    Key addition: After CMA-ES + NM polish, compute gradient at best solution.
    ...
    def __init__(
        ...
        gradient_eps: float = 0.02,  # Finite difference step size
        gradient_threshold: float = 0.1,  # Threshold for "large" gradient
        step_size: float = 0.05,  # Step size in gradient descent direction
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _verify_with_gradient(self, pos_params, n_sources, objective_fn, lb, ub):
        """
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run CMA-ES optimization with verification pass."""
        ...
        # === NEW: GRADIENT VERIFICATION PASS ===
        if self.enable_verification:
            # Verify best candidate
            candidates_raw.sort(key=lambda x: x[2])  # Sort by RMSE
            best = candidates_raw[0]
            best_pos = best[4]  # Position params

            new_pos, new_rmse, n_extra, improved = self._verify_with_gradient(
                best_pos, n_sources, objective_fine, lb, ub
            )
            n_sims[0] += n_extra
            ...
            if improved:
                ...
                candidates_raw.append((sources, full_params, final_rmse, 'verified', new_pos))
