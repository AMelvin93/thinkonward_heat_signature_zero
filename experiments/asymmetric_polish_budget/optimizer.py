"""
Asymmetric Polish Budget Optimizer

Hypothesis: 1-source problems converge faster and need fewer polish iterations,
while 2-source problems are harder and benefit from more polish iterations.

This optimizer uses different final_polish_maxiter values for 1-src vs 2-src:
- 1-source: fewer iterations (converges quickly)
- 2-source: more iterations (needs extra refinement)
"""
...
class AsymmetricPolishOptimizer:
    """
    Optimizer with asymmetric polish budget:
    - 1-source: fewer NM polish iterations (converges fast)
    - 2-source: more NM polish iterations (needs more refinement)
    """
    ...
    def __init__(
        ...
        final_polish_maxiter_1src: int = 6,   # Fewer for 1-source (easier)
        final_polish_maxiter_2src: int = 10,  # More for 2-source (harder)
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.final_polish_maxiter_1src = final_polish_maxiter_1src
        self.final_polish_maxiter_2src = final_polish_maxiter_2src
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run optimization with asymmetric final polish."""
        ...
        # Choose polish iterations based on n_sources
        final_polish_maxiter = (self.final_polish_maxiter_1src if n_sources == 1
                               else self.final_polish_maxiter_2src)
        ...
        if final_polish_maxiter > 0 and candidates_raw:
            # Find best candidate
            ...
            result = minimize(
                ...
                best_pos_params,
                method='Nelder-Mead',
                options={'maxiter': final_polish_maxiter, 'xatol': 0.005, 'fatol': 0.0005}
            )
