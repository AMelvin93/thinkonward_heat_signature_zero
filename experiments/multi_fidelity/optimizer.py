"""
Multi-Fidelity Optimization for Heat Source Identification.

Key Innovation: Use coarse grid (50x25) for CMA-ES exploration (4x faster),
then evaluate final candidates on fine grid (100x50) for accurate RMSE.

This allows more fevals within the same time budget.
"""
...
class MultiFidelityOptimizer:
    """
    Multi-fidelity optimizer: coarse grid for exploration, fine grid for evaluation.
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        max_fevals_1src: int = 20,  # More fevals since coarse is faster
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Coarse solver for exploration
        solver_coarse = self._create_solver(kappa, bc, coarse=True)
        # Fine solver for final evaluation
