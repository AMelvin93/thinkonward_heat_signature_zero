#!/usr/bin/env python3
"""Run script for Larger Candidate Pool experiment."""
...
def main():
    parser = argparse.ArgumentParser(description='Run Larger Candidate Pool')
    ...
    parser.add_argument('--candidate-pool-size', type=int, default=15)  # Increased from 10
    ...
    print(f"\nLarger Candidate Pool Experiment")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Candidate pool size: {args.candidate_pool_size} (baseline: 10)")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Larger Candidate Pool (pool_size={args.candidate_pool_size})")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (1.1688 @ 58.4 min, pool_size=10):")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"larger_pool_{args.candidate_pool_size}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param('experiment_id', 'EXP_LARGER_CANDIDATE_POOL_001')
            mlflow.log_param('worker', 'W2')
            mlflow.log_param('optimizer', 'TemporalFidelityWithPolishOptimizer')
            mlflow.log_param('n_samples', n_samples)
            mlflow.log_param('n_workers', args.workers)
            mlflow.log_param('candidate_pool_size', args.candidate_pool_size)
            ...
            mlflow.log_metric('cands_under_3_count', cands_1 + cands_2)
