#!/usr/bin/env python3
"""Run script for Polar Parameterization experiment with MLflow logging."""
...
from optimizer import PolarParameterizationOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = PolarParameterizationOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run Polar Parameterization Experiment')
    ...
    print(f"\nPolar Parameterization Experiment")
    print(f"=" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Polar Parameterization")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (Cartesian): 1.1688 @ 58.4 min (documented)")
    print(f"This run (Polar):     {score:.4f} @ {projected_400:.1f} min")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"polar_param_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_POLAR_PARAM_001")
            mlflow.log_param("worker", "W2")
            mlflow.log_param("tuning_run", args.tuning_run)
            mlflow.log_param("platform", "wsl")
            mlflow.log_param("n_samples", n_samples)
            mlflow.log_param("parameterization", "polar")
