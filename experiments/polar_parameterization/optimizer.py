"""
Polar Parameterization Optimizer - Uses polar coordinates for CMA-ES optimization

Key insight: Reparameterizing source positions from Cartesian (x, y) to polar (r, theta)
centered on the domain centroid may change the optimization landscape characteristics,
potentially improving CMA-ES convergence.

Transforms:
  x = cx + r * cos(theta)
  y = cy + r * sin(theta)

where (cx, cy) is the domain centroid (Lx/2, Ly/2).
"""
...
class PolarParameterizationOptimizer:
    """
    Optimizer that uses polar coordinates for CMA-ES optimization.

    Key parameters:
    - timestep_fraction: Fraction of timesteps to simulate during CMA-ES
    - Positions are optimized in polar (r, theta) instead of Cartesian (x, y)
    """
    ...
    def _cartesian_to_polar(self, x, y):
        """Convert Cartesian to polar coordinates centered on domain."""
        dx = x - self.cx
        dy = y - self.cy
        r = np.sqrt(dx**2 + dy**2)
    ...
    def _polar_to_cartesian(self, r, theta):
        """Convert polar to Cartesian coordinates and clip to domain bounds."""
        x = self.cx + r * np.cos(theta)
        y = self.cy + r * np.sin(theta)
        # Clip to domain with margin
        margin_x = 0.05 * self.Lx
        margin_y = 0.05 * self.Ly
        x = np.clip(x, margin_x, self.Lx - margin_x)
        y = np.clip(y, margin_y, self.Ly - margin_y)
        return x, y

    def _get_polar_bounds(self, n_sources):
        """Get bounds for polar parameters (r, theta)."""
        # Max r is roughly the diagonal from center to corner
        max_r = np.sqrt((self.Lx/2)**2 + (self.Ly/2)**2) * 0.9  # ~0.9 to stay within domain
        lb, ub = [], []
        for _ in range(n_sources):
            # r in [0.01, max_r], theta in [-pi, pi]
            lb.extend([0.01, -np.pi])
            ub.extend([max_r, np.pi])
        return lb, ub

    def _cartesian_to_polar_params(self, xy_params, n_sources):
        """Convert Cartesian params to polar."""
        polar_params = []
        for i in range(n_sources):
            x, y = xy_params[2*i], xy_params[2*i + 1]
            r, theta = self._cartesian_to_polar(x, y)
            polar_params.extend([r, theta])
        return np.array(polar_params)

    def _polar_to_cartesian_params(self, polar_params, n_sources):
        """Convert polar params to Cartesian."""
        xy_params = []
        for i in range(n_sources):
            r, theta = polar_params[2*i], polar_params[2*i + 1]
            x, y = self._polar_to_cartesian(r, theta)
            xy_params.extend([x, y])
        return np.array(xy_params)
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """
        Run a single optimization pass using POLAR coordinates for CMA-ES.
        """
        ...
        # Objective function works in POLAR coordinates
        if n_sources == 1:
            def objective_polar(polar_params):
                r, theta = polar_params
                x, y = self._polar_to_cartesian(r, theta)
                n_sims[0] += 1
            ...
            def objective_polar(polar_params):
                r1, theta1, r2, theta2 = polar_params
                x1, y1 = self._polar_to_cartesian(r1, theta1)
                x2, y2 = self._polar_to_cartesian(r2, theta2)
                n_sims[0] += 2
        ...
        lb, ub = self._get_polar_bounds(n_sources)
        fevals_per_init = max(5, max_fevals // len(initializations))

        all_solutions = []

        for init_params_cartesian, init_type in initializations:
            # Convert Cartesian init to polar
            init_params_polar = self._cartesian_to_polar_params(init_params_cartesian, n_sources)
            ...
            es = cma.CMAEvolutionStrategy(init_params_polar.tolist(), sigma0, opts)

            while not es.stop():
                solutions = es.ask()
                fitness = [objective_polar(s) for s in solutions]
                es.tell(solutions, fitness)
                for sol, fit in zip(solutions, fitness):
                    # Store solution in Cartesian for later use
                    xy_params = self._polar_to_cartesian_params(np.array(sol), n_sources)
                    all_solutions.append((xy_params, fit, init_type))

        # Sort by coarse fitness
        all_solutions.sort(key=lambda x: x[1])

        # Refine top N solutions on COARSE grid with reduced timesteps (in Cartesian now)
        if n_sources == 1:
    ...
    def _final_polish(self, sample, meta, q_range, solver_fine, best_sources, n_sources, n_sims_counter):
        """Apply final Nelder-Mead polish on best solution using FULL simulation."""
        sensors_xy = np.array(sample['sensors_xy'])
        Y_observed = sample['Y_noisy']
        dt = meta['dt']
        nt_full = sample['sample_metadata']['nt']
        ...
        # Extract position params from best sources
        if n_sources == 1:
            x, y, q = best_sources[0]
            ...
            x1, y1, q1 = best_sources[0]
            x2, y2, q2 = best_sources[1]
            init_pos = np.array([x1, y1, x2, y2])
        ...
        # Reconstruct sources from polished params
        if n_sources == 1:
            x, y = result.x
            q, _, final_rmse = compute_optimal_intensity_1src(
                x, y, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
            n_sims_counter[0] += 1
            return [(float(x), float(y), float(q))], final_rmse
        else:
            x1, y1, x2, y2 = result.x
            (q1, q2), _, final_rmse = compute_optimal_intensity_2src(
                x1, y1, x2, y2, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
            n_sims_counter[0] += 2
            return [(float(x1), float(y1), float(q1)), (float(x2), float(y2), float(q2))], final_rmse
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Compute reduced timesteps
        nt_reduced = max(10, int(nt_full * self.timestep_fraction))

        if verbose:
            print(f"  Using POLAR parameterization with {nt_reduced}/{nt_full} timesteps ({self.timestep_fraction*100:.0f}%)")

        # Primary initializations (in Cartesian, will be converted to polar)
        primary_inits = []
        ...
        # Run primary optimization with polar coordinates
        ...
        # Apply final NM polish on best candidate
        n_sims_counter = [n_sims]
        best_candidate = min(candidates_raw, key=lambda x: x[2])
        polished_sources, polished_rmse = self._final_polish(
            sample, meta, q_range, solver_fine, best_candidate[0], n_sources, n_sims_counter
        )
        n_sims = n_sims_counter[0]

        # Add polished result if it's an improvement
        if polished_rmse < best_candidate[2]:
            if n_sources == 1:
                x, y, q = polished_sources[0]
                full_params = np.array([x, y, q])
            else:
                x1, y1, q1 = polished_sources[0]
                x2, y2, q2 = polished_sources[1]
                full_params = np.array([x1, y1, q1, x2, y2, q2])
            candidates_raw.append((polished_sources, full_params, polished_rmse, 'polished'))
