#!/usr/bin/env python3
"""
Run script for W2 Intermediate Sigma + Reduced Fevals optimizer.

EXP012: intermediate_sigma_reduced_fevals
Config:
- sigma0_1src: 0.18 (intermediate)
- sigma0_2src: 0.22 (intermediate)
- threshold_1src: 0.35 (aggressive)
- threshold_2src: 0.45 (aggressive)
- fevals_1src: 18 (reduced)
- fevals_2src: 32 (reduced)
- fallback_fevals: 18

Hypothesis: Combine intermediate sigma with reduced fevals for double time savings.
Based on: EXP010 showed sigma 0.18/0.22 = 63.5 min. Reduced fevals saves ~2-3 min. Target: under 60 min!
"""
...
def process_single_sample(args):
    ...
    optimizer = SigmaBaselineOptimizer(
        ...
        max_fevals_1src=config.get('max_fevals_1src', 18),  # Reduced
        max_fevals_2src=config.get('max_fevals_2src', 32),  # Reduced
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Intermediate Sigma + Reduced Fevals optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP012 specific
    parser.add_argument('--sigma0-1src', type=float, default=0.18)  # Intermediate
    parser.add_argument('--sigma0-2src', type=float, default=0.22)  # Intermediate
    parser.add_argument('--threshold-1src', type=float, default=0.35)  # Aggressive
    parser.add_argument('--threshold-2src', type=float, default=0.45)  # Aggressive
    parser.add_argument('--fallback-fevals', type=int, default=18)
    # Reduced primary fevals
    parser.add_argument('--max-fevals-1src', type=int, default=18)  # Reduced from 20
    parser.add_argument('--max-fevals-2src', type=int, default=32)  # Reduced from 36
    # Other parameters
    ...
    print(f"\n[W2] Intermediate Sigma + Reduced Fevals Optimizer")
    print(f"=" * 70)
    print(f"EXP012: intermediate_sigma_reduced_fevals")
    print(f"-" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Sigma: 1-src={args.sigma0_1src}, 2-src={args.sigma0_2src} (INTERMEDIATE)")
    print(f"Threshold: 1-src={args.threshold_1src}, 2-src={args.threshold_2src}")
    print(f"PRIMARY fevals: 1-src={args.max_fevals_1src}, 2-src={args.max_fevals_2src} (REDUCED)")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Intermediate Sigma + Reduced Fevals (EXP012)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Config: sigma={args.sigma0_1src}/{args.sigma0_2src} (intermediate), fevals={args.max_fevals_1src}/{args.max_fevals_2src} (reduced)")
    print(f"Baseline: 1.1247 @ 57.2 min")
    print(f"EXP010 (intermediate sigma only): 1.1337 @ 63.5 min")
