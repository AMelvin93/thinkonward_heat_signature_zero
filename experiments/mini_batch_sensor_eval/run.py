"""
Run Mini-Batch Sensor Evaluation experiment with MLflow logging.
"""
...
from experiments.mini_batch_sensor_eval.optimizer import MiniBatchSensorOptimizer


def process_sample(args):
    """Process a single sample."""
    sample_idx, sample, meta, q_range, sensor_fraction = args
    
    optimizer = MiniBatchSensorOptimizer(
    ...
    try:
        ...
        return {
            ...
            'time': elapsed,
            'n_sims': n_sims,
            'n_sensors': len(sample['sensors_xy']),
            'success': True
        }
...
def run_experiment(
    ...
    """Run the mini-batch sensor experiment."""
    ...
    print(f"[MiniBatch] Running with sensor_fraction={sensor_fraction}")
    print(f"[MiniBatch] Processing {len(samples)} samples with {n_workers} workers")
    
    start_time = time.time()
    
    # Process samples in parallel
    args_list = [
        (i, sample, meta, q_range, sensor_fraction)
    ...
    print(f"  Avg sensors per sample: {avg_sensors:.1f}")
    print(f"  Sensors used in CMA-ES: {int(avg_sensors * sensor_fraction)}")
...
def main():
    ...
    parser.add_argument('--sensor-fraction', type=float, default=0.5)
    ...
    run_name = f"mini_batch_{args.sensor_fraction}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    with mlflow.start_run(run_name=run_name) as run:
        # Log parameters
        mlflow.log_param("experiment_id", "EXP_MINI_BATCH_SENSORS_001")
        mlflow.log_param("worker", "W2")
        mlflow.log_param("sensor_fraction", args.sensor_fraction)
        ...
        mlflow.log_metric("avg_sensors", result['avg_sensors'])
        
        print(f"\n[MiniBatch] MLflow run ID: {run.info.run_id}")
