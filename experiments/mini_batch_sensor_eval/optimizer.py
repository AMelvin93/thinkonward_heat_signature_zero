"""
Mini-Batch Sensor Evaluation Optimizer

Uses a random subset of sensors during CMA-ES exploration to reduce per-evaluation
cost, similar to stochastic gradient descent. Full sensor evaluation is used only
for final polish and candidate ranking.
"""
...
def compute_optimal_intensity_1src(x, y, Y_observed, solver, dt, nt, T0, sensors_xy,
                                    q_range=(0.5, 2.0), sensor_mask=None):
    """Compute optimal intensity with optional sensor masking."""
    ...
    if sensor_mask is not None:
        Y_unit_masked = Y_unit[:, sensor_mask]
        Y_obs_masked = Y_obs_trunc[:, sensor_mask]
    else:
        Y_unit_masked = Y_unit
        Y_obs_masked = Y_obs_trunc

    Y_unit_flat = Y_unit_masked.flatten()
    Y_obs_flat = Y_obs_masked.flatten()
    ...
    # RMSE on masked sensors
    Y_pred_masked = q_optimal * Y_unit_masked
    rmse = np.sqrt(np.mean((Y_pred_masked - Y_obs_masked) ** 2))
    return q_optimal, Y_unit, rmse


def compute_optimal_intensity_2src(x1, y1, x2, y2, Y_observed, solver, dt, nt, T0, sensors_xy,
                                    q_range=(0.5, 2.0), sensor_mask=None):
    """Compute optimal intensities with optional sensor masking."""
    ...
    # Apply sensor mask if provided
    if sensor_mask is not None:
        Y1_masked = Y1[:, sensor_mask]
        Y2_masked = Y2[:, sensor_mask]
        Y_obs_masked = Y_obs_trunc[:, sensor_mask]
    else:
        Y1_masked = Y1
        Y2_masked = Y2
    ...
    Y1_flat = Y1_masked.flatten()
    Y2_flat = Y2_masked.flatten()
    Y_obs_flat = Y_obs_masked.flatten()
    A = np.array([
    ...
    Y_pred_masked = q1 * Y1_masked + q2 * Y2_masked
    rmse = np.sqrt(np.mean((Y_pred_masked - Y_obs_masked) ** 2))
    return (q1, q2), (Y1, Y2), rmse


class MiniBatchSensorOptimizer:
    """
    Optimizer that uses a random subset of sensors during CMA-ES exploration.
    """
    ...
    def __init__(
        ...
        sensor_fraction: float = 0.5,  # Fraction of sensors to use during CMA-ES
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.sensor_fraction = sensor_fraction
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Create sensor mask for mini-batch evaluation
        n_sample_sensors = max(1, int(n_sensors * self.sensor_fraction))
        # Create a fixed random mask for this optimization run
        sensor_mask = np.random.choice(n_sensors, n_sample_sensors, replace=False)

        if n_sources == 1:
            def objective_coarse(xy_params, use_full=False):
                x, y = xy_params
                n_sims[0] += 1
                mask = None if use_full else sensor_mask
                q, Y_unit, rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_coarse, dt, nt, T0, sensors_xy, q_range,
                    sensor_mask=mask)
                return rmse

            def objective_fine(xy_params, use_full=True):
                x, y = xy_params
                n_sims[0] += 1
                mask = None if use_full else sensor_mask
                q, Y_unit, rmse = compute_optimal_intensity_1src(
            ...
            def objective_coarse(xy_params, use_full=False):
                ...
                n_sims[0] += 2
                mask = None if use_full else sensor_mask
                (q1, q2), (Y1, Y2), rmse = compute_optimal_intensity_2src(
        ...
        for init_params, init_type in initializations:
            ...
            while not es.stop():
                solutions = es.ask()
                # Use mini-batch sensors during CMA-ES
                fitness = [objective_coarse(s, use_full=False) for s in solutions]
        ...
        # Refine top N with coarse NM using full sensors
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                result = minimize(
                    lambda x: objective_coarse(x, use_full=True),  # Full sensors for refinement
                    pos_params,
                    method='Nelder-Mead',
                    options={
        ...
        # Evaluate on fine grid with FULL sensors
        candidates_raw = []
        for pos_params, rmse_coarse, init_type in refined_solutions:
            if n_sources == 1:
                x, y = pos_params
                q, Y_unit, final_rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_fine, dt, nt, T0, sensors_xy, q_range,
                    sensor_mask=None)  # Full sensors
                n_sims[0] += 1
                full_params = np.array([x, y, q])
                sources = [(float(x), float(y), float(q))]
            else:
                x1, y1, x2, y2 = pos_params
                (q1, q2), (Y1, Y2), final_rmse = compute_optimal_intensity_2src(
                    x1, y1, x2, y2, Y_observed, solver_fine, dt, nt, T0, sensors_xy, q_range,
                    sensor_mask=None)  # Full sensors
                n_sims[0] += 2
        ...
        # Final polish on best candidate with FULL sensors
        if self.polish_maxiter > 0 and candidates_raw:
            ...
            result = minimize(
                lambda x: objective_fine(x, use_full=True),  # Full sensors for polish
                best_pos_params,
                method='Nelder-Mead',
