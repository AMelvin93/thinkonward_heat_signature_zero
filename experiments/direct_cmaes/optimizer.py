"""
DIRECT + Nelder-Mead Hybrid Optimizer.

Key idea: Use DIRECT for deterministic global exploration (fewer function evaluations,
more reliable than random-based CMA-ES), then refine with Nelder-Mead.

DIRECT is a deterministic algorithm that systematically divides the search space,
providing more consistent global coverage than stochastic methods.
"""
...
class DirectCMAESOptimizer:
    """
    Hybrid optimizer using DIRECT for global exploration + Nelder-Mead for refinement.
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        direct_maxfun_1src: int = 50,
        direct_maxfun_2src: int = 100,
        ...
        self.direct_maxfun_1src = direct_maxfun_1src
        self.direct_maxfun_2src = direct_maxfun_2src
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if n_sources == 1:
            def objective_coarse(xy_params):
                ...
                all_evaluated.append((np.array([x, y]), rmse_early))
                return rmse_early
        else:
            def objective_coarse(xy_params):
                ...
                (q1, q2), Y_pred, rmse_early, rmse_full = compute_optimal_intensity_2src(
                    ...
                    early_fraction=early_frac)
                all_evaluated.append((np.array([x1, y1, x2, y2]), rmse_early))
                return rmse_early

        lb, ub = self._get_position_bounds(n_sources)
        bounds = [(lb[i], ub[i]) for i in range(len(lb))]
        maxfun = self.direct_maxfun_1src if n_sources == 1 else self.direct_maxfun_2src

        # Run DIRECT for global exploration
        try:
            result = direct(
                objective_coarse,
                bounds=bounds,
                maxfun=maxfun,
                locally_biased=True,  # DIRECT-L variant (faster convergence)
            )
            direct_best = result.x
        except Exception as e:
            # Fallback to smart init if DIRECT fails
            direct_best = self._smart_init_positions(sample, n_sources)

        # Also evaluate smart init and triangulation init for diversity
        smart_init = self._smart_init_positions(sample, n_sources)
        smart_rmse = objective_coarse(smart_init)

        tri_init = self._triangulation_init_positions(sample, meta, n_sources, q_range)
        if tri_init is not None:
            tri_rmse = objective_coarse(tri_init)

        # Get top solutions from all evaluated points
        all_evaluated.sort(key=lambda x: x[1])
        top_solutions = all_evaluated[:5]

        # Refine top solutions with Nelder-Mead on COARSE grid
        refined_solutions = []
        for pos_params, rmse_coarse in top_solutions[:3]:
            result = minimize(
                objective_coarse,
                pos_params,
                method='Nelder-Mead',
            ...
            if result.fun < rmse_coarse:
                ...
                refined_solutions.append((pos_params, rmse_coarse, 'direct'))

        # Add remaining top solutions
        for pos_params, rmse in top_solutions[3:]:
            refined_solutions.append((pos_params, rmse, 'direct'))
