#!/usr/bin/env python3
"""
Run script for DIRECT + Nelder-Mead Hybrid optimizer.
"""
...
from optimizer import DirectCMAESOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = DirectCMAESOptimizer(
        direct_maxfun_1src=config.get('direct_maxfun_1src', 50),
        direct_maxfun_2src=config.get('direct_maxfun_2src', 100),
...
def main():
    parser = argparse.ArgumentParser(description='Run DIRECT+NM Hybrid optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--direct-maxfun-1src', type=int, default=50)
    parser.add_argument('--direct-maxfun-2src', type=int, default=100)
    parser.add_argument('--refine-maxiter', type=int, default=10)
    ...
    print(f"DIRECT maxfun: 1-src={args.direct_maxfun_1src}, 2-src={args.direct_maxfun_2src}")
    print(f"NM refine maxiter: {args.refine_maxiter}")
    print(f"=" * 60)

    config = {
        'direct_maxfun_1src': args.direct_maxfun_1src,
        'direct_maxfun_2src': args.direct_maxfun_2src,
