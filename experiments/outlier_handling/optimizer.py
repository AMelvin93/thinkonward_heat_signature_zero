"""
Outlier-Handling Optimizer with Adaptive Function Evaluations.

Key idea: Detect "hard" samples early based on initial RMSE, then allocate
extra function evaluations to those samples. Easy samples get standard budget,
hard samples get additional fevals to escape local minima.

Based on: experiments/robust_fallback/optimizer.py
"""
...
class OutlierHandlingOptimizer:
    """
    Optimizer with adaptive function evaluations for hard samples.

    Detects "hard" samples early based on initial RMSE and allocates
    extra function evaluations to improve their results.
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        # Base fevals for normal samples
        base_fevals_1src: int = 20,
        base_fevals_2src: int = 36,
        # Extra fevals for hard samples
        hard_sample_extra_fevals: int = 10,
        # Threshold for detecting hard samples
        hard_sample_threshold: float = 0.4,
        ...
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.35,  # Fallback threshold
        rmse_threshold_2src: float = 0.45,  # Fallback threshold
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.hard_sample_extra_fevals = hard_sample_extra_fevals
    ...
    def _quick_initial_eval(self, sample, meta, q_range, solver_coarse, n_sources):
        """
        Quick evaluation at initial position to detect hard samples.
        Returns the best RMSE found with minimal simulations.
        """
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Step 1: Quick initial evaluation to detect hard samples
        initial_rmse, quick_sims = self._quick_initial_eval(
            sample, meta, q_range, solver_coarse, n_sources
        )
        total_sims += quick_sims

        # Step 2: Determine feval budget based on sample difficulty
        is_hard_sample = initial_rmse > self.hard_sample_threshold

        if n_sources == 1:
            max_fevals = self.base_fevals_1src
            if is_hard_sample:
                max_fevals += self.hard_sample_extra_fevals
        else:
            max_fevals = self.base_fevals_2src
            if is_hard_sample:
                max_fevals += self.hard_sample_extra_fevals

        # Step 3: Primary optimization with adaptive fevals
        primary_inits = []
        tri_init = self._triangulation_init_positions(sample, meta, n_sources, q_range)
        if tri_init is not None:
        ...
        candidates_raw, n_sims = self._run_optimization(
            ...
            primary_inits, n_sources, early_frac, max_fevals
        )
        total_sims += n_sims
        ...
        if best_rmse_initial > threshold:
            ...
            # For fallback, also use adaptive fevals if hard sample
            fallback_fevals = max_fevals  # Same budget as primary

            fallback_candidates, fallback_sims = self._run_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, early_frac, fallback_fevals
            )
            total_sims += fallback_sims
        ...
        return candidate_sources, best_rmse, results, total_sims, is_hard_sample
