"""
Surrogate NN Pre-filter for CMA-ES.

Key idea: Train a small MLP on (params -> RMSE) pairs from initial samples,
then use NN to pre-filter CMA-ES candidates, simulating only top 50%.

Phases:
1. Bootstrap: Run baseline CMA-ES on first N samples, collect (params, RMSE) pairs
2. Train: Fit MLP to predict RMSE from parameters
3. Filter: For remaining samples, generate candidates but only simulate NN-top-50%

Expected speedup: ~40-50% reduction in simulator calls
"""
...
class SimpleMLP:
    """Simple MLP surrogate model using numpy only (no sklearn dependency)."""

    def __init__(self, input_dim, hidden_sizes=(64, 32), learning_rate=0.01, epochs=100):
        self.layers = []
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(len(self.layers) - 1, -1, -1):
                ...
                if i > 0:
                    delta = (delta @ layer['W'].T) * self._relu_grad(self.pre_activations[i-1])

        self.trained = True
        return self

    def predict(self, X):
        """Predict RMSE for candidates."""
        if not self.trained:
            return np.ones(X.shape[0]) * 0.5  # Default high uncertainty
        return self._forward(X).flatten()


class SurrogateNNOptimizer:
    """
    CMA-ES with NN surrogate pre-filtering.

    Uses a simple MLP to predict RMSE from source parameters,
    allowing us to filter out bad candidates before simulation.
    """
    ...
    def __init__(
        ...
        # Surrogate-specific params
        filter_ratio: float = 0.5,  # Keep top 50% of candidates
        use_surrogate: bool = True,  # Toggle surrogate filtering
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        # Surrogates for 1-src and 2-src problems (trained dynamically)
        self.surrogate_1src = None
        self.surrogate_2src = None

        # Training data collected during optimization
        self.training_data_1src = {'X': [], 'y': []}
        self.training_data_2src = {'X': [], 'y': []}
    ...
    def _collect_training_data(self, params, rmse, n_sources):
        """Collect data point for surrogate training."""
        if n_sources == 1:
            self.training_data_1src['X'].append(params.copy())
            self.training_data_1src['y'].append(rmse)
        else:
            self.training_data_2src['X'].append(params.copy())
            self.training_data_2src['y'].append(rmse)

    def _train_surrogate(self, n_sources):
        """Train or update the surrogate model."""
        if n_sources == 1:
            data = self.training_data_1src
            if len(data['X']) < 20:  # Need minimum data to train
                return
            X = np.array(data['X'])
            y = np.array(data['y'])
            self.surrogate_1src = SimpleMLP(input_dim=2, hidden_sizes=(32, 16), epochs=50)
            self.surrogate_1src.fit(X, y)
        else:
            data = self.training_data_2src
            if len(data['X']) < 30:  # Need more data for 4D problem
                return
            X = np.array(data['X'])
            y = np.array(data['y'])
            self.surrogate_2src = SimpleMLP(input_dim=4, hidden_sizes=(64, 32), epochs=50)
            self.surrogate_2src.fit(X, y)

    def _filter_candidates_with_surrogate(self, candidates, n_sources):
        """Filter candidates using surrogate, return top filter_ratio."""
        surrogate = self.surrogate_1src if n_sources == 1 else self.surrogate_2src
        if surrogate is None or not self.use_surrogate:
            return candidates  # No filtering if surrogate not trained

        # Predict RMSE for all candidates
        X = np.array([c[0] for c in candidates])
        predicted_rmse = surrogate.predict(X)

        # Sort by predicted RMSE and keep top ratio
        indices = np.argsort(predicted_rmse)
        n_keep = max(4, int(len(candidates) * self.filter_ratio))  # Keep at least 4
        filtered_indices = indices[:n_keep]

        return [candidates[i] for i in filtered_indices]

    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, early_frac):
        """Run CMA-ES with optional surrogate pre-filtering."""
        ...
        if n_sources == 1:
            def objective_coarse(xy_params):
                ...
                self._collect_training_data(xy_params, rmse_early, n_sources)
                return rmse_early
        else:
            def objective_coarse(xy_params):
        ...
        for init_params, init_type in initializations:
            ...
            while not es.stop():
                ...
                # Apply surrogate filtering if trained
                if self.use_surrogate:
                    candidates = [(np.array(s), None, init_type) for s in solutions]
                    filtered = self._filter_candidates_with_surrogate(candidates, n_sources)
                    n_filtered[0] += len(solutions) - len(filtered)
                    solutions_to_eval = [f[0] for f in filtered]
                else:
                    solutions_to_eval = solutions

                # Evaluate filtered solutions
                fitness_map = {}
                for sol in solutions_to_eval:
                    sol_tuple = tuple(sol)
                    fitness_map[sol_tuple] = objective_coarse(sol)

                # Assign fitness (filtered-out get high penalty)
                fitness = []
                for sol in solutions:
                    sol_tuple = tuple(sol)
                    if sol_tuple in fitness_map:
                        fit = fitness_map[sol_tuple]
                    else:
                        # Penalty for filtered-out candidates (use surrogate prediction)
                        surrogate = self.surrogate_1src if n_sources == 1 else self.surrogate_2src
                        if surrogate is not None:
                            fit = float(surrogate.predict(np.array([sol]))[0]) + 0.1  # Slight penalty
                        else:
                            fit = 1.0  # High penalty
                    fitness.append(fit)

                es.tell(solutions, fitness)
                for sol, fit in zip(solutions_to_eval, [fitness_map[tuple(s)] for s in solutions_to_eval]):
        ...
        return candidates_raw, n_sims[0], n_filtered[0]
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        candidates_raw, n_sims, n_filtered = self._run_single_optimization(
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims, fallback_filtered = self._run_single_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, early_frac
            )
            n_sims += fallback_sims
            n_filtered += fallback_filtered

            # Combine results from both runs
            candidates_raw.extend(fallback_candidates)

        # Update surrogate with new data periodically
        self._train_surrogate(n_sources)
        ...
        return candidate_sources, best_rmse, results, n_sims, n_filtered
