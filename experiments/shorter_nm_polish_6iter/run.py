"""
Test different NM polish iterations WITHOUT perturbation.

Based on discovery that perturbation adds overhead without benefit,
we test how many NM iterations are actually needed.
"""
...
# Use the tabu hopping optimizer with tabu disabled (clean baseline)
...
def run_experiment(config, config_name, data):
    ...
    print(f"Parameters: refine_maxiter={config.get('refine_maxiter', 8)}")
...
def main():
    ...
    base_config = {
        ...
        'enable_tabu_hopping': False,  # NO perturbation!
    }

    # Test different NM polish iterations
    configs = [
        ({**base_config, 'refine_maxiter': 6}, 'nm_6iter'),
        ({**base_config, 'refine_maxiter': 7}, 'nm_7iter'),
        ({**base_config, 'refine_maxiter': 8}, 'nm_8iter_baseline'),
    ]
