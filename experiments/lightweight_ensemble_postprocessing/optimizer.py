"""
Lightweight Ensemble Postprocessing Optimizer

Based on production baseline (optimizer_with_polish.py) with MINIMAL ensemble addition.
Key insight: Ensemble averaging improves accuracy +0.03 but previous implementation
had overhead from full optimizer rewrite. This adds ONLY the ensemble step.

Changes from baseline:
1. Add align_2source_solutions() helper function
2. Add _compute_ensemble_solution() method
3. After fine-grid evaluation, compute ensemble from top-5 candidates
4. Add ensemble solution to candidate pool (1-2 extra simulations)
"""
...
# === NEW: Ensemble helper function ===
def align_2source_solutions(solutions):
    """
    Align 2-source solutions so that source 1 and source 2 are consistent.
    Uses first solution as reference and permutes others to minimize distance.

    solutions: list of [(x1, y1), (x2, y2)] position tuples
    Returns: list of aligned position tuples
    """
    if len(solutions) < 2:
        return solutions
    ...
    for sol in solutions[1:]:
        ...
        perm1 = sol  # (S1, S2)
        perm2 = [sol[1], sol[0]]  # (S2, S1)

        # Distance to reference for each permutation
        dist1 = sum(np.linalg.norm(np.array(ref) - np.array(s))
                    for ref, s in zip(reference, perm1))
        dist2 = sum(np.linalg.norm(np.array(ref) - np.array(s))
                    for ref, s in zip(reference, perm2))

        if dist1 <= dist2:
            aligned.append(perm1)
        else:
            aligned.append(perm2)

    return aligned


class LightweightEnsembleOptimizer:
    """
    Production baseline + lightweight ensemble postprocessing.

    After CMA-ES + NM polish, compute weighted average of top candidates.
    Adds only 1-2 extra simulations per sample.
    """
    ...
    def __init__(
        ...
        ensemble_top_n: int = 5,  # Number of solutions to average
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    # === NEW: Ensemble computation method ===
    def _compute_ensemble_solution(self, candidates_raw, n_sources, Y_observed,
                                    solver_fine, dt, nt_full, T0, sensors_xy, q_range):
        """
        Compute weighted average ensemble from top candidates.
        Returns (sources, full_params, rmse, n_sims) or None if not enough candidates.
        """
        if len(candidates_raw) < 2:
            return None, 0

        # Sort by RMSE and take top N
        sorted_candidates = sorted(candidates_raw, key=lambda x: x[2])[:self.ensemble_top_n]

        if n_sources == 1:
            # Extract positions and RMSEs
            positions = [c[4] for c in sorted_candidates]  # pos_params
            rmses = [c[2] for c in sorted_candidates]

            # Weights inversely proportional to RMSE
            weights = [1.0 / (r + 1e-6) for r in rmses]
            total_weight = sum(weights)
            weights = [w / total_weight for w in weights]

            # Weighted average
            avg_params = np.zeros(2)
            for pos, w in zip(positions, weights):
                avg_params += w * np.array(pos)

            # Evaluate ensemble
            x, y = avg_params
            q, _, ensemble_rmse = compute_optimal_intensity_1src(
                x, y, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
            full_params = np.array([x, y, q])
            sources = [(float(x), float(y), float(q))]
            return (sources, full_params, ensemble_rmse, 'ensemble', avg_params), 1

        else:  # 2-source
            # Extract positions as [(x1, y1), (x2, y2)] tuples
            pos_tuples = []
            for c in sorted_candidates:
                params = c[4]  # pos_params
                pos_tuples.append([(params[0], params[1]), (params[2], params[3])])

            # Align permutations
            aligned = align_2source_solutions(pos_tuples)
            ...
            # Weighted average for each source
            avg_s1 = np.zeros(2)
            avg_s2 = np.zeros(2)
            for pos, w in zip(aligned, weights):
                avg_s1 += w * np.array(pos[0])
                avg_s2 += w * np.array(pos[1])

            avg_params = np.array([avg_s1[0], avg_s1[1], avg_s2[0], avg_s2[1]])

            # Evaluate ensemble
            x1, y1, x2, y2 = avg_params
            (q1, q2), _, ensemble_rmse = compute_optimal_intensity_2src(
                x1, y1, x2, y2, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
            full_params = np.array([x1, y1, q1, x2, y2, q2])
            sources = [(float(x1), float(y1), float(q1)),
                       (float(x2), float(y2), float(q2))]
            return (sources, full_params, ensemble_rmse, 'ensemble', avg_params), 2
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # === NEW: ENSEMBLE STEP (lightweight - only 1-2 extra sims) ===
        ensemble_result, ensemble_sims = self._compute_ensemble_solution(
            candidates_raw, n_sources, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range
        )
        n_sims[0] += ensemble_sims

        # Track if ensemble was better
        ensemble_was_best = False
        if ensemble_result is not None:
            best_individual_rmse = min(c[2] for c in candidates_raw)
            if ensemble_result[2] < best_individual_rmse:
                ensemble_was_best = True
        ...
        candidates_raw_clean = [(c[0], c[1], c[2], c[3]) for c in candidates_raw]

        return candidates_raw_clean, n_sims[0], ensemble_was_best
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run optimization with ensemble and final polish
        candidates_raw, n_sims, ensemble_was_best = self._run_single_optimization(
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims, fallback_ensemble_best = self._run_single_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Track ensemble wins
        ensemble_win = ensemble_was_best or fallback_ensemble_best

        return candidate_sources, best_rmse, results, n_sims, ensemble_win
