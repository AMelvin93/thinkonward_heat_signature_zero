#!/usr/bin/env python3
"""
Run script for W1 Sigma 0.185/0.225 optimizer.

EXP020: Testing sigma 0.185/0.225 - slightly above sigma 0.18/0.22 which works.
Goal: Explore score-time tradeoff between 0.18/0.22 and 0.20/0.25.
...
from optimizer import Sigma0185_0225Optimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = Sigma0185_0225Optimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Sigma 0.185/0.225 optimizer (EXP020)')
    ...
    parser.add_argument('--sigma0-1src', type=float, default=0.185)
    parser.add_argument('--sigma0-2src', type=float, default=0.225)
    ...
    print(f"\nW1 Sigma 0.185/0.225 Optimizer (EXP020)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Sigma 0.185/0.225 (EXP020)")
    print(f"{'='*70}")
    ...
    print(f"EXP010 ref:    1.1337 @ 63.5 min (sigma 0.18/0.22)")
    print(f"EXP018 ref:    1.1271 @ 67.7 min (sigma 0.18/0.22)")
    print(f"This run:      {score:.4f} @ {projected_400:.1f} min (sigma 0.185/0.225)")
