"""
TV Regularization Optimizer - Adds Total Variation penalty to RMSE objective

Based on arxiv 2507.02677: "For recovering atomic measures, total variation (TV)
is more suitable than Tikhonov"

TV regularization for point sources = L1 norm of intensities = sum(|q_i|)

objective = RMSE + lambda * sum(|q_i|)

WARNING: Prior experiments changing the loss function have FAILED:
- log_rmse_loss: FAILED (monotonic transformations don't help CMA-ES)
- weighted_sensor_loss: FAILED (optimizing different objective than scored)

The hypothesis is that TV regularization might help STABILITY not accuracy.
"""
...
class TVRegularizationOptimizer:
    """
    Optimizer with Total Variation (TV) regularization on source intensities.

    Objective = RMSE + lambda_tv * sum(|q_i|)

    TV regularization encourages sparse solutions (fewer high-intensity sources).
    This may help stability for inverse problems with noise.
    """
    ...
    def __init__(
        ...
        # NEW: TV regularization parameter
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run optimization with TV regularization."""
        ...
        # Objective with TV regularization
        if n_sources == 1:
            def objective_coarse(xy_params):
                ...
                # TV regularization: lambda * |q|
                tv_penalty = self.lambda_tv * abs(q)
                return rmse + tv_penalty
            ...
            def objective_coarse(xy_params):
                ...
                # TV regularization: lambda * (|q1| + |q2|)
                tv_penalty = self.lambda_tv * (abs(q1) + abs(q2))
                return rmse + tv_penalty
        ...
        # Evaluate on fine grid (using PURE RMSE for final scoring)
        candidates_raw = []
        ...
        # Final polish on best candidate (uses pure RMSE, not TV)
        if self.final_polish_maxiter > 0 and candidates_raw:
            ...
            # Use pure RMSE for polish (not regularized)
            if n_sources == 1:
                def polish_objective(xy_params):
                    x, y = xy_params
                    n_sims[0] += 1
