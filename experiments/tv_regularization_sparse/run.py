#!/usr/bin/env python3
"""
Run script for TV Regularization optimizer.

Tests Total Variation (TV) regularization on the optimization objective.
TV penalty = lambda * sum(|q_i|) encourages sparse solutions.
"""
...
from optimizer import TVRegularizationOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = TVRegularizationOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run TV Regularization optimizer')
    ...
    # MLflow
    parser.add_argument('--experiment-id', type=str, default='EXP_TV_REGULARIZATION_001')
    ...
    run_name = f"tv_regularization_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    with mlflow.start_run(run_name=run_name) as run:
        ...
        mlflow.log_param("lambda_tv", args.lambda_tv)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - TV Regularization (lambda={args.lambda_tv})")
    print(f"{'='*70}")
