"""
Perturbation + Gradient Verification Optimizer

Combines two successful approaches:
1. Perturbation + local restart (basin hopping) from perturbed_extended_polish
2. Gradient verification from solution_verification_pass

Hypothesis: These two approaches address different failure modes:
- Perturbation: escapes shallow local minima
- Verification: ensures we're actually at a local minimum

Combined, they could stack improvements.
"""
...
class PerturbationPlusVerificationOptimizer:
    """
    Optimizer combining perturbation + local restart AND gradient verification.
    """
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # GRADIENT VERIFICATION PASS (on COARSE grid for speed)
        if self.enable_verification:
            candidates_raw.sort(key=lambda x: x[2])
            best = candidates_raw[0]
            best_pos = best[4]

            # Use COARSE grid for gradient verification (much faster)
            new_pos, new_rmse_coarse, n_extra, improved = self._verify_with_gradient(
                best_pos, n_sources, objective_coarse, lb, ub
            )
            n_sims[0] += n_extra

            if improved:
                # Re-evaluate improved position on FINE grid
                if n_sources == 1:
                    x, y = new_pos
                    q, _, final_rmse = compute_optimal_intensity_1src(
                        x, y, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
