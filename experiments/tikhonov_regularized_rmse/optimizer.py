"""
Tikhonov Regularized RMSE Optimizer

Adds regularization to the CMA-ES loss function to prevent overfitting to sensor noise.
L = RMSE + lambda * ||params - prior||^2

Prior is the triangulation estimate.
"""
...
class TikhonovRegularizedOptimizer:
    """
    CMA-ES optimizer with Tikhonov regularization on the loss function.
    """
    ...
    def __init__(
        ...
        regularization_lambda: float = 0.01,  # Regularization strength
        enable_regularization: bool = True,
        # Perturbation parameters (from baseline)
    ...
    def _generate_perturbations(self, pos_params, lb, ub):
        perturbations = []
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                  initializations, n_sources, nt_reduced, prior_params):
        ...
        def regularization_penalty(pos_params):
            if not self.enable_regularization or prior_params is None:
                return 0.0
            # Normalize by domain size for scale-invariant penalty
            diff = pos_params - prior_params
            diff[0::2] /= self.Lx  # x-coordinates
            diff[1::2] /= self.Ly  # y-coordinates
            return self.regularization_lambda * np.sum(diff**2)
        ...
        if n_sources == 1:
            def objective_coarse(xy_params):
                ...
                reg = regularization_penalty(xy_params)
                return rmse + reg

            def objective_coarse_no_reg(xy_params):
                x, y = xy_params
                n_sims[0] += 1
                q, Y_pred, rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_coarse, dt, nt_reduced, T0, sensors_xy, q_range)
        ...
        for init_params, init_type in initializations:
            ...
            while not es.stop():
                solutions = es.ask()
                # Use regularized objective for CMA-ES
                fitness = [objective_coarse(s) for s in solutions]
                es.tell(solutions, fitness)
                for sol, fit in zip(solutions, fitness):
                    # Store pure RMSE (without regularization) for ranking
                    pure_rmse = objective_coarse_no_reg(np.array(sol))
                    all_solutions.append((np.array(sol), pure_rmse, init_type))

        # Sort by pure RMSE (not regularized loss)
        all_solutions.sort(key=lambda x: x[1])

        # NM polish (without regularization)
        refined_solutions = []
        for i, (pos_params, rmse_coarse, init_type) in enumerate(all_solutions[:self.refine_top_n]):
            if self.refine_maxiter > 0:
                result = minimize(
                    objective_coarse_no_reg,  # Polish without regularization
                    pos_params,
                    method='Nelder-Mead',
                    options={
        ...
        # Perturbation (without regularization)
        if self.enable_perturbation:
            ...
            for idx in range(n_to_perturb):
                ...
                perturbations = self._generate_perturbations(pos_params, lb, ub)

                for perturbed_pos in perturbations:
                    result = minimize(
                        objective_coarse_no_reg,  # No regularization for perturbation
                        perturbed_pos,
                        method='Nelder-Mead',
                        options={
        ...
        # Evaluate on FINE grid (pure RMSE)
        candidates_raw = []
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Get triangulation prior for regularization
        tri_init = self._triangulation_init_positions(sample, meta, n_sources, q_range)
        prior_params = tri_init  # Use triangulation as prior

        if verbose:
            print(f"  Using {nt_reduced}/{nt_full} timesteps ({self.timestep_fraction*100:.0f}%)")
            print(f"  Regularization: lambda={self.regularization_lambda if self.enable_regularization else 'DISABLED'}")
        ...
        candidates_raw, n_sims = self._run_single_optimization(
            ...
            primary_inits, n_sources, nt_reduced, prior_params
        )
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims = self._run_single_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, nt_reduced, prior_params
            )
