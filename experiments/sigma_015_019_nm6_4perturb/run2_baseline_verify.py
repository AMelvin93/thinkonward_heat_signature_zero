"""
Run 2: Verify baseline config (sigma 0.15/0.19, 8 NM, 2 perturb)
"""
...
def run_experiment(config, config_name, data):
    ...
    print(f"Expected: 1.173 @ 50.4 min, Delta: {score - 1.173:+.4f}")

    return {'config_name': config_name, 'score': score, 'avg_n_cands': avg_n_cands, 'projected_400_min': projected_400}


def main():
    data = load_data()

    # Baseline config (should achieve ~1.173)
    config = {
        'sigma0_1src': 0.15,
        'sigma0_2src': 0.19,
        'max_fevals_1src': 20,
    ...
    result = run_experiment(config, 'baseline_verify', data)
