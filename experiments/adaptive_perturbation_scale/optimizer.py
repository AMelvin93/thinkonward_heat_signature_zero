"""
Adaptive Perturbation Scale Optimizer

Extends perturbed_extended_polish with adaptive perturbation scaling:
- Larger perturbations for worse solutions (stuck in poor basins)
- Smaller perturbations for better solutions (fine-tuning)

Hypothesis: Different RMSE levels need different perturbation magnitudes.
"""
...
class AdaptivePerturbationOptimizer:
    """
    Optimizer with adaptive perturbation scaling based on RMSE quality.
    """
    ...
    def __init__(
        ...
        perturbation_scale_high: float = 0.10,  # For RMSE > high_threshold
        perturbation_scale_mid: float = 0.05,   # For mid_threshold < RMSE <= high_threshold
        perturbation_scale_low: float = 0.02,   # For RMSE <= mid_threshold
        rmse_high_threshold: float = 0.30,
        rmse_mid_threshold: float = 0.15,
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.perturbation_scale_high = perturbation_scale_high
        self.perturbation_scale_mid = perturbation_scale_mid
        self.perturbation_scale_low = perturbation_scale_low
        self.rmse_high_threshold = rmse_high_threshold
        self.rmse_mid_threshold = rmse_mid_threshold
    ...
    def _get_adaptive_perturbation_scale(self, rmse):
        """Get perturbation scale based on current RMSE."""
        if rmse > self.rmse_high_threshold:
            return self.perturbation_scale_high
        elif rmse > self.rmse_mid_threshold:
            return self.perturbation_scale_mid
        else:
            return self.perturbation_scale_low

    def _generate_perturbations(self, pos_params, rmse, lb, ub):
        """Generate perturbations with adaptive scale based on RMSE."""
        perturbation_scale = self._get_adaptive_perturbation_scale(rmse)
        perturbations = []
        for _ in range(self.n_perturbations):
            direction = np.random.randn(len(pos_params))
            direction = direction / (np.linalg.norm(direction) + 1e-8)
            perturbed = pos_params + direction * perturbation_scale
            perturbed = np.clip(perturbed, lb, ub)
            perturbations.append(perturbed)
        return perturbations, perturbation_scale
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # ADAPTIVE PERTURBATION + LOCAL RESTART
        if self.enable_perturbation:
            ...
            for idx in range(n_to_perturb):
                ...
                # Generate perturbations with ADAPTIVE scale based on RMSE
                perturbations, scale_used = self._generate_perturbations(pos_params, rmse, lb, ub)
                ...
                for perturbed_pos in perturbations:
                    ...
                    if result.fun < rmse:
                        new_candidates.append((result.x, result.fun, f'perturbed_{scale_used:.2f}'))
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            ...
            print(f"  Adaptive perturbation: high={self.perturbation_scale_high}, mid={self.perturbation_scale_mid}, low={self.perturbation_scale_low}")
