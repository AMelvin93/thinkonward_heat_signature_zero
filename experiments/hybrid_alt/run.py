#!/usr/bin/env python
"""Run A11: Hybrid Alternating + Standard CMA-ES experiment."""
...
from experiments.hybrid_alt.optimizer import HybridAltOptimizer, extract_enhanced_features, N_MAX
...
def process_sample(sample, meta, config, history_1src, history_2src):
    optimizer = HybridAltOptimizer(
        ...
        n_candidates=3,
        k_similar=1,
        n_alt_rounds=config.get('n_alt_rounds', 2),
    )
...
def main():
    parser = argparse.ArgumentParser(description='Run Hybrid Alternating Optimizer')
    ...
    parser.add_argument('--n-alt-rounds', type=int, default=2)
    ...
    config = {
        ...
        'n_alt_rounds': args.n_alt_rounds,
    }
    ...
    print("=" * 70)
    print("A11: Hybrid Alternating + Standard CMA-ES")
    print("=" * 70)
    ...
    print(f"Alt rounds: {config['n_alt_rounds']}")
    print("=" * 70)
    ...
    for batch_idx in range(n_batches):
        ...
        for r in batch_results:
            if r['n_sources'] == 1:
                history_1src.append((r['features'], r['best_positions']))
            else:
                history_2src.append((r['features'], r['best_positions']))
    ...
    init_counts = {}
    for r in all_results:
        init_counts[r['best_init_type']] = init_counts.get(r['best_init_type'], 0) + 1

    if is_g4dn and not args.max_samples:
        import mlflow
        mlflow.set_tracking_uri("mlruns")
        mlflow.set_experiment("heat-signature-zero")
        run_name = f"hybrid_alt_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param("optimizer", "HybridAltOptimizer")
    ...
    for it, cnt in sorted(init_counts.items(), key=lambda x: -x[1]):
