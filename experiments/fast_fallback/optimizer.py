"""
Fast Fallback Optimizer.

Same as robust_fallback but uses only 1 init for fallback to reduce overhead.
This allows using more aggressive thresholds while staying within budget.

Primary: tri + smart (2 inits)
Fallback: cluster only (1 init) for 2-source
         centroid only (1 init) for 1-source
"""
...
class FastFallbackOptimizer:
    """
    Fast fallback optimizer with single fallback init.
    """
    ...
    def __init__(
        ...
        rmse_threshold_2src: float = 0.40,
        fallback_fevals: int = 18,  # Fewer fevals for fallback
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _cluster_init_2src(self, sample):
        """K-means clustering for 2-source problems."""
        readings = sample['Y_noisy']
        sensors = np.array(sample['sensors_xy'])
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Primary initializations: tri + smart
        primary_inits = []
        ...
        if best_rmse_initial > threshold:
            # Fast fallback: single init only
            fallback_inits = []
            if n_sources == 2:
                # Use cluster init for 2-source
                cluster_init = self._cluster_init_2src(sample)
                fallback_inits.append((cluster_init, 'cluster'))
            else:
                # Use centroid init for 1-source
                centroid_init = self._weighted_centroid_init(sample, n_sources)
                fallback_inits.append((centroid_init, 'centroid'))

            # Use fewer fevals for fallback
            fallback_candidates, fallback_sims = self._run_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, early_frac, self.fallback_fevals
            )
