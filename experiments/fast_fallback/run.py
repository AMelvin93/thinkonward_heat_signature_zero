#!/usr/bin/env python3
"""
Run script for Fast Fallback optimizer.
"""
...
from optimizer import FastFallbackOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = FastFallbackOptimizer(
        ...
        rmse_threshold_2src=config.get('rmse_threshold_2src', 0.40),
        fallback_fevals=config.get('fallback_fevals', 18),
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Fast Fallback optimizer')
    ...
    parser.add_argument('--threshold-2src', type=float, default=0.40)
