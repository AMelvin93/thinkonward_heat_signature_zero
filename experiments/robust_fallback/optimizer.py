"""
Robust Optimizer with Fallback Strategy.

Key idea: If CMA-ES gets stuck in a poor local minimum (high RMSE),
automatically restart with a different initialization.

This should eliminate outliers that significantly hurt the score.
"""
...
class RobustFallbackOptimizer:
    """
    Robust optimizer with automatic fallback on bad results.

    If the best RMSE exceeds a threshold, re-runs optimization
    ...
    def __init__(
        ...
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.4,  # Retry if RMSE > this
        rmse_threshold_2src: float = 0.5,  # Retry if RMSE > this
    ):
        self.Lx = Lx
        self.Ly = Ly
