...
Based on the baseline temporal fidelity optimizer, but with multiple NM polish
runs from perturbed starting points to potentially escape local minima.

Hypothesis: NM may converge to different local minima from different starting
points. Running multiple NM polishes from perturbed positions may find better
final solution.
"""
...
class MultiRestartNMPolishOptimizer:
    """
    Multi-restart NM polish optimizer.

    Key change from baseline: After CMA-ES, run multiple NM polishes from
    perturbed starting points and select the best result.
    """
    ...
    def __init__(
        ...
        # NEW: Multi-restart parameters
        final_polish_maxiter: int = 5,  # NM iterations per restart
        n_polish_restarts: int = 3,  # Number of NM restarts
        perturbation_scale: float = 0.05,  # Relative perturbation (5% of domain)
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.n_polish_restarts = n_polish_restarts
    ...
    def _perturb_position(self, pos_params, lb, ub):
        """Add random perturbation to position parameters."""
        n_params = len(pos_params)
        perturbed = pos_params.copy()
        for i in range(n_params):
            scale = (ub[i] - lb[i]) * self.perturbation_scale
            perturbed[i] += np.random.randn() * scale
            perturbed[i] = np.clip(perturbed[i], lb[i], ub[i])
        return perturbed

    def _multi_restart_polish(self, best_pos_params, objective_fine_full, n_sources, lb, ub, n_sims):
        """Run multiple NM polishes from perturbed starting points."""
        all_results = []

        # Run from original position
        result = minimize(
            objective_fine_full,
            best_pos_params,
            method='Nelder-Mead',
        ...
        all_results.append((result.x, result.fun, 'original'))

        # Run from perturbed positions
        for i in range(self.n_polish_restarts - 1):
            perturbed = self._perturb_position(best_pos_params, lb, ub)
            result = minimize(
                objective_fine_full,
                perturbed,
                method='Nelder-Mead',
            ...
            all_results.append((result.x, result.fun, f'perturbed_{i+1}'))

        # Select best result
        best_idx = min(range(len(all_results)), key=lambda i: all_results[i][1])
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run CMA-ES optimization with multi-restart NM polish."""
        ...
        for pos_params, rmse_coarse, init_type in refined_solutions:
            if n_sources == 1:
                x, y = pos_params
                q, Y_pred, final_rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed, solver_fine, dt, nt_full, T0, sensors_xy, q_range)
                n_sims[0] += 1
        ...
        # MULTI-RESTART NM POLISH on best candidate
        if self.final_polish_maxiter > 0 and self.n_polish_restarts > 0 and candidates_raw:
            ...
            # Run multi-restart polish
            polished_pos, polished_rmse, polish_type = self._multi_restart_polish(
                best_pos_params, objective_fine_full, n_sources, lb, ub, n_sims
            )
            ...
            if polished_rmse < best_rmse:
                ...
                candidates_raw[best_idx] = (sources, full_params, final_rmse, f'polished_{polish_type}', polished_pos)
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Run optimization with multi-restart polish
        ...
        # Format output
        results = []
        for sources, full_params, rmse, init_type in final_candidates:
            results.append(CandidateResult(full_params, rmse, init_type, n_sims))

        best_rmse = results[0].rmse if results else float('inf')
        candidates_output = [(list(c[0]), c[2]) for c in final_candidates]

        if verbose:
            print(f"  Sources: {n_sources}, Best RMSE: {best_rmse:.4f}, Sims: {n_sims}")

        return candidates_output, best_rmse, results, n_sims
