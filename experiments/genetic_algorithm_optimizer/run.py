#!/usr/bin/env python3
"""
Run script for Genetic Algorithm optimizer.

Tests if a simpler GA can match CMA-ES performance.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run GA optimizer')
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Genetic Algorithm ({args.timestep_fraction:.0%} timesteps)")
    print(f"{'='*70}")
    ...
    if not args.no_mlflow:
        ...
        run_name = args.run_name or f"ga_optimizer_run{args.tuning_run}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_GENETIC_ALGORITHM_001")
            mlflow.log_param("worker", "W1")
            mlflow.log_param("tuning_run", args.tuning_run)
            mlflow.log_param("platform", "wsl")
            mlflow.log_param("optimizer", "genetic_algorithm")
            ...
            mlflow.log_param("mutation_rate", args.mutation_rate)
            mlflow.log_param("crossover_rate", args.crossover_rate)
