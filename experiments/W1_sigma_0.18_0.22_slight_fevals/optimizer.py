"""
EXP031: Sigma 0.18/0.22 with Slight Fevals Reduction optimizer.

Key idea: Less aggressive fevals reduction than EXP028 to preserve more score.
EXP028 (16/30 fevals) gave 1.1171 @ 58.1 min - hurt score too much.
This tries 18/33 fevals - less aggressive reduction.
...
class Sigma018_022SlightFevalsOptimizer:
    """
    EXP031: Sigma 0.18/0.22 with slight fevals reduction.

    Less aggressive than EXP028 (16/30 -> 18/33) to preserve more score.
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        max_fevals_1src: int = 18,
        max_fevals_2src: int = 33,
        ...
        refine_top_n: int = 2,
        rmse_threshold_1src: float = 0.35,  # STANDARD
        rmse_threshold_2src: float = 0.45,  # STANDARD
        fallback_fevals: int = 16,  # Reduced fallback fevals
    ):
        self.Lx = Lx
        self.Ly = Ly
    ...
    def _weighted_centroid_init(self, sample, n_sources):
        ...
        if n_sources == 1:
            ...
            # For 2-source, offset the centroid in two directions
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        # Use override fevals for fallback, otherwise use primary
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if best_rmse_initial > threshold:
            # Fallback: try alternative initializations with reduced fevals
            fallback_inits = []
