#!/usr/bin/env python3
"""
Run script for W1 Sigma 0.18/0.22 Slight Fevals optimizer.

EXP031: Less aggressive fevals reduction than EXP028 to preserve more score.

Config:
- sigma0_1src: 0.18
- sigma0_2src: 0.22
- threshold_1src: 0.35 (STANDARD - NEVER CHANGE)
- threshold_2src: 0.45 (STANDARD - NEVER CHANGE)
- fevals_1src: 18 (reduced from 20, ~10% reduction)
- fevals_2src: 33 (reduced from 36, ~8% reduction)
- fallback_fevals: 16 (reduced from 18)

Goal: Achieve score > 1.1247 (baseline) within 60 min budget.
Reference: EXP028 got 1.1171 @ 58.1 min with 16/30 fevals - too aggressive.
"""
...
from optimizer import Sigma018_022SlightFevalsOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = Sigma018_022SlightFevalsOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Sigma 0.18/0.22 Slight Fevals (EXP031)')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--max-fevals-1src', type=int, default=18)  # Reduced from 20
    parser.add_argument('--max-fevals-2src', type=int, default=33)  # Reduced from 36
    parser.add_argument('--sigma0-1src', type=float, default=0.18)  # Sigma 0.18
    parser.add_argument('--sigma0-2src', type=float, default=0.22)  # Sigma 0.22
    ...
    parser.add_argument('--fallback-fevals', type=int, default=16)  # Reduced from 18
    ...
    print(f"\nW1 Sigma 0.18/0.22 Slight Fevals (EXP031)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Sigma 0.18/0.22 Slight Fevals (EXP031)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline:      1.1247 @ 57.2 min (sigma 0.15/0.20, thresh 0.35/0.45, fevals 20/36)")
    print(f"EXP028 ref:    1.1171 @ 58.1 min (sigma 0.18/0.22, thresh 0.35/0.45, fevals 16/30)")
    print(f"This run:      {score:.4f} @ {projected_400:.1f} min (sigma 0.18/0.22, thresh 0.35/0.45, fevals 18/33)")
    print(f"Delta vs baseline: {score - 1.1247:+.4f} score, {projected_400 - 57.2:+.1f} min")
    print(f"Delta vs EXP028:   {score - 1.1171:+.4f} score, {projected_400 - 58.1:+.1f} min")
    print()
    if projected_400 <= 60:
        if score > 1.1247:
            print("SUCCESS! In budget with improved score!")
        elif score > 1.12:
            print("PROMISING! Good score, in budget!")
        else:
            print("IN BUDGET but lower score")
    else:
        print("OVER BUDGET")
    print(f"{'='*70}\n")
