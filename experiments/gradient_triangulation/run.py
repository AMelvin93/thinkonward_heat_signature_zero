#!/usr/bin/env python
"""Run A14: Gradient-Based Triangulation experiment."""
...
from experiments.gradient_triangulation.optimizer import GradientTriangulationOptimizer, N_MAX
...
def process_sample(sample, meta, config):
    optimizer = GradientTriangulationOptimizer(
        max_fevals_1src=config['max_fevals_1src'],
        max_fevals_2src=config['max_fevals_2src'],
        sigma0_1src=config.get('sigma0_1src', 0.15),
        sigma0_2src=config.get('sigma0_2src', 0.20),
        use_onset_triangulation=config.get('use_onset_triangulation', True),
        use_gradient_triangulation=config.get('use_gradient_triangulation', True),
        n_candidates=3,
    )
...
def main():
    parser = argparse.ArgumentParser(description='Run Gradient Triangulation Optimizer')
    ...
    parser.add_argument('--no-onset-triangulation', action='store_true')
    parser.add_argument('--no-gradient-triangulation', action='store_true')
    ...
    config = {
        ...
        'use_onset_triangulation': not args.no_onset_triangulation,
        'use_gradient_triangulation': not args.no_gradient_triangulation,
    }
    ...
    print("=" * 70)
    print("A14: GRADIENT-BASED TRIANGULATION OPTIMIZER")
    print("=" * 70)
    ...
    print(f"Onset triangulation: {config['use_onset_triangulation']}")
    print(f"Gradient triangulation: {config['use_gradient_triangulation']}")
    print()
    print("KEY INNOVATION: Temperature gradient direction for triangulation")
    print("  - Gradient points AWAY from source")
    print("  - Trace rays in -gradient direction to find source")
    print("  - Angle-of-Arrival (AOA) localization for thermal fields")
    print("=" * 70)
    ...
    if is_g4dn and not args.max_samples:
        ...
        run_name = f"gradient_triangulation_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param("optimizer", "GradientTriangulationOptimizer")
