"""
Perturbation with 40% Temporal Fidelity Optimizer

Combines two successful approaches:
1. W2's 40% temporal config (1.1688): timestep_fraction=0.40, sigma0_1src=0.18, sigma0_2src=0.22
2. Perturbed local restart (1.1452): perturb_top_n=1, n_perturbations=2, perturbation_scale=0.05

Hypothesis: If perturbation improved score by +0.0079 on simpler config (25% temporal),
applying it to W2's better config (40% temporal) could yield even higher scores.
"""
...
class PerturbationTemporalOptimizer:
    """
    Combines 40% temporal fidelity with perturbation + local restart.
    """
    ...
    def __init__(
        ...
        # W2's optimal 40% temporal fidelity
        timestep_fraction: float = 0.40,
        # Perturbation parameters (from successful perturbed_local_restart)
        enable_perturbation: bool = True,
        perturb_top_n: int = 1,  # Only perturb best candidate
        n_perturbations: int = 2,  # 2 perturbations per candidate
        perturbation_scale: float = 0.05,  # 5% of parameter range
        perturb_nm_iters: int = 3,  # Quick NM for perturbed points
    ):
        self.Lx = Lx
        self.Ly = Ly
