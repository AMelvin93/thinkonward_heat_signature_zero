#!/usr/bin/env python
"""
Run Coarse-to-Fine Optimizer experiment.
...
Usage:
    # Test with default 12/30 fevals (higher 2-src due to coarse speedup)
    uv run python experiments/coarse_to_fine/run.py --workers 7 --shuffle

    # Test with very high fevals
...
from experiments.coarse_to_fine.optimizer import (
...
def process_sample(sample, meta, config, history_1src, history_2src):
    """Process a single sample with coarse-to-fine optimizer."""
...
def main():
    parser = argparse.ArgumentParser(description='Run Coarse-to-Fine Optimizer')
    parser.add_argument('--workers', type=int, default=7,
                        help='Number of workers (7 for G4dn, -1 for all CPUs)')
    parser.add_argument('--batch-size', type=int, default=20,
                        help='Samples per batch for transfer learning')
    # Default fevals: 12 for 1-src, 30 for 2-src (higher due to coarse speedup)
    parser.add_argument('--max-fevals-1src', type=int, default=12,
                        help='Max CMA-ES evaluations for 1-source (coarse phase)')
    parser.add_argument('--max-fevals-2src', type=int, default=30,
                        help='Max CMA-ES evaluations for 2-source (coarse phase)')
    parser.add_argument('--polish-fevals-1src', type=int, default=3,
                        help='Polish iterations for 1-source (fine phase)')
    parser.add_argument('--polish-fevals-2src', type=int, default=5,
                        help='Polish iterations for 2-source (fine phase)')
    ...
    parser.add_argument('--pool-size', type=int, default=5,
                        help='Number of top solutions to consider for polish')
    ...
    parser.add_argument('--polish-mode', type=str, default='eval_only',
                        choices=['eval_only', 'light_cmaes'],
                        help='Polish mode: eval_only (fast) or light_cmaes (slower but more accurate)')
    ...
    # Print header
    print("=" * 70)
    print("COARSE-TO-FINE OPTIMIZER")
    print("=" * 70)
    ...
    print()
    print("KEY INNOVATION: Coarse-to-Fine Grid")
    print("  - CMA-ES exploration on COARSE grid (50x25) - ~4x faster")
    print("  - Final polish on FINE grid (100x50) - maintains accuracy")
    print("  - Enables 30+ 2-source fevals within budget")
    print()
    print(f"Config:")
    print(f"  max_fevals: 1-src={config['max_fevals_1src']}, 2-src={config['max_fevals_2src']} (coarse)")
    print(f"  polish_fevals: 1-src={config['polish_fevals_1src']}, 2-src={config['polish_fevals_2src']} (fine)")
    print(f"  polish_mode: {config['polish_mode']}")
    ...
    if is_g4dn_simulation:
        ...
        run_name = f"coarse_to_fine_{config['max_fevals_1src']}_{config['max_fevals_2src']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            mlflow.log_param("optimizer", "CoarseToFineOptimizer")
            ...
            mlflow.log_param("polish_mode", config['polish_mode'])
    ...
    print("RESULTS")
    print("=" * 70)
    print(f"Config:           {config['max_fevals_1src']}/{config['max_fevals_2src']} coarse + {config['polish_fevals_1src']}/{config['polish_fevals_2src']} polish")
    ...
    print(f"Coarse-to-Fine ({config['max_fevals_1src']}/{config['max_fevals_2src']}+{config['polish_fevals_2src']}):  Score {final_score:.4f} @ {projected_400:.1f} min")
    print()
