#!/usr/bin/env python3
"""
Run script for W2 Sigma 0.20/0.25 + Aggressive Thresholds optimizer.

EXP025: sigma_0.20_0.25_aggressive_thresh
Config:
- sigma0_1src: 0.20 (best sigma for score)
- sigma0_2src: 0.25 (best sigma for score)
- threshold_1src: 0.40 (aggressive - reduce time)
- threshold_2src: 0.50 (aggressive - reduce time)
- fallback_fevals: 18

Hypothesis: Best sigma (0.20/0.25) with aggressive thresholds to reduce time from 69.2 min toward 60 min.
Based on: EXP002 got best score 1.1362 but 69.2 min. Try saving 9+ min with higher thresholds.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run W2 Sigma 0.20/0.25 + Aggressive Thresholds')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    parser.add_argument('--seed', type=int, default=42)
    # Key experiment parameters - EXP025 specific
    ...
    parser.add_argument('--threshold-1src', type=float, default=0.40)  # Aggressive
    parser.add_argument('--threshold-2src', type=float, default=0.50)  # Aggressive
    ...
    print(f"\n[W2] Sigma 0.20/0.25 + Aggressive Thresholds Optimizer")
    print(f"=" * 70)
    print(f"EXP025: sigma_0.20_0.25_aggressive_thresh")
    print(f"-" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Sigma: 1-src={args.sigma0_1src}, 2-src={args.sigma0_2src} (BEST)")
    print(f"Threshold: 1-src={args.threshold_1src}, 2-src={args.threshold_2src} (AGGRESSIVE)")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: [W2] Sigma 0.20/0.25 + Aggressive Thresholds (EXP025)")
    print(f"{'='*70}")
    ...
    print()
    print(f"Config: sigma={args.sigma0_1src}/{args.sigma0_2src} (BEST), thresh={args.threshold_1src}/{args.threshold_2src} (AGGRESSIVE)")
    print(f"Baseline: 1.1247 @ 57.2 min (sigma 0.15/0.20)")
    print(f"EXP002 (sigma 0.20/0.25, thresh 0.35/0.45): 1.1362 @ 69.2 min")
    print(f"Target: ~60 min with score near EXP002")
