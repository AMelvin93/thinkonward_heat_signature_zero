"""
Cluster-Smart Optimizer.

Key idea: Use cluster init instead of smart init for 2-source problems.
K-means clustering on sensor temperatures should give better 2-source initialization
than just picking the two hottest sensors.

- 1-source: triangulation + smart (normal)
- 2-source: triangulation + cluster (replace smart with cluster)

This focuses 2-source fevals on more informative initializations.
"""
...
class ClusterSmartOptimizer:
    """
    Use cluster init for 2-source problems instead of smart init.
    """
    ...
    def _cluster_init_2src(self, sample):
        """
        K-means clustering for 2-source initialization.
        """
        readings = sample['Y_noisy']
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # For 2-source: use cluster init instead of smart init
        if n_sources == 2:
            cluster_init = self._cluster_init_2src(sample)
            primary_inits.append((cluster_init, 'cluster'))
        else:
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_inits = []
            if n_sources == 2:
                # For 2-source fallback, use smart init (we used cluster primary)
                smart_init = self._smart_init_positions(sample, n_sources)
                fallback_inits.append((smart_init, 'smart'))
            else:
                # For 1-source fallback, use centroid init
