...
Strategy:
    1. Exploration: dt_coarse = dt * factor, nt_coarse = nt // factor
    2. Compare at subsampled observed timesteps
    3. Polish at full resolution (original dt, nt)
...
class TimestepSubsamplingOptimizer:
    """
    Timestep Subsampling optimizer with smart init selection.

    Strategy:
        1. Init selection: Evaluate all inits at SUBSAMPLED timesteps (fast)
        2. CMA-ES exploration: Run at SUBSAMPLED timesteps (2-4x faster)
        3. Fine polish: Full resolution for final accuracy

    Key Insight: Heat2D time is dominated by timesteps, not grid size.
    Using dt*factor and nt/factor gives genuine 2-4x speedup.
    """
    ...
    def __init__(
        ...
        nx: int = 100,
        ny: int = 50,
        subsample_factor: int = 2,  # 2 = 2x faster, 4 = 4x faster
        max_fevals_1src: int = 12,
        max_fevals_2src: int = 28,  # Higher due to subsample speedup
    ...
    def _evaluate_init_subsampled(self, init_params: np.ndarray, n_sources: int,
                                   Y_observed_sub: np.ndarray, solver: Heat2D,
                                   dt_sub: float, nt_sub: int, T0: float,
                                   sensors_xy: np.ndarray, q_range: Tuple[float, float]) -> float:
        """Quickly evaluate an init at SUBSAMPLED timesteps."""
        if n_sources == 1:
            x, y = init_params
            _, _, rmse = compute_optimal_intensity_1src(
                x, y, Y_observed_sub, solver, dt_sub, nt_sub, T0, sensors_xy, q_range)
        else:
            x1, y1, x2, y2 = init_params
            _, _, rmse = compute_optimal_intensity_2src(
                x1, y1, x2, y2, Y_observed_sub, solver, dt_sub, nt_sub, T0, sensors_xy, q_range)
        return rmse
    ...
    def estimate_sources(
        ...
        """
        Estimate sources using timestep subsampling strategy.

        Phase 1: Init selection at SUBSAMPLED timesteps (fast)
        Phase 2: CMA-ES exploration at SUBSAMPLED timesteps (2-4x faster)
        Phase 3: Polish top solutions at FULL resolution (accurate)
        """
        ...
        # Create solver (same grid, different timesteps)
        solver = self._create_solver(kappa, bc)

        # Subsampled parameters
        factor = self.subsample_factor
        dt_sub = dt * factor  # Larger timestep
        nt_sub = nt // factor  # Fewer timesteps

        # Subsample observed data to match
        # Original Y_observed has shape (nt+1, n_sensors)
        # We need indices [0, factor, 2*factor, ...] up to nt_sub+1 points
        subsample_indices = np.arange(0, nt + 1, factor)[:nt_sub + 1]
        Y_observed_sub = Y_observed[subsample_indices]

        if verbose:
            print(f"  Subsampling: dt={dt:.4f}→{dt_sub:.4f}, nt={nt}→{nt_sub} ({factor}x speedup)")
        ...
        # === PHASE 1: INIT SELECTION AT SUBSAMPLED TIMESTEPS ===
        if verbose:
            print(f"  Phase 1: Init selection (subsampled {factor}x)")
        ...
        # Evaluate each init at SUBSAMPLED timesteps (fast!)
        init_evaluations = []
        n_sub_sims = 0
        for init_params, init_type in initializations:
            rmse = self._evaluate_init_subsampled(
                init_params, n_sources, Y_observed_sub, solver,
                dt_sub, nt_sub, T0, sensors_xy, q_range
            )
            init_evaluations.append((init_params, init_type, rmse))
            n_sub_sims += (1 if n_sources == 1 else 2)
        ...
        if verbose:
            print(f"    Evaluated {len(initializations)} inits ({n_sub_sims} subsampled sims)")
            for params, itype, rmse in init_evaluations:
                marker = " <-- BEST" if itype == best_init_type else ""
                print(f"      {itype}: RMSE={rmse:.4f}{marker}")

        # === PHASE 2: CMA-ES AT SUBSAMPLED TIMESTEPS (FAST EXPLORATION) ===
        if verbose:
            print(f"  Phase 2: CMA-ES exploration (subsampled {factor}x)")
        ...
        if n_sources == 1:
            def objective_subsampled(xy_params):
                x, y = xy_params
                n_cmaes_sims[0] += 1
                _, _, rmse = compute_optimal_intensity_1src(
                    x, y, Y_observed_sub, solver, dt_sub, nt_sub, T0, sensors_xy, q_range)
        ...
        # Run CMA-ES at subsampled resolution
        opts = cma.CMAOptions()
        ...
        while not es.stop():
            solutions = es.ask()
            fitness = [objective_subsampled(s) for s in solutions]
        ...
        if verbose:
            print(f"    CMA-ES completed: {n_cmaes_sims[0]} subsampled sims, best RMSE={es.result.fbest:.4f}")

        # === PHASE 3: POLISH TOP SOLUTIONS AT FULL RESOLUTION ===
        if verbose:
            print(f"  Phase 3: Full resolution polish")

        polish_fevals = self.polish_fevals_1src if n_sources == 1 else self.polish_fevals_2src

        # Sort by subsampled RMSE and take top solutions for polishing
        ...
        # Polish each top solution with full-resolution CMA-ES
        polished_solutions = []
        for pos_params, sub_rmse, init_type in top_solutions:
            # Quick polish with full-resolution CMA-ES
            ...
            while not es_polish.stop():
                ...
                fitness = [objective_full(s) for s in solutions]
        ...
        if verbose:
            print(f"    Polish completed: {n_full_sims[0]} full-resolution sims")
        ...
        total_sims = n_sub_sims + n_cmaes_sims[0] + n_full_sims[0]
        ...
        if verbose:
            print(f"  Total: {total_sims} sims ({n_sub_sims} init + {n_cmaes_sims[0]} CMA-ES + {n_full_sims[0]} polish)")
            print(f"  Best RMSE (full-res): {best_rmse:.4f}")
