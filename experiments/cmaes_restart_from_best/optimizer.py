"""
Two-Phase CMA-ES Restart Optimizer

Key idea: Run CMA-ES in two phases:
1. Phase 1: Broad exploration with large sigma (0.25/0.30)
2. Phase 2: Local refinement starting from best with small sigma (0.05/0.08)

This differs from IPOP/BIPOP which increase population size on restart.
Here we keep same population but change sigma.
"""
...
class TwoPhaseRestartOptimizer:
    """
    Optimizer that runs CMA-ES in two phases:
    1. Phase 1: Broad exploration with large sigma
    2. Phase 2: Local refinement with small sigma starting from best
    """
    ...
    def __init__(
        ...
        nx_coarse: int = 50,
        ny_coarse: int = 25,
        # Phase 1 parameters (broad exploration)
        phase1_fevals_1src: int = 12,  # 60% of total fevals
        phase1_fevals_2src: int = 22,
        phase1_sigma_1src: float = 0.25,
        phase1_sigma_2src: float = 0.30,
        # Phase 2 parameters (local refinement)
        phase2_fevals_1src: int = 8,  # 40% of total fevals
        phase2_fevals_2src: int = 14,
        phase2_sigma_1src: float = 0.05,
        phase2_sigma_2src: float = 0.08,
        # Shared parameters
        use_triangulation: bool = True,
        n_candidates: int = N_MAX,
        candidate_pool_size: int = 10,
        refine_maxiter: int = 8,  # NM polish iterations
        refine_top_n: int = 3,
        rmse_threshold_1src: float = 0.4,
        rmse_threshold_2src: float = 0.5,
        timestep_fraction: float = 0.40,  # 40% timesteps for CMA-ES
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.phase1_fevals_1src = phase1_fevals_1src
        self.phase1_fevals_2src = phase1_fevals_2src
        self.phase1_sigma_1src = phase1_sigma_1src
        self.phase1_sigma_2src = phase1_sigma_2src
        self.phase2_fevals_1src = phase2_fevals_1src
        self.phase2_fevals_2src = phase2_fevals_2src
        self.phase2_sigma_1src = phase2_sigma_1src
        self.phase2_sigma_2src = phase2_sigma_2src
    ...
    def _run_two_phase_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
                                     initializations, n_sources, nt_cmaes, nt_full):
        """
        Run two-phase CMA-ES optimization:
        Phase 1: Broad exploration with large sigma
        Phase 2: Local refinement from best with small sigma
        """
        ...
        # Phase parameters
        if n_sources == 1:
            phase1_fevals = self.phase1_fevals_1src
            phase1_sigma = self.phase1_sigma_1src
            phase2_fevals = self.phase2_fevals_1src
            phase2_sigma = self.phase2_sigma_1src
        else:
            phase1_fevals = self.phase1_fevals_2src
            phase1_sigma = self.phase1_sigma_2src
            phase2_fevals = self.phase2_fevals_2src
            phase2_sigma = self.phase2_sigma_2src

        lb, ub = self._get_position_bounds(n_sources)
        phase1_fevals_per_init = max(3, phase1_fevals // len(initializations))

        # Objective function using coarse solver with reduced timesteps
        if n_sources == 1:
        ...
        all_solutions_phase1 = []

        for init_params, init_type in initializations:
            opts = cma.CMAOptions()
            opts['maxfevals'] = phase1_fevals_per_init
            opts['bounds'] = [lb, ub]
            opts['verbose'] = -9
            opts['tolfun'] = 1e-6
            opts['tolx'] = 1e-6

            es = cma.CMAEvolutionStrategy(init_params.tolist(), phase1_sigma, opts)
            ...
            while not es.stop():
                ...
                for sol, fit in zip(solutions, fitness):
                    all_solutions_phase1.append((np.array(sol), fit, init_type))

        # Sort by fitness and get best solution from phase 1
        all_solutions_phase1.sort(key=lambda x: x[1])
        best_phase1 = all_solutions_phase1[0]

        # ==================== PHASE 2: Local Refinement ====================
        # Restart from best solution with small sigma
        all_solutions_phase2 = []

        opts = cma.CMAOptions()
        opts['maxfevals'] = phase2_fevals
        opts['bounds'] = [lb, ub]
        opts['verbose'] = -9
        opts['tolfun'] = 1e-6
        opts['tolx'] = 1e-6

        es = cma.CMAEvolutionStrategy(best_phase1[0].tolist(), phase2_sigma, opts)
        ...
        while not es.stop():
            ...
            es.tell(solutions, fitness)
            for sol, fit in zip(solutions, fitness):
                all_solutions_phase2.append((np.array(sol), fit, 'phase2'))

        # Combine solutions from both phases, keeping diverse candidates
        all_solutions = all_solutions_phase1 + all_solutions_phase2
        all_solutions.sort(key=lambda x: x[1])

        # NM Polish using FULL timesteps
        if n_sources == 1:
        ...
        for i, (pos_params, rmse_cmaes, init_type) in enumerate(all_solutions[:self.refine_top_n]):
        ...
        for pos_params, rmse_cmaes, init_type in all_solutions[self.refine_top_n:self.candidate_pool_size]:
            full_rmse = objective_full(pos_params)
            refined_solutions.append((pos_params, full_rmse, init_type))
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        # Compute reduced timesteps for CMA-ES
        nt_cmaes = max(10, int(nt_full * self.timestep_fraction))

        if verbose:
            print(f"  CMA-ES: {nt_cmaes}/{nt_full} ({self.timestep_fraction*100:.0f}%)")
            print(f"  Phase 1 sigma: {self.phase1_sigma_1src if n_sources==1 else self.phase1_sigma_2src}")
            print(f"  Phase 2 sigma: {self.phase2_sigma_1src if n_sources==1 else self.phase2_sigma_2src}")
        ...
        candidates_raw, n_sims = self._run_two_phase_optimization(
            sample, meta, q_range, solver_coarse, solver_fine,
            primary_inits, n_sources, nt_cmaes, nt_full
        )
        ...
        if best_rmse_initial > threshold:
            ...
            fallback_candidates, fallback_sims = self._run_two_phase_optimization(
                sample, meta, q_range, solver_coarse, solver_fine,
                fallback_inits, n_sources, nt_cmaes, nt_full
            )
