#!/usr/bin/env python3
"""
Run script for Two-Phase CMA-ES Restart optimizer.

Tests the hypothesis that two-phase CMA-ES (broad exploration then local refinement)
can improve accuracy over single-phase optimization.
"""
...
from optimizer import TwoPhaseRestartOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = TwoPhaseRestartOptimizer(
        phase1_fevals_1src=config['phase1_fevals_1src'],
        phase1_fevals_2src=config['phase1_fevals_2src'],
        phase1_sigma_1src=config['phase1_sigma_1src'],
        phase1_sigma_2src=config['phase1_sigma_2src'],
        phase2_fevals_1src=config['phase2_fevals_1src'],
        phase2_fevals_2src=config['phase2_fevals_2src'],
        phase2_sigma_1src=config['phase2_sigma_1src'],
        phase2_sigma_2src=config['phase2_sigma_2src'],
...
def main():
    parser = argparse.ArgumentParser(description='Run Two-Phase CMA-ES Restart optimizer')
    parser.add_argument('--workers', type=int, default=7)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--shuffle', action='store_true')
    # Phase 1 parameters (broad exploration)
    parser.add_argument('--phase1-fevals-1src', type=int, default=12)
    parser.add_argument('--phase1-fevals-2src', type=int, default=22)
    parser.add_argument('--phase1-sigma-1src', type=float, default=0.25)
    parser.add_argument('--phase1-sigma-2src', type=float, default=0.30)
    # Phase 2 parameters (local refinement)
    parser.add_argument('--phase2-fevals-1src', type=int, default=8)
    parser.add_argument('--phase2-fevals-2src', type=int, default=14)
    parser.add_argument('--phase2-sigma-1src', type=float, default=0.05)
    parser.add_argument('--phase2-sigma-2src', type=float, default=0.08)
    # Shared parameters
    ...
    total_fevals_1src = args.phase1_fevals_1src + args.phase2_fevals_1src
    total_fevals_2src = args.phase1_fevals_2src + args.phase2_fevals_2src

    print(f"\nTwo-Phase CMA-ES Restart Optimizer")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"Phase 1 (exploration): fevals={args.phase1_fevals_1src}/{args.phase1_fevals_2src}, sigma={args.phase1_sigma_1src}/{args.phase1_sigma_2src}")
    print(f"Phase 2 (refinement):  fevals={args.phase2_fevals_1src}/{args.phase2_fevals_2src}, sigma={args.phase2_sigma_1src}/{args.phase2_sigma_2src}")
    print(f"Total fevals: {total_fevals_1src}/{total_fevals_2src}")
    print(f"CMA-ES timesteps: {args.timestep_fraction:.0%}")
    print(f"NM Polish: {args.refine_maxiter} iters on top-{args.refine_top}")
    print(f"=" * 70)

    config = {
        'phase1_fevals_1src': args.phase1_fevals_1src,
        'phase1_fevals_2src': args.phase1_fevals_2src,
        'phase1_sigma_1src': args.phase1_sigma_1src,
        'phase1_sigma_2src': args.phase1_sigma_2src,
        'phase2_fevals_1src': args.phase2_fevals_1src,
        'phase2_fevals_2src': args.phase2_fevals_2src,
        'phase2_sigma_1src': args.phase2_sigma_1src,
        'phase2_sigma_2src': args.phase2_sigma_2src,
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Two-Phase CMA-ES Restart")
    print(f"{'='*70}")
    ...
    run_name = args.mlflow_run_name or f"twophase_restart_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    with mlflow.start_run(run_name=run_name) as run:
        mlflow.log_param("experiment_id", "EXP_CMAES_RESTART_BEST_001")
        mlflow.log_param("worker", "W1")
        mlflow.log_param("algorithm", "twophase_cmaes_restart")
        mlflow.log_param("platform", "wsl")
        mlflow.log_param("phase1_fevals_1src", args.phase1_fevals_1src)
        mlflow.log_param("phase1_fevals_2src", args.phase1_fevals_2src)
        mlflow.log_param("phase1_sigma_1src", args.phase1_sigma_1src)
        mlflow.log_param("phase1_sigma_2src", args.phase1_sigma_2src)
        mlflow.log_param("phase2_fevals_1src", args.phase2_fevals_1src)
        mlflow.log_param("phase2_fevals_2src", args.phase2_fevals_2src)
        mlflow.log_param("phase2_sigma_1src", args.phase2_sigma_1src)
        mlflow.log_param("phase2_sigma_2src", args.phase2_sigma_2src)
