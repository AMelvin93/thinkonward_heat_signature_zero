#!/usr/bin/env python3
"""
Run script for W1 Baseline Reduced Fevals V2 optimizer.

EXP026: Baseline sigma with reduced fevals and slightly lower thresholds.
Goal: Stay in budget (<60 min) while potentially gaining some score via lower thresholds.

Config:
- sigma0_1src: 0.15 (baseline)
- sigma0_2src: 0.20 (baseline)
- threshold_1src: 0.33 (slightly lower than 0.35)
- threshold_2src: 0.43 (slightly lower than 0.45)
- fevals_1src: 18 (reduced from 20)
- fevals_2src: 32 (reduced from 36)
...
from optimizer import BaselineReducedFevalsOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = BaselineReducedFevalsOptimizer(
...
def main():
    parser = argparse.ArgumentParser(description='Run W1 Baseline Reduced Fevals V2 (EXP026)')
    ...
    parser.add_argument('--sigma0-1src', type=float, default=0.15)  # Baseline
    parser.add_argument('--sigma0-2src', type=float, default=0.20)  # Baseline
    parser.add_argument('--threshold-1src', type=float, default=0.33)  # Slightly lower
    parser.add_argument('--threshold-2src', type=float, default=0.43)  # Slightly lower
    ...
    print(f"\nW1 Baseline Reduced Fevals V2 (EXP026)")
    print(f"=" * 60)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: W1 Baseline Reduced Fevals V2 (EXP026)")
    print(f"{'='*70}")
    ...
    print(f"This run:      {score:.4f} @ {projected_400:.1f} min (sigma 0.15/0.20, thresh 0.33/0.43, fevals 18/32)")
    ...
    if projected_400 <= 60:
        if score > 1.1247:
            ...
            print("IN BUDGET but no score improvement")
    else:
        print("OVER BUDGET")
    print(f"{'='*70}\n")
