#!/usr/bin/env python
"""
Run ICA Decomposition Optimizer experiment.

Key Innovation: Uses ICA to decompose 2-source sensor signals and extract
position estimates, providing much better initialization for the main bottleneck.

Expected Benefits:
- Better 2-source initialization (targets main RMSE bottleneck)
- Millisecond execution (signal processing vs optimization)
- Adds to innovation score (novel approach)
- Combined with analytical intensity for optimal results

Usage:
    # G4dn simulation (7 workers, MLflow logging)
    uv run python experiments/ica_decomposition/run.py --workers 7 --shuffle

    # Prototype mode (all CPUs)
    uv run python experiments/ica_decomposition/run.py --workers -1 --shuffle
"""
...
from experiments.ica_decomposition.optimizer import (
    ICADecompositionOptimizer,
...
def process_sample(sample, meta, config, history_1src, history_2src):
    """Process a single sample with ICA decomposition optimizer."""
    optimizer = ICADecompositionOptimizer(
        ...
        use_nmf=config['use_nmf'],
        ica_replaces_triangulation=config['ica_replaces_triangulation'],
    ...
    start = time.time()
    candidates, best_rmse, results, features, best_positions, n_transferred, ica_info = optimizer.estimate_sources(
...
def main():
    parser = argparse.ArgumentParser(description='Run ICA Decomposition Optimizer')
    ...
    parser.add_argument('--no-ica', action='store_true',
                        help='Disable ICA init for 2-source')
    parser.add_argument('--use-nmf', action='store_true',
                        help='Enable NMF init (alternative to ICA)')
    parser.add_argument('--ica-replaces-tri', action='store_true',
                        help='Use ICA instead of triangulation for 2-source (not in addition)')
    ...
    config = {
        ...
        'ica_replaces_triangulation': args.ica_replaces_tri,
    ...
    # Print header
    print("=" * 70)
    print("ICA DECOMPOSITION OPTIMIZER")
    print("=" * 70)
    ...
    print(f"  ICA: {config['use_ica']}")
    print(f"  ICA replaces tri (2-src): {config['ica_replaces_triangulation']}")
    print(f"  NMF: {config['use_nmf']}")
    ...
    if config['ica_replaces_triangulation']:
        print(f"KEY INNOVATION: ICA REPLACES triangulation for 2-source!")
    else:
        print(f"KEY INNOVATION: ICA signal decomposition for 2-source init!")
    ...
    for batch_idx in range(n_batches):
        ...
        # Count ICA wins in this batch
        ica_wins = sum(1 for r in batch_results if r['best_init_type'] == 'ica')

        print(f"  -> Batch RMSE: {np.mean(batch_rmses):.4f}, "
              f"Avg transfers: {np.mean(batch_transfers):.1f}, "
              f"ICA wins: {ica_wins}/{len(batch_results)}, "
    ...
    # Track ICA success for 2-source
    ica_success_count = sum(1 for r in all_results
                           if r['n_sources'] == 2 and r['ica_info'].get('success', False))
    ica_win_count = sum(1 for r in all_results if r['best_init_type'] == 'ica')
    n_2src = sum(1 for r in all_results if r['n_sources'] == 2)
    ...
    if is_g4dn_simulation:
        ...
        run_name = f"ica_decomposition_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            # ICA-specific metrics
            mlflow.log_metric("ica_success_rate", ica_success_count / n_2src * 100 if n_2src > 0 else 0)
            mlflow.log_metric("ica_win_rate", ica_win_count / n_2src * 100 if n_2src > 0 else 0)
            ...
            # Parameters
            mlflow.log_param("optimizer", "ICADecompositionOptimizer")
            ...
            mlflow.log_param("use_nmf", config['use_nmf'])
            mlflow.log_param("ica_replaces_triangulation", config['ica_replaces_triangulation'])
            ...
            with open(output_path, 'wb') as f:
                pickle.dump({
                    ...
                    'ica_success_rate': ica_success_count / n_2src * 100 if n_2src > 0 else 0,
                    'ica_win_rate': ica_win_count / n_2src * 100 if n_2src > 0 else 0,
                }, f)
            try:
    ...
    print()
    print("ICA Decomposition Stats (2-source only):")
    print(f"  ICA Success:    {ica_success_count}/{n_2src} ({ica_success_count/n_2src*100:.1f}%)" if n_2src > 0 else "  No 2-source samples")
    print(f"  ICA Best Init:  {ica_win_count}/{n_2src} ({ica_win_count/n_2src*100:.1f}%)" if n_2src > 0 else "")
    print()
