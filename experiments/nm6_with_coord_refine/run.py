"""
Test 6 NM iterations + coordinate refinement.

Based on findings:
- 6 NM iter alone: 53.5 min, score 1.1489
- 7 NM + coord refine: 63.8 min, score 1.1637 (over budget)

Goal: Combine 6 NM iter + coord refine to stay in budget (~56 min)
while gaining accuracy benefit of coord refinement.
"""
...
# Import optimizer from coordinate_wise_refinement
sys.path.insert(0, os.path.join(_project_root, 'experiments', 'coordinate_wise_refinement'))
...
def run_experiment(config, config_name, data):
    ...
    print(f"Parameters: refine_maxiter={config.get('refine_maxiter', 6)}")
...
def main():
    ...
    # Test different NM iterations with coord refinement
    configs = [
        ({**base_config, 'refine_maxiter': 6, 'coord_step': 0.01}, 'nm6_coord001'),
        ({**base_config, 'refine_maxiter': 6, 'coord_step': 0.015}, 'nm6_coord0015'),
        ({**base_config, 'refine_maxiter': 5, 'coord_step': 0.01}, 'nm5_coord001'),
    ]
