"""
Separable CMA-ES (Diagonal Covariance) Optimizer

Tests whether diagonal-only covariance is sufficient for this 2-4D problem.

Key change from baseline:
- CMA_diagonal=True: Uses diagonal covariance only, no parameter correlations
- This enables higher learning rates and faster adaptation
- Trade-off: loses correlation information between x,y positions

Research: Akimoto & Hansen 2020 (Evolutionary Computation) shows sep-CMA-ES
converges faster on separable/nearly-separable functions.
"""
...
class SeparableCMAESOptimizer:
    """
    Optimizer using Separable CMA-ES (diagonal-only covariance).

    Key change: CMA_diagonal=True removes correlation modeling between parameters.
    This enables faster adaptation at the cost of potentially missing important
    ...
    def __init__(
        ...
        # NEW: Separable CMA-ES option
        use_diagonal_cma: bool = True,
    ):
        self.Lx = Lx
        self.Ly = Ly
        ...
        self.use_diagonal_cma = use_diagonal_cma
    ...
    def _run_single_optimization(self, sample, meta, q_range, solver_coarse, solver_fine,
        ...
        """Run CMA-ES with diagonal-only covariance."""
        ...
        for init_params, init_type in initializations:
            ...
            # KEY CHANGE: Enable diagonal-only covariance (Separable CMA-ES)
            if self.use_diagonal_cma:
    ...
    def estimate_sources(self, sample, meta, q_range=(0.5, 2.0), verbose=False):
        ...
        if verbose:
            cma_mode = "diagonal" if self.use_diagonal_cma else "full"
            print(f"  CMA-ES mode: {cma_mode}, {nt_reduced}/{nt_full} timesteps ({self.timestep_fraction*100:.0f}%)")
