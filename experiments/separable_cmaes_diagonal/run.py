#!/usr/bin/env python3
"""Run script for Separable CMA-ES experiment with MLflow logging."""
...
from optimizer import SeparableCMAESOptimizer


def process_single_sample(args):
    idx, sample, meta, config = args
    optimizer = SeparableCMAESOptimizer(
        ...
        use_diagonal_cma=config['use_diagonal_cma'],
    )
    start = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Run Separable CMA-ES Experiment')
    ...
    parser.add_argument('--no-diagonal', action='store_true', help='Use full covariance (baseline)')
    ...
    cma_mode = "diagonal" if use_diagonal else "full"

    print(f"\nSeparable CMA-ES Experiment")
    print(f"=" * 70)
    print(f"Samples: {n_samples}, Workers: {args.workers}")
    print(f"CMA-ES mode: {cma_mode}")
    print(f"Timesteps: {args.timestep_fraction*100:.0f}%")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS - Separable CMA-ES ({cma_mode})")
    print(f"{'='*70}")
    ...
    print()
    print(f"Baseline (full cov): 1.1688 @ 58.4 min")
    print(f"This run ({cma_mode}): {score:.4f} @ {projected_400:.1f} min")
    ...
    if not args.no_mlflow:
        ...
        run_name = f"sep_cmaes_run{args.tuning_run}_{cma_mode}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.log_param("experiment_id", "EXP_SEP_CMAES_001")
            mlflow.log_param("worker", "W2")
            mlflow.log_param("tuning_run", args.tuning_run)
            mlflow.log_param("platform", "wsl")
            mlflow.log_param("n_samples", n_samples)
            mlflow.log_param("cma_mode", cma_mode)
            mlflow.log_param("use_diagonal_cma", use_diagonal)
