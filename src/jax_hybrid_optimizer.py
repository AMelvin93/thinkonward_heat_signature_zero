"""
Hybrid JAX-Forward Optimizer.

Uses JAX JIT-compiled simulator for FAST forward passes,
but scipy L-BFGS-B for optimization (numerical gradients).

This avoids the expensive autodiff through time-stepping while
still benefiting from JAX's GPU acceleration and JIT compilation.

Expected speedup: 10-50x over numpy forward passes.
"""
...
# Import JAX simulator functions
try:
    from .jax_simulator import simulate_and_sample, check_gpu
except ImportError:
    from jax_simulator import simulate_and_sample, check_gpu

# Default to n-1 workers for CPU parallelism
...
class JAXHybridOptimizer:
    """
    Hybrid optimizer using JAX for fast forward simulation.

    Strategy:
    1. Use JAX JIT-compiled PDE solver for fast forward passes (GPU accelerated)
    2. Use scipy L-BFGS-B for optimization (numerical gradients, but fast forward calls)
    3. Multi-start with smart + random initializations
    4. Candidate diversity filtering

    This is faster than:
    - Pure numpy: JAX forward passes are 10-50x faster
    - Pure JAX autodiff: Avoids expensive gradient through time-stepping
    """
    ...
    def _create_objective(
        ...
        """
        Create objective function using JAX forward simulation.

        The objective uses JAX's JIT-compiled simulator for fast forward passes.
        """
        ...
        def objective(params):
            # Reshape params to (n_sources, 3)
            sources = jnp.array(params).reshape(n_sources, 3)

            # Fast JAX forward simulation (JIT compiled, GPU accelerated)
            ...
            # Compute RMSE
            rmse = float(jnp.sqrt(jnp.mean((Y_pred - Y_obs_jax) ** 2)))
            return rmse
    ...
    def estimate_sources(
        ...
        """
        Estimate source parameters using JAX-accelerated forward simulation.
        ...
        # Create JAX-accelerated objective
        objective = self._create_objective(
            n_sources, sensors_xy, Y_observed,
            kappa, dt, nt, bc, T0
        )

        # Warmup JIT compilation with a dummy call
        if verbose:
            print("  Warming up JIT compilation...")
        dummy_params = self._random_init(n_sources, q_range)
        _ = objective(dummy_params)
        ...
        if verbose:
            print(f"  Running {len(initial_points)} L-BFGS-B optimizations with JAX forward sim...")

        # Run optimizations sequentially (JAX handles GPU parallelism internally)
        candidates = []
        for init_type, x0 in initial_points:
            try:
                ...
                if verbose:
                    print(f"    {init_type}: RMSE={result.fun:.4f}")
